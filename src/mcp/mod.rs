            ...
            "properties": {
                ...
                "query": {
                    ...
                    "description": "Derived-relation query kind. output_support (default): each \
                                    ...
                                    support cone of its transition-select input sel. node_drivers: \
                                    per IR node its immediate (1-hop) driver adjacency — kind, width, \
                                    gate op (for a gate), and the list of its direct operand drivers \
                                    ...
                                    module's graph (opening the opaque InstanceOutput leaf). \
                                    ...
                                    cone_depth). node_reach: the transitive complement to node_readers \
            ...
            "tools": [
                ...
                },
                {
                    ...
                    "description": "Answer a derived-RELATION query over the DUT (seed, config) IR by \
                                    ...
                                    query=node_drivers returns per IR node its immediate (1-hop) driver \
                                    adjacency (kind, width, gate op, and its direct operand drivers — \
                                    each operand's id, kind, and resolved handle — in operand order) — \
                                    the atomic node-level primitive under the transitive support cone, \
                                    surfacing each node's gate op; \
    ...
    fn run_analyze(&mut self, id: Value, seed: u64, cfg: &Config, args: &Value) -> Value {
        ...
        let (analysis, doc) = if cfg.effective_hierarchy_depth_range().is_some() {
            ...
            let analysis = match query {
                ...
                introspect::analyze::QUERY_NODE_DRIVERS => {
                    introspect::analyze::design_node_drivers(&design, target)
                }
            ...
            let analysis = match query {
                ...
                introspect::analyze::QUERY_NODE_DRIVERS => {
                    introspect::analyze::module_node_drivers(&m, target)
                }
                introspect::analyze::QUERY_NODE_READERS => {
...
mod tests {
    ...
    #[test]
    fn analyze_returns_memory_provenance_and_caches_it() {
        let mut s = McpServer::new();
        // memory_prob = 1.0 makes the DUT likely to carry an inferrable memory; the
        // routing + schema + caching assertions below hold regardless of memory count.
        let cfg = Config {
            seed: 7,
            memory_prob: 1.0,
            ..Config::default()
        ...
        assert!(doc["analysis"]["results"].as_array().unwrap().is_empty());
        assert!(doc["analysis"].get("reach_results").is_none());
        assert!(doc["analysis"].get("flop_dependencies").is_none());
        // Cached + served under the memory_provenance query key.
        let run_id = doc["request"]["run_id"].as_str().unwrap().to_string();
        let read = s
            .handle(&req(
                2,
                "resources/read",
                json!({ "uri": format!("anvil://artifact/{run_id}/analysis/memory_provenance") }),
            ))
            .unwrap();
        let text = read["result"]["contents"][0]["text"].as_str().unwrap();
        let cached: Value = serde_json::from_str(text).unwrap();
        assert_eq!(cached["analysis"]["query"], "memory_provenance");
    }

    #[test]
    fn analyze_memory_provenance_unknown_target_is_invalid_params() {
        let mut s = McpServer::new();
        // No DUT has memory id 99999 ⇒ an unresolvable target ⇒ -32602.
        let cfg = Config {
            seed: 7,
            memory_prob: 1.0,
            ..Config::default()
    ...
    #[test]
    fn analyze_returns_node_drivers_and_caches_it() {
        let mut s = McpServer::new();
        // Every DUT has IR nodes, so node_drivers always returns a non-empty adjacency.
        let cfg = Config {
            seed: 7,
            ..Config::default()
        };
        ...
        assert!(!nds.is_empty()); // a real DUT has nodes
                                  // Node ids are the ascending 0..len index.
        for (i, nd) in nds.iter().enumerate() {
            assert_eq!(nd["node"].as_u64(), Some(i as u64));
        }
        // At least one gate node carries an `op` + a non-empty operand-driver list.
        assert!(nds.iter().any(|nd| nd["kind"] == "gate"
            && nd.get("op").is_some()
            && !nd["drivers"].as_array().unwrap().is_empty()));
        // The other queries' vecs are not populated by this kind.
        assert!(doc["analysis"]["results"].as_array().unwrap().is_empty());
        assert!(doc["analysis"].get("reach_results").is_none());
        assert!(doc["analysis"].get("fsm_provenance").is_none());
        // Cached + served under the node_drivers query key.
        let run_id = doc["request"]["run_id"].as_str().unwrap().to_string();
        let read = s
            .handle(&req(
                2,
                "resources/read",
                json!({ "uri": format!("anvil://artifact/{run_id}/analysis/node_drivers") }),
            ))
            .unwrap();
        let text = read["result"]["contents"][0]["text"].as_str().unwrap();
        let cached: Value = serde_json::from_str(text).unwrap();
        assert_eq!(cached["analysis"]["query"], "node_drivers");
    }

    #[test]
    fn analyze_node_drivers_unknown_target_is_invalid_params() {
        let mut s = McpServer::new();
        // No DUT has node id 999999 ⇒ an out-of-range target ⇒ -32602.
        let cfg = Config {
            seed: 7,
            ..Config::default()
        };
        ...
        let resp = call(
            ...
            1,
            "analyze",
            json!({ "seed": 7, "config": cfg_json, "query": "node_drivers", "target": "node:999999" }),
        );
        assert_eq!(resp["error"]["code"].as_i64(), Some(INVALID_PARAMS));
    }

    #[test]
    fn analyze_returns_node_readers_and_caches_it() {
        let mut s = McpServer::new();
        // Every DUT has IR nodes, so node_readers always returns a non-empty adjacency.
        let cfg = Config {
            seed: 7,
            ..Config::default()
        };
        ...
        for (i, nr) in nrs.iter().enumerate() {
            assert_eq!(nr["node"].as_u64(), Some(i as u64));
        }
        // At least one node is read by a gate ⇒ a non-empty readers list; every reader
        // list is ascending + deduped and every reader is a gate (only gates have operands).
        assert!(nrs
            .iter()
            .any(|nr| !nr["readers"].as_array().unwrap().is_empty()));
        for nr in nrs {
            let readers = nr["readers"].as_array().unwrap();
            ...
            sorted.dedup();
            assert_eq!(ids, sorted, "readers must be ascending + deduped");
            assert!(readers.iter().all(|r| r["kind"] == "gate"));
        }
        // The other queries' vecs are not populated by this kind.
        assert!(doc["analysis"]["results"].as_array().unwrap().is_empty());
        assert!(doc["analysis"].get("node_drivers").is_none());
        assert!(doc["analysis"].get("reach_results").is_none());
        // Cached + served under the node_readers query key.
        let run_id = doc["request"]["run_id"].as_str().unwrap().to_string();
        let read = s
            .handle(&req(
                2,
                "resources/read",
                json!({ "uri": format!("anvil://artifact/{run_id}/analysis/node_readers") }),
            ))
            .unwrap();
        let text = read["result"]["contents"][0]["text"].as_str().unwrap();
        let cached: Value = serde_json::from_str(text).unwrap();
        assert_eq!(cached["analysis"]["query"], "node_readers");
    }

    #[test]
    fn analyze_node_readers_unknown_target_is_invalid_params() {
        let mut s = McpServer::new();
        // No DUT has node id 999999 ⇒ an out-of-range target ⇒ -32602.
        let cfg = Config {
            seed: 7,
        ...
        let resp = call(
            ...
            1,
            "analyze",
            json!({ "seed": 7, "config": cfg_json, "query": "node_readers", "target": "node:999999" }),
        );
        assert_eq!(resp["error"]["code"].as_i64(), Some(INVALID_PARAMS));
    }
    ...
    #[test]
    fn analyze_returns_instance_input_bindings_and_caches_it() {
        let mut s = McpServer::new();
        // A hierarchy config ⇒ a design artifact ⇒ design_instance_input_bindings reads the
        // top instances' input bindings.
        let cfg = Config {
            seed: 42,
            hierarchy_depth: 1,
            num_leaf_modules: 2,
        ...
        assert!(doc["analysis"]["results"].as_array().unwrap().is_empty());
        assert!(doc["analysis"].get("instance_provenance").is_none());
        assert!(doc["analysis"].get("node_readers").is_none());
        let iibs = doc["analysis"]["instance_input_bindings"]
            .as_array()
            .unwrap();
        assert!(!iibs.is_empty(), "a hierarchy top instantiates >= 1 child");
        // Each instance carries its facts; across them at least one child input is bound, and
        // every binding resolves to a parent node (id + kind + handle).
        let mut total_bindings = 0;
        for ib in iibs {
            assert!(!ib["instance"].as_str().unwrap().is_empty());
            assert!(!ib["module"].as_str().unwrap().is_empty());
            let role = ib["role"].as_str().unwrap();
            assert!(role == "planned_child" || role == "parent_cone");
            for b in ib["input_bindings"].as_array().unwrap() {
                assert!(!b["port"].as_str().unwrap().is_empty());
                assert!(b["driver"]["node"].is_number());
                assert!(!b["driver"]["kind"].as_str().unwrap().is_empty());
                assert!(!b["driver"]["name"].as_str().unwrap().is_empty());
                total_bindings += 1;
            }
        }
        assert!(
            total_bindings >= 1,
            "a depth-1 hierarchy routes >= 1 parent source into a child input"
        );
        // Cached + served under the instance_input_bindings query key.
        let run_id = doc["request"]["run_id"].as_str().unwrap().to_string();
        let read = s
            .handle(&req(
                2,
                "resources/read",
                json!({ "uri": format!("anvil://artifact/{run_id}/analysis/instance_input_bindings") }),
            ))
            .unwrap();
        let text = read["result"]["contents"][0]["text"].as_str().unwrap();
        let cached: Value = serde_json::from_str(text).unwrap();
        assert_eq!(cached["analysis"]["query"], "instance_input_bindings");
    }

    #[test]
    fn analyze_instance_input_bindings_unknown_target_is_invalid_params() {
        let mut s = McpServer::new();
        let cfg = Config {
            seed: 42,
            hierarchy_depth: 1,
    ...
    #[test]
    fn analyze_returns_node_reach_and_caches_it() {
        let mut s = McpServer::new();
        // A default single-module DUT ⇒ module_node_reach over every IR node.
        let cfg = Config {
            seed: 7,
            ..Config::default()
        };
        ...
        let nrs = doc["analysis"]["node_reach"].as_array().unwrap();
        assert!(!nrs.is_empty(), "a DUT has >= 1 IR node");
        let mut saw_a_node_reaching_an_output = false;
        for nr in nrs {
            assert!(nr["node"].is_number());
            assert!(!nr["kind"].as_str().unwrap().is_empty());
            assert!(nr["width"].is_number());
            let outs = nr["reaches_outputs"].as_array().unwrap();
            let flops = nr["reaches_flops"].as_array().unwrap();
            // fanout_targets is the total sink count.
            assert_eq!(
                nr["fanout_targets"].as_u64().unwrap() as usize,
                outs.len() + flops.len(),
                "fanout_targets == reaches_outputs + reaches_flops"
            );
            if !outs.is_empty() {
                saw_a_node_reaching_an_output = true;
            }
        }
        assert!(
            saw_a_node_reaching_an_output,
            "a DUT has >= 1 driven output ⇒ some node reaches it"
        );
        // Cached + served under the node_reach query key.
        let run_id = doc["request"]["run_id"].as_str().unwrap().to_string();
        let read = s
            .handle(&req(
                2,
                "resources/read",
                json!({ "uri": format!("anvil://artifact/{run_id}/analysis/node_reach") }),
            ))
            .unwrap();
        let text = read["result"]["contents"][0]["text"].as_str().unwrap();
        let cached: Value = serde_json::from_str(text).unwrap();
        assert_eq!(cached["analysis"]["query"], "node_reach");
    }

    #[test]
    fn analyze_node_reach_unknown_target_is_invalid_params() {
        let mut s = McpServer::new();
        let cfg = Config {
            seed: 7,
            ..Config::default()
    ...
    fn adapter_catalog_resource_lists_the_registry() {
        ...
        // It reads back as the five registered adapters (the three originals then
        // the new adapters `sv2v`, `slang`), in registry order, each with the four
        // projected fields.
        let resp = s
            .handle(&req(
                2,
        ...
        for entry in entries {
            ...
            // slang is the first fact-bearing adapter (`extract_facts` JSON-AST);
            // the others are accept/warn/reject only.
            let expects_facts = entry["id"] == json!("slang");
            assert_eq!(entry["supports_facts"], json!(expects_facts));
        }
    }
    ...
    /// `DOWNSTREAM-ADAPTER-EXPANSION.2c.1` — the fact-bearing `slang` adapter is
    /// selectable through the same `tools` arg (decision `0017` API-completeness),
    /// and the allow-list error now names it too.
    #[test]
    fn parse_validate_tools_accepts_slang() {
        assert_eq!(
            parse_validate_tools(&json!({ "tools": ["verilator", "slang"] })).unwrap(),
            vec![AcceptanceTool::Verilator, AcceptanceTool::Slang]
        );
        let err = parse_validate_tools(&json!({ "tools": ["nope"] })).unwrap_err();
        assert!(
            err.contains("slang"),
