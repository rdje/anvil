...
/// The lane's top-module name for `seed`. The single source of truth
/// shared by [`emit_sv`], [`emit_manifest`], and the umbrella lane's
/// `LaneArtifact::top` (`LANE-OUT-FILENAME.1`) — so the emitted SV,
/// the manifest's `top` field, and the `--out` filename stem can
/// never disagree.
pub fn top_name(seed: u64) -> String {
    format!("mc_{seed}")
}
