//! The canonicalization engine: structural interning, constant folding,
//! associative flattening, and the peephole rewriter.
//!
//! Extracted from `types.rs` by `IR-TYPES-DECOMPOSITION.3` as a **verbatim
//! move** — every line below is byte-identical to the code it replaced, so
//! generated RTL is unchanged for every `(seed, knobs)` pair.
//!
//! **Why it is a separate module.** `types.rs` answers *"what is a circuit?"*;
//! this file answers *"is this node the same expression as one I already
//! have?"* — a different question, with its own invariants (commutative
//! operand normalization, XOR pair cancellation, width-modular constant
//! evaluation) that a reader of the data model should not have to page past.
//!
//! **Why an inherent `impl Module` here is legal.** Rust permits an inherent
//! impl in any module of the crate that defines the type, so no call site
//! changes and no field visibility widens: `Module::gate_instances` and
//! `const_instances` are already `pub(crate)`, which a sibling module of
//! `crate::ir` can reach.

use super::types::{DepSet, GateOp, Module, Node, NodeId};
