...
/// `docs/AGENT_INTROSPECTION_SCHEMA.md` §7 (`MAJOR.MINOR`). `1.15` is the
/// additive (backward-compatible) MINOR bump that adds the
/// `Metrics::num_emitted_mux_if_blocks` field — the count of 2:1 `Mux` gates the
/// emitter projects as a procedural `always_comb` `if`/`else` block
/// (`STRUCTURED-EMISSION-EXPANSION.15b`, decision `0027`). It is a SCHEMA-DERIVED
/// projection of an existing emitter-surface annotation (a count of
/// `Module::mux_if_gates`), not new computed truth; default-off modules report `0`
/// and the default-`dut` **artifact** (`.sv`) stays byte-identical. The prior `1.14`
/// added `Metrics::num_emitted_multi_output_tasks` (decision `0025`); a `1.14`
/// consumer simply ignores the new integer key. MINOR is an integer, so
/// `1.14 → 1.15` (fifteen), not a decimal fraction. See the schema-doc §7
/// changelog for the full `1.0 → … → 1.14 → 1.15` history.
pub const SCHEMA_VERSION: &str = "1.15";
