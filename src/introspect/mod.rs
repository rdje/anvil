...
/// `docs/AGENT_INTROSPECTION_SCHEMA.md` §7 (`MAJOR.MINOR`). `1.9` is the
/// additive (backward-compatible) MINOR bump that surfaces the new
/// [`Metrics::num_emitted_generate_loops`](crate::metrics::Metrics)
/// field in `module_metrics` — the count of `{N{x}}` replication gates
/// emitted as a `generate for` loop projection (the `generate_loop_emit_prob`
/// knob, `STRUCTURED-EMISSION-EXPANSION.4b.2a`). The field is
/// `#[serde(default)]`, so a `1.8` consumer ignores the new key and an absent
/// key reads back as `0`; the default-`dut` artifact stays byte-identical and
/// determinism is preserved (the metric is a post-hoc structural count, not
/// new computed truth). See the schema-doc §7 changelog for the full
/// `1.0 → … → 1.8 → 1.9` history.
...
mod tests {
    ...
    fn module_document_is_schema_v1_and_dut() {
        ...
        assert_eq!(doc.schema_version, "1.9");
