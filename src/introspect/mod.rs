...
/// `docs/AGENT_INTROSPECTION_SCHEMA.md` §7 (`MAJOR.MINOR`). `1.25` is the
/// additive (backward-compatible) MINOR bump that adds the **twelfth** derived
/// `analyze` query kind `longest_path` — one representative **longest combinational
/// fan-in path** of a target (an output port / `"flop:<id>"`): the ordered chain of
/// interior gates (each with its op) realizing the `output_support` cone's scalar
/// `cone_depth`, terminating at a boundary leaf, carried by a twelfth
/// `DerivedAnalysis.longest_path` parallel vec (`SEMANTIC-INTROSPECTION-EXPANSION.13b`;
/// the eighth query beyond decision `0011`'s four named kinds — the **witness** for the
/// scalar `cone_depth`, structural gate-depth NOT timing). It is a SCHEMA-DERIVED
/// projection over the already-emitted IR (a pure two-pass walk of the node graph — not
/// new computed truth); the `longest_path` key is `skip_serializing_if`-omitted on every
/// other query document, so the eleven prior `analyze` documents and the default-`dut`
/// **artifact** (`.sv`) stay byte-identical. The prior `1.24` added the eleventh `analyze`
/// query kind `instance_input_bindings` (`SEMANTIC-INTROSPECTION-EXPANSION.12b`); a `1.24`
/// consumer simply ignores the new query kind. MINOR is an integer, so `1.24 → 1.25`
/// (twenty-five), not a decimal fraction. See the schema-doc §7 changelog for the full
/// `1.0 → … → 1.24 → 1.25` history.
pub const SCHEMA_VERSION: &str = "1.25";
