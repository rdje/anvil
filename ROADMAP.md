...
`168` scenarios, `672` total designs, `coverage_gaps = []`, and clean Verilator + Yosys
elaboration/synthesis on the broadened hierarchy matrix
(`672/0` in Verilator plus both repo-owned Yosys modes). The `r73` report
...
the full downstream-clean `r49` report first proved it, `r50` banked the accumulated mixed-support surface, and `r51` through `r73` carry it forward with
`coverage_gaps = []` and `672/0` pass-fail in Verilator plus both
repo-owned Yosys modes.
...
`r73` is the current full downstream-clean 168-scenario recursive
