...
`183` scenarios, `732` total designs, `coverage_gaps = []`, and clean Verilator + Yosys
elaboration/synthesis on the broadened hierarchy matrix
(`732/0` in Verilator plus both repo-owned Yosys modes). The `r78` report
...
the full downstream-clean `r49` report first proved it, `r50` banked the accumulated mixed-support surface, and `r51` through `r78` carry it forward with
...
depth-6 sweep.
`r78` is the current full downstream-clean 183-scenario recursive
