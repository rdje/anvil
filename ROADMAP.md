...
`165` scenarios, `660` total designs, `coverage_gaps = []`, and clean Verilator + Yosys
elaboration/synthesis on the broadened hierarchy matrix
(`660/0` in Verilator plus both repo-owned Yosys modes). The `r72` report
...
the full downstream-clean `r49` report first proved it, `r50` banked the accumulated mixed-support surface, and `r51` through `r72` carry it forward with
`coverage_gaps = []` and `660/0` pass-fail in Verilator plus both
repo-owned Yosys modes.
...
hierarchy artifact.
`r72` is the current full downstream-clean 165-scenario recursive
...
fact) on the expanded 165-scenario matrix.
