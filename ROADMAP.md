...
- Current slice constraints:
  ...
  - the latest full downstream-clean repo-owned Phase 4 matrix is banked at
    ...
    and `552/0` pass-fail in Verilator plus both repo-owned Yosys modes.
...
`138` scenarios, `552` total designs, `coverage_gaps = []`, and clean Verilator + Yosys
elaboration/synthesis on the broadened hierarchy matrix
(`552/0` in Verilator plus both repo-owned Yosys modes). The `r63` report
...
the full downstream-clean `r49` report first proved it, `r50` banked the accumulated mixed-support surface, and `r51` through `r63` carry it forward with
`coverage_gaps = []` and `552/0` pass-fail in Verilator plus both
repo-owned Yosys modes.
...
`r63` is the current full downstream-clean 138-scenario recursive
