...
`195` scenarios, `780` total designs, `coverage_gaps = []`, and clean Verilator + Yosys
elaboration/synthesis on the broadened hierarchy matrix
(`780/0` in Verilator plus both repo-owned Yosys modes). The `r82` report
...
the full downstream-clean `r49` report first proved it, `r50` banked the accumulated mixed-support surface, and `r51` through `r82` carry it forward with
...
hierarchy artifact.
`r82` is the current full downstream-clean 195-scenario recursive
depth-7 stateful unregistered parent-composed mixed-support child-input
gated coverage hierarchy artifact (2,2 calibrated) — closes the
depth-7 sweep.
