...
`180` scenarios, `720` total designs, `coverage_gaps = []`, and clean Verilator + Yosys
elaboration/synthesis on the broadened hierarchy matrix
(`720/0` in Verilator plus both repo-owned Yosys modes). The `r77` report
...
the full downstream-clean `r49` report first proved it, `r50` banked the accumulated mixed-support surface, and `r51` through `r77` carry it forward with
`coverage_gaps = []` and `720/0` pass-fail in Verilator plus both
repo-owned Yosys modes.
...
hierarchy artifact.
`r77` is the current full downstream-clean 180-scenario recursive
...
child-instance calibration adopted by r74).
