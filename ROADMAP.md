...
`135` scenarios, `540` total designs, `coverage_gaps = []`, and clean Verilator + Yosys
elaboration/synthesis on the broadened hierarchy matrix
(`540/0` in Verilator plus both repo-owned Yosys modes). The `r62` report
...
the full downstream-clean `r49` report first proved it, `r50` banked the accumulated mixed-support surface, and `r51` through `r62` carry it forward with
...
hierarchy artifact.
`r62` is the current full downstream-clean 135-scenario recursive
...
fact) on the expanded 135-scenario matrix.
