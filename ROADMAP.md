...
`177` scenarios, `708` total designs, `coverage_gaps = []`, and clean Verilator + Yosys
elaboration/synthesis on the broadened hierarchy matrix
(`708/0` in Verilator plus both repo-owned Yosys modes). The `r76` report
...
the full downstream-clean `r49` report first proved it, `r50` banked the accumulated mixed-support surface, and `r51` through `r76` carry it forward with
`coverage_gaps = []` and `708/0` pass-fail in Verilator plus both
repo-owned Yosys modes.
...
artifact.
`r76` is the current full downstream-clean 177-scenario recursive
...
fact) on the expanded 177-scenario matrix.
