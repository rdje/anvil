...
`186` scenarios, `744` total designs, `coverage_gaps = []`, and clean Verilator + Yosys
elaboration/synthesis on the broadened hierarchy matrix
(`744/0` in Verilator plus both repo-owned Yosys modes). The `r79` report
...
the full downstream-clean `r49` report first proved it, `r50` banked the accumulated mixed-support surface, and `r51` through `r79` carry it forward with
...
the depth-7 axis.
`r79` is the current full downstream-clean 186-scenario recursive
