...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   closed at `/tmp/anvil-tool-matrix-phase4-hierarchy-r73` with 672/0
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  (672 designs, `coverage_gaps = []`, and 672/0 pass-fail in Verilator
  ...
  are now historical policy breadcrumbs because the full `r73` bank
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  numerically in focused smokes and the full downstream-clean `r73`
  hierarchy bank. The `r73` bank requires
