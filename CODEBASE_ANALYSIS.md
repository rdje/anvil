...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   closed at `/tmp/anvil-tool-matrix-phase4-hierarchy-r41` with 348/0
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  are now historical policy breadcrumbs because the full `r41` bank
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  numerically in focused smokes and the full downstream-clean `r41`
  hierarchy bank. The `r41` bank requires
  ...
  recursive non-top registered parent-composed helper, recursive non-top parent-output helper, recursive non-top stateful parent-output helper, recursive non-top multi-helper budget, direct sibling,
...
- `cargo test` — clean (293 passing tests: 195 lib + 5 main + 26 tool_matrix + 67 integration).
- `cargo build` — clean.
