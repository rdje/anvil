...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   closed at `/tmp/anvil-tool-matrix-phase4-hierarchy-r46` with 396/0
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  (396 designs, `coverage_gaps = []`, and 396/0 pass-fail in Verilator
  ...
  are now historical policy breadcrumbs because the full `r46` bank
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  or parent-composed D logic. Direct sibling helper routing, direct
  registered sibling
  ...
  numerically in focused smokes and the full downstream-clean `r46`
  hierarchy bank. The `r46` bank requires
...
- `cargo test` — clean (298 passing tests: 195 lib + 5 main + 26 tool_matrix + 72 integration).
- `cargo build` — clean.
