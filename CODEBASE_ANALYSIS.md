...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   closed at `/tmp/anvil-tool-matrix-phase4-hierarchy-r74` with 684/0
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  (684 designs, `coverage_gaps = []`, and 684/0 pass-fail in Verilator
  ...
  are now historical policy breadcrumbs because the full `r74` bank
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  numerically in focused smokes and the full downstream-clean `r74`
  hierarchy bank. The `r74` bank requires
