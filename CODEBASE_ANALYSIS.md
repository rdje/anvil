...
│   │                 requested interval, and still keeps the
│   │                 parent-side layer combinational. Both hierarchy
...
│   │                 leaves. Local parent flops, richer hierarchy-local
...
| 4 — Hierarchy                | in progress  | `gen/hierarchy.rs`, `ir/types.rs`, `ir/validate.rs`, `emit/sv.rs`, `main.rs`, `metrics.rs`, `bin/tool_matrix.rs` | Current live slice now has two lanes: the legacy exact depth-1 wrapper planner and the newer bounded recursive planner. The wrapper lane still exercises exact, reuse, and under-instantiation profiles via `num_child_instances`; the recursive lane uses `min_hierarchy_depth..=max_hierarchy_depth` plus `min_child_instances_per_module..=max_child_instances_per_module`, now keeping every leaf depth inside the requested interval and mixing shallow/deep branches when the interval is open and the structure allows it. Repeated `child_instances_per_depth` overrides are now also live and let branching be steered by parent depth (`0` = top, `1` = its children, ...) on top of that global fallback range. Both hierarchy lanes now also expose an explicit child-sourcing axis, `hierarchy_child_source_mode = library | on-demand`: `library` keeps reusable child-definition pools, while the currently-landed `on-demand` slice synthesizes children against parent-planned exact data-interface profiles per planned instance slot. Both lanes build a parent-side combinational output layer over child `InstanceOutput` leaves, expose combinational sibling-routed child-input binding via `hierarchy_sibling_route_prob`, and now expose parent-local combinational child-input cones via `hierarchy_child_input_cone_prob` over parent data inputs, earlier sibling outputs, and earlier parent-side route gates. Hierarchy manifests/reports now carry exact per-design `DesignMetrics`, so hierarchy quality is measurable without SV inspection (coverage, reuse, single-use-vs-multiuse child structure, exact profiled interface realization, child-input provenance, sibling-routing fractions, parent-composed child-input fractions, top-interface shape, direct-vs-composed outputs, child-output dependency fractions/support, realized depth, depth histograms, leaf-depth histograms, branching histograms, per-depth branching summaries, control fanout, weighted child load). The emitted-boundary doctrine is explicit too: pure comb-only modules omit `clk` / `rst_n`, while wrappers keep them visible iff they carry sequential descendants. The fully banked repo-owned Phase 4 matrix is now `/tmp/anvil-tool-matrix-phase4-hierarchy-r15/tool_matrix_report.json` (84 designs, `coverage_gaps = []`, 84/0 in Verilator and both repo-owned Yosys modes), and it covers wrapper exact/reuse/under-instantiation, recursive depth `2`, mixed recursive depth range `2:3`, explicit child-sourcing modes `library` and `on-demand`, child-instance profiles `2`, `4`, `2:3`, `1:3`, the per-depth override profile `0=4:4,1=2:2`, real mixed shallow/deep recursive realization, real on-demand child sourcing, exact profiled child-interface synthesis, real sibling-routed child inputs, real parent-composed child-input bindings, and real parent-side composition. The focused clean smokes at `/tmp/anvil-hier-parent-compose-smoke-r1/manifest.json`, `/tmp/anvil-hier-range-smoke-r1/manifest.json`, `/tmp/anvil-hier-depth-profile-smoke-r1/manifest.json`, `/tmp/anvil-hier-mixed-depth-smoke-r1/manifest.json`, `/tmp/anvil-hier-profiled-ondemand-smoke-r1/manifest.json`, `/tmp/anvil-hier-sibling-routing-smoke-r1/manifest.json`, and `/tmp/anvil-hier-child-input-cone-smoke-r1/manifest.json` remain useful targeted evidence. Local parent state, later registered child-to-child routing, and future hierarchy-aware identity remain open. |
...
- `tests/pipeline.rs` — 41 integration tests covering cross-seed validity, reproducibility across strategies, motif sweeps, both constant- and variable-shift surfaces, the landed procedural case/casez/for-fold surfaces, the landed selectable `Slice` / `Concat` surface, the hierarchy surface (legacy depth-1 wrapper exact/reuse/under-instantiation plus bounded recursive tree shape, per-depth branching profiles, exact profiled on-demand child interfaces, sibling-routed child inputs, and parent-composed child-input bindings), the first parent-side composition surface over child outputs, all live gate categories, zero-orphan / zero-duplicate-operand doctrine guards, input-surface finalisation, associative / constant-fold / peephole / compaction counters, and knob-roll telemetry.
- Current executed counts (`cargo test`, 2026-04-23): **215 unit-target tests + 41 integration tests = 256 passing tests**. Doc-tests: 0.
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  Phase 4 hierarchy gate is now closed too at `/tmp/anvil-tool-matrix-phase4-hierarchy-r15/tool_matrix_report.json`
  (84 designs, `coverage_gaps = []`, and 84/0 pass-fail in Verilator
  ...
  exact profiled child-interface synthesis, real parent-side composition above instance outputs, real sibling-routed hierarchy child inputs, and real parent-composed child-input bindings. The focused smokes at
  ...
  `/tmp/anvil-hier-sibling-routing-smoke-r1`, and
  `/tmp/anvil-hier-child-input-cone-smoke-r1` remain useful targeted
  ...
  bank, `r11` is the first explicit child-sourcing bank, and the aborted `r8` rerun is historical
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  child-input cone surface, proved numerically in focused smokes and
  the banked `r15` hierarchy report. The next honest work is local parent state, registered
  child-to-child routing where it is structurally warranted, and future
...
- `cargo test` — clean (256 passing tests: 184 lib + 5 main + 26 tool_matrix + 41 integration).
- `cargo build` — clean.
