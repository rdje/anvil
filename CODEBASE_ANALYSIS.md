...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   closed at `/tmp/anvil-tool-matrix-phase4-hierarchy-r82` with 780/0
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  (780 designs, `coverage_gaps = []`, and 780/0 pass-fail in Verilator
  ...
  are now historical policy breadcrumbs because the full `r82` bank
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  numerically in focused smokes and the full downstream-clean `r82`
  hierarchy bank. The `r82` bank requires
