...
│                     and per-definition instantiation histograms.
...
│   │                 parent cone choice is now live for child-input
│   │                 and parent-output cones; broader helper
│   │                 placement/budgeting and hierarchy-aware identity
│   │                 remain open.
...
| 4 — Hierarchy                | in progress  | `gen/hierarchy.rs`, `ir/types.rs`, `ir/compact.rs`, `ir/validate.rs`, `emit/sv.rs`, `main.rs`, `metrics.rs`, `bin/tool_matrix.rs` | Current live slice now has two lanes: the legacy exact depth-1 wrapper planner and the newer bounded recursive planner. The wrapper lane still exercises exact, reuse, and under-instantiation profiles via `num_child_instances`; the recursive lane uses `min_hierarchy_depth..=max_hierarchy_depth` plus `min_child_instances_per_module..=max_child_instances_per_module`, now keeping every leaf depth inside the requested interval and mixing shallow/deep branches when the interval is open and the structure allows it. Repeated `child_instances_per_depth` overrides are now also live and let branching be steered by parent depth (`0` = top, `1` = its children, ...) on top of that global fallback range. Both hierarchy lanes now also expose an explicit child-sourcing axis, `hierarchy_child_source_mode = library | on-demand`: `library` keeps reusable child-definition pools, while the currently-landed `on-demand` slice synthesizes children against parent-planned exact data-interface profiles per planned instance slot. Both lanes build parent-side output cones over the full parent source pool, then repair finalized parent outputs so every output retains child `InstanceOutput` support and can also carry parent data-port support when such ports are live. Both lanes expose direct sibling-routed child-input binding via `hierarchy_sibling_route_prob`, registered sibling-routed child-input binding through one parent-local flop via `hierarchy_registered_sibling_route_prob`, registered parent-composed child-input binding through parent logic plus one parent-local flop via `hierarchy_registered_child_input_cone_prob`, parent-local child-input cones via `hierarchy_child_input_cone_prob` over parent data inputs / earlier sibling outputs / earlier parent-side route gates, optional local parent flops via `hierarchy_parent_flop_prob`, and parent-cone helper instances via `hierarchy_parent_cone_instance_prob` as internal sources for parent-composed child-input cones and parent-output cones. `max_parent_cone_instances_per_module` now controls the per-parent helper budget. The registered parent-composed route now builds from the full parent source pool too, so it can mix parent data ports with sibling outputs when both supports are live and later routes can chain through earlier parent-local Qs. Hierarchy manifests/reports now carry exact per-design `DesignMetrics`, so hierarchy quality is measurable without SV inspection (coverage, reuse, single-use-vs-multiuse child structure, exact profiled interface realization, child-input provenance including parent-flop, registered-instance-output, registered-parent-composed, registered-mixed-support, and registered-multistage parent-composed bindings, sibling-routing fractions, registered sibling-routing fractions, registered parent-composed fractions, registered mixed-support fractions, registered multistage parent-composed fractions, parent-composed child-input fractions, parent-flop child-input fractions, parent-cone helper-instance counts/fractions and per-parent maximum, parent-output helper-instance support counts/fractions, local parent-state counts, top-interface shape, direct-vs-composed outputs, parent-port-composed output counts/fractions, child-output dependency fractions/support, realized depth, depth histograms, leaf-depth histograms, branching histograms, per-depth branching summaries, control fanout, weighted child load). The emitted-boundary doctrine is explicit too: pure comb-only modules omit `clk` / `rst_n`, while hierarchy parents keep them visible iff they carry local state or sequential descendants. Module names are reserved from one generator-global sequence, so repeated hierarchy designs in one output directory cannot reuse the same definition filename. The fully banked repo-owned Phase 4 matrix remains `/tmp/anvil-tool-matrix-phase4-hierarchy-r21/tool_matrix_report.json` (132 designs, `coverage_gaps = []`, 132/0 in Verilator and both repo-owned Yosys modes), and it covers wrapper exact/reuse/under-instantiation, recursive depth `2`, mixed recursive depth range `2:3`, explicit child-sourcing modes `library` and `on-demand`, child-instance profiles `2`, `4`, `2:3`, `1:3`, the per-depth override profile `0=4:4,1=2:2`, real mixed shallow/deep recursive realization, real on-demand child sourcing, exact profiled child-interface synthesis, real sibling-routed child inputs, real registered sibling-routed child inputs, real registered parent-composed child-input bindings, real registered mixed-support child-input bindings, real multi-stage registered parent-composed child-input bindings, real parent-composed child-input bindings, real parent-cone helper-instance child-input bindings, real local parent flops, real parent-side composition, and real mixed parent-port / child-output parent outputs. Current HEAD adds focused parent-cone helper-instance parent-output composition and budgeted multi-helper allocation after that bank, proved by `cargo test hierarchy_parent_outputs_can_depend_on_helper_instance_outputs` and `cargo test hierarchy_parent_cone_helper_budget_allows_multiple_helpers`; the next full Phase 4 bank should cover the new 39-scenario / 156-design matrix. Earlier current-code coverage-only probes first kept the Phase 4 coverage policy honest for mixed parent-output composition, registered mixed-support routing, and multi-stage registered routing: `/tmp/anvil-tool-matrix-phase4-parent-port-coverage-r1/tool_matrix_report.json` records `saw_hierarchy_parent_port_composed_outputs = true`, `/tmp/anvil-tool-matrix-phase4-registered-mixed-r1/tool_matrix_report.json` records `saw_hierarchy_registered_mixed_support_routing = true`, and `/tmp/anvil-tool-matrix-phase4-registered-multistage-r1/tool_matrix_report.json` records `saw_hierarchy_registered_multistage_routing = true`; all have `coverage_gaps = []` with Verilator/Yosys skipped, and `r21` now carries those facts plus parent-cone helper-instance child-input routing through the full downstream-clean gate. The focused clean smokes at `/tmp/anvil-hier-parent-compose-smoke-r1/manifest.json`, `/tmp/anvil-hier-range-smoke-r1/manifest.json`, `/tmp/anvil-hier-depth-profile-smoke-r1/manifest.json`, `/tmp/anvil-hier-mixed-depth-smoke-r1/manifest.json`, `/tmp/anvil-hier-profiled-ondemand-smoke-r1/manifest.json`, `/tmp/anvil-hier-sibling-routing-smoke-r1/manifest.json`, `/tmp/anvil-hier-child-input-cone-smoke-r1/manifest.json`, `/tmp/anvil-hier-parent-state-smoke-r1/manifest.json`, `/tmp/anvil-hier-registered-sibling-smoke-r1/manifest.json`, `/tmp/anvil-hier-registered-child-input-cone-smoke-r2/manifest.json`, `/tmp/anvil-hier-parent-output-mix-smoke-r1/manifest.json`, `/tmp/anvil-hier-registered-mixed-child-input-smoke-r1/manifest.json`, and `/tmp/anvil-hier-registered-multistage-child-input-smoke-r1/manifest.json`, and `/tmp/anvil-parent-cone-instance-smoke-r1/manifest.json` remain useful targeted evidence. Broader helper-instance placement beyond the current child-input, parent-output, and per-parent-budget slices, broader registered hierarchy routing/composition patterns, and future hierarchy-aware identity remain open. |
...
- `src/metrics.rs` — 13 inline unit tests for empty-module, per-kind gate, flop-shape metrics, constant-vs-variable shift-rhs classification, and hierarchy design metrics for reuse, under-instantiation, parent-side composition, parent-cone helper-instance output support, budgeted parent-cone helper allocation, bounded recursive tree shape, per-depth branching profiles, and profiled on-demand interface realization.
...
- `src/bin/tool_matrix.rs` — 26 inline unit tests covering scenario-name uniqueness, full factorization-rung coverage, full construction-strategy coverage, coverage-gap detection, the Phase-1 / Phase-2 / Phase-3 / Phase-4 gate run-plan math, representative `share_prob`-sweep coverage, Phase-3 structured-surface coverage, the refreshed Phase-4 hierarchy coverage facts (wrapper and recursive depths, child-instance profiles, per-depth override profiles, reuse, under-instantiation, mixed parent-output coverage, parent-cone helper-output coverage, registered mixed-support routing coverage, multi-stage registered routing coverage, recursive fact derivation from `DesignMetrics`), design-level metrics/report embedding, design-level Yosys invocation shaping, legacy `.sv` bootstrap resume, same-binary generator-checkpoint resume for both module and design artifacts, `sv`-hash mismatch rejection, and legacy-checkpoint upgrade.
- `tests/pipeline.rs` — 49 integration tests covering cross-seed validity, reproducibility across strategies, motif sweeps, both constant- and variable-shift surfaces, the landed procedural case/casez/for-fold surfaces, the landed selectable `Slice` / `Concat` surface, the hierarchy surface (legacy depth-1 wrapper exact/reuse/under-instantiation plus bounded recursive tree shape, per-depth branching profiles, exact profiled on-demand child interfaces, sibling-routed child inputs, parent-composed child-input bindings, parent-cone helper-instance child-input bindings, parent-cone helper-instance parent-output composition, budgeted parent-cone helper allocation, local parent flops, registered sibling-routed child-input bindings, registered parent-composed child-input bindings, registered mixed-support child-input bindings, multi-stage registered parent-composed child-input bindings, mixed parent-port / child-output parent outputs, and module-name uniqueness across batched hierarchy designs), the first parent-side composition surface over child outputs, all live gate categories, zero-orphan / zero-duplicate-operand doctrine guards, input-surface finalisation, associative / constant-fold / peephole / compaction counters, and knob-roll telemetry.
- Current executed counts (`cargo test`, 2026-04-24): **217 unit-target tests + 49 integration tests = 266 passing tests**. Doc-tests: 0.
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  Phase 4 hierarchy gate is now closed too at `/tmp/anvil-tool-matrix-phase4-hierarchy-r21/tool_matrix_report.json`
  (132 designs, `coverage_gaps = []`, and 132/0 pass-fail in Verilator
  ...
  exact profiled child-interface synthesis, real parent-side composition above instance outputs, real sibling-routed hierarchy child inputs, real registered sibling-routed child inputs, real registered parent-composed child-input bindings, registered mixed-support child-input bindings, multi-stage registered parent-composed child-input bindings, real parent-composed child-input bindings, parent-cone helper-instance child-input bindings, real mixed parent-port / child-output parent outputs, and real local parent flops. Current HEAD adds focused parent-cone helper-instance parent-output composition and budgeted multi-helper allocation after that bank; `cargo test hierarchy_parent_outputs_can_depend_on_helper_instance_outputs` proves the output route through `top_outputs_reaching_parent_cone_instances` and `hierarchy_outputs_reaching_parent_cone_instances`, while `cargo test hierarchy_parent_cone_helper_budget_allows_multiple_helpers` proves `max_parent_cone_instances_per_internal_module = 3`. The Phase 4 scenario plan now includes dedicated `phase4_hier2_inst4_parent_output_cone_instance` and `phase4_hier2_inst4_parent_cone_instance_budget3` axes for the next full bank. The `r21` full downstream-clean report records
  ...
  `saw_hierarchy_parent_cone_instance_routing = true`, so the earlier
  ...
  `/tmp/anvil-hier-registered-child-input-cone-smoke-r2`, and
  `/tmp/anvil-parent-cone-instance-smoke-r1` remain useful targeted
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  proved numerically in focused smokes and the banked `r21` hierarchy
  ...
  `saw_hierarchy_parent_cone_instance_routing`. The next honest work
  is broader helper-instance placement beyond the current child-input,
...
- `cargo test` — clean (266 passing tests: 186 lib + 5 main + 26 tool_matrix + 49 integration).
- `cargo build` — clean.
