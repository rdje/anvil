...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  Phase 4 hierarchy gate is now closed too at `/tmp/anvil-tool-matrix-phase4-hierarchy-r17/tool_matrix_report.json`
  (108 designs, `coverage_gaps = []`, and 108/0 pass-fail in Verilator
  ...
  exact profiled child-interface synthesis, real parent-side composition above instance outputs, real sibling-routed hierarchy child inputs, real parent-composed child-input bindings, and real local parent flops. The focused smokes at
  ...
  `/tmp/anvil-hier-child-input-cone-smoke-r1`, and
  `/tmp/anvil-hier-parent-state-smoke-r1` remain useful targeted
  ...
  pre-parent-state bank, and the aborted `r8` rerun is historical
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  cone surface, optional local parent flops, and a first one-flop
  registered sibling-route surface, proved numerically in focused
  smokes and the banked `r17` hierarchy report. The next honest work is
  richer registered child-to-child routing where it is structurally
...
- `cargo test` — clean (258 passing tests: 184 lib + 5 main + 26 tool_matrix + 43 integration).
- `cargo build` — clean.
