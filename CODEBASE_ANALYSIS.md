...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   closed at `/tmp/anvil-tool-matrix-phase4-hierarchy-r44` with 372/0
...
| 4 — Hierarchy                | in progress  | `gen/hierarchy.rs`, `ir/types.rs`, `ir/compact.rs`, `ir/validate.rs`, `emit/sv.rs`, `main.rs`, `metrics.rs`, `bin/tool_matrix.rs` | Current live slice now has two lanes: the legacy exact depth-1 wrapper planner and the newer bounded recursive planner. The wrapper lane still exercises exact, reuse, and under-instantiation profiles via `num_child_instances`; the recursive lane uses `min_hierarchy_depth..=max_hierarchy_depth` plus `min_child_instances_per_module..=max_child_instances_per_module`, now keeping every leaf depth inside the requested interval and mixing shallow/deep branches when the interval is open and the structure allows it. Repeated `child_instances_per_depth` overrides are now also live and let branching be steered by parent depth (`0` = top, `1` = its children, ...) on top of that global fallback range. Both hierarchy lanes now also expose an explicit child-sourcing axis, `hierarchy_child_source_mode = library | on-demand`: `library` keeps reusable child-definition pools, while the currently-landed `on-demand` slice synthesizes children against parent-planned exact data-interface profiles per planned instance slot. Both lanes build parent-side output cones over the full parent source pool, then repair finalized parent outputs so every output retains child `InstanceOutput` support and can also carry parent data-port support when such ports are live. Both lanes expose direct sibling-routed child-input binding via `hierarchy_sibling_route_prob`, registered sibling-routed child-input binding through parent-local flops via `hierarchy_registered_sibling_route_prob` (including multi-stage chains through earlier parent-local Qs), registered parent-composed child-input binding through parent logic plus one parent-local flop via `hierarchy_registered_child_input_cone_prob`, parent-local child-input cones via `hierarchy_child_input_cone_prob` over parent data inputs / earlier sibling outputs / earlier parent-side route gates, optional local parent flops via `hierarchy_parent_flop_prob`, and parent-cone helper instances via `hierarchy_parent_cone_instance_prob` as internal sources for parent-composed child-input cones, direct sibling routes, direct registered sibling routes, registered child-input D cones, and parent-output cones. `max_parent_cone_instances_per_module` now controls the per-parent helper budget, and parent-output helper collection can spend multiple helper instances directly instead of sharing one optional helper source across the whole parent-output set; parent-output helper sources can also route through parent-local flops when `hierarchy_parent_flop_prob` is enabled, and parent-composed child-input helper sources can route through parent-local helper Qs while keeping the final child binding unregistered parent logic. The recursive lane now proves helper-through-state parent-composed routing, direct sibling helper routing, direct registered sibling helper routing, multi-stage direct registered sibling helper routing, registered parent-composed helper D-cone routing, multi-stage registered parent-composed helper routing, parent-output helper routing, stateful parent-output helper routing, and registered mixed-support routing below the top parent in exact-depth-2 trees. The registered parent-composed route now builds from the full parent source pool too, so it can mix parent data ports with sibling outputs when both supports are live, later routes can chain through earlier parent-local Qs, and helper outputs can be folded into the D cone when helper placement is enabled. Hierarchy manifests/reports now carry exact per-design `DesignMetrics`, so hierarchy quality is measurable without SV inspection (coverage, reuse, single-use-vs-multiuse child structure, exact profiled interface realization, child-input provenance including parent-flop, registered-instance-output, registered-parent-composed, registered-mixed-support, registered-multistage parent-composed, registered-multistage sibling-routed, registered parent-cone helper bindings, and registered-multistage parent-cone helper bindings, registered-multistage parent-composed helper bindings, parent-composed helper-through-state bindings, sibling-routing fractions, registered sibling-routing fractions, registered parent-composed fractions, registered mixed-support fractions, registered multistage parent-composed fractions, registered multistage sibling-route fractions, registered helper-route fractions, parent-composed child-input fractions, parent-flop child-input fractions, parent-cone helper-instance counts/fractions and per-parent maximum, parent-output helper-instance support counts/fractions, parent-output helper-through-flop counts/fractions, local parent-state counts, top-interface shape, direct-vs-composed outputs, parent-port-composed output counts/fractions, child-output dependency fractions/support, realized depth, depth histograms, leaf-depth histograms, branching histograms, per-depth branching summaries, control fanout, weighted child load). The emitted-boundary doctrine is explicit too: pure comb-only modules omit `clk` / `rst_n`, while hierarchy parents keep them visible iff they carry local state or sequential descendants. Module names are reserved from one generator-global sequence, so repeated hierarchy designs in one output directory cannot reuse the same definition filename. The latest full downstream-clean repo-owned Phase 4 bank is `/tmp/anvil-tool-matrix-phase4-hierarchy-r44/tool_matrix_report.json` (372 designs, `coverage_gaps = []`, 372/0 in Verilator and both repo-owned Yosys modes), and it covers wrapper exact/reuse/under-instantiation, recursive depth `2`, mixed recursive depth range `2:3`, explicit child-sourcing modes `library` and `on-demand`, child-instance profiles `2`, `4`, `2:3`, `1:3`, the per-depth override profile `0=4:4,1=2:2`, real mixed shallow/deep recursive realization, real on-demand child sourcing, exact profiled child-interface synthesis, real sibling-routed child inputs, real registered sibling-routed child inputs, real registered parent-composed child-input bindings, real registered mixed-support child-input bindings, recursive non-top registered mixed-support child-input bindings, real multi-stage registered parent-composed child-input bindings, real multi-stage registered sibling-routed child-input bindings, real parent-composed child-input bindings, real parent-cone helper-instance child-input bindings, parent-output helper-instance composition, recursive non-top parent-output helper routing, recursive non-top stateful parent-output helper routing, recursive non-top multi-helper budget evidence, recursive non-top child-input multi-helper budget evidence, recursive non-top stateful multi-helper budget evidence, stateful parent-output helper routing through parent-local flops, recursive non-top stateful parent-output helper routing through parent-local flops, stateful parent-composed helper child-input routing through parent-local flops, recursive non-top stateful parent-composed helper child-input routing through parent-local flops, recursive non-top direct sibling helper routing, recursive non-top direct registered sibling helper routing, recursive non-top multi-stage direct registered sibling helper routing, recursive non-top multi-stage registered parent-composed helper routing, recursive non-top registered parent-composed helper routing, budgeted multi-helper allocation, registered parent-composed helper-sourced child-input D cones, real local parent flops, real parent-side composition, real mixed parent-port / child-output parent outputs, direct sibling helper routing, direct registered sibling helper routing, multi-stage direct registered sibling helper routing, multi-stage registered parent-composed helper routing, and multi-stage registered sibling routing. The clean `r22` run is retained as root-cause evidence for the stale 126-design total-budget mismatch, `r23` is the historical pre-direct-helper full bank, `r24` is the historical coverage-only proof that first folded the direct helper scenarios into the 48-scenario policy, `r25` is the previous direct-helper full bank, `r26` is the previous multi-stage registered sibling bank, `r27` is the previous stateful parent-output helper bank, `r28` is the previous multi-stage direct registered sibling helper bank, `r29` is the previous multi-stage registered parent-composed helper bank, `r30` is the previous stateful parent-composed helper full bank, `r31` is the previous recursive helper-state full bank, `r32` is useful root-cause evidence for exact-selector `CaseMux` / `CasezMux` shift cleanup, `r33` is the pre-compact-normalization recursive direct-helper bank, `r34` is the previous recursive direct-helper full bank, `r35` is the previous recursive direct registered-helper full bank, `r36` is the previous recursive registered parent-composed helper full bank, `r37` is the previous recursive non-top multi-stage direct registered helper full bank, `r38` is the previous recursive non-top multi-stage registered parent-composed helper full bank, `r39` is the previous recursive non-top parent-output helper full bank, `r40` is the previous recursive non-top stateful parent-output helper full bank, `r41` is the previous recursive non-top parent-output multi-helper budget full bank, `r42` is the previous recursive non-top stateful multi-helper budget full bank, `r43` is the previous recursive non-top child-input multi-helper budget full bank, and `r44` is the latest recursive non-top registered mixed-support routing full bank. The focused clean smokes at `/tmp/anvil-hier-parent-compose-smoke-r1/manifest.json`, `/tmp/anvil-hier-range-smoke-r1/manifest.json`, `/tmp/anvil-hier-depth-profile-smoke-r1/manifest.json`, `/tmp/anvil-hier-mixed-depth-smoke-r1/manifest.json`, `/tmp/anvil-hier-profiled-ondemand-smoke-r1/manifest.json`, `/tmp/anvil-hier-sibling-routing-smoke-r1/manifest.json`, `/tmp/anvil-hier-child-input-cone-smoke-r1/manifest.json`, `/tmp/anvil-hier-parent-state-smoke-r1/manifest.json`, `/tmp/anvil-hier-registered-sibling-smoke-r1/manifest.json`, `/tmp/anvil-hier-registered-child-input-cone-smoke-r2/manifest.json`, `/tmp/anvil-hier-parent-output-mix-smoke-r1/manifest.json`, `/tmp/anvil-hier-registered-mixed-child-input-smoke-r1/manifest.json`, and `/tmp/anvil-hier-registered-multistage-child-input-smoke-r1/manifest.json`, and `/tmp/anvil-parent-cone-instance-smoke-r1/manifest.json`, plus `cargo test hierarchy_parent_composed_helper_routes_can_use_parent_flops`, `cargo test recursive_hierarchy_parent_composed_helper_routes_can_use_parent_flops_below_top`, `cargo test recursive_hierarchy_sibling_routes_can_use_helper_instances_below_top`, and `cargo test recursive_hierarchy_registered_sibling_routes_can_use_helper_instances_below_top`, `cargo test recursive_hierarchy_registered_sibling_routes_can_chain_helper_instances_below_top`, `cargo test recursive_hierarchy_registered_parent_composed_routes_can_chain_helper_instances_below_top`, `cargo test recursive_hierarchy_parent_outputs_can_depend_on_helper_instances_below_top`, `cargo test recursive_hierarchy_parent_outputs_can_spend_helper_budget_below_top`, `cargo test recursive_hierarchy_parent_cone_helper_budget_allows_multiple_helpers_below_top`, `cargo test recursive_hierarchy_parent_outputs_can_spend_stateful_helper_budget_below_top`, `cargo test recursive_hierarchy_parent_outputs_can_route_helper_instances_through_parent_flops_below_top`, `cargo test recursive_hierarchy_registered_mixed_support_routes_below_top`, and `cargo test recursive_hierarchy_registered_child_input_cones_can_use_helper_instances_below_top`, remain useful targeted evidence. Broader helper-instance placement beyond the current parent-composed child-input, stateful parent-composed child-input, recursive non-top stateful parent-composed child-input, recursive non-top direct sibling, recursive non-top direct registered sibling, recursive non-top multi-stage direct registered sibling, recursive non-top multi-stage registered parent-composed helper, recursive non-top registered parent-composed helper, recursive non-top parent-output helper, recursive non-top stateful parent-output helper, recursive non-top parent-output multi-helper budget, recursive non-top child-input multi-helper budget, recursive non-top stateful multi-helper budget, direct sibling, direct registered sibling, registered child-input, budgeted parent-output helper, stateful parent-output helper, multi-stage direct registered helper, and multi-stage registered parent-composed helper slices, broader registered hierarchy routing/composition patterns, and future hierarchy-aware identity remain open. |
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  (372 designs, `coverage_gaps = []`, and 372/0 pass-fail in Verilator
  ...
  exact profiled child-interface synthesis, real parent-side composition above instance outputs, real sibling-routed hierarchy child inputs, real registered sibling-routed child inputs, real registered parent-composed child-input bindings, registered mixed-support child-input bindings, recursive non-top registered mixed-support child-input bindings, multi-stage registered parent-composed child-input bindings, multi-stage registered sibling-routed child-input bindings, multi-stage direct registered sibling helper bindings, recursive non-top multi-stage direct registered sibling helper bindings, recursive non-top multi-stage registered parent-composed helper bindings, real parent-composed child-input bindings, parent-cone helper-instance child-input bindings, parent-output helper-instance composition, recursive non-top parent-output helper routing, recursive non-top stateful parent-output helper routing, recursive non-top parent-output multi-helper budget evidence, recursive non-top child-input multi-helper budget evidence, recursive non-top stateful multi-helper budget evidence, stateful parent-output helper routing through parent-local flops, recursive non-top stateful parent-output helper routing through parent-local flops, stateful parent-composed helper child-input routing through parent-local flops, recursive non-top stateful parent-composed helper child-input routing through parent-local flops, recursive non-top direct sibling helper routing, recursive non-top direct registered sibling helper routing, recursive non-top multi-stage direct registered sibling helper routing, recursive non-top multi-stage registered parent-composed helper routing, recursive non-top registered parent-composed helper routing, budgeted multi-helper allocation, registered parent-composed helper-sourced child-input D cones, real mixed parent-port / child-output parent outputs, and real local parent flops. The `r44` full downstream-clean report also records the direct sibling helper route, direct registered sibling helper route, stateful parent-output helper route, multi-stage direct registered sibling helper route, multi-stage registered parent-composed helper route, recursive non-top parent-output helper route, recursive non-top stateful parent-output helper route, recursive non-top parent-output multi-helper budget evidence, recursive non-top child-input multi-helper budget evidence, recursive non-top stateful multi-helper budget evidence, and recursive non-top registered mixed-support routing. The stale-total-budget `r22` run is clean but insufficient root-cause evidence at 126 designs; `r23` is the historical pre-direct-helper full bank; `r24` is the historical coverage-only direct-helper policy proof; `r25` is the previous direct-helper full bank, `r26` is the previous multi-stage registered sibling bank, `r27` is the previous stateful parent-output helper bank, `r28` is the previous multi-stage direct registered sibling helper bank, `r29` is the previous multi-stage registered parent-composed helper bank, `r30` is the previous stateful parent-composed helper full bank, `r31` is the previous recursive helper-state full bank, `r32` is root-cause evidence for the exact-selector `CaseMux` / `CasezMux` shift-cleanup fix, `r33` is the pre-compact-normalization recursive direct-helper bank, `r34` is the previous recursive direct-helper full bank, `r35` is the previous recursive direct registered-helper full bank, `r36` is the previous recursive registered parent-composed helper full bank, `r37` is the previous recursive non-top multi-stage direct registered helper full bank, `r38` is the previous recursive non-top multi-stage registered parent-composed helper full bank, `r39` is the previous recursive non-top parent-output helper full bank, `r40` is the previous recursive non-top stateful parent-output helper full bank, `r41` is the previous recursive non-top parent-output multi-helper budget full bank, `r42` is the previous recursive non-top stateful multi-helper budget full bank, `r43` is the previous recursive non-top child-input multi-helper budget full bank, and `r44` is the latest recursive non-top registered mixed-support routing full bank. The `r44` full downstream-clean report records
  ...
  and
  `saw_recursive_hierarchy_parent_composed_parent_cone_instance_flop_routing = true`, so the earlier
  ...
  are now historical policy breadcrumbs because the full `r44` bank
  ...
  `cargo test recursive_hierarchy_registered_sibling_routes_can_chain_helper_instances_below_top`, and
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  hierarchy. Direct sibling helper routing, direct registered sibling
  helper routing, multi-stage direct registered sibling helper routing,
  registered parent-composed helper D-cone routing, parent-output helper
  routing, stateful parent-output helper routing, and multi-helper budget evidence are also proved below the top parent in the recursive
  exact-depth-2 lane. These surfaces are proved
  numerically in focused smokes and the full downstream-clean `r44`
  hierarchy bank. The `r44` bank requires
  ...
  `saw_hierarchy_registered_multistage_sibling_routing`, and
  ...
  `saw_recursive_hierarchy_registered_parent_composed_parent_cone_instance_routing`, and
...
- `cargo test` — clean (294 passing tests: 195 lib + 5 main + 26 tool_matrix + 68 integration).
- `cargo build` — clean.
