...
3. **Tool-clean confidence is still under-automated**
   ...
   `/tmp/anvil-tool-matrix-phase1-real-r21` already banked through 500
   completed checkpoints / 501 emitted files with zero warning
   ...
   saved-`sv` hash still match, and the latest focused current-code
   `e-graph` proof
...
│                     (`r21`) at 500 completed checkpoints / 501
...
│                     newer fast-resume metadata only exists on
│                     checkpoints written by the current binary, so
│                     pre-upgrade trees still pay the one-time strict
│                     replay cost before they become fast-resumable.
│
├── ir/
