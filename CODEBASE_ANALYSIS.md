...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   tree at `anvil-tool-matrix-phase1-real-r21` with a final
   ...
   is also closed at `anvil-tool-matrix-phase3-structured-r4`,
   and the latest full downstream-clean Phase 4 hierarchy gate is
   closed at `anvil-tool-matrix-phase4-hierarchy-r87` with 840/0
   ...
   Banked downstream-clean at `anvil-generate-loop-gate-r1` (3
   ...
   run. Banked downstream-clean at `anvil-task-emit-gate-r1` (3
   ...
   downstream-clean at `anvil-cone-function-gate-r1` (3 scenarios, 12
   ...
   downstream-clean at `anvil-multi-output-task-gate-r1` (3 scenarios, 12
...
│                     (`anvil-fe-r2/`). Repo-owned gate + coverage
│                     fact = `.2b.2`.
│
...
│                     (`anvil-gl-r1/`). Repo-owned gate + coverage fact
│                     = `.4b.2`.
│
...
│                     both modes + Icarus (`anvil-te-r1/`). Metric +
...
│                     Verilator `--lint-only` (+`-Wall` Δ=0 vs OFF) + Yosys
│                     both modes + Icarus (`anvil-cf-sweep/`). Metric +
...
│                     (`anvil-mo-sweep/`). Metric + repo-owned gate +
│                     coverage fact = `.12b.2`.
│
...
│                     is sim-equiv over 20000 vectors (`anvil-muxif-genproof.*`).
...
│                     `anvil-casez-mux-if-gate-r1`: 3 scenarios / 12 modules /
...
│                     (`SEMANTIC-INTROSPECTION-EXPANSION.11b.2`), and
│                     1.23→1.24 the eleventh `instance_input_bindings`
...
│                     (`SEMANTIC-INTROSPECTION-EXPANSION.14b.2`), and
│                     1.26→1.27 the fourteenth `reach_path` analyze query
...
│                     ascending child PortId). Wired to the MCP surface in `.12b.2`: in
│                     `supported_query_kinds()` + the `run_analyze` dispatch
│                     (`module`/`design_instance_input_bindings`) + the `analyze_schema`
...
│                     deterministic tie-break proof). Wired to the MCP surface in `.15b.2`: in
│                     `supported_query_kinds()` + the `run_analyze` dispatch
│                     (`module`/`design_reach_path`) in both arms + the vec-aware `-32602` guard +
│                     the `analyze_schema` enum + schema `1.26 → 1.27`; 2 mcp proofs (incl. a live
│                     reach_path↔node_reach cross-check). DUT byte-identical.
...
│                     `.6b.2`/`.7b.2`/`.8b.2`/`.9b.2` adding the
│                     `flop_dependencies`/`memory_provenance`/`fsm_provenance`/`node_drivers`
│                     branches (`{module,design}_{flop_dependencies,memory_provenance,
│                     fsm_provenance,node_drivers}`, schema → `1.21`); the
...
│                     (`anvil-tool-matrix-phase2-share-r1`) adds
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  exists too at `anvil-hierarchy-smoke-r1`, clean in Verilator,
  ...
  exists too at `anvil-hier-parent-compose-smoke-r1`, clean in the
  ...
  and `anvil-tool-matrix-phase4-registered-multistage-r1/tool_matrix_report.json`
  ...
  `anvil-hier-range-smoke-r1`, and
  `anvil-hier-depth-profile-smoke-r1`, and
  `anvil-hier-mixed-depth-smoke-r1`, and
  `anvil-hier-profiled-ondemand-smoke-r1`, and
  ...
  `anvil-parent-cone-instance-smoke-r1`, and
  ...
  proofs, while the old `anvil-tool-matrix-phase4-hierarchy-r7`
...
- `scripts/check_doctrines.sh` — the doctrine-enforcement registry+driver
  ...
  `BOOK-LINK-TARGETS` → `scripts/check_book_link_targets.sh`<!--/enum:doctrine-ids-->). The driver
...
- Generator-output smoke: focused current default `tool_matrix`
  ...
  including `anvil-tool-matrix-phase1-real-r21` (1005/0 in
