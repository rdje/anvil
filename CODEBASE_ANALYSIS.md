...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   closed at `/tmp/anvil-tool-matrix-phase4-hierarchy-r65` with 576/0
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  `/tmp/anvil-tool-matrix-phase4-hierarchy-r65/tool_matrix_report.json`
  (576 designs, `coverage_gaps = []`, and 576/0 pass-fail in Verilator
  ...
  are now historical policy breadcrumbs because the full `r65` bank
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  numerically in focused smokes and the full downstream-clean `r65`
  hierarchy bank. The `r65` bank requires
