...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  exact profiled child-interface synthesis, real parent-side composition above instance outputs, real sibling-routed hierarchy child inputs, real registered sibling-routed child inputs, real registered parent-composed child-input bindings, real parent-composed child-input bindings, and real local parent flops. A current-code coverage-only probe at
  `/tmp/anvil-tool-matrix-phase4-parent-port-coverage-r1/tool_matrix_report.json`
  also records `coverage_gaps = []` and
  `saw_hierarchy_parent_port_composed_outputs = true`, proving the
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  parent outputs. The first five are proved numerically in focused
  smokes and the banked `r19` hierarchy report; the mixed parent-output
  slice is covered by a focused integration regression and a
  coverage-only Phase 4 matrix probe that now requires
  `saw_hierarchy_parent_port_composed_outputs`. The next honest
  work is first-class module instantiation inside parent cone choice,
  richer registered hierarchy routing/composition where it is
