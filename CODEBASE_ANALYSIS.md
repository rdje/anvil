...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   closed at `/tmp/anvil-tool-matrix-phase4-hierarchy-r55` with 456/0
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  (456 designs, `coverage_gaps = []`, and 456/0 pass-fail in Verilator
  ...
  are now historical policy breadcrumbs because the full `r55` bank
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  numerically in focused smokes and the full downstream-clean `r55`
  hierarchy bank. The `r55` bank requires
