...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  exact profiled child-interface synthesis, real parent-side composition above instance outputs, real sibling-routed hierarchy child inputs, real registered sibling-routed child inputs, real registered parent-composed child-input bindings, registered mixed-support child-input bindings, multi-stage registered parent-composed child-input bindings, real parent-composed child-input bindings, parent-cone helper-instance child-input bindings, real mixed parent-port / child-output parent outputs, and real local parent flops. Current HEAD adds focused parent-cone helper-instance parent-output composition, budgeted multi-helper allocation, and registered helper-sourced child-input D cones after that bank; `cargo test hierarchy_parent_outputs_can_depend_on_helper_instance_outputs` proves the output route through `top_outputs_reaching_parent_cone_instances` and `hierarchy_outputs_reaching_parent_cone_instances`, `cargo test hierarchy_parent_cone_helper_budget_allows_multiple_helpers` proves `max_parent_cone_instances_per_internal_module = 3`, and `cargo test hierarchy_registered_child_input_cones_can_use_helper_instances` proves `child_input_bindings_from_registered_parent_cone_instances > 0`. The Phase 4 scenario plan now includes dedicated `phase4_hier2_inst4_parent_output_cone_instance`, `phase4_hier2_inst4_parent_cone_instance_budget3`, and `phase4_hier2_inst4_registered_parent_cone_instance_state` axes for the next full bank. The `r21` full downstream-clean report records
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  is broader helper-instance placement beyond the current unregistered
  child-input, registered child-input, parent-output, and
  per-parent-budget slices, broader registered
...
- `cargo test` — clean (268 passing tests: 187 lib + 5 main + 26 tool_matrix + 50 integration).
- `cargo build` — clean.
