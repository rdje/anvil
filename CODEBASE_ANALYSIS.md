...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  Phase 4 hierarchy gate is now closed too at `/tmp/anvil-tool-matrix-phase4-hierarchy-r19/tool_matrix_report.json`
  (120 designs, `coverage_gaps = []`, and 120/0 pass-fail in Verilator
  ...
  exact profiled child-interface synthesis, real parent-side composition above instance outputs, real sibling-routed hierarchy child inputs, real registered sibling-routed child inputs, real registered parent-composed child-input bindings, registered mixed-support child-input bindings, multi-stage registered parent-composed child-input bindings, real parent-composed child-input bindings, and real local parent flops. Current-code coverage-only probes at
  `/tmp/anvil-tool-matrix-phase4-parent-port-coverage-r1/tool_matrix_report.json`
  and `/tmp/anvil-tool-matrix-phase4-registered-mixed-r1/tool_matrix_report.json`
  and `/tmp/anvil-tool-matrix-phase4-registered-multistage-r1/tool_matrix_report.json`
  also record `coverage_gaps = []`,
  `saw_hierarchy_parent_port_composed_outputs = true`, and
  `saw_hierarchy_registered_mixed_support_routing = true`, and
  `saw_hierarchy_registered_multistage_routing = true`, proving the
  Phase 4 coverage policy now requires mixed parent-output composition,
  registered mixed-support routing, and multi-stage registered routing.
  ...
  `/tmp/anvil-hier-registered-sibling-smoke-r1`, and
  `/tmp/anvil-hier-registered-child-input-cone-smoke-r2` remain useful targeted
  ...
  is the first registered-parent-composed bank, and the aborted `r8`
  rerun is historical
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  cone surface, optional local parent flops, a first one-flop
  ...
  outputs and chain through earlier parent-local Qs. The first five are
  proved numerically in focused smokes and the banked `r19` hierarchy
  report; the mixed parent-output, registered mixed-support, and
  multi-stage registered slices are covered by focused regressions and
  coverage-only Phase 4 matrix probes that now require
  `saw_hierarchy_parent_port_composed_outputs`,
  `saw_hierarchy_registered_mixed_support_routing`, and
  `saw_hierarchy_registered_multistage_routing`. The next honest work
  is first-class module instantiation inside parent cone choice,
