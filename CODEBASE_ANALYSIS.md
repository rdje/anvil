...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   closed at `/tmp/anvil-tool-matrix-phase4-hierarchy-r75` with 696/0
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  (696 designs, `coverage_gaps = []`, and 696/0 pass-fail in Verilator
  ...
  are now historical policy breadcrumbs because the full `r75` bank
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  numerically in focused smokes and the full downstream-clean `r75`
  hierarchy bank. The `r75` bank requires
