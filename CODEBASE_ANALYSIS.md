...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  exact profiled child-interface synthesis, real parent-side composition above instance outputs, real sibling-routed hierarchy child inputs, real registered sibling-routed child inputs, real registered parent-composed child-input bindings, registered mixed-support child-input bindings, real parent-composed child-input bindings, and real local parent flops. Current-code coverage-only probes at
  ...
  and registered mixed-support routing.
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  outputs. The first five are proved numerically in focused smokes and
  the banked `r19` hierarchy report; the mixed parent-output and
  registered mixed-support slices are covered by focused regressions
  and coverage-only Phase 4 matrix probes that now require
  `saw_hierarchy_parent_port_composed_outputs` and
  `saw_hierarchy_registered_mixed_support_routing`. The next honest
  work is first-class module instantiation inside parent cone choice,
  richer multi-stage registered hierarchy routing/composition where it
  is structurally warranted, and future hierarchy-aware identity.
