...
1. **Feature breadth / legal surface area / artifact-family breadth**
   ...
   now landed, but the phase is still not closed because the structured
   surface does not yet have a dedicated closure gate like Phase 1 /
   Phase 2. Hierarchy, parameterization,
...
- `tests/pipeline.rs` — 29 integration tests covering cross-seed validity, reproducibility across strategies, motif sweeps, both constant- and variable-shift surfaces, the landed procedural case/casez/for-fold surfaces, the landed selectable `Slice` / `Concat` surface, all live gate categories, zero-orphan / zero-duplicate-operand doctrine guards, input-surface finalisation, associative / constant-fold / peephole / compaction counters, and knob-roll telemetry.
- Current executed counts (`cargo test`, 2026-04-22): **162 unit-target tests + 29 integration tests = 191 passing tests**. Doc-tests: 0.
...
- `cargo test` — clean (191 passing tests: 142 lib + 5 main + 15 tool_matrix + 29 integration).
- `cargo build` — clean.
