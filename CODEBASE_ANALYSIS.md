...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   closed at `/tmp/anvil-tool-matrix-phase4-hierarchy-r53` with 432/0
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  (432 designs, `coverage_gaps = []`, and 432/0 pass-fail in Verilator
  ...
  are now historical policy breadcrumbs because the full `r53` bank
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  numerically in focused smokes and the full downstream-clean `r53`
  hierarchy bank. The `r53` bank requires
...
- `cargo test` — clean (306 passing tests: 197 lib + 5 main + 26 tool_matrix + 78 integration).
- `cargo build` — clean.
