...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   closed at `/tmp/anvil-tool-matrix-phase4-hierarchy-r51` with 408/0
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  (408 designs, `coverage_gaps = []`, and 408/0 pass-fail in Verilator
  ...
  are now historical policy breadcrumbs because the full `r51` bank
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  numerically in focused smokes and the full downstream-clean `r51`
  hierarchy bank. The `r51` bank requires
...
- `cargo test` — clean (302 passing tests: 196 lib + 5 main + 26 tool_matrix + 75 integration).
- `cargo build` — clean.
