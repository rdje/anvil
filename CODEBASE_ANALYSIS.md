...
- `tests/pipeline.rs` — 39 integration tests covering cross-seed validity, reproducibility across strategies, motif sweeps, both constant- and variable-shift surfaces, the landed procedural case/casez/for-fold surfaces, the landed selectable `Slice` / `Concat` surface, the hierarchy surface (legacy depth-1 wrapper exact/reuse/under-instantiation plus bounded recursive tree shape, per-depth branching profiles, and exact profiled on-demand child interfaces), the first parent-side composition surface over child outputs, all live gate categories, zero-orphan / zero-duplicate-operand doctrine guards, input-surface finalisation, associative / constant-fold / peephole / compaction counters, and knob-roll telemetry.
- Current executed counts (`cargo test`, 2026-04-23): **212 unit-target tests + 39 integration tests = 251 passing tests**. Doc-tests: 0.
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  Phase 4 hierarchy gate is now closed too at `/tmp/anvil-tool-matrix-phase4-hierarchy-r12/tool_matrix_report.json`
  ...
  exact profiled child-interface synthesis, and real parent-side composition above instance outputs. The focused smokes at
  ...
  `/tmp/anvil-hier-profiled-ondemand-smoke-r1` remain useful targeted
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  state, richer hierarchy-local routing/composition, and future
...
- `cargo test` — clean (251 passing tests: 181 lib + 5 main + 26 tool_matrix + 39 integration).
- `cargo build` — clean.
