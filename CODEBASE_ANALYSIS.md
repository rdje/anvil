...
3. **Downstream-acceptance confidence still needs broader automation beyond current phase gates**
   ...
   closed at `/tmp/anvil-tool-matrix-phase4-hierarchy-r47` with 396/0
...
- No external Verilator / Yosys smoke tests are wired into `cargo test`
  ...
  are now historical policy breadcrumbs because the full `r47` bank
...
- Phase 4 is no longer only the first depth-1 slice. The legacy exact
  ...
  numerically in focused smokes and the full downstream-clean `r47`
  hierarchy bank. The `r47` bank requires
...
- `cargo test` — clean (299 passing tests: 195 lib + 5 main + 26 tool_matrix + 73 integration).
- `cargo build` — clean.
