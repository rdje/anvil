...
| `NO-BOOT-VOLUME-REFS` | structural | `scripts/check_no_boot_volume_refs.sh` | no tracked file stores on or points at the boot volume (`/tmp`, `/private/tmp`, `/var/folders`), and `std::env::temp_dir()` is named in exactly one file — `src/paths.rs`, the runtime resolver (decision `0031`). **Its allow-list is load-bearing:** policy documents keep the strings they forbid (a doctrine cannot state what it prohibits without naming it), and `CHANGES.md` / `DEVELOPMENT_NOTES.md` are append-only history the owner has directed must stay raw — a check that flagged them would pressure authors into the history rewrite the project forbids |
