...
| `CODE-CHANGE-EVIDENCE` | evidence (scope-aware) | `scripts/check_diagnosis_evidence.sh` | a staged code change co-stages the mandatory live-doc evidence (`CHANGES.md`) per `COMMIT.md`; pure non-code commits exempt. **`MEMORY.md` was dropped from this assertion at `RESUME-POINTER-COMMIT-PATH-COUPLING.2` (decision `0051`), and the reason is a general one about evidence checks.** Requiring it here made `MEMORY-ARCH`'s **fail-closed** 6,144 B cap mandatory on the path **820 of 840 (97.6 %)** commits took; when that cap fired, the remedy taken was the **prose trim its own routing hint forbids**, recorded as compliance, with no check comparing remedy-taken against remedy-prescribed. But frequency was never the argument ([[gate-frequency-is-not-evidence]] stands). The fault is that the assertion was **evidence-shaped without being evidence**: `CHANGES.md` is per-*change* — one entry describing this commit, so staging it is a claim that can be false — while `MEMORY.md` is a `bounded_snapshot` of where the *work* stopped. A commit that changes no resumable state has nothing true to write in it, so the requirement could only ever be discharged by a no-op diff in an overwrite-only file: **the cheapest possible box to tick, and therefore §6.1's self-tick rather than proof.** The rule that generalizes: *assert co-staging only where the artifact's content is a function of the diff.* `MEMORY.md` keeps `MEMORY-ARCH` for size, shape and required fields — one mechanism per job (`feedback_full_factorization`) |
