...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r74`. Its final
`tool_matrix_report.json` records:
...
The `r74` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r74` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `171` scenarios, `684` total designs,
  ...
  `r74` is the current bank that extends the depth-6 axis with recursive non-top mixed-support child inputs at exact hierarchy depth 6 without helpers,
