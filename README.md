...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r66`. Its final
`tool_matrix_report.json` records:
...
The `r66` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r66` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `147` scenarios, `588` total designs,
  ...
  `r66` is the current bank that extends the depth-4 axis with recursive non-top stateful parent-port-composed parent outputs at exact hierarchy depth 4 without helpers,
