...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r68`. Its final
`tool_matrix_report.json` records:
...
The `r68` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r68` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `153` scenarios, `612` total designs,
  ...
  `r68` is the current bank that opens the depth-5 axis with recursive non-top parent-local flops at exact hierarchy depth 5,
