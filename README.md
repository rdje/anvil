...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r80`. Its final
`tool_matrix_report.json` records:
...
The `r80` report is the latest fully banked downstream-clean repo-owned
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `189` scenarios, `756` total designs,
  ...
  `r80` is the current bank that extends the depth-7 axis with recursive non-top parent-port-composed parent outputs at exact hierarchy depth 7 without helpers or state,
