...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r70`. Its final
`tool_matrix_report.json` records:
...
The `r70` report is the latest fully banked downstream-clean repo-owned
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `159` scenarios, `636` total designs,
  ...
  `r70` is the current bank that extends the depth-5 axis with recursive non-top parent-port-composed parent outputs at exact hierarchy depth 5 without helpers or state,
