...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r82`. Its final
`tool_matrix_report.json` records:
...
- `coverage_gaps = []`
- `Verilator pass/fail = 780/0`
- `Yosys without-abc pass/fail = 780/0`
- `Yosys with-abc pass/fail = 780/0`
...
The `r82` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r82` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `195` scenarios, `780` total designs,
  ...
  `r82` is the current bank that closes the depth-7 sweep with recursive non-top stateful unregistered parent-composed mixed-support child inputs at exact hierarchy depth 7 without helpers (2,2 calibrated),
