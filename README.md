...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r56`. Its final
`tool_matrix_report.json` records:
...
The `r56` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r56` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `117` scenarios, `468` total designs,
  ...
  `r56` is the current recursive stateful parent-composed mixed-support child-input hierarchy bank,
