...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r83`. Its final
`tool_matrix_report.json` records:
...
- `coverage_gaps = []`
- `Verilator pass/fail = 792/0`
- `Yosys without-abc pass/fail = 792/0`
- `Yosys with-abc pass/fail = 792/0`
...
The `r83` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r83` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `198` scenarios, `792` total designs,
  ...
  `r83` is the current bank that proves recursive non-top registered parent-composed child-input bindings can chain through three parent-local flop stages without helpers,
