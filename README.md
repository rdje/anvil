...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r67`. Its final
`tool_matrix_report.json` records:
...
The `r67` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r67` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `150` scenarios, `600` total designs,
  ...
  `r67` is the current bank that closes the depth-4 sweep with recursive non-top stateful parent-composed mixed-support child inputs at exact hierarchy depth 4 without helpers,
