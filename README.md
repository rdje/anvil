...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r57`. Its final
`tool_matrix_report.json` records:
...
The `r57` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r57` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `120` scenarios, `480` total designs,
  ...
  `r57` is the current bank that gates recursive non-top parent-local flops as first-class coverage,
