...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r69`. Its final
`tool_matrix_report.json` records:
...
The `r69` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r69` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `156` scenarios, `624` total designs,
  ...
  `r69` is the current bank that extends the depth-5 axis with recursive non-top mixed-support child inputs at exact hierarchy depth 5 without helpers,
