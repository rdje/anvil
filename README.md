...
- `anvil --sv-version <2012|2017|2023>` is the opt-in IEEE 1800 emission-target capability gate (`SV-VERSION-TARGETING.2b.1`, decision `0009`). Default `2012` is the honest floor: ANVIL's current emitted subset is 1800-2012-valid, so the default reproduces today's output byte-for-byte (`tests/snapshots.rs` untouched, and all three targets are byte-identical over the current subset). It is a **down-gating guarantee** — the emitter never emits a construct newer than the target — threaded into the emitter as a read-only capability bound (`SvVersion::permits`). Surfaced in `--dump-config` and `--introspect` (introspection schema MINOR-bumped `1.1`→`1.2`). Deliberately emitting a higher standard's distinctive synthesizable constructs (**up-opting**) lands as `SV-VERSION-TARGETING.3`; the per-version downstream acceptance axis lands as `.2b.2`. Orthogonal to `--identity-mode` / `--factorization-level`.
