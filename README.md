...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r84`. Its final
`tool_matrix_report.json` records:
...
- `coverage_gaps = []`
- `Verilator pass/fail = 804/0`
- `Yosys without-abc pass/fail = 804/0`
- `Yosys with-abc pass/fail = 804/0`
...
The `r84` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r84` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `201` scenarios, `804` total designs,
  ...
  `r84` is the current bank that proves a recursive non-top internal parent can saturate a parent-cone helper budget of 5 helpers,
