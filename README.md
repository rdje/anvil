...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r58`. Its final
`tool_matrix_report.json` records:
...
The `r58` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r58` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `123` scenarios, `492` total designs,
  ...
  `r58` is the current bank that pushes recursive parent-local flops to exact hierarchy depth 3,
