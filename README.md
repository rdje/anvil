...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r64`. Its final
`tool_matrix_report.json` records:
...
The `r64` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r64` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `141` scenarios, `564` total designs,
  ...
  `r64` is the current bank that extends the depth-4 axis with recursive non-top mixed-support child inputs at exact hierarchy depth 4 without helpers,
