...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r60`. Its final
`tool_matrix_report.json` records:
...
The `r60` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r60` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `129` scenarios, `516` total designs,
  ...
  `r60` is the current bank that pushes recursive non-top parent-port-composed parent outputs to exact hierarchy depth 3 without helpers or state,
