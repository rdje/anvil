...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r81`. Its final
`tool_matrix_report.json` records:
...
- `coverage_gaps = []`
- `Verilator pass/fail = 768/0`
- `Yosys without-abc pass/fail = 768/0`
- `Yosys with-abc pass/fail = 768/0`
...
The `r81` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r81` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `192` scenarios, `768` total designs,
  ...
  `r81` is the current bank that extends the depth-7 axis with recursive non-top stateful parent-port-composed parent outputs at exact hierarchy depth 7 without helpers,
