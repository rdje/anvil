...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r59`. Its final
`tool_matrix_report.json` records:
...
The `r59` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r59` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `126` scenarios, `504` total designs,
  ...
  `r59` is the current bank that pushes recursive non-top mixed-support child inputs to exact hierarchy depth 3 without helpers,
