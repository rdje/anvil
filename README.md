...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r73`. Its final
`tool_matrix_report.json` records:
...
The `r73` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r73` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `168` scenarios, `672` total designs,
  ...
  `r73` is the current bank that opens the depth-6 axis with recursive non-top parent-local flops at exact hierarchy depth 6,
