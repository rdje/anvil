...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r76`. Its final
`tool_matrix_report.json` records:
...
- `coverage_gaps = []`
- `Verilator pass/fail = 708/0`
- `Yosys without-abc pass/fail = 708/0`
- `Yosys with-abc pass/fail = 708/0`
...
The `r76` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r76` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `177` scenarios, `708` total designs,
  ...
  `r76` is the current bank that extends the depth-6 axis with recursive non-top stateful parent-port-composed parent outputs at exact hierarchy depth 6 without helpers,
