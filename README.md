...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r65`. Its final
`tool_matrix_report.json` records:
...
The `r65` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r65` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `144` scenarios, `576` total designs,
  ...
  `r65` is the current bank that extends the depth-4 axis with recursive non-top parent-port-composed parent outputs at exact hierarchy depth 4 without helpers or state,
