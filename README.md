...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r75`. Its final
`tool_matrix_report.json` records:
...
- `coverage_gaps = []`
- `Verilator pass/fail = 696/0`
- `Yosys without-abc pass/fail = 696/0`
- `Yosys with-abc pass/fail = 696/0`
...
The `r75` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r75` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `174` scenarios, `696` total designs,
  ...
  `r75` is the current bank that extends the depth-6 axis with recursive non-top parent-port-composed parent outputs at exact hierarchy depth 6 without helpers or state,
