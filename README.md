...
exists at `/tmp/anvil-tool-matrix-phase4-hierarchy-r63`. Its final
`tool_matrix_report.json` records:
...
The `r63` report is the latest fully banked downstream-clean repo-owned
...
are superseded by the full downstream-clean `r63` bank.
...
- Current scope: single-module combinational **and sequential**
  ...
  `tool_matrix`, with `138` scenarios, `552` total designs,
  ...
  `r63` is the current bank that opens the depth-4 axis with recursive non-top parent-local flops at exact hierarchy depth 4,
