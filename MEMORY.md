...
- Latest full downstream-clean Phase 4 hierarchy bank is:
  ...
  stateful parent-output helper routing.
  Focused regressions:
...
- **Last completed slice:** Banked recursive non-top stateful
  parent-output helper routing through full downstream
  tools at
  `/tmp/anvil-tool-matrix-phase4-hierarchy-r40/tool_matrix_report.json`
  ...
  registered parent-composed helper bank, and the `r39` recursive
  non-top parent-output helper bank.
...
- **Prior slice:** Landed registered parent-composed child-input
  ...
  The current full `r40` gate supersedes that historical report with
