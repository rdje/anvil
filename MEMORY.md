...
- **Active task trees:**
  ...
  - `INSTA-SNAPSHOTS` — **`.1`+`.2` done** (`2026-05-18`): `insta` pinned `=1.47.2` (Cargo.lock unchanged); `tests/snapshots.rs` now **6 fully-deterministic byte-stable shapes** — canonical leaf / bounded recursive library / bounded recursive on-demand / sibling-route (helper-instance) / parent-composed route / dedup-canonical-signatures (`hierarchy_module_dedup=true`, so `HIERARCHY-AWARE-IDENTITY` dedup drift breaks a snapshot). All proven from `tests/pipeline.rs` shapes; generated via `INSTA_UPDATE=always`, re-verified stable; full `cargo test` green, no regression. Frontier → `.3` (`COMMIT.md` checklist + book acceptance-protocol — closes the tree; docs/workflow only). Quality lane: reproducibility regressions.
