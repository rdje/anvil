...
- **Active task trees:**
  ...
  - **Every remaining roadmap phase is now task-tree-tracked (2026-05-16 owner directive):**
    ...
    - `PHASE-5-PARAMETERIZATION` — **active PNT lane.** `.1` design **done** (architecture (C); (A)/(B)/(C') rejected — DEVELOPMENT_NOTES.md "Phase 5 parameterization design"). `.2.1` scaffold **done**: `WidthExpr{Lit,Param}`+`ParamEnv`, additive `Module` annotation fields, opt-in `Config::width_parameterization_prob` (default 0.0), `src/ir/param.rs` annotation-only post-construction pass (5 unit tests), emitter `#(parameter int W=D)` + `[W-1:0]`, focused round-trip/default-off proof. Architecture (C) confirmed implementable with zero changes to the invasive width math. `.2.2.1` soundness primitives **done** (`is_width_generic` gate + fully-width-generic emitter; param.rs 6/0; default-off byte-identical). **Rules-first pivot found:** a 64-seed sweep produced zero organically width-homogeneous modules → post-hoc filtering is inert + generate-then-filter; `.2.2` split into `.2.2.1` (done), `.2.2.2` (rules-first parameterizable-leaf **constructor** — makes the feature fire; **frontier**), `.2.2.3` (instantiation substitution + `Instance.param_bindings` + resolved-width validate). Then `.2.3` parameter-aware identity, `.2.4` matrix gate + Phase 5 closure.
