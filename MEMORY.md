...
## Recent commits
- `<pending>` — PHASE-6-ADVANCED-MOTIFS.3.3: cargo-portable proof `fsm_block_matches_probed_template_and_is_factorization_opaque` — 4 `ConstructionStrategy` × 4 `FactorizationLevel` (incl. EGraph) × 6 seeds (96 designs): 1 `FsmOut`/0 `Gate` survives every factorization level (FSM never enters NodeId graph — CSE/EGraph-opaque); emitted state constants are EXACTLY the chosen `FsmEncoding`'s (`state_width`/`state_const`) + exact async-low-reset state register + sel-selected next-state/Moore cases; all 3 encodings reachable (deterministic seeds 0..6). Proof-only (no `src/`); full `cargo` gate green. Frontier → `.3.4` (last Phase 6 leaf) (‖ `.2.4` gate-blocked).
