...
- **Last completed slice:** Emit `{N{expr}}` replication for same-operand Concat. See `CHANGES.md` entry `2026-04-16-0036`. `render_gate` for `Concat` detects all-operands-identical and emits the canonical SV replication form instead of the flat list. Clean-up triggered by user seeing `{eq_0, eq_0, … × 22}` in seed-42 output; now reads `{22{eq_0}}`. Semantics unchanged. Emitter unit test updated. 47 tests pass.
