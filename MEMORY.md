...
- **Active task trees:**
  ...
  - **Every remaining roadmap phase is now task-tree-tracked (2026-05-16 owner directive):**
    ...
    - `PHASE-5-PARAMETERIZATION` — **active PNT lane.** `.1` design **done** (architecture (C) post-construction parameterization pass + monomorphic instantiation chosen; (A)/(B)/(C') rejected — see DEVELOPMENT_NOTES.md "Phase 5 parameterization design"). Phase 4 `done` unblocked `.2`, split into `.2.1` IR+emit scaffold → `.2.2` instantiation substitution → `.2.3` parameter-aware identity → `.2.4` matrix gate + Phase 5 closure. Frontier `.2.1`.
