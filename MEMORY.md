...
- next_action: **`IR-TYPES-DECOMPOSITION.3`** — **owner redirect `2026-08-01`: *"return to real coding task… we cleaned enough."*** Live-doc/doctrine work is **PAUSED, not abandoned**: `OVERFLOW-DESTINATION-INSTRUMENTATION` stays `active` with `.9` (separate the index journal, `0042`) and `.4` (portable policy) **registered and pending** — they are tracked in the tree, so nothing is lost; do **not** resume them without an owner nudge. `CHANGES-ENTRY-PLACEMENT.4` likewise deferred. **Every commit subject must name a LEAF** (`TREE.N`) — the `commit-msg` hook rejects a bare tree name.
