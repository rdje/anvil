...
## Recent commits
- `<pending>` — Docs: BOOK-EXAMPLES-RUNNABLE.1 — new quality tree + design-only DEVELOPMENT_NOTES.md entry (62-bash/8-rust/9-sv/4-text inventory; `cargo run --release --` convention; `tests/book_examples.rs` integration harness + `mdbook test` + skip-sentinel; 3 rejected alternatives; CI-wired). No code; mdbook clean.
