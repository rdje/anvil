...
- latest_commit: **`EMIT-SURFACE-INTERACTION-GATE.3`** (`401d72d`) — the interaction gate + its banked digest. Prior: `.2` (`d73b154`), `.1` (`7664761`). Session history: `git log --oneline` (do not re-narrate it here).
- active_work_unit: **`EMIT-SURFACE-INTERACTION-GATE`** → `.1` + `.2` + `.3` **done**, frontier **`.4`** (last leaf; closes the tree). Then **`EVIDENCE-BANK-DURABILITY.6`** — that tree was **reopened** `2026-07-30` because banking the *second* real digest exposed two `scripts/evidence_digest.sh` defects: (A) the re-runnable `--command` is rebuilt from a **hardcoded** 14-entry gate-flag list, so a newer gate yields a **flagless** command that would re-verify the *default* scenario set — silent and wrong; (B) `commit:` is `HEAD` at derivation time = the *parent* of the banking commit, unusable for a gate introduced alongside its own first bank. Closed this session: `CARGO-TMPDIR-SWEEP-REGRESSION`.
- next_action: `EMIT-SURFACE-INTERACTION-GATE.4` — harden `cone_function_emit::compute_use_counts` to count `Memory` (`we`/`waddr`/`wdata`/`raddr`) and `Fsm` (`sel`) consumers. Today it omits them, so a gate consumed once by a cone edge **and** once by a memory port reads as single-use, gets absorbed, and has its module wire deleted out from under the memory block. Unreachable today **only** because `build_memory_leaf`/`build_fsm_block` (`src/gen/module.rs:125,248`) build gate-free modules — an accident of shape, not a stated invariant. Add a regression test pinning that a memory-port-feeding gate is never absorbed; a provable no-op on every currently-constructible module ⇒ `tests/snapshots.rs` untouched. Consider factoring the census into a shared `ir::use_counts` helper (`feedback_full_factorization`).
