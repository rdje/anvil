...
- latest_commit: **`9ad7385`** (`BOOK-LINK-INTEGRITY.3`, tree CLOSED). Prior: `6539c32`, `3a48cc4`, `5e3e9a0`. Recent decisions: **`0040`–`0046`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight.** `active`: **`NEGATIVE-CONTROL-HARNESS`** (`.1`, registered not started) · **`CAPABILITY-BREADTH-EXPANSION`** (`.1`, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (**PAUSED, do not resume without a nudge**). CLOSED `2026-08-01`: `USER-GUIDE-CLI-TABLE-SHADOW` `.7`, `RESUME-POINTER-CONTRACT`, **`BOOK-LINK-INTEGRITY` `.1`–`.3`** — registry now **11** doctrines ⇒ [[mdbook-md-to-html-rewrite-trap]].
- next_action: **`NEGATIVE-CONTROL-HARNESS.1`** — measure, do NOT build the helper first. The finding: *"assert the mutation landed"* cost **2 of 3** ad-hoc control probes in one leaf while costing **0 of 10** written inside the `probe` helper — a **carrier** problem, not a diligence one. Read the tree; the honest obstacle (a control leaves no commit artifact, so it may not be doctrine-shaped) is stated there. **Every commit subject must name a LEAF** (`TREE.N`).
