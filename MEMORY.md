...
- **Active task trees:**
  ...
  - `COVERAGE-INSTRUMENTATION` — **`.2` triage done** (`2026-05-18`, triage-only): `DEVELOPMENT_NOTES.md` "Coverage baseline triage — top-5 under-covered files" per-file (a)/(b)/(c) disposition. **No confirmed dead code**; headline 3314 uncovered lines = gate-exclusive (`tool_matrix`) / intentional-defensive (`validate.rs`) / integration-only (`config.rs`,`main.rs`) by design; the only high-value `.3` target is a handful of `gen/cone.rs` focused proofs (retry-exhaustion / anti-collapse-reject / `pick_terminal` adapter-fallback) + an optional `config.rs` orphan-knob spot-audit. Frontier → `.3` (right-sized; code slice). Baseline at `docs/coverage-baseline.md` (85.26% lines).
