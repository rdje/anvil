...
- **Last completed slice:** Knob measurement doctrine + effectiveness map (docs only). See `CHANGES.md` entry `2026-04-16-0040`. `book/src/knobs.md` gains (1) a "Measurement doctrine" opening: no knob is privileged, every knob's effect must be empirically measurable via `Metrics` and/or `--trace`, with three landing requirements; (2) a dedicated "AST uniqueness / duplication" sub-section covering `max_ast_instances` and `mux_arm_duplication_rate`; (3) a knob-to-metric effectiveness map at the bottom listing which metric measures each knob, with *pending* entries flagging known gaps. No code changed. mdbook builds cleanly. 50 tests unchanged.
