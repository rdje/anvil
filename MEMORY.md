...
- latest_commit: **`PARITY-EXTRACTOR-CHARSET-GAP.1`** (backfill next commit). Prior: `18e75ad`, `c38024b`, `eb79534`. Recent decisions: **`0040`–`0044`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
...
- blockers: none. **A doctrine check can pass VACUOUSLY on a value it never reads** — `PARITY-EXTRACTOR-CHARSET-GAP` (closed `2026-08-01`): capture the charset the SOURCE permits, not the one its current members use; a count floor is *shrink-coupled*, so it cannot see an id born invisible. **A decision you already made is DISCLOSED, never re-opened as a question** — [`0041`](docs/decisions/0041-owner-standing-directives-recorded-in-layer-c.md) §(e). **Adding to this file requires routing, not appending.**
