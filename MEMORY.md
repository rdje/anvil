...
- **Last completed slice:** Coefficient fits operand width (Rule 19). See `CHANGES.md` entry `2026-04-15-0031`. `pick_coefficient` is now width-aware — it clamps the draw range to `[max(min_coefficient,1), min(max_coefficient, 2^W-1)]` so the `1'h6` truncation bug cannot occur. Book Rule 19 added. One new unit test. 30 unit + 15 integration = 45 tests. First of the three source-of-generation root-cause fixes queued from the defect catalogue.
...
- **Next up (closing small-to-medium Phase 3+ motifs first, per user direction):**
  0. **Fix remaining source-of-generation defects.** (a) done: coefficient-width clamp landed as Rule 19 in `2026-04-15-0031`. (b) next: position-dependent leaf rules — `pick_terminal` in mux-select position must reject constants (the `2'h2 == 2'h2` defect). (c) N-arity operand-multiset distinctness — broaden `violates_anti_collapse` beyond pairwise for idempotent/self-inverse ops (the `i_2^i_2^i_2^i_2 = 0` defect). (d) Rule 18 enforcement decided = α (construction-time demand-driven) per doctrine; graph-first will be reworked after b/c land.
