...
- **Last completed slice:** `interleaved` construction strategy landed. New `ConstructionStrategy::Interleaved` variant. New `cone::build_outputs_interleaved` frame state machine: one global `SignalFrame` queue popped at random each step; `GateFrame` in-flight table holds pending-operand gates; gates finalize when their last operand resolves. Blocks (flop allocation, comb-mux) still build depth-first within a single frame step; flop D-cones are still drained synchronously at the end (full symmetry awaits `graph-first`). Anti-collapse and DAG-sharing paths match the recursive path. Two new integration tests: `interleaved_reproducibility`, `interleaved_differs_from_sequential`. `all_strategies_produce_valid_modules` extended to cover all three. 25 unit + 7 integration = 32 tests total. See `CHANGES.md` entry `2026-04-15-0022`.
...
- **Next up:**
  1. **`graph-first` construction strategy + default flip:** architectural shift to pool-first DAG construction — no per-output cone recursion at all; grow a gate pool, then pick a drive-root per output from the pool. Full symmetric sharing including through block internals (flop D-cones, comb-mux sub-cones). When it lands, the `construction_strategy` default flips to `GraphFirst`; users wanting prior behavior pin to `sequential` / `shuffled` / `interleaved`. Depth budget is re-interpreted (emergent from pool growth rather than direct recursion depth).
