...
- latest_commit: **`da73827`** (`NEGATIVE-CONTROL-HARNESS.0`). Prior: `3b0d2c2`, `425c0ca`, `9ad7385`. Recent decisions: **`0040`–`0046`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight.** `active`: **`NEGATIVE-CONTROL-HARNESS`** (`.2`; `.1` done) · **`CAPABILITY-BREADTH-EXPANSION`** (`.1`, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (**PAUSED, do not resume without a nudge**). CLOSED `2026-08-01`: `USER-GUIDE-CLI-TABLE-SHADOW` `.7`, `RESUME-POINTER-CONTRACT`, **`BOOK-LINK-INTEGRITY` `.1`–`.3`** — registry now **11** doctrines ⇒ [[mdbook-md-to-html-rewrite-trap]].
- next_action: **`NEGATIVE-CONTROL-HARNESS.2`** — decide the CARRIER; do NOT build first. `.1` measured it: leg 3 is reported in **2 of 39** episodes, both on the day the rule was written; the same file-vs-baseline check runs **27×** on the *revert* and **2×** on the *mutation*, because only a dirty tree has a gate watching it. Subject is **one primitive** — `sed -i`/`perl -pi` exit `0` whether or not they matched. **Every commit subject must name a LEAF** (`TREE.N`).
