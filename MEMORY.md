...
- **Last completed slice:** Dep-bearing source at elaboration-sensitive positions (Rule 20). See `CHANGES.md` entry `2026-04-16-0032`. New `pick_terminal_dep_bearing` picker (dep-bearing pool entry or dep-bearing width-adapter; panics if none exists). Seven pool-mode call sites migrated: mux selects (comb + flop, encoded + one-hot), priority-encoder request bits, const-comparand LHS, const-shift value. 31 unit + 15 integration = 46 tests. Spot-check across six seeds shows zero folded literal-vs-literal comparisons. Second of the three source-of-generation root-cause fixes.
- **Prior slice:** Coefficient fits operand width (Rule 19). See `CHANGES.md` entry `2026-04-15-0031`.
...
- **Next up (closing small-to-medium Phase 3+ motifs first, per user direction):**
  0. **Fix remaining source-of-generation defects.** (a) done: coefficient-width clamp — Rule 19 / `2026-04-15-0031`. (b) done: dep-bearing select/req/LHS/value — Rule 20 / `2026-04-16-0032`. (c) next: N-arity operand-multiset distinctness — broaden `violates_anti_collapse` beyond pairwise for idempotent/self-inverse ops (the `i_2^i_2^i_2^i_2 = 0` and identical-one-hot-arms defects). (d) Rule 18 enforcement = α (construction-time demand-driven) per doctrine; graph-first rework follows (c).
