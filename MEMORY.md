...
- latest_commit: **`f2d282e`** (`CHANGES-ENTRY-PLACEMENT.5` hash backfill). Prior: `2203574`, `beb1ebb`, `8c93d1e`. Recent decisions: **`0040`–`0045`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight.** `USER-GUIDE-CLI-TABLE-SHADOW.2` **done**: the CLI flag table is **exhaustive over the KNOBS** — a *derived* set (`config.rs::Overrides` + 3 alias flags), **not** over `anvil --help`; mode flags keep their own sections. Table **75 → 93**; the partition is **total**: 93 + 14 = **107**. Still `active`: **`USER-GUIDE-CLI-TABLE-SHADOW`** (`.3`) · **`CAPABILITY-BREADTH-EXPANSION`** (only `.1`, SV up-opt, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (`.9`/`.4` — **PAUSED by owner redirect, do not resume without a nudge**).
- next_action: **`USER-GUIDE-CLI-TABLE-SHADOW.3`** — the mechanism question. `.2` supplied both its precondition (the set is derivable) and the trap: **any flag extractor must be COMMAND-SCOPED**. `.4` also now open (the book's `## CLI coverage` is the same shadow, missing **11 of 107**). **Every commit subject must name a LEAF** (`TREE.N`).
