...
- latest_commit: **`dbc8d62`** (`NEGATIVE-CONTROL-HARNESS.3`). Prior: `c5d6580`, `46f2af1`, `83a2a5b`. Recent decisions: **`0040`–`0047`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight.** `active`: **`UNGATED-PRACTICE-AUDIT`** (`.1`, registered not started) · **`CAPABILITY-BREADTH-EXPANSION`** (`.1`, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (**PAUSED, do not resume without a nudge**). CLOSED `2026-08-01`: `USER-GUIDE-CLI-TABLE-SHADOW`, `RESUME-POINTER-CONTRACT`, `BOOK-LINK-INTEGRITY`, **`NEGATIVE-CONTROL-HARNESS` `.1`–`.3`** — registry now **11** doctrines ⇒ [[mdbook-md-to-html-rewrite-trap]].
- next_action: **`UNGATED-PRACTICE-AUDIT.1`** — DERIVE the candidate set from an authoritative source (`COMMIT.md` checklist / `TOOLBOX.md` Part 2 / the §10 registry's complement), state the denominator, classify on evidence. It is a **hypothesis under test**: *"the rule predicts nothing here"* is a legitimate result. Watch for the third class — a **self-ticked** box is watched by something that cannot see it. **Every commit subject must name a LEAF** (`TREE.N`).
