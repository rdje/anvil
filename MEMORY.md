...
- **Last completed slice:** construction-strategy machinery landed, with two strategies implemented. New `ConstructionStrategy` enum (`Sequential`, `Shuffled`) with serde + clap::ValueEnum. New `construction_strategy` knob (default `Sequential`) threaded through Config / Overrides / CLI (`--construction-strategy`). `generate_leaf_module` dispatches on strategy: Sequential iterates outputs in declaration order; Shuffled draws a seeded random permutation. Drives recorded in declaration order regardless. Three new integration tests: `shuffled_reproducibility`, `shuffled_differs_from_sequential`, `all_strategies_produce_valid_modules`. 25 unit + 5 integration = 30 tests total. See `CHANGES.md` entry `2026-04-15-0021`.
...
- **Next up:**
  1. **`interleaved` construction strategy:** moderate rework of `build_cone` into an explicit frame state machine. Frames from all cones popped in random order from a work queue; cones grow in lockstep. Near-symmetric within-module sharing. See `book/src/construction-strategies.md`.
  2. **`graph-first` construction strategy + default flip:** architectural shift to pool-first DAG construction. When it lands, the `construction_strategy` default flips to `GraphFirst`; users wanting prior behavior pin to `sequential` or `shuffled`.
