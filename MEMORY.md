...
- active_work_unit: **none mid-flight.** `active`: **`BOOK-PARAGRAPH-BLOBS`** (owner finding; frontier **`.1`**) · **`UNGATED-PRACTICE-AUDIT`** (`.1` **done**; frontier **`.2`**) · **`CAPABILITY-BREADTH-EXPANSION`** (`.1`, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (**PAUSED, do not resume without a nudge**). `.1` **refuted its own rule** ⇒ [[practice-survives-as-a-by-product-not-by-a-gate]].
- next_action: **`BOOK-PARAGRAPH-BLOBS.1`** — whitespace-only split of the **6 of 1,244** over-1,500-char rendered paragraphs (worst **22,908**); **no wording change**; re-census after. Owner: **not urgent**. Then **`UNGATED-PRACTICE-AUDIT.2`** — a **stale SPEC** (`COMMIT.md` item 2), a **blind gate** (`MEMORY-ARCH` checks field *names*, not freshness), a **scope gap** (`0031` §1 covers agent scratch; the check is tracked-only). `0047`: **removing the need outranks watching harder**. **Every subject must name a LEAF.**
