...
- latest_commit: **`OVERFLOW-DESTINATION-INSTRUMENTATION.3`** (pending — backfill next). Prior: `6c0c953`+`c1b765c` (`.5b`), `4f3d508` (`.5a` = decision `0041`), `4a91c45` (`.2` = decision `0040`). Recent decisions: **`0039`–`0041`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (`active`; `.1`/`.2`/`.5a`/`.5b`/`.3` done, frontier **`.6`**). **This file was the subject and is now inside its own instrument** — 30 lines / ~6.0 KB against caps of 50 / **6,144**, down from 19,885 B. Decision [`0040`](docs/decisions/0040-overflow-destination-classification-and-the-unmeasured-axis.md) classifies every overflow destination in **three** kinds and measures that **a cap binds only on the axis it measures**; `.5a`/`.5b` routed the layer-C content out ([`0041`](docs/decisions/0041-owner-standing-directives-recorded-in-layer-c.md) + 12 fact cards); `.3` added the byte axis. **Read the records, not a summary here.** Also `active`: **`CHANGES-ENTRY-PLACEMENT`** (frontier `.4`; 181 of 574 provenance lines never hash-backfilled) · **`IR-TYPES-DECOMPOSITION`** (`.3`/`.4` — the ~1830-line interning engine, the big code leaf).
- next_action: **`OVERFLOW-DESTINATION-INSTRUMENTATION.6`** (measure the long-line class tree-wide — `CODEBASE_ANALYSIS.md` carries a **24,990-char** line; a measured *"not worth the churn"* closes it), then **`.4`** — carry the correction into the **portable** standards, which still ship the hole `.3` closed locally: `MEMORY_ARCHITECTURE.md` §6 states the cap as *"~50 lines"* and its §9 reference script enforces **lines only**. Then **`CHANGES-ENTRY-PLACEMENT.4`**, then **`IR-TYPES-DECOMPOSITION.3`**. **Every commit subject must name a LEAF** (`TREE.N`) — the `commit-msg` hook rejects a bare tree name.
- in_flight_uncommitted: none once `.3` lands. `scripts/` + docs only ⇒ **DUT byte-identical**; `src/`, `tests/`, `examples/` untouched. `check_doctrines.sh` **8/8** after `git add`; KM **106** facts / **1,022** keys.
