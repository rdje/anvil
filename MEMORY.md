...
- latest_commit: **`0e4654f`** (`UNGATED-PRACTICE-AUDIT.1`). Prior: `1dedbd8`, `ef6413c`, `b536139`. Recent decisions: **`0040`–`0047`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight.** `active`: **`UNGATED-PRACTICE-AUDIT`** (`.1` **done** `2026-08-02`; frontier **`.2`**) · **`CAPABILITY-BREADTH-EXPANSION`** (`.1`, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (**PAUSED, do not resume without a nudge**). `.1` **refuted its own rule** and replaced it ⇒ [[practice-survives-as-a-by-product-not-by-a-gate]].
- next_action: **`UNGATED-PRACTICE-AUDIT.2`** — decide per candidate. `.1` found **one** eroding item and it is a **stale SPECIFICATION** (`COMMIT.md` item 2's five-section template, 100 %→32 %, substance intact under bespoke headings), a **blind gate** (`MEMORY-ARCH` asserts field *names*, never freshness), and a **scope gap** (`0031` §1 covers agent scratch; the check is tracked-only). `0047`: **removing the need outranks watching harder**. **Every commit subject must name a LEAF** (`TREE.N`).
- in_flight_uncommitted: none. **OWNER FINDING `2026-08-02`, tree-owned NEXT, not urgent:** rendered book has paragraph blobs — `architecture.html` is ONE `<p>` of **22,908** chars (`hierarchy` 4,371, `ir` 3,211). **`.cache/local-references/` is NEVER tracked** ⇒ [`0043`](docs/decisions/0043-reference-cache-stays-untracked-public-repo.md). **Never trust a piped exit status.**
