...
- latest_commit: **`c5d6580`** (`NEGATIVE-CONTROL-HARNESS.2`). Prior: `46f2af1`, `83a2a5b`, `4ad09a4`. Recent decisions: **`0040`–`0047`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight.** `active`: **`CAPABILITY-BREADTH-EXPANSION`** (`.1`, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (**PAUSED, do not resume without a nudge**). CLOSED `2026-08-01`: `USER-GUIDE-CLI-TABLE-SHADOW`, `RESUME-POINTER-CONTRACT`, `BOOK-LINK-INTEGRITY`, **`NEGATIVE-CONTROL-HARNESS` `.1`–`.3`** — registry now **11** doctrines ⇒ [[mdbook-md-to-html-rewrite-trap]].
- next_action: **pick the next tree** — no frontier is mid-flight. `NEGATIVE-CONTROL-HARNESS` closed at `.3`: `scripts/negative_control.sh` refuses a zero-count substitution (exit `9`), distinguishes a broken expression (`2`), and is controlled on itself through a seam ⇒ [`0047`](docs/decisions/0047-negative-control-carrier-is-the-mutation.md). **Reach for a mutation with NO match step first** (R1). **Every commit subject must name a LEAF** (`TREE.N`).
