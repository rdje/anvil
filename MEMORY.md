...
- latest_commit: **`CAPABILITY-BREADTH-EXPANSION.4b.3`** (backfill next commit). Prior: `955e387`, `c1cc6a1`, `34b8d14`. Recent decisions: **`0040`–`0044`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight.** The `unique`/`priority` **case-qualifier** strand (decision **`0044`**) is **CLOSED** — live, queryable (schema **`1.28`**), gated (`anvil-case-qualifier-gate-r1`) and documented; `CAPABILITY-BREADTH-EXPANSION` stays `active` with only **`.1`** (SV up-opt) **deferred-not-retired**. Also `active`: **`IR-TYPES-DECOMPOSITION`** (`.4`) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (`.9`/`.4` — **PAUSED by owner redirect, do not resume without a nudge**) · **`CHANGES-ENTRY-PLACEMENT`** (`.4`, deferred).
- next_action: **pick a new work unit** — `.1` is evidence-deferred, so the live candidates are **`IR-TYPES-DECOMPOSITION.4`** and **`CHANGES-ENTRY-PLACEMENT.4`** (the latter now carries a *second measured instance* of its class: `docs/AGENT_INTROSPECTION_SCHEMA.md` §7's changelog runs in **three** different orderings — see that tree's `.4` Notes). **Every commit subject must name a LEAF** (`TREE.N`).
