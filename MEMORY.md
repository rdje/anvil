...
- **Active task trees:**
  ...
  - **Every remaining roadmap phase is now task-tree-tracked (2026-05-16 owner directive):**
    ...
    - `PHASE-5B-AGGREGATES` — **active**, frontier `.2.2`. `.1` design **done** (architecture **(P)** emitter-only projection; rejected (A)/(B)/(C); identity Open Question resolved — annotation not hashed into `canonical_module_signature`). `.2` split (`.2.1` scaffold / `.2.2` soundness+identity / `.2.3` matrix scenario+gap no-promo / `.2.4` real-gate verify → ROADMAP Phase 5b done + closure; mirrors Phase 5 `.2.1`–`.2.4`, r87 no-aspirational-claims). **`.2.1` scaffold done** (`2026-05-17`): additive `Module.aggregate_layout` + `AggregateKind{StructPacked}`/`AggregateGroup` (`src/ir/types.rs`), `Config::aggregate_prob` serde-default 0.0, non-rolling `src/ir/aggregate.rs::annotate_aggregate` (6 unit tests), seeded per-module roll at `gen/mod.rs` post-pass scoped to **non-instantiated** modules, boundary-alias emitter projection (typedef struct packed + 1 aggregate port/side + alias wires/assigns; flat IR body byte-identical). Focused proof green (default-off byte-identical 4 strat × 6 seeds; forced-on projects + validates); real `verilator --lint-only` of a projected hierarchy design EXIT 0. Scoping (recorded): StructPacked-only / non-instantiated-only / param_env skipped. **`.2.2` done** (`2026-05-17`, proofs only): organic existence 68/80 ≈ 85% with DEFAULT port ranges (4 strat × 20 seeds) → projection NOT inert, **no rules-first pivot** (threshold ≥50%); identity-invariance proven (`canonical_signature_is_invariant_under_projection` + `aggregate_projected_twin_dedup_collapses`; annotation not hashed → projected twin dedup-collapses). `aggregate::` 10/0. **`.2.3` done** (`2026-05-17`): `DesignMetrics.num_packed_aggregate_modules` + `phase5b_packed_aggregate_focus_config`/scenario (dedup-anchor shape → shape sets unperturbed) + `saw_packed_aggregate_design` set/merge + Phase4Hierarchy gap; bin 213→216 / 852→864; `phase5b_packed_aggregate_scenario_is_non_vacuous` proves the wrapper projects (coverage fact reachable); phase4 bin 3/3; ROADMAP unchanged. **Frontier `.2.4`** = run the real repo-owned `Phase4Hierarchy` gate, verify `coverage_gaps=[]` + Verilator/both-Yosys all-pass + `saw_packed_aggregate_design=true`, THEN author ROADMAP Phase 5b exit criteria + promote Phase 5b `done` + reconcile `book/src/ir.md`+`knobs.md` + close the tree (r87 no-aspirational-claims).
