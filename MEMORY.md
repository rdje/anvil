...
- active_work_unit: **none mid-flight.** `active`: **`CAPABILITY-BREADTH-EXPANSION`** (`.1`, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (**PAUSED, do not resume without a nudge**). CLOSED `2026-08-01`: `USER-GUIDE-CLI-TABLE-SHADOW` `.7`, `RESUME-POINTER-CONTRACT` (⇒ [[gate-frequency-is-not-evidence]]), **`BOOK-LINK-INTEGRITY` `.1`–`.3`** — doctrine registry now **11**, `BOOK-LINK-TARGETS` added ⇒ [[mdbook-md-to-html-rewrite-trap]].
- next_action: **pick a new tree — no frontier is open.** Candidates: resume `CAPABILITY-BREADTH-EXPANSION.1`, or register the finding `BOOK-LINK-INTEGRITY.3` surfaced: **the *"assert the mutation landed"* rule cost 2 false control results in one leaf because it is a HABIT, not a mechanism** — it never failed inside the `probe` helper that asserts it, only in ad-hoc probes. **Every commit subject must name a LEAF** (`TREE.N`).
