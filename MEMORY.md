...
- latest_commit: **`614e977`** (`RESUME-POINTER-CONTRACT.1`); `BOOK-LINK-INTEGRITY.1` lands next — **backfill its hash**. Recent decisions: **`0040`–`0045`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight.** `active`: **`BOOK-LINK-INTEGRITY`** (frontier **`.2`**) · **`CAPABILITY-BREADTH-EXPANSION`** (`.1`, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (**PAUSED, do not resume without a nudge**). CLOSED `2026-08-01`: `USER-GUIDE-CLI-TABLE-SHADOW` at `.7` (all three clap registries gated), `RESUME-POINTER-CONTRACT` (hypothesis FALSE ⇒ [[gate-frequency-is-not-evidence]]).
- next_action: **`BOOK-LINK-INTEGRITY.2`** — `.1` measured both classes: **1** offending site (`book/src/recipes.md:857`), **anchor class EMPTY** (0 of 71 authored / 1136 rendered, negative-controlled; the tree's own prediction refuted). `.2` **records the repair form BEFORE editing**. ⇒ [[mdbook-md-to-html-rewrite-trap]]: the target `.md` **exists**, so *"does the target exist?"* is **vacuous** against this defect. **Every commit subject must name a LEAF** (`TREE.N`).
