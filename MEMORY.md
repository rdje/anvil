...
- latest_commit: **`OVERFLOW-DESTINATION-INSTRUMENTATION.7`** (backfill next commit). Prior: `bb446fc` (`.6`), `f33601e`+`e801617` (`.3`), `6c0c953` (`.5b`), `4f3d508` (`.5a` = decision `0041`), `4a91c45` (`.2` = decision `0040`). Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (`active`; `.1`/`.2`/`.5a`/`.5b`/`.3`/`.6`/`.7` done, frontier **`.8`**). **This file is inside its own instrument** — 30 lines / ~6.0 KB against caps of 50 / **6,144**, down from 19,885 B. `0040` classifies every overflow destination in **three** kinds and measures that **a cap binds only on the axis it measures**; `.3` added the byte axis; **`.6` found what no size cap can see — 36 malformed table rows silently dropping 57,283 *rendered* chars — and `.7` repaired all 36 and registered the NINTH doctrine `TABLE-RENDER-FIDELITY`.** **Read the records, not a summary here.** Also `active`: **`CHANGES-ENTRY-PLACEMENT`** (frontier `.4`) · **`IR-TYPES-DECOMPOSITION`** (`.3`/`.4`, the big code leaf).
- next_action: **`OVERFLOW-DESTINATION-INSTRUMENTATION.8`** — classify `docs/TASK_TREE.md`: `0040` never did (it classified `docs/tasks/`, a different path) and neither router names it, the **second** instance of `0040` §(g); **648 B/line** vs the 406 that made `MEMORY.md` the tree's finding, **79 %** of it in the *Current frontier* column. **Read each dated cell before claiming mixed-surface** — the distinct-date probe fires on 8/74 and `.2` retired that instrument for crying wolf. Then **`.4`** (carry the correction into the **portable** standards — `MEMORY_ARCHITECTURE.md` §6/§9 still ship the single-axis hole `.3` closed locally). Then **`CHANGES-ENTRY-PLACEMENT.4`**, then **`IR-TYPES-DECOMPOSITION.3`**. **Every commit subject must name a LEAF** (`TREE.N`) — the `commit-msg` hook rejects a bare tree name.
- in_flight_uncommitted: none once `.7` lands. Docs + `scripts/` only ⇒ **DUT byte-identical**; `src/`, `tests/`, `examples/` untouched. `check_doctrines.sh` **9/9** after `git add`; KM **107** facts / **1,029** keys.
