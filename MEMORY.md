...
- **Active task trees:**
  ...
  - **Every remaining roadmap phase is now task-tree-tracked (2026-05-16 owner directive):**
    - `PHASE-4-HIERARCHY` — **tree complete** (`.1` done — surface landed-proven; `.2` superseded; `.3` done — Phase 4 closed via deliberate evidence-backed scope cut against explicit ROADMAP exit criteria, closing artifact r87). **ROADMAP Phase 4 = done.** The PNT-to-exhaustion directive for Phase 4 is fulfilled; Phase 5 (Parameterization, now unblocked) is the next numbered phase.
    - `PHASE-5-PARAMETERIZATION` — **active PNT lane.** `.1` design **done** (architecture (C); (A)/(B)/(C') rejected — DEVELOPMENT_NOTES.md "Phase 5 parameterization design"). `.2.1` scaffold **done**: `WidthExpr{Lit,Param}`+`ParamEnv`, additive `Module` annotation fields, opt-in `Config::width_parameterization_prob` (default 0.0), `src/ir/param.rs` annotation-only post-construction pass (5 unit tests), emitter `#(parameter int W=D)` + `[W-1:0]`, focused round-trip/default-off proof. Architecture (C) confirmed implementable with zero changes to the invasive width math. `.2.2.1` soundness primitives **done** + **rules-first pivot** (post-hoc filter inert/generate-then-filter). `.2.2.2` **done**: `src/gen/module.rs::build_parameterizable_leaf` constructs a width-homogeneous combinational leaf by rule (single opt-in roll; param.rs refactored rolling→non-rolling `annotate_parameterized`, no double-roll). Feature now **fires by construction** — at prob 1.0 every forced-on single-module design is a parameterized width-generic leaf (4 strategies), validates, body fully `[W-1:0]`, default-off byte-identical. `.2.2.3` **done** (`.2.2.3a` IR field+emitter + `.2.2.3b` hierarchy instantiation pick + resolved-width validate). **Phase 5 width parameterization is end-to-end functional**: rules-first parameterizable leaves, multi-width `#(.W(v))` instantiation (legacy wrapper library mode), valid by construction, default-off byte-identical. Soundness scoping: only the planned-child loop overrides; helper/default instances use default elaboration = concrete template = already valid. `.2.3` **done** (param-aware `canonical_module_signature`; dedup unchanged; r87/H-A-I regression-clean). `.2.4` split (r87 no-aspirational-claims precedent): `.2.4a` **done** — `phase5_width_parameterized` tool_matrix scenario + `DesignMetrics.num_width_parameterized_modules`/`num_param_override_instances` + `saw_width_parameterized_design` fact/gap; bin counts 213/852; phase4 bin tests 3/3; **ROADMAP unchanged**. **Frontier `.2.4b`**: run the real repo-owned Phase 4 hierarchy gate (now includes phase5_width_parameterized), verify downstream-clean (coverage_gaps=[], Verilator+both Yosys all-pass, saw_width_parameterized_design=true), THEN author ROADMAP Phase 5 exit criteria + promote Phase 5 `done` + sync README/CODEBASE_ANALYSIS/book + close the tree. Promotion strictly follows the verified artifact.
