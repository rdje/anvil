...
- latest_commit: **`c793cf3`** (`USER-GUIDE-CLI-TABLE-SHADOW.2` backfill). Prior: `cd7f24b`, `f2d282e`, `2203574`. Recent decisions: **`0040`–`0045`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight.** `USER-GUIDE-CLI-TABLE-SHADOW` `.2`+`.3` **done**: the CLI flag table is **exhaustive over the KNOBS** (a *derived* set, **not** `anvil --help`), repaired **75 → 93**, and now **gated as `ENUMERATION-PARITY` pair 5** — a pair, not an 11th doctrine. Still `active`: **`USER-GUIDE-CLI-TABLE-SHADOW`** (`.4`) · **`CAPABILITY-BREADTH-EXPANSION`** (only `.1`, SV up-opt, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (`.9`/`.4` — **PAUSED by owner redirect, do not resume without a nudge**).
- next_action: **`USER-GUIDE-CLI-TABLE-SHADOW.4`** — `book/src/knobs.md` §*CLI coverage* is the same shadow, **missing 11 of 107**, under a sentence promising it is accurate. `.3` deliberately left it **ungated**: gating a copy freezes it; `0033` rung **R1** is to delete it and point at the reference. **Every commit subject must name a LEAF** (`TREE.N`).
