...
- next_action: do **`AGENT-MCP-EXPANSION.4`** (code) — optional HTTP transport for `anvil-mcp` beside stdio, driving the **same** `McpServer::handle` dispatcher behind an explicit opt-in flag (bias: `--http <addr>` on the existing bin, **loopback-only default**); stdio stays the default; per-call `downstream` guardrails unchanged; transport-level test. Per decision `0005` + `.1`. NOTE: `.5` closeout then syncs `book/src/agent-mcp.md` + `USER_GUIDE.md` + `README.md` for the whole expanded surface (coverage_gaps + non-DUT lanes + HTTP).
