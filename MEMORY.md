...
- latest_commit: **`4f36cde`** (`USER-GUIDE-CLI-TABLE-SHADOW.7`, tree CLOSED). Prior: `702695d`, `6e95494`, `04ef949`. Recent decisions: **`0040`–`0045`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight. `USER-GUIDE-CLI-TABLE-SHADOW` is CLOSED** at `.7`: a census of `src/` finds **three** clap registries and **all three are gated** — `anvil` knob flags (pair 5), `tool_matrix` options (pair 6), `anvil hunt` flags (pair 7); both book copies deleted by `0033` rung **R1**. Still `active`: **`BOOK-LINK-INTEGRITY`** (`.1`) · **`CAPABILITY-BREADTH-EXPANSION`** (only `.1`, SV up-opt, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (`.9`/`.4` — **PAUSED by owner redirect, do not resume without a nudge**).
