...
## Recent commits
- `<pending>` — PHASE-6-ADVANCED-MOTIFS.3.1: generated-encoding FSM motif design (design-only, no code). `DEVELOPMENT_NOTES.md` entry — codebase grounding (FSM is a block), empirical downstream probe of the exact emitted SV in all 3 encodings (binary/one-hot/gray) → every one `verilator --lint-only` exit 0 + both repo Yosys modes `check -assert` clean (encoding-selectability structural); architecture (F) `Vec<Fsm>` + opaque `Node::FsmOut` + encoding-derived emitter + opt-in `fsm_prob`; 4 rejected/deferred. `.3` split → container `.3.1`(done)/`.3.2`/`.3.3`/`.3.4` (mirrors memory `.2.1`–`.2.4`; FSM = last Phase 6 motif → `.3.4` closes Phase 6 + tree). mdbook/fmt clean; no `src/`/`tests/` change. Continuous-PNT while the `.2.4` memory gate runs.
