...
## Current state
- **Memory architecture adoption in progress (2026-06-04):** `MEMORY-ARCHITECTURE-DOC` opened to adopt `MEMORY_ARCHITECTURE.md` in five leaves. `.1` added the portable standard + README pointer; `.2` added `docs/decisions/` with three initial records; `.3` will demote this file to a bounded resume pointer; `.4` will install enforcement; `.5` will close with focused end-to-end validation. Full `cargo test` is intentionally not required for these workflow-doc leaves per owner instruction/resource policy; monitor RAM and stop any future full suite if memory exceeds 90%.
...
- **Live-doc path portability (2026-06-04, `LIVE-DOC-PATH-HYGIENE.1`):** live docs, task trees, and mdBook-facing docs must use repo-root-relative paths for project files. Local checkout prefixes were scrubbed from the live-doc/book surface; `/tmp` banked validation artifacts intentionally remain absolute because they identify evidence outside the repository. Stale closed-tree `active` statuses found by the same audit were aligned to `done`. Validation clean: local-path `rg` no matches; active-status `rg` only template/current tree before closure; `git diff --check`; `mdbook build book`; `cargo check --all-targets`; `cargo test` (pipeline 121/121 in 634.05s); `cargo clippy --all-targets -- -D warnings`; `cargo fmt --all --check`.
...
- **Phase:** Phase 0 done. Phase 1 (Single-module MVP) is done. Phase 2 (Signal sharing / DAG cones) is done. Phase 3 (structured combinational ops) is done. **Phase 4 (hierarchy) is done** (2026-05-16, closed by `PHASE-4-HIERARCHY.3` as a deliberate evidence-backed scope cut against explicit ROADMAP exit criteria; closing artifact r87, `coverage_gaps=[]`, 840/0). **Phase 5 (parameterization) is done** (2026-05-17, `PHASE-5-PARAMETERIZATION` tree complete: rules-first width-parameterizable leaves + `#(.W(v))` instantiation + parameter-aware identity, closed against explicit ROADMAP exit criteria; closing artifact `/tmp/anvil-tool-matrix-phase5-p1` — 213 scenarios / 852 designs, `coverage_gaps=[]`, Verilator+both-Yosys 852/0, `saw_width_parameterized_design=true`). **Phase 5b (synthesizable aggregates) is done** (2026-05-18, `PHASE-5B-AGGREGATES` tree complete: opt-in `aggregate_prob` packed-`struct` emitter projection over the flat IR — annotation-only, default-off byte-identical; closed against explicit ROADMAP exit criteria; closing artifact `/tmp/anvil-tool-matrix-phase5b-p1` — 216 scenarios / 864 designs, `coverage_gaps=[]`, Verilator+both-Yosys 864/0, `saw_packed_aggregate_design=true`, Phase 4/5 regressions clean). **Phase 6 (advanced motifs) is DONE** (2026-05-20, `PHASE-6-ADVANCED-MOTIFS` tree complete): both substantive motifs landed and verified downstream-clean against banked `Phase4Hierarchy` gates. **Memory motif** (delivered 2026-05-18, `.2.4`): closing artifact `/tmp/anvil-tool-matrix-phase6-p1` — 219 scenarios / 876 designs, `coverage_gaps=[]`, Verilator+both-Yosys 876/0, `saw_inferrable_memory_design=true`. **Generated-encoding FSM motif** (delivered 2026-05-20, `.3.4b`): first-class `Fsm` block + opaque `Node::FsmOut` + encoding-derived emitter (binary / one-hot / gray; Moore outputs) + opt-in `fsm_prob` (default-off byte-identical); closing artifact `/tmp/anvil-tool-matrix-phase6-fsm-p1` — **222 scenarios / 888 designs, `coverage_gaps=[]`, Verilator+both-Yosys 888/0, `saw_fsm_design=true` AND `saw_inferrable_memory_design=true`**, P4/P5/P5b regressions proven in the same banked artifact. ROADMAP Phase 6 closed; book/README/CODEBASE reconciled; r87 no-aspirational-claims observed throughout. Multi-clock CDC remains the explicitly-optional, separately-prioritised deferral (every module stays fully synchronous to a single clock). **Phase 9 (multi-artifact umbrella) is DONE** (2026-05-20, `PHASE-9-MULTI-ARTIFACT-UMBRELLA` tree complete): delivered the artifact-family selector + shared plumbing — `pub trait ArtifactLane` in `src/umbrella/` (`name`/`validate_knobs`/`generate(seed)`/`check_plan`) + `LaneArtifact{lane, seed, sv, manifest: Option<String>}` carrier + `CheckPlan{SynthAccept, ParityVsManifest}` + all 3 lane impls (`DutLane` wrapping `src/gen/`, `MicrodesignLane` wrapping `src/microdesign/`, `FrontendLane` wrapping `src/frontend/`). New `--artifact <lane>` top-level CLI flag on the `anvil` binary (default `dut`) dispatches via `Box<dyn ArtifactLane>`; `--artifact dut` falls through to the historical code path UNCHANGED via early-return guard at the top of `fn main`. **8 cargo-portable umbrella proofs** incl. per-lane byte-identical regression (L1/L2/L3) + cross-lane heterogeneous dispatch through `Box<dyn ArtifactLane>`. **Book/CI byte-identical contract verified by `tests/book_examples::every_runnable_book_bash_block_succeeds` passing 3/3 in 80s AFTER the CLI change** — the load-bearing proof that `--artifact dut` preserves every existing book example + CI gate. Explicit anti-goal from `.1` preserved: only plumbing unifies; the three lanes' rules-first generators stay decoupled. **ANVIL now ships THREE complementary lanes selectable via one tool.** **All 9 numbered roadmap phases delivered.** Remaining open follow-up trees: multi-clock CDC (explicitly-optional deferral from Phase 6 closure); `DIFFERENTIAL-SIMULATION.2b` (iverilog↔verilator harness implementation; quality lane).

**Phase 8 (frontend-accept) is DONE** (2026-05-20, `PHASE-8-FRONTEND-ACCEPT` tree complete): delivered the source-level frontend/elaboration accept lane — a separate top-level module `src/frontend/` with its own AST IR (`SourceUnit` → `Package` → `Module` → `ModuleItem` with `Instance`/`GenerateIf` arms; depth-1 hierarchies sufficient for every elaboration axis the parity gate checks) + construction-time elaboration-evaluator + un-elaborated-`.sv` emitter + elaborated-facts JSON manifest + hierarchy-aware parity comparator (`ToolReport`/`InstanceToolReport`/`Divergence` × 23 variants with load-bearing `Instance*` additions) + yosys-specific `hierarchy + write_json` extractor reading `.cells[<inst>].{type, parameters}`; closing artifact `/tmp/anvil-frontend-parity-phase8-yosys-p1/` — 5 reproducibility seeds × {`.sv`, `.json`, `.yosys.json`}, `parity gate clean across 5 seeds` on **first try** (no fix-and-retry — contrast with Phase 7's `.2c.2b.1`), per-seed fact agreement verified including both generate branches (seed 12345 takes `g_else`) AND the load-bearing hierarchy-aware Phase-8 axis (2 instances × 4 per-instance per-binding values each). **Cross-tree reuse of Phase 7's `expr_to_sv` carried `.2c.2b.1`'s non-negative-modulo-idiom fix forward at zero incremental cost** — full-factorization doctrine vindicated. Scope caveat: yosys covers 5 of 7 manifest fact categories (Seed/Top/TopParams/Instances/GenerateBranches); top_localparams + package_constants are folded — richer-AST coverage via slang/verilator-with-debug is a recorded post-Phase-8 follow-up that does NOT retract closure since ANVIL's by-construction oracle already covers all 7 categories. ANVIL now ships THREE complementary lanes: DUT (Phases 1–6) + microdesign (Phase 7) + frontend (Phase 8). DUT lane stays byte-identical by construction (`frontend` is never invoked from `gen::*`; default-off).

**Phase 7 (oracle-backed micro-design) is DONE** (2026-05-20, `PHASE-7-ORACLE-MICRODESIGN` tree complete): delivered the `rtl_const_expr`-family lane — a separate top-level module `src/microdesign/` with its own source-level const-expr/parameter IR + construction-time-resolved oracle + un-resolved-`.sv` emitter + JSON expected-facts manifest + scoped parity comparator + yosys-specific `write_json` extractor; closing artifact `/tmp/anvil-microdesign-parity-phase7-yosys-p1/` — 5 reproducibility seeds × {`.sv`, `.json`, `.yosys.json`}, `parity gate clean across 5 seeds`, per-seed fact agreement verified (incl. seed 7 P4=-1 → bits=8 on both sides after `.2c.2b.1`'s non-negative-modulo-idiom fix, and both generate branches exercised — seed 12345 takes `g_else`); the closing run found and fixed an ANVIL-self-consistency bug in `width_expr` (oracle used `rem_euclid`, SV used `%`; diverged for negative `last.value`) — exactly what `.1` designed the gate to do. Scope caveat: yosys 0.64 covers 4 of 7 manifest fact categories (Seed/Top/Params/Widths/Generate); richer-AST coverage via slang/verilator-with-debug is a recorded post-Phase-7 follow-up that does NOT retract closure since ANVIL's by-construction oracle already covers all 7 manifest categories. DUT lane stays byte-identical by construction (microdesign is never invoked from the DUT generate path; default-off).
- **Active task trees:**
  ...
  - **Every remaining roadmap phase is now task-tree-tracked (2026-05-16 owner directive):**
    ...
    - `PHASE-6-ADVANCED-MOTIFS` — **tree COMPLETE (`2026-05-20`)**: Phase 6 closed (memory `.2.4` 2026-05-18 + FSM `.3.4b` 2026-05-20). Historical milestone trail below. `.1` memory design **done** (`2026-05-18`, design-only): `DEVELOPMENT_NOTES.md` "Phase 6 inferrable-memory motif design" — empirical Yosys probe (single-port + simple-dual-port → `1 $mem_v2`, clean both repo modes + Verilator); architecture **(M)** first-class `Memory` block (additive `Vec<Memory>` on `Module`, Default-empty) + opaque `Node::MemRead` leaf (sibling to `FlopQ`, never CSE'd) + emitter renders the validated inferrable template on shared `clk` + opt-in `Config::memory_prob` serde-default 0.0; rejected (A) flop-array+mux (not `$mem`-inferred) / (B) emitter-only string template / (C) generic unpacked-array datatype. `.2` split (`2026-05-18`) per Splitting Rules + r87 into `.2.1` scaffold / `.2.2` Yosys-inference proof + `MemRead` CSE-opacity / `.2.3` matrix scenario+metric+gap no-advance / `.2.4` real-gate verify → ROADMAP **memory delivered** note (Phase 6 stays open for `.3` FSM — no tree closure). `.2.1` further split → `.2.1a`/`.2.1b` on a discovered compaction-reachability dependency (opaque stateful leaf ≠ mechanical FlopQ-mirroring). **`.2.1` container complete (`2026-05-18`)**: `.2.1a` IR core (`MemId`/`MemKind`/`Memory`/`Node::MemRead`/`DepAtom::MemVirtual`/`has_local_memories`) + `MemRead` through ~21 matches + load-bearing `compact.rs` reachability + emitter inferrable template + validator step-5b + 3 unit proofs; `.2.1b` `Config::memory_prob` (serde-default 0.0) + rules-first `build_memory_leaf` + single opt-in roll (after the param lane, mutually exclusive) + default-off/forced-on focused proof; real spot-check of generated memory → `1 $mem_v2`, verilator + both-yosys clean. **`.2.2` done** (`2026-05-18`, proof only): `inferrable_memory_matches_yosys_template_and_is_factorization_opaque` — 64 combos (4 strategies × 4 FactorizationLevel incl. EGraph × 4 seeds) prove the generated SV is exactly the `.1` Yosys-`$mem_v2` template + `MemRead`/array never enter the NodeId graph (CSE/factorization-opaque); tool-level `$mem_v2`/Verilator proof scoped to `.2.1b` spot-check (interim) + `.2.4` real gate (cargo can't shell yosys — Decisions). **`.2.3` done** (`2026-05-18`): `DesignMetrics.num_memory_modules` + `phase6_inferrable_memory_focus_config`/scenario (clone of phase5b/dedup anchor → shape sets unperturbed; library leaves = memory blocks) + `saw_inferrable_memory_design` set/merge + Phase4Hierarchy gap; bin 216→219 / 864→876; `phase6_inferrable_memory_scenario_is_non_vacuous` proves ≥1 memory module/strategy (coverage fact reachable); phase4 bin 3/3; ROADMAP unchanged. **Frontier `.2.4`** = run the real repo-owned `Phase4Hierarchy` gate, verify `coverage_gaps=[]` + Verilator/both-Yosys all-pass + `saw_inferrable_memory_design=true` (+ P4/P5/P5b regressions clean), THEN record memory **delivered** in ROADMAP Phase 6 + reconcile `book/src/ir.md`/`knobs.md` (Phase 6 stays open for `.3` FSM — no tree closure; r87 no-aspirational-claims). **`.2.4` is gate-blocked: the real `Phase4Hierarchy` gate `/tmp/anvil-tool-matrix-phase6-p1` is currently RUNNING (~halfway, 105+/219); `.2.4` is verify/record-only — actioned when it completes.** **`.3` split + `.3.1` design done (`2026-05-18`, design-only)**: `DEVELOPMENT_NOTES.md` "Phase 6 generated-encoding FSM motif design" — empirical probe of the exact emitted SV for a 4-state Moore FSM in all 3 generated encodings (binary 2-bit / one-hot 4-bit / gray 2-bit): every one `verilator --lint-only` exit 0 + `yosys synth -noabc; check -assert` + `synth; abc -fast; check -assert` clean (both repo modes); width/constants differ by encoding ⇒ encoding-selectability structural. Architecture **(F)**: additive `Vec<Fsm>` on `Module` + opaque `Node::FsmOut` leaf (sibling to `FlopQ`/`MemRead`, never CSE'd, same `compact.rs` reachability obligation as `.2.1a`) + encoding-derived-constant emitter on shared `clk` + opt-in `Config::fsm_prob` serde-default 0.0; rejected/deferred (A) primitives-only / (B) emitter-only string / (C) generic enum-datatype / (D) Mealy (deferred). `.3` container `.3.1`(done)/`.3.2`/`.3.3`/`.3.4` mirrors memory `.2.1`–`.2.4`. **FSM is the LAST Phase 6 motif** → a verified-clean `.3.4` gate closes ROADMAP Phase 6 + the `PHASE-6-ADVANCED-MOTIFS` tree (memory delivered at `.2.4`; multi-clock CDC = explicitly-optional separately-prioritised deferral, not a blocker). **`.3.2` split (`2026-05-18`) → `.3.2a` (FSM IR core + opaque `Node::FsmOut` through all exhaustive `Node` matches + load-bearing `compact.rs` reachability keeping `fsm.sel` alive + emitter + validator + 3 unit proofs; no generator/knob ⇒ default-off byte-identical) + `.3.2b` (`Config::fsm_prob` + rules-first `build_fsm_block` + focused proof) — the opaque-stateful-leaf reachability is correctness-critical, known concretely from the landed memory `.2.1a` (`FsmOut` ≡ `MemRead` obligation). **`.3.2a` done** (`2026-05-18`): FSM IR core + opaque `Node::FsmOut` threaded through every exhaustive `Node` match + the load-bearing `compact.rs` reachability (keeps `fsm.sel` alive) + emitter (.3.1-probed-clean template) + validator 5c + 3 unit proofs; full `cargo test` green; default-off trivially byte-identical. **`.3.2` container done** (`2026-05-18`): `.3.2a` IR core + `.3.2b` `Config::fsm_prob` + rules-first `build_fsm_block` (single opt-in roll mutually exclusive with the param + memory lanes; default-off byte-identical; forced-on every single-module design a 1-`Fsm` leaf emitting the `.3.1`-probed-clean template, all 3 encodings reachable); full `cargo` gate green. **`.3.3` done** (proof-only); **`.3.4` split + `.3.4a` done** (`2026-05-18`): `.3.4a` mirrors memory `.2.3` — `num_fsm_modules` metric + `phase6_fsm` scenario (anchor clone, only `fsm_prob=1.0`) + `saw_fsm_design` fact/gap + `phase6_fsm_scenario_is_non_vacuous`; bin **222 scenarios / 888 modules** (verified); tool_matrix 29/29; full `cargo` gate green; no ROADMAP advance. **`.2.4` DONE (`2026-05-18`) → memory delivered, `.2` container CLOSED:** the real `Phase4Hierarchy` gate completed (bg `bd3jdtx31`, ~10h, 219/219) and `/tmp/anvil-tool-matrix-phase6-p1/tool_matrix_report.json` was verified clean before promotion (r87): 219 scenarios / 876 designs, `coverage_gaps=[]`, Verilator 876/0 + yosys-without-abc 876/0 + yosys-with-abc 876/0, `saw_inferrable_memory_design=true`, P4/P5/P5b facts true in the same banked artifact; ROADMAP Phase 6 `(not started)→(in progress)` + "Memory motif delivered" + "Exit criteria (memory met; phase closes at FSM)", `book/src/ir.md` "Unpacked arrays"→Delivered(Phase 6), `book/src/knobs.md` += `memory_prob`/`fsm_prob`, README + CODEBASE phase-map reconciled. **Single remaining Phase 6 leaf = `.3.4b`** (the machine is now FREE — run a *fresh* real `Phase4Hierarchy` gate with the post-`.3.4a` binary, expected 222 scenarios / 888 designs, verify `coverage_gaps=[]` + Verilator/both-Yosys all-pass + `saw_fsm_design`+`saw_inferrable_memory_design`=true + P4/P5/P5b clean → records FSM delivered + **closes ROADMAP Phase 6 + the PHASE-6-ADVANCED-MOTIFS tree**). Multi-clock CDC = optional separately-prioritised deferral; r87 no-aspirational-claims.**
    - `PHASE-7-ORACLE-MICRODESIGN` — **tree COMPLETE (`2026-05-20`)**: Phase 7 closed; verified-clean parity gate against yosys 0.64 on closing artifact `/tmp/anvil-microdesign-parity-phase7-yosys-p1/`. Historical milestone trail below. **`.1` design done** (`2026-05-18`, design-only): `DEVELOPMENT_NOTES.md` "Phase 7 oracle-backed micro-design artifact family design" — oracle-by-construction (generator evaluates every const-expr/param node as it builds it; emits `.sv` + JSON manifest from the same resolved values; no analysis pass/re-parse); its own source-level const/parameter IR + separate generator path (circuit IR has no param/generate/package concept); `rtl_const_expr` family; expected-facts JSON schema; new parity harness (≠ `tool_matrix` DUT gate; cargo structural-equiv + repo-owned gate); Phase-8/9 boundaries; 4 rejected alternatives; both Open Questions resolved. **`.2` split (`2026-05-18`, tree-planning docs-only) → `.2a` (const-expr/parameter IR + construction-time evaluator/oracle; unit-proven, no emit/harness), `.2b` (SV + JSON-manifest emitters, artifact-family flag, default-off DUT-byte-identical), `.2c` (parity harness + tool-gated repo-owned gate → ROADMAP Phase 7 only after a verified clean run, r87)** — the boundaries `.1` named; `.2a`+`.2b` evaluator/manifest core is the reuse PHASE-8.2 + Phase-9 plumbing depend on. **`.2a` DONE (`2026-05-19`)**: new separate top-level module `src/microdesign/` (`pub mod microdesign`; NOT in `src/ir/`) — `ConstExpr`/`ParamDecl`(+`value` oracle)/`ConstExprUnit` IR + `eval`/`resolve` construction-time evaluator (SV-const-expr `i128` semantics; defensive `EvalError`) + reproducible rules-first `build_constexpr_unit(seed,n)` (ChaCha8, project convention). 4 unit proofs incl. the load-bearing oracle-no-drift invariant; full `cargo test` green (lib→225); no emit/harness/ROADMAP/book change. **`.2b` DONE (`2026-05-19`)**: `src/microdesign/` extended — `expr_to_sv` (fully-parenthesized printer), `emit_sv` (un-resolved `rtl_const_expr` SV: `package mc_<seed>_pkg`/`K`, symbolic `parameter`/`localparam` chains, `PKG_REF=mc_<seed>_pkg::K`, expr-derived `W_SIG`+`sig`, `generate if/else`), `Manifest`+`build_manifest`/`emit_manifest` (the `.1` JSON schema, all facts from the `.2a` oracle, `BTreeMap` ⇒ byte-stable). Default-off DUT-byte-identical is structural (separate module, never invoked by the DUT path). 7 microdesign unit proofs total; full `cargo test` green. Frontier → `.2c` (tool-gated parity harness + repo-owned gate → ROADMAP Phase 7, r87). PHASE-8.2 / Phase-9 reuse this evaluator+manifest core.
    - `PHASE-8-FRONTEND-ACCEPT` — **tree COMPLETE (`2026-05-20`)**: Phase 8 closed; verified-clean parity gate against yosys 0.64 on closing artifact `/tmp/anvil-frontend-parity-phase8-yosys-p1/` (**first try clean**, no fix-and-retry needed — cross-tree reuse of Phase 7's `expr_to_sv` carried `.2c.2b.1`'s non-negative-modulo-idiom fix forward at zero incremental cost). Historical milestone trail below. **`.1` design done** (`2026-05-18`, design-only): `DEVELOPMENT_NOTES.md` "Phase 8 frontend/elaboration accept-corpus source-IR design" — compact elaboratable hierarchies emitted with params UNRESOLVED in SV text + a manifest of what elaboration must resolve; dedicated source-level AST IR (separate generator path; post-elaboration circuit IR can't express modules/params/packages/generate); source-IR sketch; instance-tree manifest schema (extends Phase 7's); oracle-by-construction reusing Phase 7's evaluator/manifest core; hierarchy-aware parity harness; 4 rejected alternatives; Open Question resolved (`.2` sequences after `PHASE-7-ORACLE-MICRODESIGN.2`). Frontier → `.2` (expected to split).
    - `PHASE-9-MULTI-ARTIFACT-UMBRELLA` — **tree COMPLETE (`2026-05-20`)**: Phase 9 closed; `src/umbrella/` carries the `ArtifactLane` trait + all 3 lane impls + 8 cargo-portable proofs; `src/main.rs` carries the `--artifact <lane>` CLI flag (default `dut`); load-bearing byte-identical default-`dut` contract verified by `tests/book_examples::every_runnable_book_bash_block_succeeds` 3/3 in 80s AFTER the CLI change. **All 9 numbered roadmap phases delivered.** Historical milestone trail below. **`.1` design done** (`2026-05-18`, design-only): `DEVELOPMENT_NOTES.md` "Phase 9 multi-artifact umbrella selector design" — anti-goal (unify plumbing, not generators); `ArtifactLane` trait + umbrella-owned shared seed/manifest/knob/output/check plumbing; **default-`dut` `--artifact` flag** keeping the CI-gated book + CI byte-identical (load-bearing vs `BOOK-EXAMPLES-RUNNABLE`); L1-wrap lane-migration (no retrofit; Phases 7/8 built to the contract); 4 rejected alternatives; Open Question resolved. Frontier → `.2` (blocked until ≥2 delivered lanes: DUT + ≥1 of Phase 7/8).
  - `INSTA-SNAPSHOTS` — **tree COMPLETE** (`2026-05-18`, all 3 leaves done): `.1` `insta` pinned `=1.47.2` (Cargo.lock unchanged) + `tests/snapshots.rs` baseline → `.2` 6 fully-deterministic byte-stable shapes (canonical leaf / recursive library / recursive on-demand / sibling-route / parent-composed / dedup-canonical-signatures[`hierarchy_module_dedup=true`]) → `.3` `COMMIT.md` non-negotiable snapshot-acceptance protocol (failing snapshot never silently regenerated; deliberate reviewed accept in the same slice as the explaining generator change) + `book/src/knobs.md` "### Snapshot guard-rails" (no runnable bash fence ⇒ BOOK-EXAMPLES-RUNNABLE harness unaffected). The byte-identical contract is now enforced (by `cargo test`) and its acceptance discipline codified.
  - `DIFFERENTIAL-SIMULATION` — **`.1` done** (`2026-05-18`, research-only): installed Icarus Verilog 13.0; empirically proved iverilog ingests **all 4** ANVIL output categories (comb leaf / seq-flop leaf / recursive hierarchy / helper-sibling routes) with **zero config** beyond `-g2012` (`iverilog -o /dev/null` exit 0 silent; Verilator clean). Differential pair = **Verilator (2-state, cycle) ↔ iverilog (4-state, event)** — engine-independent. Sole divergence (pre-reset 4-state) documented as a `.2` harness design constraint (reset → single canonical post-reset sample point → compare defined bits); 4 rejected alternatives. `iverilog` now installed locally. **`.2` split + `.2a` design done** (`2026-05-18`, design-only): `DEVELOPMENT_NOTES.md` "Single-design differential harness design" — IR-driven testbench (ports from `Design`/`Module`, not SV re-parse), reset + single canonical post-reset sample point (neutralises the pre-reset 4-state gap), **baked** deterministic stimulus (not per-sim `$random`), `iverilog -g2012`+`vvp` / `verilator --binary` orchestration, byte-equal compare w/ retained counterexamples, **tool-gated `#[ignore]` test so portable `cargo test` stays green tool-less** (Phase-1 doctrine; `.2b` ≈ zero mandatory-gate runtime), 5 rejected alternatives. Frontier → `.2b` (implement). Quality lane: signoff-level downstream consistency.
  - `COVERAGE-INSTRUMENTATION` — **tree COMPLETE** (`2026-05-18`, all 3 leaves done): `.1` cargo-llvm-cov baseline → `.2` top-5 triage (no dead code; right-sized `.3`) → `.3` acted on it — added the focused proof `constant_pressure_exhausts_cone_retry_and_stays_valid_and_reproducible` (the one real proof-gap: `pick_terminal` constant branch + `build_cone_with_retry` rollback/budget-exhausted accept; invariant = constant pressure can't break the pipeline), `config.rs` orphan-knob audit (3/74 fields unconsumed but all documented-reserved in `book/src/knobs.md` ⇒ no dead code, leave as-is), defensive/gate-exclusive/integration-only left as documented, baseline refreshed via `cargo llvm-cov --release` (= the COMMIT.md cargo-test gate) + `docs/coverage-baseline.md` `.3` addendum. Only the one isolated `tests/pipeline.rs` proof is new code.
  - `BOOK-EXAMPLES-RUNNABLE` — **tree complete** (`2026-05-18`, all leaves `done`; Quality lane: user-facing book correctness). `.1` design → `.2.1` convention migration (45 line-leading `anvil`→`cargo run --release --`) → `.2.2` embedded migration (1 `$()`-embedded) + 9 reasoned skip sentinels + 32 bare ``` → ```text + `tests/book_examples.rs` (std-only cargo integration harness: parse every ```bash fence, honour skip sentinel, substitute `cargo run --release --`→`"$ANVIL"`, classification-guard panic on residual, run via `bash -eu -o pipefail` in fresh temp CWD offline, exit-0; + negative control + skip-reason tests) + `.github/workflows/ci.yml` `mdbook test book` step. **Pipe-buffer deadlock root-caused & fixed** (default module ≈86 KB > ≈64 KB OS pipe; `Stdio::piped()`+undrained `try_wait()` → 12 false 600s timeouts ≈7273s; fix = child stdio to temp files + reap after kill; examples themselves run 0.03–0.15s). `cargo test --test book_examples` 3/3 green, 54 runnable exit-0, 9 skipped, 76.4s; full `cargo test`/clippy/fmt/`mdbook test` clean. Every book example now CI-gated against drift.
...
## Recent commits
- `<pending>` — **DIFFERENTIAL-SIMULATION.2b.1 IR-driven testbench harness + dual-sim orchestration + cargo-portable helper proofs + #[ignore] scaffold.** New `tests/diff_sim.rs` (~400 lines) carries the single-design iverilog↔verilator differential harness per `.2a`'s design: `baked_input_vectors` (canonical-edge-case + seeded-ChaCha8); `fmt_sv_hex` (fixed-width SV hex via `u32::div_ceil(4)`); `is_sequential` (`top.has_local_flops() || has_local_memories() || has_local_fsms()` — NOT `top.clock.is_some()` since every ANVIL module declares `clk`/`rst_n` per the synchronous-design discipline); `emit_testbench` (reads `top.inputs`/`top.outputs` from IR; combinational hold+settle+sample / sequential clock+reset+warmup+per-cycle); `run_iverilog`/`run_verilator` shell the respective tools; `normalize_trace` filters to hex-only lines. **5 cargo-portable helper proofs** + **2 tool-gated `#[ignore]` tests** (combinational + sequential). Fixed 3 clippy hits (unused-imports, manual-div-ceil, field-reassign-with-default). `cargo fmt`/`clippy --all-targets -- -D warnings`/`check --all-targets` clean. Full `cargo test` green: `tests/diff_sim` 5 portable + 2 ignored; all other tests unchanged. Portable stays green tool-less. **Split rationale (`.2b` → `.2b.1`/`.2b.2`)**: the first real-tool gate run surfaced an iverilog↔verilator trace-alignment issue (iverilog samples one extra time at vector[0]; underlying sequences match shifted by one) — the harness SV is correct; the alignment discipline needs one more iteration. Per Splitting Rules + `PHASE-7-ORACLE-MICRODESIGN.2c.2a`/`.2c.2b` precedent, `.2b.1` lands the scaffold + portable proofs + documented alignment issue; `.2b.2` is the alignment fix + verified end-to-end run. Frontier → `DIFFERENTIAL-SIMULATION.2b.2`.
- `2f7e6a5` — **Phase 9: PHASE-9-MULTI-ARTIFACT-UMBRELLA.2c `--artifact` CLI flag + book/CI byte-identical — closes Phase 9 + tree.** `src/main.rs` extended with the `--artifact <lane>` top-level CLI flag: new `ArtifactKind` enum (`Dut` default / `Microdesign` / `Frontend`) + `--lane-n-params` + `--lane-n-children` knobs. Early-return guard at the top of `fn main`: `if cli.artifact != ArtifactKind::Dut { return run_non_dut_lane(&cli); }` — so `--artifact dut` (the default) falls through to the historical DUT path UNCHANGED. New `run_non_dut_lane` helper constructs `Box<dyn ArtifactLane>` per the flag (`MicrodesignLane` or `FrontendLane`), calls `lane.generate(seed)`, emits SV to stdout (or `<out>/<top>.sv`) + manifest to stderr (or `<out>/<top>.json`); helper `parse_top_name(sv)` does best-effort filename extraction. Smoke-tested both non-DUT lanes locally (output matches `.2b`'s per-lane byte-identical regression proofs). Fixed 1 clippy `unused-imports` hit. **Book/CI byte-identical verification (LOAD-BEARING)**: `cargo test --test book_examples` clean — 3/3 in 80s — the 3rd test runs every runnable bash block in the book + asserts exit 0; Phase 9's load-bearing default-`dut`-byte-identical contract is verified. Full `cargo test` green: lib 244 unchanged; microdesign_parity 15+1; frontend_parity 12+2; pipeline 121 (657s); snapshots 6; bin 5+29+3; book_examples 3 passed (80s); doc 0. Promotion strictly followed the artifact: ROADMAP Phase 9 `(not started)`→`(done)` with 3-lane-selector closure note; `book/src/ir.md` Phase-9 entry; README + CODEBASE phase-coverage-map Phase-9 row done. **Notable**: the cross-lane heterogeneous dispatch proof landed in `.2b` made the CLI dispatch correct-by-construction the moment it compiled. **Closes `.2c` + `.2` containers + `PHASE-9-MULTI-ARTIFACT-UMBRELLA` tree + ROADMAP Phase 9. ALL 9 NUMBERED ROADMAP PHASES NOW DELIVERED.** Remaining open follow-up trees (multi-clock CDC — explicitly-optional separately-prioritised deferral from Phase 6 closure; DIFFERENTIAL-SIMULATION.2b harness implementation — quality lane, not a numbered phase) carry on as post-phase work.
- `537ba6e` — **Phase 9: PHASE-9-MULTI-ARTIFACT-UMBRELLA.2b L2 MicrodesignLane + L3 FrontendLane impls of the ArtifactLane trait.** `src/umbrella/mod.rs` extended with `pub struct MicrodesignLane{n_params: usize}` (full standard derives) + `impl ArtifactLane for MicrodesignLane` calling Phase 7's `microdesign::{build_constexpr_unit, emit_sv, emit_manifest}` (returns `LaneArtifact{lane="microdesign", manifest=Some(...)}`, `check_plan=ParityVsManifest`) AND `pub struct FrontendLane{n_params, n_children}` + `impl ArtifactLane for FrontendLane` calling Phase 8's `frontend::{build_acceptable_unit, emit_sv, emit_manifest}` (`lane="frontend"`, `manifest=Some(...)`, `ParityVsManifest`). 4 new unit proofs (8 total in `umbrella::tests`): `microdesign_and_frontend_lane_identities` (smoke); **`microdesign_lane_is_byte_identical_to_direct_path`** + **`frontend_lane_is_byte_identical_to_direct_path`** (the L2 + L3 LOAD-BEARING byte-identical regression proofs across reproducibility seeds — BOTH SV AND manifest); `cross_lane_dispatch_through_dyn_artifact_lane` (heterogeneous `Vec<Box<dyn ArtifactLane>>` with all 3 lanes — each lane's name/lane-field/check-plan/manifest-presence matches; important because `.2c`'s CLI will hold a `Box<dyn ArtifactLane>` chosen by `--artifact <name>`). Fixed 5 clippy `doc-lazy-continuation` hits (a `--` token at line-start was being parsed as a markdown list marker; reworded). `cargo fmt --all --check`/`clippy --all-targets -- -D warnings`/`check --all-targets` clean. Full `cargo test` green: lib **244 passed** (was 240 + 4 new `umbrella::tests`); `tests/microdesign_parity` 15+1 unchanged; `tests/frontend_parity` 12+2 unchanged; `tests/pipeline` 121 passed; `tests/snapshots` 6 passed; bin tests 5+29+3 passed; doc-tests 0. No CLI flag (`.2c`); no ROADMAP advance. Frontier → `PHASE-9-MULTI-ARTIFACT-UMBRELLA.2c` (`--artifact <lane>` top-level CLI flag with default `dut` + book/CI byte-identical verification + record ROADMAP Phase 9 → done; may further split per Phase 7/8 `.2c` → `.2c.1`/`.2c.2` precedent on a discovered regression).
- `fb76bc6` — **Phase 9: PHASE-9-MULTI-ARTIFACT-UMBRELLA.2a ArtifactLane trait + L1 DutLane wrap + byte-identical regression proof.** New separate top-level module `src/umbrella/mod.rs` registered via `pub mod umbrella` (plumbing-only per `.1`'s anti-goal). `pub trait ArtifactLane` (`name`/`validate_knobs`/`generate(seed)`/`check_plan`) + `pub struct LaneArtifact{lane, seed, sv, manifest: Option<String>}` (typed-Option manifest expresses "L1 has no semantic manifest" cleanly) + `pub enum CheckPlan{SynthAccept, ParityVsManifest}` + `pub enum LaneError{UnknownKnob, Construction}` (placeholder for `.2b`/`.2c`). `pub struct DutLane{base_config: Config}` (NOT `Eq` because `Config` has `f64` knobs) + `impl ArtifactLane for DutLane`: `generate(seed)` clones `base_config`, overrides `.seed`, calls `Generator::new(cfg).generate_design()`, then `emit::to_sv_design(&design)` — **identical to today's invocation pattern, zero behavioural change**. 4 unit proofs (all green): `dut_lane_identity_and_check_plan` (smoke); **`dut_lane_is_byte_identical_to_direct_generator_path`** — the LOAD-BEARING byte-identical regression proof comparing trait-dispatch vs direct-`Generator` output across reproducibility seeds `{0,1,7,42,12345}` (if this breaks, every book example + CI gate that depends on `--artifact dut` regresses — `BOOK-EXAMPLES-RUNNABLE` depends on it); `dut_lane_is_byte_identical_through_dyn_artifact_lane` (`dyn` dispatch via `&dyn ArtifactLane` + `Box<dyn ArtifactLane>` produces the same byte-identical output as concrete-type dispatch — important because `.2c`'s CLI hands around `Box<dyn ArtifactLane>`); `dut_lane_is_reproducible_on_repeated_calls` (no state accumulation across calls). Fixed 2 clippy `doc-lazy-continuation` hits (a `+` at line-start was being parsed as a markdown list marker; reworded) + 1 `field-reassign-with-default` hit (used struct-update `Config{seed, ..Config::default()}`). `cargo fmt --all --check`/`clippy --all-targets -- -D warnings`/`check --all-targets` clean. Full `cargo test` green: lib **240 passed** (was 236 + 4 new `umbrella::tests`); `tests/microdesign_parity` 15+1 unchanged; `tests/frontend_parity` 12+2 unchanged; `tests/pipeline` 121 passed (656s); `tests/snapshots` 6 passed; bin tests 5+29+3 passed; doc-tests 0 (unchanged). DUT lane stays byte-identical by construction. No new lane impls (`.2b`); no CLI flag (`.2c`); no ROADMAP advance. Frontier → `PHASE-9-MULTI-ARTIFACT-UMBRELLA.2b` (L2 `MicrodesignLane` + L3 `FrontendLane` impls of the trait + cross-lane byte-identical regression proof).
- `ae8f3d1` — Docs: split PHASE-9-MULTI-ARTIFACT-UMBRELLA.2 into `.2a` (`ArtifactLane` trait + shared plumbing + L1 DUT wrap + byte-identical regression proof — load-bearing for `BOOK-EXAMPLES-RUNNABLE` + every CI gate) + `.2b` (L2 microdesign + L3 frontend lane impls under the trait + cross-lane byte-identical proof) + `.2c` (`--artifact <lane>` top-level CLI flag with default `dut` + book/CI byte-identical verification + ROADMAP Phase 9 → done; may further split per Phase 7/8 `.2c` → `.2c.1`/`.2c.2` precedent on a discovered regression). **UNBLOCKED 2026-05-20** by Phase 7 closure at `20a7b4a` + Phase 8 closure at `997b0a6` (all 3 delivered lanes now exist: DUT + microdesign + frontend; `.1`'s "DUT + ≥1 of Phase 7/8" blocker condition is satisfied). Mirrors the proven Phase 7/8 `.2` → `.2a`/`.2b`/`.2c` decomposition. Tree-planning, docs-only: `docs/tasks/PHASE-9-MULTI-ARTIFACT-UMBRELLA.md` (Metadata last-updated 2026-05-20 + tree-root Children annotation + `.2` container + `.2a`/`.2b`/`.2c` leaves + Frontier `.2`→`.2a` + Decisions entry + Verification-Log + Commit-Log + Changelog) + `docs/TASK_TREE.md` (Phase 9 row updated) + CHANGES.md + MEMORY.md. `cargo fmt`/clippy/check clean; no `src/`/`tests/` (`cargo` unchanged-green vs `997b0a6`); mdbook unchanged. Continuous-PNT immediately after closing Phase 8 + the `PHASE-8-FRONTEND-ACCEPT` tree at `997b0a6`. Frontier → `PHASE-9-MULTI-ARTIFACT-UMBRELLA.2a` (`ArtifactLane` trait + DUT wrap + byte-identical regression proof; unblocked, code-bearing).
- `997b0a6` — **Phase 8: PHASE-8-FRONTEND-ACCEPT.2c.2b parity gate clean against yosys (first try) — closes Phase 8 + tree.** Real tool-equipped run of the `.2c.2a` `#[ignore]` parity gate against yosys 0.64 (no preceding fix needed — UNLIKE Phase 7 `.2c.2b.1`, came back clean on first try): `cargo test --test frontend_parity -- --ignored parity_against_real_yosys_hierarchy_write_json --nocapture` exited 0 with stdout "parity gate clean across 5 seeds" and "test result: ok. 1 passed; 0 failed; 0 ignored; 0 measured; 13 filtered out; finished in 0.04s". Per-seed fact agreement verified BEFORE any promotion (r87): seed 0 — manifest `{P0=57, P1=38, P2=36, P3=54; u_0_0 with 4 bindings; u_0_1 with 4 bindings; gen=true}` matches yosys `.cells` + `.parameter_default_values` + `g_taken.gflag` netname; seeds 1/7/42 similarly clean with `gen=true`; **seed 12345** — `{P0=15, ...; gen=false}` matches with yosys netname `g_else.gflag` — the else-branch case exercised. **Both generate branches** in the corpus (4 seeds `g_taken`, 1 seed `g_else`). **The load-bearing hierarchy-aware Phase-8 axis** is exercised (every seed has 2 instances × 4 per-instance per-binding values verified against yosys's `.cells[<inst>].parameters`). Banked artifact copied to `/tmp/anvil-frontend-parity-phase8-yosys-p1/` per established convention (15 files: 5 × `{acc_<seed>.sv, acc_<seed>.json, acc_<seed>.yosys.json}`). Promotion strictly followed the artifact: ROADMAP Phase 8 `(not started)`→`(done)` + verified-clean citation + explicit **yosys-supported-categories scope caveat** (Seed/Top/TopParams/Instances/GenerateBranches; top_localparams + package_constants folded — richer-AST coverage via slang/verilator-with-debug is the recorded post-Phase-8 follow-up that does NOT retract closure since ANVIL's by-construction oracle already covers all 7 categories); `book/src/ir.md` gains the Phase 8 frontend-accept lane delivered note; README + CODEBASE phase-coverage-map Phase-8 row done. **Notable**: the gate came back clean on the **FIRST** try (contrast with Phase 7's `.2c.2b.1` `width_expr` non-negative-modulo-idiom fix). Cross-tree reuse of Phase 7's `expr_to_sv` carried forward Phase 7's hard-won fix at the expression layer for free — the kind of factorization payoff the user's `full-factorization` doctrine targets. No `src/`/`tests/` change in this slice (code in `.2a`/`.2b`/`.2c.1`/`.2c.2a`); mdbook build clean; cargo unchanged-green. **Closes the `.2c.2b` + `.2c.2` + `.2c` + `.2` containers + `PHASE-8-FRONTEND-ACCEPT` tree + ROADMAP Phase 8.** ANVIL now ships THREE complementary lanes (DUT, microdesign, frontend) with cross-tree-shared `ConstExpr`/`eval`/`expr_to_sv` at the expression layer. Continuous-PNT proceeds → `PHASE-9-MULTI-ARTIFACT-UMBRELLA.2` (now unblocked with two delivered post-DUT lanes) or `DIFFERENTIAL-SIMULATION.2b`.
- `4f7867d` — **Phase 8: PHASE-8-FRONTEND-ACCEPT.2c.2a yosys hierarchy write_json extractor + end-to-end-runnable #[ignore] harness.** `tests/frontend_parity.rs` extended with `parse_yosys_binary_param(s) -> Option<i128>` (signed-32-bit sign-extension symmetric to Phase 7) + `yosys_hierarchy_write_json_to_tool_report(json, seed)` (reads `.parameter_default_values` → `top_params`; reads **`.cells[<inst>].{type, parameters}` → `instances` with `child_module` + per-binding `resolved_bindings` — the load-bearing hierarchy-aware Phase-8 axis**; reads `.netnames` key prefix `g_taken.`/`g_else.` → `generate_branches["g_taken"]`; folded axes [package_constants, top_localparams] deliberately empty since yosys doesn't expose them by name) + `yosys_hierarchy_scope() -> ParityScope` (`only(&[Seed, Top, TopParams, Instances, GenerateBranches])`). `parity_against_real_yosys_hierarchy_write_json` rewritten end-to-end: per-seed `emit_sv` → `CARGO_TARGET_TMPDIR/frontend-parity-phase8-yosys/acc_<seed>.sv`; `emit_manifest` → `.json`; shell `yosys -q -p "read_verilog -sv <sv>; hierarchy -top acc_<seed>; write_json <out.json>"` (**no `proc; opt`** — the `.2c.2` probe confirmed it collapses empty-bodied child instances out of `.cells`); parse → ToolReport; scoped compare with `yosys_hierarchy_scope`; panic on non-empty counterexample list (else `eprintln "parity gate clean across N seeds"`); yosys-presence guard. `.2c.1`'s `parity_against_real_downstream_elaborator` preserved as a friendly no-op pointing at the named yosys test. 2 new cargo-portable extractor proofs (12 portable + 2 ignored total): `yosys_extractor_reads_a_synthetic_hierarchy_write_json_correctly` (hand-built JSON for seed 0 with 2 instances × 2 bindings each — exercises every branch); `yosys_extractor_reports_g_else_when_else_branch_survives` (g_else-survives → `generate.g_taken=false`). `cargo fmt --all --check`/`clippy --all-targets -- -D warnings`/`check --all-targets` clean. Full `cargo test` green: `tests/frontend_parity` **12 passed + 2 ignored** (was 10 + 1); rest unchanged. Portable `cargo test` stays green tool-less. **NO real cargo-test run of the `#[ignore]`** (that is `.2c.2b`). No ROADMAP advance; no book/ change. Frontier → `PHASE-8-FRONTEND-ACCEPT.2c.2b` (run `cargo test -- --ignored parity_against_real_yosys_hierarchy_write_json` against real yosys + verify exact-agreement on yosys-supported 5-of-7 categories + bank artifact at `/tmp/anvil-frontend-parity-phase8-yosys-p1/` + record ROADMAP Phase 8 → done with the explicit scope caveat + reconcile book/README/CODEBASE; may further split per Phase 7 `.2c.2b` → `.2c.2b.1`/`.2c.2b.2` on a discovered ANVIL self-consistency bug).
- `c86906e` — Docs: split PHASE-8-FRONTEND-ACCEPT.2c.2 into `.2c.2a` (yosys-specific extractor + end-to-end-runnable `#[ignore]`; cargo stays green tool-less) + `.2c.2b` (real `--ignored` gate run + verify + bank + record ROADMAP Phase 8 → done; gate-blocked, r87 no-aspirational-claims; may further split per Phase 7 `.2c.2b` → `.2c.2b.1`/`.2c.2b.2` on a discovered ANVIL self-consistency bug). Triggered by today's empirical probe of `yosys hierarchy -top acc_0; write_json` on a Phase-8 `acc_<seed>.sv` immediately after `.2c.1` landed at `977c632`: yosys covers **5 of 7 manifest fact categories** (top params via `.parameter_default_values` + **instances + per-instance per-binding values via `.cells[<inst>].{type, parameters}` — the load-bearing hierarchy-aware Phase-8 axis** + generate-branch via netname-prefix scan; top_localparams + package-constants folded — richer-AST tools see them); AND that `proc; opt` collapses the empty-bodied child instances away from `.cells` (so the yosys invocation pattern omits `proc; opt`). Mirrors `PHASE-7-ORACLE-MICRODESIGN.2c.2` → `.2c.2a`/`.2c.2b` discovered-dependency precedent. Tree-planning, docs-only: `docs/tasks/PHASE-8-FRONTEND-ACCEPT.md` (Metadata last-updated 2026-05-20 + `.2`/`.2c` Children annotations + `.2c.2` container + `.2c.2a`/`.2c.2b` leaves + Frontier `.2c.2`→`.2c.2a` + Decisions entry + Verification-Log + Commit-Log + Changelog) + `docs/TASK_TREE.md` (Phase 8 frontier updated) + CHANGES.md + MEMORY.md. `cargo fmt`/clippy/check clean; no `src/`/`tests/` (`cargo` unchanged-green vs `977c632`); mdbook unchanged. Continuous-PNT immediately after `.2c.1` landed. Frontier → `PHASE-8-FRONTEND-ACCEPT.2c.2a` (unblocked, code-bearing).
- `977c632` — **Phase 8: PHASE-8-FRONTEND-ACCEPT.2c.1 hierarchy-aware parity harness — comparator core + cargo-portable proofs + tool-gated #[ignore] scaffold.** `src/frontend/mod.rs` extended with the Phase-8-specific parity comparator core (parallel to Phase 7's microdesign types, NOT derived — different artifact shape: Phase 8 carries instances + per-instance bindings): `ToolReport{seed, top, package_constants, top_params, top_localparams, instances: Vec<InstanceToolReport>, generate_branches}`, `InstanceToolReport{inst_name, child_module, resolved_bindings}`, `Divergence` (**23 variants** — `SeedMismatch`/`TopMismatch` + per-category `{MissingInTool, MissingInManifest, Mismatch}` × `{PackageConstants, TopParams, TopLocalparams, GenerateBranches}` + **load-bearing hierarchy-aware additions** `InstanceMissingInTool`/`InstanceMissingInManifest`/`InstanceChildModuleMismatch`/`InstanceBindingMissingInTool`/`InstanceBindingMissingInManifest`/`InstanceBindingMismatch`), `FactCategory` (7 axes), `ParityScope` with `all()`/`none()`/`only(&[...])`, `compare_manifest_to_tool_report` + `_in_scope` (the `Instances` arm builds name-keyed `BTreeMap` indices for order-independent presence + per-binding compares), `synthetic_tool_report_from_manifest`. New `tests/frontend_parity.rs` carries **10 cargo-portable comparator proofs** (all green): baseline agreement across `{0,1,7,42,12345}` + per-axis perturbation for top-param / top-localparam / package-constant / **instance-binding (the hierarchy-aware addition — perturbs ONE binding on ONE instance + asserts no spurious divergence on the OTHER instance)** / generate-branch / **instance-presence (drop + add — the other hierarchy-aware addition)** / seed+top + scoped-comparator + empty-scope self-check. Plus 1 tool-gated `#[ignore]` `parity_against_real_downstream_elaborator` scaffold (any-of-yosys/slang/verilator presence guard; corpus-driver loop wired against same SEEDS/N_PARAMS/N_CHILDREN; placeholder for `.2c.2`-owned `emit_sv`→shell→extract→compare wiring). `cargo fmt --all --check`/`clippy --all-targets -- -D warnings`/`check --all-targets` clean. Full `cargo test` green: `tests/frontend_parity` **10 passed + 1 ignored**; `tests/microdesign_parity` 15+1 unchanged; `tests/pipeline` 121 passed (758s); `tests/snapshots` 6 passed; lib **236 passed** (unchanged); bin 5+29+3; doc 0. Portable `cargo test` stays green tool-less. No ROADMAP advance (that is `.2c.2` on a verified clean banked artifact, r87 no-aspirational-claims). No book/ change. Frontier → `PHASE-8-FRONTEND-ACCEPT.2c.2` (real `--ignored` run against a downstream elaborator + verify exact-agreement + bank artifact + record ROADMAP Phase 8 → done; may further split per Phase 7 `.2c.2` → `.2c.2a`/`.2c.2b` precedent on a discovered tool-capability dependency).
- `686afd8` — Docs: split PHASE-8-FRONTEND-ACCEPT.2c into `.2c.1` (build the hierarchy-aware parity harness — cargo-portable comparator + tool-gated `#[ignore]` real-tool scaffold; no real run, no ROADMAP advance, cargo stays green tool-less) + `.2c.2` (real `--ignored` run + verify exact-agreement + bank artifact + record ROADMAP Phase 8 → done; gate-blocked, r87 no-aspirational-claims; may further split per `PHASE-7-ORACLE-MICRODESIGN.2c.2` → `.2c.2a`/`.2c.2b` precedent on a discovered tool-capability dependency). Exactly mirrors the proven `PHASE-7-ORACLE-MICRODESIGN.2c` → `.2c.1`/`.2c.2` decomposition that closed Phase 7 on 2026-05-20. Tree-planning, docs-only: `docs/tasks/PHASE-8-FRONTEND-ACCEPT.md` (Metadata last-updated 2026-05-20 + `.2` Children annotation + `.2c` container + `.2c.1`/`.2c.2` leaves + Frontier `.2c`→`.2c.1` + Decisions entry + Verification-Log + Commit-Log + Changelog entries) + `docs/TASK_TREE.md` + CHANGES.md + MEMORY.md. `cargo fmt`/clippy/check clean; no `src/`/`tests/` (`cargo` unchanged-green vs `d67df0c`); mdbook unchanged. Continuous-PNT immediately after `.2b` landed. Frontier → `PHASE-8-FRONTEND-ACCEPT.2c.1` (hierarchy-aware parity harness build leaf; unblocked, code-bearing).
- `d67df0c` — **Phase 8: PHASE-8-FRONTEND-ACCEPT.2b un-elaborated SV emitter + elaborated-facts JSON manifest emitter (from the .2a oracle).** `src/frontend/mod.rs` extended with `pub fn emit_sv(unit) -> String` (full un-elaborated SV — provenance comment + package + child stubs + top with symbolic parameter ports / chained localparams / named-binding instance instantiations / generate-if with g_taken/g_else gflag markers; all expressions via cross-tree-reused `crate::microdesign::expr_to_sv`) and manifest types `PackageFacts`/`ParamFact`/`InstanceFact`/`GenerateFact`/`Manifest` (all `pub`+serde `Serialize+Deserialize`, `BTreeMap` for deterministic key order) + `build_manifest(unit) -> Manifest` (reads `.2a` oracle directly; no re-evaluation — load-bearing on the oracle-no-drift proof) + `emit_manifest(unit) -> String` (`serde_json::to_string_pretty`; byte-stable). 3 new lib proofs (10 total in `frontend::tests`): `emit_sv_is_valid_unresolved_shape` (full structural pin); `manifest_mirrors_the_oracle` (across seeds `{0,1,7,42,12345}` parsed JSON matches the oracle on every fact axis); `sv_and_manifest_are_byte_reproducible` (same `(seed, shape)` → byte-identical .sv + .json). Fixed 2 `single_char_add_str` clippy hits (`push_str(")")` → `push(')')`). `cargo fmt --all --check`/`clippy --all-targets -- -D warnings`/`check --all-targets` clean. Full `cargo test` green: lib **236 passed** (was 233 + 3 new); `frontend::tests` **10/10** (was 7); `microdesign::tests` 8/8 unchanged; `tests/microdesign_parity` 15 passed + 1 ignored; `tests/pipeline` 121 passed (654s); `tests/snapshots` 6 passed; bin 5+29+3 passed; doc 0. DUT lane stays byte-identical-by-construction. No ROADMAP advance (`.2c`); no book/ change (`.2c`). Frontier → `PHASE-8-FRONTEND-ACCEPT.2c` (hierarchy-aware parity harness + repo-owned gate; reuses Phase 7's scoped comparator with hierarchy-aware variants; expected to split per the proven `PHASE-7-ORACLE-MICRODESIGN.2c` → `.2c.1`/`.2c.2` etc. decomposition).
- `603aaa5` — **Phase 8: PHASE-8-FRONTEND-ACCEPT.2a source-level AST IR + construction-time elaboration-evaluator (oracle).** New separate top-level module `src/frontend/mod.rs` (registered via `pub mod frontend`; sits between `emit` and `gen` after `cargo fmt`'s alphabetical sort). AST IR types: `SourceUnit{seed, packages, children, top}` (depth-1 instance tree), `Package{name, items}`, `PackageItem::Localparam(ParamDecl)`, `Module{name, params, body}`, `ParamDecl{name, kind, expr, value}`, `ModuleItem::Localparam | Instance | GenerateIf`, `Instance{inst_name, child_module, param_bindings}` (named bindings only in `.2a`), `ParamBinding{name, expr, resolved}`, `GenerateIf{label, else_label, condition, taken, then_branch, else_branch}`. Every type derives `Debug+Clone+PartialEq+Eq`. **Cross-tree reuse** per `.1`'s full-factorization plan: `use crate::microdesign::{eval, BinOp, ConstExpr, EvalError, ParamKind}` — Phase 7's `ConstExpr`/`eval` are the expression layer. Construction-time elaboration-evaluator `pub fn elaborate(&mut SourceUnit) -> Result<BTreeMap<String, i128>, EvalError>` walks packages → top params → top body (Localparams extend env in declaration order; Instance bindings resolve in parent's env and populate `ParamBinding.resolved`; `GenerateIf.taken = eval(condition) != 0`; else-branch sandboxed clone). **Builder IS the oracle**: every `.value`/`.resolved`/`.taken` set in place. Rules-first reproducible `build_acceptable_unit(seed, n_params, n_children)`: one `ChaCha8Rng`; package `acc_<seed>_pkg`/`K`; child stub `child_<seed>`; top `acc_<seed>` with N params + N chained localparams + M named-binding instances + 1 `GenerateIf(P0 >= pkg::K)`. **4 unit proofs (all green):** `build_acceptable_unit_has_the_documented_shape` (smoke), `unit_is_reproducible_and_seed_sensitive` (load-bearing reproducibility — same `(seed, shape)` → byte-identical across rebuilds across reproducibility-set seeds `{0,1,7,42,12345}`), `elaboration_evaluator_resolves_every_axis` (package K + literal-rooted top params + localparam re-eval + GenerateIf.taken), `elaborated_facts_match_a_fresh_reeval_across_the_seed_set` (**load-bearing oracle-no-drift invariant** — Phase-8 counterpart of Phase 7's `stored_values_are_consistent_with_a_fresh_reeval`; every stored fact equals a fresh eval; covers all 5 axes; seeds 0..=8). `cargo fmt`/clippy/check clean; full `cargo test` green (lib **233 passed** ← was 229 + 4 new; microdesign 8/8 unchanged; microdesign_parity 15+1; pipeline 121; snapshots 6; bin 5+29+3; doc 0). DUT lane stays byte-identical by construction (`frontend` never invoked from `gen::*`). No SV/manifest emit (`.2b`); no harness (`.2c`); no ROADMAP/book change. Frontier → `PHASE-8-FRONTEND-ACCEPT.2b` (un-elaborated-SV emitter + elaborated-facts JSON manifest emitter from the same `.2a` oracle).
- `d36020e` — Docs: split PHASE-8-FRONTEND-ACCEPT.2 into `.2a` (source-level AST IR + construction-time elaboration-evaluator) + `.2b` (un-elaborated-SV + elaborated-facts JSON manifest emitters) + `.2c` (hierarchy-aware parity harness + repo-owned gate → ROADMAP Phase 8). Exactly mirrors the proven `PHASE-7-ORACLE-MICRODESIGN.2`→`.2a`/`.2b`/`.2c` decomposition that closed Phase 7 on 2026-05-20. Unblocked now that Phase 7 closed: `src/microdesign/` in-tree, the Phase-7 `ConstExpr` set ready to be cross-tree-imported as the Expr layer of Phase 8's source IR, the scoped comparator (`ToolReport`/`Divergence`/`FactCategory`/`ParityScope`/`compare_manifest_to_tool_report_in_scope`) the shape `.2c` extends with hierarchy-aware variants. Tree-planning, docs-only: `docs/tasks/PHASE-8-FRONTEND-ACCEPT.md` (Metadata last-updated 2026-05-20 + tree-root Children annotation + `.2` container + `.2a`/`.2b`/`.2c` leaves + Frontier `.2`→`.2a` + Decisions entry + Verification-Log + Commit-Log + Changelog) + `docs/TASK_TREE.md` (Phase 8 row frontier updated) + CHANGES.md + MEMORY.md. `cargo fmt --all --check`/`clippy --all-targets -- -D warnings`/`check --all-targets` clean; no `src/`/`tests/` (`cargo` unchanged-green vs `20a7b4a`); mdbook unchanged. Continuous-PNT immediately after closing Phase 7. Frontier → `PHASE-8-FRONTEND-ACCEPT.2a` (source-IR-and-evaluator code-bearing slice; unblocked).
- `20a7b4a` — **Phase 7: PHASE-7-ORACLE-MICRODESIGN.2c.2b.2 parity gate clean against yosys — closes Phase 7 + tree.** Real tool-equipped re-run of the `.2c.2a` `#[ignore]` parity gate against yosys 0.64 (with `.2c.2b.1`'s width_expr non-negative-modulo idiom in place): `cargo test --test microdesign_parity -- --ignored parity_against_real_yosys_write_json --nocapture` exited 0 with stdout "parity gate clean across 5 seeds" and "test result: ok. 1 passed; 0 failed; 0 ignored; 0 measured; 15 filtered out; finished in 0.04s". Per-seed fact agreement verified BEFORE any promotion (r87): seed 0 {P0=46, bits=6, taken=true} ≡ matching; seed 1 {P0=26, P2=27, bits=8, taken=true} ≡ matching; **seed 7 (the previously-divergent case)** {P0=9, P3=3, P4=-1, bits=8, taken=true} ≡ {P4 sign-extended from "11..1" → -1, sig 8-bit, g_taken} — the `.2c.2b.1` fix worked perfectly; seed 42 {P0=44, bits=3, taken=true} ≡ matching; seed 12345 {P0=43, P3=47, P4=186, bits=3, taken=false} ≡ {matching, netname g_else.gflag}. **Both generate branches** exercised (4 seeds g_taken, 1 seed g_else); **negative parameter sign-extension** correct; **previously-divergent seed 7 widths["sig"] now agrees**. Banked artifact copied to `/tmp/anvil-microdesign-parity-phase7-yosys-p1/` per established convention (15 files: 5 × {`.sv`, `.json`, `.yosys.json`}). Promotion strictly followed the artifact: ROADMAP Phase 7 `(not started)`→`(done)` + verified-clean citation + explicit **yosys-supported-categories scope caveat** (Seed/Top/Params/Widths/Generate; localparams+package-constants folded by yosys; richer-AST coverage via slang/verilator-with-debug is a recorded post-closure follow-up that does NOT retract Phase 7 closure since ANVIL's by-construction oracle already covers all 7 categories); `book/src/ir.md` gains the Phase 7 micro-design lane delivered note; README + CODEBASE phase-coverage-map Phase-7 row done. No `src/`/`tests/` change (code in `.2a`/`.2b`/`.2c.1`/`.2c.2a`/`.2c.2b.1`); mdbook build clean; cargo unchanged-green. **Closes the `.2c.2b` + `.2c.2` + `.2c` + `.2` containers + `PHASE-7-ORACLE-MICRODESIGN` tree + ROADMAP Phase 7.** Continuous-PNT proceeds → `PHASE-8-FRONTEND-ACCEPT.2`.
- `590be2d` — **Phase 7: PHASE-7-ORACLE-MICRODESIGN.2c.2b.1 width_expr oracle/SV semantic alignment.** `src/microdesign/mod.rs::width_expr` SV text changed from `(<last> % 8) + 1` to the standard SV non-negative-modulo idiom `((<last> % 8) + 8) % 8 + 1`. A SV elaborator now evaluates the new text to exactly the same value Rust's `rem_euclid` produces for the oracle, for any `last.value` (positive or negative). The oracle's bits formula was left UNCHANGED because it was already correct — only the SV text needed to catch up. New lib regression proof `width_expr_uses_sv_non_negative_modulo_idiom_and_agrees_for_negative_last_values` (3 axes): (a) the `.2c.2a` counterexample fixture — seed 7's reproducible `P4=-1` → `widths.sig.bits=8`; (b) non-negative collapse — seed 0 `P4=365` → 6 (matches old formula); (c) cross-seed structural pin — every reproducibility-set seed's W_SIG line uses the new idiom textually. Existing `emit_sv_is_valid_unresolved_shape` substring pin updated to match the new emit. `cargo fmt --all --check`/`clippy --all-targets -- -D warnings`/`check --all-targets` clean. Full `cargo test` green: lib **229 passed** (was 228 + 1 regression); microdesign **8/8** (was 7/7); `tests/microdesign_parity` 15+1 (every `.2c.1`+`.2c.2a` portable proof still green — `manifest_mirrors_the_oracle` continues to hold because both sides moved in lockstep); pipeline 121 (658s); snapshots 6; bin 5+29+3; doc 0. Portable `cargo test` stays green tool-less. **The `#[ignore]` real-tool parity gate is now unblocked** — the seed-7 counterexample is structurally impossible after this fix. No ROADMAP advance (that is `.2c.2b.2` on a verified clean banked artifact, r87 no-aspirational-claims). No book/ change. Frontier → `PHASE-7-ORACLE-MICRODESIGN.2c.2b.2` (run `cargo test -- --ignored parity_against_real_yosys_write_json` against real yosys; verify exact-agreement on yosys-supported categories; bank artifact; record ROADMAP Phase 7 → done with explicit scope caveat; reconcile book/README/CODEBASE).
- `ae7b592` — Docs: split PHASE-7-ORACLE-MICRODESIGN.2c.2b on a real-tool-surfaced ANVIL-self-consistency bug. **First real-tool `--ignored` run of `.2c.2a`'s gate against locally-installed yosys 0.64 retained ONE counterexample**: seed 7, `WidthMismatch { sig, expected bits=8, actual bits=2 }`. Root cause is an ANVIL-self-consistency bug in `width_expr`: oracle uses Rust's `last.value.rem_euclid(8) + 1` (mathematical non-negative modulo, gives 8 for `(-1).rem_euclid(8) + 1`) but emits SV text `((<last> % 8) + 1)` which evaluates `(-1 % 8) + 1 = 0` (SV `%` on signed = truncated toward zero, matches Rust's `%`); yosys interprets `logic [-1:0] sig` as 2 bits ⇒ oracle ≠ SV. NOT a yosys bug. The parity gate is doing exactly what `.1` designed it to do — surface an oracle/downstream semantic disagreement. Per ANVIL's "valid-by-construction + downstream-acceptance-quality" north-star + r87 no-aspirational-claims, fix must land before ROADMAP Phase 7 can be promoted. Split per memory `.2.1`→`.2.1a`/`.2.1b` discovered-dependency precedent (one level deeper this time): `.2c.2b.1` (semantic-alignment fix: change BOTH oracle + SV text to the standard SV non-negative-modulo idiom `((x % 8) + 8) % 8 + 1` so the width is always in `[1, 8]` and oracle ≡ SV; regression proof exercising a negative-value seed; cargo gates green; no real run, no ROADMAP advance) + `.2c.2b.2` (re-run `--ignored` gate against real yosys + verify clean + bank artifact + record ROADMAP Phase 7 → done with explicit yosys-supported-categories scope caveat + reconcile book/README/CODEBASE; gate-blocked). Tree-planning, docs-only: `docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md` (Metadata last-updated 2026-05-20 + `.2c.2` Children annotation + `.2c.2b` container + `.2c.2b.1`/`.2c.2b.2` leaves + Frontier `.2c.2b`→`.2c.2b.1` + Decisions entry + Verification-Log split row + Commit-Log row + Changelog entry) + `docs/TASK_TREE.md` + CHANGES.md + MEMORY.md. `cargo fmt`/clippy/check clean; no `src/`/`tests/` (`cargo` unchanged-green vs `900061c`). Frontier → `PHASE-7-ORACLE-MICRODESIGN.2c.2b.1`.
- `900061c` — **Phase 7: PHASE-7-ORACLE-MICRODESIGN.2c.2a scoped comparator + yosys write_json extractor + end-to-end-runnable #[ignore] harness.** `src/microdesign/mod.rs`: `pub enum FactCategory` (7 axes — Seed/Top/Params/Localparams/Widths/Generate/PackageConstants), `pub struct ParityScope { categories: BTreeSet<FactCategory> }` (all()/none()/only(&[...]) + .contains), `pub fn compare_manifest_to_tool_report_in_scope` (the scoped walker). The existing strict `compare_manifest_to_tool_report` delegates with `ParityScope::all()` → backwards-compatible by construction; `.2c.1`'s 9 proofs unchanged-green. `tests/microdesign_parity.rs`: 3 new scoped-comparator proofs (`scoped_comparator_only_enforces_scoped_categories` — load-bearing scoping proof; `yosys_scope_ignores_localparams_and_package_constants` — strict-vs-scoped delta on the folded yosys axes; `empty_scope_ignores_every_disagreement` — self-check) + yosys-specific extractor `parse_yosys_binary_param` (signed-32-bit → i128 sign-extension; load-bearing for negative resolved values e.g. seed 7 P4 = -1) and `yosys_write_json_to_tool_report` (params from .parameter_default_values, generate["g_taken"] from netnames-key prefix scan g_taken./g_else., widths["sig"] from .netnames.sig.bits.len(); folded axes empty) + 3 extractor proofs (synthetic seed-0 JSON / g_else-survives / sign-extension). `parity_against_real_yosys_write_json` rewritten end-to-end (per-seed `emit_sv`→`CARGO_TARGET_TMPDIR/microdesign-parity-phase7-yosys/mc_<seed>.sv`, shell yosys, parse → ToolReport, scoped compare, accumulate counterexamples, panic on non-empty) with yosys-presence guard; NOT executed in the portable suite (`.2c.2b` runs it). `cargo fmt --all --check`/`clippy --all-targets -- -D warnings`/`check --all-targets` clean; full `cargo test` green (microdesign_parity 15+1; pipeline 121; snapshots 6; lib 228 microdesign 7/7; bin 5+29+3; doc 0). Portable `cargo test` stays green tool-less (Phase-1 doctrine). No ROADMAP advance (that is `.2c.2b` on a verified clean banked artifact, r87). No book/ change. Frontier → `PHASE-7-ORACLE-MICRODESIGN.2c.2b` (run `cargo test -- --ignored parity_against_real_yosys_write_json` against real yosys; verify exact-agreement on yosys-supported categories; bank artifact; record ROADMAP Phase 7 → done with the explicit scope caveat; book/README/CODEBASE reconcile).
- `78a35ef` — Docs: split PHASE-7-ORACLE-MICRODESIGN.2c.2 into `.2c.2a` (`FactCategory`+`ParityScope`+scoped comparator + yosys-specific `write_json` extractor + end-to-end-runnable `#[ignore]` harness + cargo-portable scoped-comparator proof; cargo stays green tool-less; no ROADMAP advance) + `.2c.2b` (real `--ignored` run + verify exact-agreement on yosys-supported categories + bank artifact + record ROADMAP Phase 7 → done + book/README/CODEBASE reconcile; gate-blocked, r87 no-aspirational-claims). Split on a discovered lower-level dependency mirroring memory `.2.1`→`.2.1a`/`.2.1b`: empirical probe today of yosys 0.64 `write_json` on the `.2b` `mc_<seed>` corpus (seeds `{0,1,7,42,12345}`, `N_PARAMS=5`) confirmed yosys exposes **4 of the 7 manifest fact categories** — `.parameter_default_values` (binary-string → SV `int` → `i128`), `.netnames` keys prefixed by `g_taken.`/`g_else.` (corpus exercises BOTH — seed 12345 takes `g_else`), and `.netnames["sig"].bits` — but **folds localparams and package_constants** (richer-AST tools like `slang`/`verilator-with-debug` see them; recorded follow-up that does NOT block Phase 7 closure). Tree-planning, docs-only: `docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md` (Metadata last-updated 2026-05-20 + `.2c` Children annotation + `.2c.2` container + `.2c.2a`/`.2c.2b` leaves + Frontier `.2c.2`→`.2c.2a` + Decisions entry + Verification-Log split row + Commit-Log row + Changelog entry) + `docs/TASK_TREE.md` (Phase 7 frontier updated) + CHANGES.md + MEMORY.md. `cargo fmt`/clippy/check clean; no `src/`/`tests/` (`cargo` unchanged-green vs `c91d35e`); mdbook unchanged. Continuous-PNT. Frontier → `PHASE-7-ORACLE-MICRODESIGN.2c.2a`.
- `c91d35e` — **Phase 7: PHASE-7-ORACLE-MICRODESIGN.2c.1 parity harness — comparator core + cargo-portable proofs + tool-gated #[ignore] scaffold.** `src/microdesign/mod.rs` extended with `ToolReport` (normalized resolved-facts view from a downstream consumer; `BTreeMap` throughout for determinism; serde `Serialize`+`Deserialize`), `Divergence` enum (17 variants across `SeedMismatch`/`TopMismatch` + {missing-in-tool, missing-in-manifest, mismatch} × {param, localparam, width, generate, package-constant} — closes `.1`'s rejected-alternative "single facts-disagree bit" gap), `compare_manifest_to_tool_report` (cargo-portable walker; accumulates the full divergence set rather than fail-fast so the gate can return Ok or retain the full counterexample profile in one pass; symbolic manifest `expr` strings deliberately NOT compared — they are un-resolved-SV documentation, not facts the tool re-emits), `synthetic_tool_report_from_manifest` (always-agreeing reference). `FactEntry`/`WidthFact`/`GenFact`/`ConstExprFact`/`Manifest` fields promoted to `pub`; derives extended to `Clone`+`PartialEq`+`Eq`+`Deserialize`. New `tests/microdesign_parity.rs` (mirrors `tests/pipeline.rs`/`snapshots.rs`): **9 cargo-portable comparator proofs green** (agreement baseline across reproducibility-set seeds `{0,1,7,42,12345}` + per-axis divergence — param/localparam/width/generate-branch/package-constant/param-missing-in-tool/param-missing-in-manifest/seed+top) + **1 tool-gated `#[ignore]` scaffold** `parity_against_real_yosys_write_json` (yosys-presence guard; corpus-driver loop wired against the same `SEEDS`/`N_PARAMS` constants; placeholder for `.2c.2`-owned `emit_sv`→shell→extract→compare end-to-end wiring). `cargo fmt --all --check`/`clippy --all-targets -- -D warnings`/`check --all-targets` clean; full `cargo test` green (microdesign_parity 9 passed + 1 ignored; pipeline 121 passed in 657s; snapshots 6 passed; lib microdesign still 7/7 — `.2a`+`.2b` unchanged). Portable `cargo test` stays green tool-less (Phase-1 doctrine). No ROADMAP advance (that is `.2c.2` on a verified clean banked artifact, r87). No book/ change. Frontier → `PHASE-7-ORACLE-MICRODESIGN.2c.2` (real tool-equipped `--ignored` run + verify exact-agreement + record ROADMAP Phase 7 → done).
- `e0d31e3` — Docs: split PHASE-7-ORACLE-MICRODESIGN.2c into `.2c.1` (build the parity harness — cargo-portable comparator proof + tool-gated `#[ignore]` real-tool harness scaffold; no real run, no ROADMAP advance, cargo stays green tool-less per the Phase-1 doctrine) + `.2c.2` (real tool-equipped run + verify exact-agreement + record ROADMAP Phase 7 → done; gate-blocked, r87 no-aspirational-claims). Mirrors the proven memory `.2.3`/`.2.4` and FSM `.3.4a`/`.3.4b` decomposition. Tree-planning, docs-only: `docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md` (Metadata last-updated 2026-05-20 + `.2` container annotation + `.2c` container + `.2c.1`/`.2c.2` leaves + Frontier `.2c`→`.2c.1` + Decisions + Verification-Log + Commit-Log + Changelog entries) + `docs/TASK_TREE.md` (Phase 7 row frontier updated) + CHANGES.md + MEMORY.md. `cargo fmt`/clippy/check clean; no `src/`/`tests/` (`cargo` unchanged-green vs `13faa77`); mdbook unchanged. Continuous-PNT immediately after closing Phase 6. Frontier → `PHASE-7-ORACLE-MICRODESIGN.2c.1`.
- `13faa77` — **Phase 6: PHASE-6-ADVANCED-MOTIFS.3.4b FSM delivered — verified gate, closes Phase 6 + tree.** Real repo-owned `Phase4Hierarchy` gate (bg `bqebtebwa`, exit 0, 222/222 dirs) → `/tmp/anvil-tool-matrix-phase6-fsm-p1/tool_matrix_report.json` VERIFIED CLEAN before any promotion (r87): 222 scenarios / 888 designs, `coverage_gaps=[]`, Verilator 888/0 + yosys_without_abc 888/0 + yosys_with_abc 888/0, `saw_fsm_design=true` AND `saw_inferrable_memory_design=true`; P4/P5/P5b regressions proven in the same banked report. Promotion strictly followed the artifact: ROADMAP Phase 6 `(in progress)`→`(done)` + FSM bullet rewritten "delivered" + closing "Exit criteria (met)" block citing BOTH banked artifacts; `book/src/ir.md` "Phase 6 is now done"; `book/src/knobs.md` `fsm_prob` flipped to "Delivered" + "This closes Phase 6"; README + CODEBASE phase-map Phase-6 row done. **Phase 6 CLOSED; `.3.4`+`.3` containers CLOSED; `PHASE-6-ADVANCED-MOTIFS` tree CLOSED.** Multi-clock CDC remains the explicitly-optional, separately-prioritised deferral. No `src/`/`tests/` change (FSM code landed in `.3.2a`/`.3.2b`/`.3.3`/`.3.4a`); mdbook build clean; cargo unchanged-green. **30-commit batch pushed** `8076e25..13faa77`. Continuous-PNT proceeds → `PHASE-7-ORACLE-MICRODESIGN.2c.1`.
- `e1b3205` — PHASE-7-ORACLE-MICRODESIGN.2b: un-resolved SV emitter + JSON expected-facts manifest emitter, both from the `.2a` oracle. `src/microdesign/`: `expr_to_sv` (fully-parenthesized), `emit_sv` (`rtl_const_expr` family: package/`K`, symbolic param/localparam chains, `PKG_REF`, expr-derived `W_SIG`+`sig`, `generate if/else`), `Manifest`+`build_manifest`/`emit_manifest` (the `.1` schema, all from the oracle, BTreeMap byte-stable serde_json). Default-off DUT-byte-identical = structural (separate module). 3 new proofs (7 total); fmt/clippy/check clean; full `cargo test` green. No harness/ROADMAP/book. Frontier → PHASE-7 `.2c`.
- `32865a1` — PHASE-7-ORACLE-MICRODESIGN.2a: const-expr/parameter IR + construction-time evaluator (the oracle). New separate `src/microdesign/mod.rs` (`pub mod microdesign`; not in `src/ir/`): `ConstExpr`/`ParamDecl`(+`value`)/`ConstExprUnit` + `eval`/`resolve` (SV-const-expr `i128`; defensive `EvalError`) + reproducible rules-first `build_constexpr_unit(seed,n)` (ChaCha8). 4 unit proofs incl. oracle-no-drift; cargo fmt/clippy/check clean; full `cargo test` green (lib→225). No emit/harness/ROADMAP/book. Frontier → PHASE-7 `.2b`. (Ran concurrent with the `.3.4b` Phase-6 gate — accepted per continuous-PNT.)
- `8836522` — PHASE-6-ADVANCED-MOTIFS.2.4: **memory delivered; `.2` container CLOSED.** Real `Phase4Hierarchy` gate (bg `bd3jdtx31`, ~10h, exit 0) report `/tmp/anvil-tool-matrix-phase6-p1` verified clean before promotion (r87): 219 scenarios / 876 designs, `coverage_gaps=[]`, Verilator 876/0 + yosys-without-abc 876/0 + yosys-with-abc 876/0, `saw_inferrable_memory_design=true`, P4/P5/P5b facts true same artifact. ROADMAP Phase 6 →(in progress)+memory-delivered note+exit-criteria; `book/src/ir.md` Unpacked-arrays→Delivered(Phase 6); `book/src/knobs.md` += `memory_prob`/`fsm_prob` (config-only list + metrics table); README + CODEBASE phase-map reconciled. Phase 6 NOT closed (`.3.4b` closes it). No `src/`/`tests/` (docs/ROADMAP/book only); mdbook build clean; cargo unchanged-green. Frontier → `.3.4b` (fresh 222/888 gate; machine now free).
- `63a5a52` — PHASE-7-ORACLE-MICRODESIGN.2 split (tree-planning, docs-only, no code): `.2` → container `.2a` (const-expr/parameter IR + construction-time evaluator/oracle), `.2b` (SV + JSON-manifest emitters behind artifact-family flag, default-off DUT-byte-identical), `.2c` (parity harness + tool-gated repo-owned gate → ROADMAP Phase 7, r87). Boundaries per `.1`'s named split candidates; `.2a`+`.2b` core is PHASE-8.2 / Phase-9 reuse. `cargo` unchanged-green vs base e550db1. Continuous-PNT zero-contention advance while Phase 6 gate near-complete. Frontier → PHASE-7 `.2a`.
- `e550db1` — DIFFERENTIAL-SIMULATION.2a: single-design differential-harness design (design-only, no code). `.2` split → `.2a` design / `.2b` implement. `DEVELOPMENT_NOTES.md` entry — IR-driven testbench (not SV re-parse), reset+canonical-post-reset-sample (neutralises `.1`'s 4-state gap), baked deterministic stimulus, `iverilog`+`vvp`/`verilator --binary` orchestration, byte-equal compare, tool-gated `#[ignore]` test (portable `cargo test` stays green tool-less), 5 rejected alternatives, `.2b` proof shape. Continuous-PNT design-first while Phase 6 gate-blocked (docs-only ⇒ ~zero contention). Frontier → DIFFERENTIAL-SIMULATION `.2b`.
- `f0df6f5` — DIFFERENTIAL-SIMULATION.1: iverilog second-simulator compatibility note (research-only, no code). Installed Icarus Verilog 13.0; `iverilog -g2012` vs `verilator --lint-only` on all 4 ANVIL categories → iverilog exit 0 SILENT everywhere ⇒ zero-config second simulator. Pair = Verilator↔iverilog (engine-independent). Sole divergence (pre-reset 4-state) = `.2` design constraint, not a blocker; 4 rejected alternatives. `DEVELOPMENT_NOTES.md` entry; iverilog installed locally. Continuous-PNT while Phase 6 gate-blocked. Frontier → DIFFERENTIAL-SIMULATION `.2`.
- `da3a00d` — INSTA-SNAPSHOTS.3: snapshot-acceptance protocol (**INSTA-SNAPSHOTS tree CLOSED**). `COMMIT.md` checklist item 1 annotated + non-negotiable "Snapshot-acceptance protocol" sub-bullet (failing snapshot ⇒ never silently regenerated; deliberate reviewed `cargo insta accept`/`INSTA_UPDATE=accept` committed in the same slice as the explaining generator change; unexplained `.snap` = workflow violation). `book/src/knobs.md` "## Reproducibility" → friendly "### Snapshot guard-rails" subsection (no runnable bash fence ⇒ book_examples harness unaffected). Docs/workflow only; mdbook build/test clean; no `src/`/`tests/`. Continuous-PNT while Phase 6 gate-blocked.
- `c1acfb5` — INSTA-SNAPSHOTS.2: `tests/snapshots.rs` expanded 2 → 6 fully-deterministic byte-stable snapshot shapes (+ bounded_recursive_ondemand / sibling_route / parent_composed_route / dedup_canonical_signatures[`hierarchy_module_dedup=true`]) spanning all the listed axes; ≥5-shapes acceptance met. Generated via `INSTA_UPDATE=always`, re-verified stable; fmt/clippy clean; full `cargo test` green, no regression. Frontier → INSTA-SNAPSHOTS `.3` (`COMMIT.md` checklist + book protocol; closes the tree).
- `d00bfe3` — INSTA-SNAPSHOTS.1: `insta` dev-dep pinned `=1.47.2` (Cargo.lock unchanged) + new `tests/snapshots.rs` with two deterministic baseline snapshots (canonical leaf + bounded recursive library). Generated via `INSTA_UPDATE=always`, re-verified byte-stable on plain re-run. Open Question resolved (one `snapshots.rs` → per-test `.snap` under `tests/snapshots/`). fmt/clippy clean; full `cargo test` green incl. the new binary, no regression. Continuous-PNT while Phase 6 gate-blocked. Frontier → INSTA-SNAPSHOTS `.2`.
- `199db6b` — COVERAGE-INSTRUMENTATION.3: cone retry-exhaustion focused proof + orphan-knob audit + baseline refresh (**COVERAGE-INSTRUMENTATION tree CLOSED**). New `tests/pipeline.rs` proof `constant_pressure_exhausts_cone_retry_and_stays_valid_and_reproducible` (constant_prob=1.0 ⇒ pick_terminal constant branch + build_cone_with_retry rollback/budget-exhausted accept; invariant = max constant pressure stays valid+reproducible). config.rs orphan-knob audit: 3/74 unconsumed but all documented-reserved (`library_prob`/`max_nodes_per_module`/`use_async_reset`) ⇒ no dead code. Baseline refreshed via `cargo llvm-cov --release` (= cargo-test gate) + `docs/coverage-baseline.md` `.3` addendum. fmt/clippy clean. Only the one `tests/pipeline.rs` proof is new code.
- `01990d1` — COVERAGE-INSTRUMENTATION.2: top-5 under-covered-file triage (triage-only, no code). `DEVELOPMENT_NOTES.md` disposition table (a dead / b rarely-fired-proof / c intentional-or-integration) for `bin/tool_matrix.rs`,`gen/cone.rs`,`ir/validate.rs`,`config.rs`,`main.rs`. **No confirmed dead code** — headline gaps gate-exclusive/defensive/integration-only by design; the only high-value `.3` work is a handful of `gen/cone.rs` focused proofs + optional `config.rs` orphan-knob spot-audit (`.3` right-sized). mdbook/fmt clean; no `src/`/`tests/`. Continuous-PNT while Phase 6 gate-blocked.
- `9806028` — PHASE-9-MULTI-ARTIFACT-UMBRELLA.1: artifact-family selector + shared-plumbing design (design-only, no code). `DEVELOPMENT_NOTES.md` entry — anti-goal (unify plumbing not generators); `ArtifactLane` trait + shared seed/manifest/knob/output/check plumbing; default-`dut` `--artifact` flag keeping book/CI byte-identical; L1-wrap lane-migration; 4 rejected alternatives; Open Question resolved; `.2` blocked until ≥2 lanes. mdbook/fmt clean; no `src/`/`tests/`. Continuous-PNT while Phase 6 gate-blocked. **All design-only `.1` leaves now cleared (Phase 7/8/9).**
- `5c9932c` — PHASE-8-FRONTEND-ACCEPT.1: source-level frontend/elaboration accept-corpus IR design (design-only, no code). `DEVELOPMENT_NOTES.md` entry — Phase 8 = compact elaboratable hierarchies emitted with params UNRESOLVED in text + elaborated-facts manifest; dedicated source-level AST IR (separate generator path); source-IR sketch (SourceUnit/Package/Module/ModuleItem/Type); instance-tree manifest schema (extends Phase 7's); oracle-by-construction reusing Phase 7 evaluator/manifest core; hierarchy-aware parity harness; 4 rejected alternatives; Open Question resolved (`.2` after `PHASE-7-ORACLE-MICRODESIGN.2`). mdbook/fmt clean; no `src/`/`tests/`. Continuous-PNT while Phase 6 gate-blocked. Frontier → Phase 8 `.2`.
- `f0cff2c` — PHASE-7-ORACLE-MICRODESIGN.1: oracle-backed micro-design artifact-family design (design-only, no code). `DEVELOPMENT_NOTES.md` entry — conceptual shift (Phase 7 = tiny `.sv` whose elaboration facts are known by construction + machine-checkable manifest); own source-level const/parameter IR + separate generator path; `rtl_const_expr` family; expected-facts JSON schema; oracle-by-construction generation (evaluate-as-you-build, emit `.sv`+manifest from same resolved values); reproducibility; new parity harness (≠ `tool_matrix`); Phase-8/9 boundaries; 4 rejected alternatives; `.2` split. mdbook/fmt clean; no `src/`/`tests/`. Continuous-PNT while Phase 6 `.2.4`/`.3.4b` gate-blocked. Frontier → Phase 7 `.2`.
- `5db4ac9` — PHASE-6-ADVANCED-MOTIFS.3.4a: `phase6_fsm` matrix scenario + `DesignMetrics.num_fsm_modules` metric + `CoverageSummary.saw_fsm_design` fact/`Phase4Hierarchy` gap + `phase6_fsm_scenario_is_non_vacuous` (mirrors memory `.2.3`). `phase6_fsm_focus_config` = exact clone of the memory anchor, only `fsm_prob=1.0` ⇒ shape-coverage sets unperturbed. Bin **219→222 scenarios / 876→888 modules** (verified; +3/+12 matches `phase6_inferrable_memory`). `cargo test --bin tool_matrix` 29/29; full `cargo` gate green. No ROADMAP advance (that is `.3.4b`). `.3.4` split → `.3.4a` (done) / `.3.4b` (gate-blocked). Frontier → `.3.4b` (‖ `.2.4` gate-blocked).
- `725e7fc` — PHASE-6-ADVANCED-MOTIFS.3.3: cargo-portable proof `fsm_block_matches_probed_template_and_is_factorization_opaque` — 4 `ConstructionStrategy` × 4 `FactorizationLevel` (incl. EGraph) × 6 seeds (96 designs): 1 `FsmOut`/0 `Gate` survives every factorization level (FSM never enters NodeId graph — CSE/EGraph-opaque); emitted state constants are EXACTLY the chosen `FsmEncoding`'s (`state_width`/`state_const`) + exact async-low-reset state register + sel-selected next-state/Moore cases; all 3 encodings reachable (deterministic seeds 0..6). Proof-only (no `src/`); full `cargo` gate green. Frontier → `.3.4` (last Phase 6 leaf) (‖ `.2.4` gate-blocked).
- `9ee5285` — PHASE-6-ADVANCED-MOTIFS.3.2b: `Config::fsm_prob` (serde-default 0.0, range-validated; mirrors `memory_prob`) + rules-first `build_fsm_block` (clk/rst_n+`sel` in, `q` out; `num_states`/`encoding`/`sel_width`/`out_width` via `g.rng`; transitions+distinct masked Moore outputs by rule; opaque `FsmOut`; no gates/flops) + single opt-in roll AFTER the param + memory lanes (mutually exclusive; default-off never enters → byte-identical). Focused proof `fsm_block_is_default_off_and_constructs_when_forced_on`: default-off byte-identical 4 strategies×6 seeds; forced-on every single-module design a 1-`Fsm` leaf validating + emitting the `.3.1`-probed-clean template; all 3 encodings reachable. Full `cargo` gate green. **`.3.2` container done.** Frontier → `.3.3` (‖ `.2.4` gate-blocked).
- `ef0eef7` — PHASE-6-ADVANCED-MOTIFS.3.2a: FSM IR core + opaque-`FsmOut`-leaf pipeline integration (mirrors memory `.2.1a`). `FsmId`; `FsmEncoding{Binary,OneHot,Gray}` (`state_width`/`state_const`); `Fsm` struct (`sel:NodeId`+`sel_width`, `transitions[N][1<<sel_width]`, `outputs[N]`, `out_width`); additive `Default`-empty `Module.fsms`; opaque `Node::FsmOut`; `DepAtom::FsmVirtual`/`from_fsm_virtual`; `has_local_fsms()` in both `carries_sequential_state` predicates. `FsmOut` threaded by compiler-as-oracle through every exhaustive `Node` match (the LOAD-BEARING `compact.rs` reachability marks `fsm.sel` alive; + cone.rs ×5, hierarchy.rs, module.rs, param.rs, metrics.rs ×3 incl. structural-hash tag 7). `validate.rs` step 5c + 5 errors. Emitter = the `.3.1`-probed-clean template (per-FSM `FSM<id>_S<k>` localparams, `always_comb` next-state `case` on `sel`, async-low-reset state `always_ff` on shared `clk`/`rst_n`, `always_comb` Moore output `case`). 3 unit proofs green; full `cargo test` green; default-off trivially byte-identical (no generator/knob). Frontier → `.3.2b` (‖ `.2.4` gate-blocked).
- `f39f66b` — PHASE-6-ADVANCED-MOTIFS.3.2 split (tree-planning, no code): `.3.2` → container `.3.2a` (FSM IR core + opaque `Node::FsmOut` pipeline integration incl. load-bearing `compact.rs` reachability keeping `fsm.sel` alive + 3 unit proofs; no generator/knob ⇒ default-off byte-identical) + `.3.2b` (`Config::fsm_prob` + rules-first `build_fsm_block` + focused proof). Dependency known concretely from the landed memory `.2.1a` (`FsmOut` ≡ `MemRead` reachability obligation). Frontier → `.3.2a` (‖ `.2.4` gate-blocked). docs-only.
- `4dddf0e` — PHASE-6-ADVANCED-MOTIFS.3.1: generated-encoding FSM motif design (design-only, no code). `DEVELOPMENT_NOTES.md` entry — codebase grounding (FSM is a block), empirical downstream probe of the exact emitted SV in all 3 encodings (binary/one-hot/gray) → every one `verilator --lint-only` exit 0 + both repo Yosys modes `check -assert` clean (encoding-selectability structural); architecture (F) `Vec<Fsm>` + opaque `Node::FsmOut` + encoding-derived emitter + opt-in `fsm_prob`; 4 rejected/deferred. `.3` split → container `.3.1`(done)/`.3.2`/`.3.3`/`.3.4` (mirrors memory `.2.1`–`.2.4`; FSM = last Phase 6 motif → `.3.4` closes Phase 6 + tree). mdbook/fmt clean; no `src/`/`tests/` change. Continuous-PNT while the `.2.4` memory gate runs.
- `0b799b6` — BOOK-EXAMPLES-RUNNABLE.2.2: `tests/book_examples.rs` (NEW, leaf-owned std-only cargo integration harness — every ```bash fence run exit-0, skip-sentinel-aware, classification-guard panic on residual, negative control) + embedded migration (1 `$()` `anvil`→cargo run) + 9 reasoned skip sentinels + 32 bare ``` → ```text + `ci.yml` `mdbook test book` step. **Root-caused & fixed a harness pipe-buffer deadlock** (default module ≈86 KB > ≈64 KB OS pipe; `Stdio::piped()`+undrained `try_wait()` → 12 false 600s timeouts ≈7273s; fix = child stdio→temp files + reap after kill). `cargo test --test book_examples` 3/3 green, 54 runnable exit-0, 76.4s; full test/clippy/fmt/mdbook clean. **BOOK-EXAMPLES-RUNNABLE tree CLOSED.**
- `481d9fc` — Docs: BOOK-EXAMPLES-RUNNABLE.2.2 scope — recorded an honest `.2.1` correction: migration/audit were LINE-LEADING only; bare `anvil` in `$(...)`/for-loops still un-migrated. `.2.2` now owns the embedded-position migration + skip sentinels + the harness (shell-script model) + mdbook-test CI step. Bash-block classification recorded (48 pure cargo-run / 8 comment+cargo-run / ~6 external-tool/install skips / $()-loop). Tree/docs only, no code.
- `415818c` — Docs: BOOK-EXAMPLES-RUNNABLE.2.1 migrate book examples to `cargo run --release --` (45 LINE-LEADING bash heads; **note: `$(...)`/loop-embedded `anvil` NOT covered — fixed in .2.2**; 9 rust sketches→rust,ignore; shorthand note; spot-runs exit 0). Book/docs only, no code.
