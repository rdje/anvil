...
- next_action: **`CAPABILITY-BREADTH-EXPANSION.1`** — its **pre-`.1` findings block is written on the tree; READ IT FIRST**, it carries the measured probe and saves re-deriving. Summary: the leaf's *"version-distinctive up-opt"* premise is contested (post-2012 SV is mostly verification features; **0** emitter literals for `enum`/`unique`/`priority`/`always_latch`/`casex`/`interface`/`modport`/`inside`) — **owner has not ruled on reframing it to *synthesizable breadth***. Recommended pick **`unique`/`priority` case qualifiers**, proven valid-by-construction on `CaseMux` (emitter always writes `default:` ⇒ full; labels are sequential `0..N-1` ⇒ parallel) but **NOT** safe as `unique` on `CasezMux` (overlapping wildcard arms) ⇒ gate per gate kind. Remaining `.1` work: downstream tool probe + the ADR. **Every commit subject must name a LEAF** (`TREE.N`).
