...
- latest_commit: **`EMIT-SURFACE-INTERACTION-GATE.0`** (`74b101b`) — registers that tree. Prior: `EVIDENCE-BANK-DURABILITY.4` (`756713c`) closed the evidence tree; `EVIDENCE-CITATIONS` live at driver **6/6**. Session history: `git log --oneline` (do not re-narrate it here).
- active_work_unit: **`EMIT-SURFACE-INTERACTION-GATE`** → `.1` **done** (decision `0032`), frontier **`.2`**. Closed this session: `CARGO-TMPDIR-SWEEP-REGRESSION`, `EVIDENCE-BANK-DURABILITY`.
- next_action: `EMIT-SURFACE-INTERACTION-GATE.2` — the preset, per decision `0032` (e). Set all eight `*_emit_prob` to **`0.25`** (NOT `1.0` — see the gotcha below) + raise `comb_mux_prob`/`case_mux_prob`/`casez_mux_prob`; correct the description; add the preset↔knob-list drift test; fix `USER_GUIDE.md:363`, `README.md:875`, and the `knob-presets-and-cli-flags` KM `reverify` line, all of which still pin the old four-knobs-at-`1.0` expectation. Then `.3` (the 3+1 gate + `docs/evidence/` digest — this tree is that mechanism's second customer) and `.4` (harden `cone_function_emit::compute_use_counts`).
