...
## Recent commits
- `<pending>` — Phase 6: PHASE-6-ADVANCED-MOTIFS.2.2 memory inference structural-contract + factorization-opacity proof (64 combos: 4 strategies × 4 FactorizationLevel × 4 seeds; template equivalence + MemRead/array never in NodeId graph incl. EGraph; tool-level $mem_v2 proof = .2.1b spot-check + .2.4 gate). Proof only, no code.
