...
- latest_commit: **`CAPABILITY-BREADTH-EXPANSION.4a`** (backfill next commit). Prior: `eb79534`, `b9b26b4`, `832a482`. Recent decisions: **`0040`–`0044`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **`CAPABILITY-BREADTH-EXPANSION`** (`active`; `.2`/`.3`/`.4a` done, frontier **`.4b.1`**). The strand (decision **`0044`**): default-off `unique`/`priority` **case qualifiers** on `CaseMux`/`CasezMux`, valid-by-construction. Also `active`: **`IR-TYPES-DECOMPOSITION`** (`.4`) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (`.9`/`.4` — **PAUSED by owner redirect, do not resume without a nudge**) · **`CHANGES-ENTRY-PLACEMENT`** (`.4`, deferred).
- next_action: **`CAPABILITY-BREADTH-EXPANSION.4b.1`** (live source) — **read the `.4a` entry in `DEVELOPMENT_NOTES.md` first, then [`0044`](docs/decisions/0044-capability-breadth-unique-priority-case-qualifiers.md)**. Between them EVERY impl point is pinned — predicate, the `case_qualifiers` **map** carrier, the emitter prefix on the two *unprojected* branches only, tenth-pass ordering, the two knobs + the `knob_group` arm they force, metrics @ schema **`1.27 → 1.28`**, the 13-scenario gate + per-qualifier tool plans, and the two-predicate FULL/PARALLEL test with its two firing negative controls. Do **not** re-derive them. Two `0044` corrections, do **not** re-inherit: `CasezMux` **IS** parallel by construction, and `.1` was **not** reframed. **Every commit subject must name a LEAF** (`TREE.N`).
...
- blockers: none. **A decision you already made is DISCLOSED, never re-opened as a question** — owner `2026-08-01`, [`0041`](docs/decisions/0041-owner-standing-directives-recorded-in-layer-c.md) §(e); *"your call"* is not a valid label for a decided call. **Adding to this file requires routing, not appending.**
