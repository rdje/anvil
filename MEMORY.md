...
- latest_commit: **`EMIT-SURFACE-INTERACTION-GATE.3` follow-up** (`99e6cc0`) — digest pointer corrected. Prior: `.3` (`401d72d`), `.2` (`d73b154`), `.1` (`7664761`). Session history: `git log --oneline` (do not re-narrate it here).
- active_work_unit: **`EVIDENCE-BANK-DURABILITY`** (reopened `2026-07-30`) → frontier **`.6`**. Closed this session: `CARGO-TMPDIR-SWEEP-REGRESSION`, **`EMIT-SURFACE-INTERACTION-GATE`** (all 4 leaves; decision `0032`, the honest preset, the interaction gate + banked digest, the absorption-census fix).
- next_action: `EVIDENCE-BANK-DURABILITY.6` — repair two `scripts/evidence_digest.sh` defects the *second* real digest exposed. **(A)** the re-runnable `--command` is rebuilt from a **hardcoded** 14-entry `*_gate` list, so a newer gate yields a **flagless** command that would re-verify the *default* scenario set — silent, plausible, wrong; derive it from the report's own `*_gate` keys and fail loudly when exactly one true flag cannot be identified. **(B)** `commit:` is `HEAD` at derivation time = the *parent* of the banking commit, unusable for a gate introduced alongside its own first bank (hand-corrected in `anvil-emit-surface-interaction-r1`). Negative-control both; the digest must regenerate to the hand-corrected value.
