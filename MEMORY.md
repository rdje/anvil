...
- Latest full downstream-clean Phase 4 hierarchy bank is:
  ...
  covers the live `87`-scenario policy at `4` designs/scenario
  (`348` total designs), with `artifact_kind = "design"`,
  ...
  evidence for parent outputs through parent-local flops.
  Focused regressions:
  ...
  parent-output helper bank are now historical breadcrumbs.
- Latest Phase 4 hierarchy slice lets parent outputs below the top
  parent spend a multi-helper parent-cone instance budget in
  exact-depth-2 recursive hierarchy. With
  `hierarchy_parent_cone_instance_prob = 1.0`,
  `max_parent_cone_instances_per_module = 3`, and child-input route
  probabilities disabled, a non-top parent can instantiate multiple
  helper children and use those helper outputs in parent-output cones
  without proving a child-input helper binding or a registered
  child-input helper D cone.
  Key metrics are
...
- **Last completed slice:** Banked recursive non-top stateful
  multi-helper budget evidence through full downstream
  tools at
  `/tmp/anvil-tool-matrix-phase4-hierarchy-r42/tool_matrix_report.json`
  (`348` designs, `coverage_gaps = []`, `348/0` in Verilator plus both
  ...
  parent-output helper bank, and the `r41` recursive non-top
...
- **Prior slice:** Landed registered parent-composed child-input
  ...
  The current full `r42` gate supersedes that historical report with
  `coverage_gaps = []`, `348/0` pass-fail in Verilator plus both
