...
- latest_commit: **`9b73e80`** (`USER-GUIDE-CLI-TABLE-SHADOW.5`). Prior: `176c868`, `ccfbc23`, `5ce2dd3`. Recent decisions: **`0040`–`0045`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight.** `USER-GUIDE-CLI-TABLE-SHADOW` `.2`–`.5` **done**: the `USER_GUIDE.md` knob table is **exhaustive over the KNOBS** (derived from source, **not** `anvil --help`), repaired **75 → 93**, **gated as `ENUMERATION-PARITY` pair 5**; `.4`+`.5` **deleted** both book copies by `0033` rung **R1**. Still `active`: **`USER-GUIDE-CLI-TABLE-SHADOW`** (`.6`) · **`BOOK-LINK-INTEGRITY`** (`.1`) · **`CAPABILITY-BREADTH-EXPANSION`** (only `.1`, SV up-opt, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (`.9`/`.4` — **PAUSED by owner redirect, do not resume without a nudge**).
- next_action: **`USER-GUIDE-CLI-TABLE-SHADOW.6`** — the mechanism question for `.5`'s new `USER_GUIDE.md` `### Options` list (`ENUMERATION-PARITY` **pair 6**?), standing to `.5` as `.3` did to `.2`. **Two constraints `.3` never faced are already measured and written into `.6`'s Acceptance — read it, do not re-derive them.** **Every commit subject must name a LEAF** (`TREE.N`).
...
- blockers: none. **A finding is not closed until something MECHANICAL fails if it recurs** (`PARITY-EXTRACTOR-CHARSET-GAP`, closed `2026-08-01`; the `total_or_fail` rule and the delimiter-is-the-spec lesson are in [[extractor-charset-narrower-than-source]], not here). **SEVEN instrument notes now** — the countermeasures are recorded, not summarised: **derive `S` twice; state the REGION measured** ⇒ [[cli-flag-audit-must-be-command-scoped]]; **audit a list's EXEMPTION half** ⇒ [[exemption-list-asserts-absence]]. **A decision you already made is DISCLOSED, never re-opened as a question** — [`0041`](docs/decisions/0041-owner-standing-directives-recorded-in-layer-c.md) §(e). **Adding to this file requires routing, not appending.**
