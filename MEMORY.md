...
- latest_commit: **`USER-GUIDE-CLI-TABLE-SHADOW.1`** (backfill next commit). Prior: `1df0071`, `928817f`, `f0eb8fb`. Recent decisions: **`0040`–`0045`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight; two trees CLOSED this session.** The `unique`/`priority` **case-qualifier** strand (decision **`0044`**) — live, queryable (schema **`1.28`**), gated (`anvil-case-qualifier-gate-r1`), documented; `CAPABILITY-BREADTH-EXPANSION` stays `active` with only **`.1`** (SV up-opt) **deferred-not-retired**. And **`IR-TYPES-DECOMPOSITION`** is `done` (`types.rs` **4,069 → 1,473**, zero call sites changed). Still `active`: **`CHANGES-ENTRY-PLACEMENT`** (`.5`) · **`USER-GUIDE-CLI-TABLE-SHADOW`** (`.2`, registered `2026-08-01`) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (`.9`/`.4` — **PAUSED by owner redirect, do not resume without a nudge**).
- next_action: **`CHANGES-ENTRY-PLACEMENT.5`** — implement decision **`0045`** as a registered doctrine. **Read `0045` and the tree's `.5` Acceptance first**; every point is pinned there, including the two easiest to get wrong (the three negative controls are **REPRODUCED, not re-derived**, and the check must **not** be scope-aware). **Every commit subject must name a LEAF** (`TREE.N`).
