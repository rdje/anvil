...
- latest_commit: **`46f2af1`** (`NEGATIVE-CONTROL-HARNESS.2`). Prior: `83a2a5b`, `4ad09a4`, `da73827`. Recent decisions: **`0040`–`0047`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
- active_work_unit: **none mid-flight.** `active`: **`NEGATIVE-CONTROL-HARNESS`** (`.3`; `.1`+`.2` done) · **`CAPABILITY-BREADTH-EXPANSION`** (`.1`, **deferred-not-retired**) · **`OVERFLOW-DESTINATION-INSTRUMENTATION`** (**PAUSED, do not resume without a nudge**). CLOSED `2026-08-01`: `USER-GUIDE-CLI-TABLE-SHADOW` `.7`, `RESUME-POINTER-CONTRACT`, **`BOOK-LINK-INTEGRITY` `.1`–`.3`** — registry now **11** doctrines ⇒ [[mdbook-md-to-html-rewrite-trap]].
- next_action: **`NEGATIVE-CONTROL-HARNESS.3`** (last leaf) — build decision [`0047`](docs/decisions/0047-negative-control-carrier-is-the-mutation.md)'s **R2** helper to the contract pinned there (`apply` refuses a zero-count substitution; `restore` verifies by `cmp`; `--self-test` is the control on the control), plus one `TOOLBOX.md` row. **NOT a doctrine** — `0047` (C) disqualifies that on structure. **Every commit subject must name a LEAF** (`TREE.N`).
