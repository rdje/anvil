...
## Recent commits
- `<pending>` — Docs: PHASE-6-ADVANCED-MOTIFS.1 inferrable-memory motif design (architecture (M) `Memory` block + `MemRead` leaf; empirical Yosys `$mem_v2` probe both modes; 3 rejected alternatives; design-only, no code; frontier → `.2`).
