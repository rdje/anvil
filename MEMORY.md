...
- **Active task trees:**
  ...
  - `INSTA-SNAPSHOTS` — **`.1` done** (`2026-05-18`): `insta` pinned `=1.47.2` (Cargo.lock unchanged); `tests/snapshots.rs` with two deterministic baseline snapshots (canonical leaf seed 1 + bounded recursive library seed 11, exact depth-2/2-instances proven-shape), generated via `INSTA_UPDATE=always` then **re-verified byte-stable on plain re-run**; full `cargo test` green incl. the new binary, no regression. Frontier → `.2` (expand to ≥5 shapes spanning library/on-demand/helper/registered-composed + a `canonical_module_signatures`-exercising design). Quality lane: reproducibility regressions.
