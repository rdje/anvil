...
- Latest Phase 4 hierarchy matrix-policy slice is now fully banked:
  ...
  covers the current `51`-scenario policy at `4` designs/scenario
  (`204` total designs), with `coverage_gaps = []` and `204/0`
  pass-fail in Verilator plus both repo-owned Yosys modes. It includes
  the direct sibling helper route and the direct registered sibling
  helper route plus the new multi-stage registered sibling route. The
  older `r23` full bank, `r24` coverage-only direct-helper proof, and
  `r25` direct-helper full bank are now historical breadcrumbs.
...
- Prior Phase 4 hierarchy gate refresh fixed the gate budget so the
  ...
  repo-owned Yosys modes. The current `r26` bank supersedes it for the
...
- The focused proof for direct sibling helper routing is
  ...
  This route is now banked in the full downstream-clean `r26` Phase 4
...
- The focused proof for the new parent-cone helper-instance
  ...
  This route is now banked in the full `r26` Phase 4 matrix through the
...
- The refreshed repo-owned Phase 4 hierarchy closure report is now `/tmp/anvil-tool-matrix-phase4-hierarchy-r26/tool_matrix_report.json`: **51 scenarios**, **4 designs/scenario**, **204 total designs**, `artifact_kind = "design"`, `coverage_gaps = []`, and **204/0** pass-fail in Verilator plus both repo-owned Yosys modes.
...
- Earlier current-code coverage-only Phase 4 matrix probes at
  ...
  --skip-yosys`; the full downstream-clean `r26` bank now carries those
  facts plus the newer helper-output, budgeted-helper, and registered
  helper-route facts with real tool validation.
...
- **Last completed slice:** Broadened direct registered sibling routing
  so later child-input routes can chain through earlier parent-local Qs
  without becoming registered parent-composed logic, then banked the
  resulting current 51-scenario Phase 4 hierarchy matrix through full
  downstream tools at
  `/tmp/anvil-tool-matrix-phase4-hierarchy-r26/tool_matrix_report.json`
  (`204` designs, `coverage_gaps = []`, `204/0` in Verilator plus both
  repo-owned Yosys modes). This supersedes the old `r23`/`r24`
  evidence split and the `r25` direct-helper full bank.
...
- **Prior slice:** Refreshed the Phase 4 hierarchy gate at full
  ...
  four-designs-per-scenario Phase 4 floor. The current full bank at
  `/tmp/anvil-tool-matrix-phase4-hierarchy-r26/tool_matrix_report.json`
...
- **Earlier slice:** Landed multi-stage registered parent-composed
  ...
  `/tmp/anvil-tool-matrix-phase4-registered-multistage-r1/tool_matrix_report.json`
  with `coverage_gaps = []`, now superseded by the full `r26` bank for
...
- **Prior slice:** Landed registered parent-composed child-input
  ...
  The current full `r26` gate supersedes that historical report with
  `coverage_gaps = []`, `204/0` pass-fail in Verilator plus both
...
- **Next up (ordered by the four-gap steering map):**
  0. **Deepen Phase 4 hierarchy beyond the current banked gate.** Mixed parent-port / child-output parent composition, the first registered sibling route plus its multi-stage parent-Q chain, first registered parent-composed child-input route, registered mixed-support child-input routing, the first multi-stage registered parent-composed chain, parent-cone helper-instance parent-composed child-input, direct sibling, direct registered sibling, registered child-input D-cone, budgeted parent-output routes, budgeted parent-cone helper allocation, generator-global module-name allocation, and the current 51-scenario / 204-design Phase 4 matrix policy are now fully downstream-banked in `r26`; the next structural work is additional helper placement beyond the current helper-route slices and broader registered hierarchy routing/composition, with hierarchy-aware identity still later.
  1. **Keep the hierarchy gate representative without letting it drift back into leaf-stress cost or stale total-budget arithmetic.** The banked `r26` result closes cleanly because the Phase 4 sequential profiles are hierarchy-focused rather than borrowing the heaviest Phase 1 leaf stress, and because the gate budget now directly preserves four designs/scenario as the scenario set grows. Future hierarchy scenarios should preserve both separation-of-concerns and per-scenario evidence density.
...
- Phase 4 hierarchy is started and has a fully banked repo-owned closure artifact at `/tmp/anvil-tool-matrix-phase4-hierarchy-r26/tool_matrix_report.json` that covers wrapper, recursive, mixed-depth recursive, explicit library-vs-on-demand child-sourcing profiles, exact profiled child-interface synthesis, sibling-routed child-input binding, registered sibling-routed child-input binding, multi-stage registered sibling-routed child-input binding, registered parent-composed child-input binding, registered mixed-support child-input binding, multi-stage registered parent-composed child-input binding, mixed parent-port / child-output parent outputs, parent-composed child-input binding, parent-cone helper-instance child-input binding, direct sibling helper binding, direct registered sibling helper binding, parent-cone helper-instance parent-output routing, budgeted parent-cone helper allocation, registered helper-sourced child-input D cones, generator-global module-name allocation, and local parent state at 51 scenarios / 204 designs. The roadmap gap is helper placement beyond the current child-input, direct sibling, direct registered sibling, registered child-input, and budgeted parent-output helper routes, broader registered hierarchy routing/composition, and future hierarchy-aware identity.
