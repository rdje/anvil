...
- latest_commit: **`CAPABILITY-BREADTH-EXPANSION.4b.1`** (backfill next commit). Prior: `c38024b`, `eb79534`, `b9b26b4`. Recent decisions: **`0040`–`0044`**. Older commits NOT listed (`MEMORY_ARCHITECTURE.md` §12) — use `git log --oneline`.
...
- blockers: none. **OWED next commit** (`0041` §(a) — a defect is handled only once a tree owns it): register a tree for the **pre-existing** `ENUMERATION-PARITY` hole `.4b.1` found — `extract_steering_categories` captures the category as `"([a-z]+)"`, so an underscored category is invisible and the gate passes **vacuously**; full write-up in the `.4b.1` `DEVELOPMENT_NOTES.md` entry. **A decision you already made is DISCLOSED, never re-opened as a question** — [`0041`](docs/decisions/0041-owner-standing-directives-recorded-in-layer-c.md) §(e). **Adding to this file requires routing, not appending.**
