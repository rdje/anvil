...
| `anvil://artifact/<run_id>/analysis/<query>` | `application/json` | a derived-relation analysis (one of the ten `analyze` queries: `output_support` / `input_reach` / `flop_reset_provenance` / `module_reachability` / `flop_dependencies` / `memory_provenance` / `fsm_provenance` / `node_drivers` / `node_readers` / `instance_provenance`) | once that `analyze` `query` has run on the artifact |
