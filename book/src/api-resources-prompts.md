...
| `anvil://catalog/adapters` | `application/json` | the **downstream adapter catalog**: `{ "adapters": [ { id, binary, present, supports_facts } … ] }`, one entry per registered acceptance tool (today `verilator` / `yosys` / `iverilog` / `sv2v` / `slang`, the last two being the adapters added beyond the three originals — `sv2v`, a SystemVerilog→Verilog-2005 transpile accept/reject column, and `slang`, a strict independent elaborator that is the first **fact-bearing** adapter: `supports_facts = true`, projecting a SCHEMA-DERIVED `--ast-json` view of the top's ports + child instances), so you can discover which tools exist and which are installed (`present` — a live `<binary> --version` probe) over the API alone. The `id` is the token you pass in a `tools` array; `supports_facts` flags an adapter that exposes richer structured facts beyond accept/warn/reject |
| `anvil://catalog/presets` | `application/json` | the **knob preset registry** (`KNOB-ERGONOMICS-AND-PRESETS.2b.2a`): `{ "presets": [ { name, description, overrides } … ] }`, one entry per `--profile` bundle (`arithmetic-heavy` / `deep-hierarchy` / `structured-emission-max` / `sv2023-upopts`). `overrides` is the SCHEMA-DERIVED projection of the preset's `Config` field overrides with the unset knobs filtered out, so you see exactly what a preset changes. Apply one via the `profile` tool argument (below) |
| `anvil://catalog/knob-schema` | `application/json` | the **per-knob catalog** (`KNOB-ERGONOMICS-AND-PRESETS.2b.2b`): `{ "knobs": [ { name, group, ty, default, validation, cli_flag, config_only } … ] }`, one entry per `Config` field — a SCHEMA-DERIVED projection (names + defaults from `Config::default()`, `cli_flag` from the `Overrides` serde shape, `group`/`validation` classified). This is the richer companion to `anvil://catalog/knobs` (the bare `Config::default()` dump, kept unchanged): use it to discover every knob's group, default, validation summary, and whether it has a CLI flag |
...
| `anvil://artifact/<run_id>/analysis/<query>` | `application/json` | a derived-relation analysis (one of the fourteen `analyze` queries: `output_support` / `input_reach` / `flop_reset_provenance` / `module_reachability` / `flop_dependencies` / `memory_provenance` / `fsm_provenance` / `node_drivers` / `node_readers` / `instance_provenance` / `instance_input_bindings` / `longest_path` / `node_reach` / `reach_path`) | once that `analyze` `query` has run on the artifact |
