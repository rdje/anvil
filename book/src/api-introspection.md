...
| Field | Meaning |
| --- | --- |
| `schema_version` | the document schema version (currently `1.24`) — see [stability](#schema_version-stability-contract) |
...
The document schema is **`1.24`** and evolves under a strict MINOR/MAJOR policy
(the full per-version changelog is the canonical §7 of
`docs/AGENT_INTROSPECTION_SCHEMA.md`):

- a **MINOR** bump (e.g. `1.23 → 1.24`) is purely **additive** — a new optional
  ...
  **byte-identical** (new sections are `skip_serializing_if`, so a query that does
  ...
  each leaving the earlier queries' bytes untouched: the **eleven** derived queries are
  ...
  design-only) — (`1.22 → 1.23`), and `instance_input_bindings` — its parent-side
  dual, the parent node driving each child input port — (`1.23 → 1.24`). The structured-emission and capability metric
