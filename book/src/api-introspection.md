...
| `category_fire_rates` | the same cell pooled over each coarse category (<!--enum:steer-categories-->`state` / `selectors` / `datapath` / `terminals` / `sharing` / `hierarchy` / `motifs` / `emission` / `qualifiers`<!--/enum:steer-categories-->) — attempt-weighted. Note that `motifs` rolls at most once per module while `emission` rolls once per candidate gate, so their attempt counts differ by orders of magnitude; that is why they are separate categories rather than one. `qualifiers` is separate from `emission` for a different reason: its knobs claim only the gates the `emission` projections declined, so pooling the two would average a self-cancelling mixture. |
...
- a **MINOR** bump (e.g. `1.26 → 1.27`) is purely **additive** — a new optional
  ...
  **byte-identical** (new sections are `skip_serializing_if`, so a query that does
  ...
  `module_metrics.num_emitted_multi_output_tasks` (decision `0025`), and
  `1.14 → 1.15`/`1.16`/`1.17` added the procedural-mux / `case`-mux / `casez`-mux
  emit counts.
