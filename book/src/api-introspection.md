...
| Field | Meaning |
| --- | --- |
| `schema_version` | the document schema version (currently `1.23`) — see [stability](#schema_version-stability-contract) |
...
The document schema is **`1.23`** and evolves under a strict MINOR/MAJOR policy
(the full per-version changelog is the canonical §7 of
`docs/AGENT_INTROSPECTION_SCHEMA.md`):

- a **MINOR** bump (e.g. `1.22 → 1.23`) is purely **additive** — a new optional
  ...
  **byte-identical** (new sections are `skip_serializing_if`, so a query that does
  ...
  each leaving the earlier queries' bytes untouched: the **ten** derived queries are
  ...
  `node_drivers` — (`1.21 → 1.22`), and `instance_provenance` — per-child-instance
  descent into the child module's graph (the third opaque-leaf boundary-opener,
  design-only) — (`1.22 → 1.23`). The structured-emission and capability metric
