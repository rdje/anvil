...
```
```json
{ "schema_version": "1.24", "anvil_version": "0.1.0", "lane": "dut",
...
| `query` | `"output_support"` \| `"input_reach"` \| `"flop_reset_provenance"` \| `"module_reachability"` \| `"flop_dependencies"` \| `"memory_provenance"` \| `"fsm_provenance"` \| `"node_drivers"` \| `"node_readers"` \| `"instance_provenance"` \| `"instance_input_bindings"` | no | `"output_support"` | the relation kind |
...
(design-only) / `instance_input_bindings` → a child instance name. Omit `target` for *every* element.
...
```
```json
{ "schema_version": "1.24", "lane": "dut", "request": { "seed": 7, "run_id": "…" },
...
```
```json
{ "schema_version": "1.24", "lane": "dut", "request": { "seed": 42, "run_id": "…" },
  "coverage": {
