...
## Flop motifs: M-to-1 one-hot mux on D
...
- **M >= 2** — an M-to-1 one-hot mux. M = 1 is excluded by design (a
  1-arm mux is just a wire).

The two motifs below describe the M >= 2 case. They differ only in
what happens when no select bit fires.
