...
strategy is the one Phase 4 hierarchy scenario whose hierarchy-routing
probabilities are all 0.0 by design; its sole job is to exercise the
canonical-signature path with multiple structurally-identical
Modules so the future dedup pass (H-A-I.4) has live exercise.

The earlier `r85` bank lands the first slice of hierarchy-aware identity by
adding per-module canonical signatures
(`DesignMetrics.canonical_module_signatures`,
`num_distinct_module_signatures`,
`num_structurally_duplicate_module_pairs`). The hash covers port shape,
node sequence, drive structure, flop structure, and instance interfaces
but intentionally excludes `instance.module` / `instance.name`, so
structurally-identical Modules with distinctly-named children share a
signature. The new focus scenario
`phase4_recur_d2_canonical_module_signatures` per construction strategy
anchors the instrumentation in the matrix.

The earlier `r84` bank extends the helper-budget axis above the previous
budget-3 baseline. The focus scenario
...
The earlier `r81` bank extended the depth-7 axis to the stateful
parent-port-composed parent-output surface (r76's depth-6 territory)
at depth 7 with parent-flop probability 1.0 and a 64-flop budget per
module. The focus scenario
`phase4_recur_d7_stateful_parent_port_composed_output` per construction
strategy uses 2,2 child-instance bounds.

The earlier `r80` bank extends the depth-7 axis to the unregistered
parent-port-composed parent-output surface (r75's depth-6 territory)
at depth 7. The new focus scenario
`phase4_recur_d7_parent_port_composed_output` per construction strategy
uses `2,2` child-instance bounds, isolating the surface across six
intermediate parent layers below the top with no helpers, no flops, and
no child-input cones.

The earlier `r79` bank extends the depth-7 axis to the unregistered
parent-composed mixed-support child-input surface (r74's depth-6
territory) at depth 7. The new focus scenario
`phase4_recur_d7_parent_composed_mixed_support_child_input` per
construction strategy uses `2,2` child-instance bounds (continuing the
calibration from depth 6).

The earlier `r78` bank opens the depth-7 axis on top of the closed depth-6
sweep, mirroring how r73 opened depth-6 above the closed depth-5
sweep, r68 opened depth-5 above depth-4, and r63 opened depth-4 above
depth-3. The new focus scenario `phase4_recur_d7_parent_state` per
construction strategy uses `min/max_hierarchy_depth = 7` and `2,2`
child-instance bounds, isolating the parent-flop surface across six
intermediate parent layers below the top.

The earlier `r77` bank closes the depth-6 sweep with the stateful unregistered
parent-composed mixed-support child-input surface (r72's depth-5
territory, r67's depth-4 territory, r62's depth-3 territory) at
depth 6. The new focus scenario
`phase4_recur_d6_stateful_parent_composed_mixed_support_child_input`
per construction strategy uses `2,2` child-instance bounds (mirroring
r74's calibration) with parent-composed child-input cones at probability
1.0, parent-flop probability 1.0, and a 64-flop budget per module. The
depth-6 axis now has all five mixed-support cells gated as first-class
coverage facts, mirroring closed depth-3 (r58..r62), depth-4
(r63..r67), and depth-5 (r68..r72) sweeps.

The earlier `r76` bank extends the depth-6 axis to the stateful
...
- the refreshed `tool_matrix` Phase 4 scenario set now explicitly
  ...
  depth 6 without helpers or state, `r76` is the previous hierarchy
  full bank that extended the depth-6 axis with recursive non-top
  stateful parent-port-composed parent outputs at exact hierarchy
  depth 6 without helpers, `r77` is the previous hierarchy full bank
  that closed the depth-6 sweep with recursive non-top stateful
  unregistered parent-composed mixed-support child inputs at exact
  hierarchy depth 6 without helpers (2,2 calibrated), `r78` is the
  previous hierarchy full bank that opened the depth-7 axis with
  recursive non-top parent-local flops at exact hierarchy depth 7,
  `r79` is the previous hierarchy full bank that extended the depth-7
  axis with recursive non-top mixed-support child inputs at exact
  hierarchy depth 7 without helpers (2,2 calibrated), `r80` is the
  previous hierarchy full bank that extended the depth-7 axis with
  recursive non-top parent-port-composed parent outputs at exact
  ...
  parent-composed three-stage chain coverage, `r84` is the previous
  hierarchy full bank that extended the helper-budget axis above the
  previous budget-3 baseline with recursive non-top parent-cone helper
  budget 5 coverage, `r85` is the current hierarchy full bank that
  adds canonical module signatures as the first slice of
  hierarchy-aware identity instrumentation, and the
  aborted
  `r8`
