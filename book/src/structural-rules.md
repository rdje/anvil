...
| `e-graph`    | Bounded semantic equivalence fragment. Under `identity_mode = node-id`, small-support combinational cones proven equal over the same canonical leaf endpoints collapse post-construction. Functionally-unused helper endpoints are minimized out of the proof, so `a & (b \| !b)` can fold to the existing `a` node; different canonical roots such as `a` versus `c` remain distinct. Full semantic equivalence remains open. |
