...
(780 designs, `artifact_kind = "design"`, `coverage_gaps = []`, and
780/0 pass-fail in Verilator plus both repo-owned Yosys modes). That
...
routes without helper instances or parent-local state, and recursive non-top stateful parent-port-composed parent-output routes without helper instances, and recursive non-top stateful unregistered parent-composed mixed-support child-input routes through parent-local Qs without helper instances at exact hierarchy depth 7 (2,2 calibrated) — closing the depth-7 sweep.
The `r82` report records
...
remain useful focused policy breadcrumbs, while the current full `r82`
