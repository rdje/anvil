...
(360 designs, `artifact_kind = "design"`, `coverage_gaps = []`, and
360/0 pass-fail in Verilator plus both repo-owned Yosys modes). That
...
parent-composed helper routing. The `r43` report records
...
remain useful focused policy breadcrumbs, while the current full `r43`
...
is the previous recursive non-top stateful multi-helper budget bank, `r43` is the latest recursive non-top child-input multi-helper budget bank, and
