...
(444 designs, `artifact_kind = "design"`, `coverage_gaps = []`, and
444/0 pass-fail in Verilator plus both repo-owned Yosys modes). That
...
routes without helper instances or parent-local state.
The `r54` report records
...
remain useful focused policy breadcrumbs, while the current full `r54`
