...
- **`instance_input_bindings: Vec<InstanceInputBindings>`** (schema `1.24`,
  `SEMANTIC-INTROSPECTION-EXPANSION.12b.2`) — the `instance_input_bindings` payload: the
  **parent-side dual of `instance_provenance`**. For each child instance in a design's top, the
  **parent node** driving each of the child's **input** ports — read from the instance's `inputs`
  binding table (`Instance.inputs: Vec<(PortId, NodeId)>`). Where `instance_provenance` *descends
  into the child* to report each output's support cone (in the child's terms), this query *ascends
  to the parent* to report what drives each input (in the parent's terms); chaining the two traces
  a parent signal across the module boundary through a child output and back. An
  `InstanceInputBindings` is, per child instance: `instance` (its name, the entity), `module` (the
  child module it instantiates), `role` (`"planned_child"` | `"parent_cone"`), and
  `input_bindings` — one `InstanceInputBinding { port, driver }` per **bound** child input port (in
  ascending child PortId order). `port` is the child input **port name** in a design (or the
  `"port<id>"` fallback in a bare module); `driver` is the parent node as a `NodeRef` (`node` /
  `kind` / resolved `name` — a parent input name, a parent `"flop:<id>"`, a sibling
  `"<sibling>.<port>"`, or a parent `"node:<id>"`), resolved in the parent's graph by the **same**
  `node_ref_of` helper `node_drivers` uses — a pure read, no IR field, no generator change.
  `instance_input_bindings` carries the same `skip_serializing_if`, so the prior ten documents stay
  byte-identical across the `1.23 → 1.24` bump; an `instance_input_bindings` document carries it
  with `results: []`. `target` is a child instance **name** (omit for every instance); an instance
  with no data bindings is a known-but-empty entry, an unknown instance name is an error. Unlike
  `instance_provenance`, the single-module (`module_instance_input_bindings`) variant is
  **non-degenerate** — the parent driver lives in the bare module's own graph, so it carries real
  bindings; only the child input port *name* degrades to `"port<id>"`. Scope boundary (the
  `node_drivers` one-hop precedent): `driver` is the immediate (1-hop) parent node — chain
  `output_support` / `node_drivers` on `driver.node` for the transitive parent cone.
- **`longest_path: Vec<LongestPath>`** (schema `1.25`,
  `SEMANTIC-INTROSPECTION-EXPANSION.13b.2`) — the `longest_path` payload: one representative
  **longest combinational fan-in path** of a target. It is the **witness for `output_support`'s
  scalar `cone_depth`** and the **transitive complement to `node_drivers`**: where `output_support`
  collapses the whole fan-in to its boundary leaves (and reports `cone_depth` only as a number) and
  `node_drivers` shows a single node's 1-hop operands, `longest_path` returns the ordered chain of
  interior gates realizing the deepest path. A `LongestPath` is, per resolved target: `target` (an
  output port name, or `"flop:<id>"` — the **same namespace as `output_support`**), `depth` (the
  gate-depth, `== path.len() == output_support(target).cone_depth`), `path` (the interior-gate chain
  ordered from the target's driver toward the leaf — one `PathStep { node, op, width }` per gate;
  every step is a `Gate`, so `op` is always present), and `leaf` (the terminal boundary leaf as a
  `NodeRef` — a primary input / flop `"flop:<id>"` / instance output / `"mem:<id>"` / `"fsm:<id>"` /
  constant; omitted only when the target is **undriven**). The chain is made unique + byte-stable by
  a deterministic tie-break: at each gate, descend into the operand of maximum gate-depth, ties
  broken by smallest operand node id. It is **structural gate-depth, NOT a timing critical path**
  (ANVIL has no delay model). A pure two-pass walk of the existing node graph — no IR field, no
  generator change. `longest_path` carries the same `skip_serializing_if`, so the prior eleven
  documents stay byte-identical across the `1.24 → 1.25` bump; a `longest_path` document carries it
  with `results: []`. `target` is an output port name or `"flop:<id>"` (omit for every output); a
  resolvable-but-undriven target is a known-but-empty entry (empty `path`, no `leaf`), an unknown
  target is an error.
- **`node_reach: Vec<NodeReach>`** (schema `1.26`,
  `SEMANTIC-INTROSPECTION-EXPANSION.14b.2`) — the `node_reach` payload: per IR node, its
  **transitive combinational fan-OUT** — the boundary sinks it reaches. It is the **transitive
  complement to `node_readers`**, completing the node-addressed driver/reader × 1-hop/transitive
  matrix (`node_drivers` = backward 1-hop, `node_readers` = forward 1-hop, `longest_path` = backward
  transitive, `node_reach` = the forward-transitive corner), and the **node-addressed generalization
  of `input_reach`** (which is seeded only at cone-leaf sources). A `NodeReach` is, per node: `node`
  (id, addressed `"node:<id>"`), `kind` / `op` / `width` (the node-family header, mirroring
  `NodeReaders`), `reaches_outputs` (the output port names whose driving node is in the node's
  forward closure, sorted), `reaches_flops` (the flop ids whose `D` node is in the closure, sorted),
  and `fanout_targets` (their total). It is computed by a **forward-closure walk** over the reader
  index (the transpose of operands) — so the **register boundary is automatic** (a flop is not a
  `Gate` ⇒ no reader edge ⇒ the walk records the flop `D` sink and never crosses into the next
  stage's `Q`), and the sinks are output ports + flop `D` cones only (symmetric with `input_reach`;
  a node that only feeds a child-instance input reaches nothing — chain `instance_input_bindings` →
  `instance_provenance` to cross the module boundary). Provable consistency:
  `node_reach("node:<PrimaryInput-of-i>").{reaches_outputs, reaches_flops} ==
  input_reach(i).{reaches_outputs, reaches_flops}`. A pure walk of the existing node graph — no IR
  field, no generator change. `node_reach` carries the same `skip_serializing_if`, so the prior
  twelve documents stay byte-identical across the `1.25 → 1.26` bump; a `node_reach` document carries
...
(<!--enum:steer-categories-->`state` / `selectors` / `datapath` / `terminals` / `motifs` / `emission` / `qualifiers` / `sharing` / `hierarchy`<!--/enum:steer-categories--> —
...
- **`1.25` → `1.26` (`SEMANTIC-INTROSPECTION-EXPANSION.14b.2`).** Additive MINOR bump:
  ...
  (`reaches_outputs` output ports + `reaches_flops` flop `D` cones + `fanout_targets`), with the
  node-family header (`node` / `kind` / `op` / `width`, mirroring `NodeReaders`), carried by a
  thirteenth `DerivedAnalysis.node_reach: Vec<NodeReach>` parallel vec
  (`#[serde(default, skip_serializing_if = "Vec::is_empty")]`). The ninth query beyond decision
  `0011`'s four named kinds — the **transitive complement to `node_readers`**, completing the
  node-addressed driver/reader × 1-hop/transitive matrix (`node_drivers` backward 1-hop,
  `node_readers` forward 1-hop, `longest_path` backward transitive, `node_reach` the
  forward-transitive corner), and the **node-addressed generalization of `input_reach`** (which is
  seeded only at cone-leaf sources) — with the provable consistency
  `node_reach("node:<PrimaryInput-of-i>") == input_reach(i)`. The register boundary is **automatic**
  (a flop is not a `Gate` ⇒ no reader edge ⇒ the forward walk records the flop `D` sink and never
  crosses into the next stage's `Q`); the sinks are output ports + flop `D` cones only (symmetric
  with `input_reach`). SCHEMA-DERIVED (a pure forward-closure walk over the reader index — the
  transpose of operands — not new computed truth). Backward compatible: the `node_reach` key is
  `skip_serializing_if`-omitted on every other `analyze` document, so the twelve prior query
  documents and the default-`dut` **artifact** (`.sv`) stay byte-identical; a `1.25` consumer
  ignores the new query kind. MINOR is an integer, so this is `1.25 → 1.26` (twenty-six), not a
  decimal.
- **`1.24` → `1.25` (`SEMANTIC-INTROSPECTION-EXPANSION.13b.2`).** Additive MINOR bump:
  added the **twelfth** derived `analyze` query kind `longest_path` — one representative
  **longest combinational fan-in path** of a target (an output port / `"flop:<id>"`, the
  `output_support` namespace): the ordered chain of interior gates (each a `PathStep { node, op,
  width }`) realizing the `output_support` cone's scalar `cone_depth`, terminating at a boundary
  leaf (a `NodeRef`), carried by a twelfth
  `DerivedAnalysis.longest_path: Vec<LongestPath>` parallel vec
  (`#[serde(default, skip_serializing_if = "Vec::is_empty")]`). The eighth query beyond decision
  `0011`'s four named kinds — the **witness for `output_support`'s scalar `cone_depth`** (provably
  `longest_path(t).depth == output_support(t).cone_depth`) and the **transitive complement to
  `node_drivers`** (the full longest chain vs one hop). It is **structural gate-depth, NOT a timing
  critical path** (ANVIL has no delay model). SCHEMA-DERIVED (a pure two-pass walk of the existing
  node graph — a `node_depth` memo = the recurrence `visit` returns, then a greedy max-child-depth
  descent with ties broken by smallest operand node id ⇒ a unique byte-stable path; not new
  computed truth). Backward compatible: the `longest_path` key is `skip_serializing_if`-omitted on
  every other `analyze` document, so the eleven prior query documents and the default-`dut`
  **artifact** (`.sv`) stay byte-identical; a `1.24` consumer ignores the new query kind. MINOR is
  an integer, so this is `1.24 → 1.25` (twenty-five), not a decimal.
- **`1.23` → `1.24` (`SEMANTIC-INTROSPECTION-EXPANSION.12b.2`).** Additive MINOR bump:
  added the **eleventh** derived `analyze` query kind `instance_input_bindings` — the
  **parent-side dual of `instance_provenance`**: for each child instance in a design's top, its
  `module` / `role` + the **parent node** (a `NodeRef`) driving each of the child's **input**
  ports (read from the instance's `inputs` binding table), carried by an eleventh
  `DerivedAnalysis.instance_input_bindings: Vec<InstanceInputBindings>` parallel vec
  (`#[serde(default, skip_serializing_if = "Vec::is_empty")]`). The seventh query beyond
  decision `0011`'s four named kinds. `instance_provenance` *descends into the child* for each
  output; `instance_input_bindings` *ascends to the parent* for each input — chaining the two
  traces a parent signal across the module boundary. SCHEMA-DERIVED (a pure read of
  `Instance.inputs`, the parent driver resolved via the same `node_ref_of` helper — not new
  computed truth). Unlike `instance_provenance` (design-only), the single-module variant is
  **non-degenerate** (the parent driver lives in the bare module's graph; only the child port
  *name* degrades to `"port<id>"`). Backward compatible: the `instance_input_bindings` key is
  `skip_serializing_if`-omitted on every other `analyze` document, so the ten prior query
  documents and the default-`dut` **artifact** (`.sv`) stay byte-identical; a `1.23` consumer
  ignores the new query kind. MINOR is an integer, so this is `1.23 → 1.24` (twenty-four), not a
  decimal.
