...
answers:
  ...
  - "does ANVIL emit a wider-lane generate-for part-select"
  - "how does ANVIL render a multi-bit lane replication as a generate loop"
...
its wider-lane part-select case is the **fourth** surface, decision
[`0015`](../decisions/0015-structured-emission-fourth-surface-wide-lane-generate-loop.md))
...
- **What qualifies:** the gen-time pass
  ...
  **same** `NodeId` — of **any lane width `LW ≥ 1`** (so result width `== N·LW`).
  The common case is the one-hot `{W{sel}}` mux-mask broadcast (a 1-bit lane); a
  wider lane (e.g. `{2{i_2}}` over a 2-bit input) is the **fourth surface**
  (decision [`0015`](../decisions/0015-structured-emission-fourth-surface-wide-lane-generate-loop.md),
  `.8b`). It runs **after** the `function_emit` pass (so a function-emit-marked
  replication is excluded — the two projections are mutually exclusive on a gate)
  and skips Phase-5 `param_env` modules.
- **Rendering (`src/emit/sv.rs`):** for a marked replication `<wire>`, a
  ...
  <wire>__gi + 1) begin : <wire>__gen <body>; end endgenerate`
  (`render_generate_loop_block` + the `generate_loop_gate` defensive accessor),
  where the body is `assign <wire>[<wire>__gi] = <x>;` for a **1-bit lane** and
  the indexed part-select `assign <wire>[<wire>__gi*LW +: LW] = <x>;` for a
  **wider lane** (`LW = m.nodes[lane].width()`, recomputed in the renderer; the
  `LW==1` form is byte-identical to the second-surface original). The per-gate
  assign loop `continue`s past the marked gate so the inline
  `assign <wire> = {N{x}};` is suppressed. The increment is the
  maximally-portable `gi = gi + 1` (`gi++` is equally valid, not foreclosed).
