...
| `PHASE-7-ORACLE-MICRODESIGN` | `active` | Phase 7 — Oracle-backed micro-design artifacts | `PHASE-7-ORACLE-MICRODESIGN.2c.1` (`.2a`+`.2b` done — `src/microdesign/` IR+evaluator/oracle + un-resolved SV emitter + JSON manifest emitter from the oracle, 7 unit proofs, byte-reproducible; **`.2c` split (`2026-05-20`)** into `.2c.1` build parity harness (cargo-portable comparator proof + tool-gated `#[ignore]` real-tool harness scaffold; no real run, no ROADMAP advance) + `.2c.2` real-tool gate + verify clean + ROADMAP Phase 7 → done; `.2c.1` unblocked) | [docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md](tasks/PHASE-7-ORACLE-MICRODESIGN.md) |
