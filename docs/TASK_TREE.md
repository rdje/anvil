...
| `AGENT-MCP-EXPANSION` | `active` | Capability — agent/MCP interface breadth | `.1` done (decision `0005`); `.2` done (coverage_gaps pure-projection MCP tool); **frontier → `.3a`** (non-DUT introspection design), then `.3b` impl; `.3` split into `.3a`/`.3b`; lane order `2 → 3 → 1` | [docs/tasks/AGENT-MCP-EXPANSION.md](tasks/AGENT-MCP-EXPANSION.md) |
