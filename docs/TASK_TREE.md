...
| `CONE-DECOMPOSITION` | `active` | Code quality / maintainability — generator core readability | `.1` (design) + `.2` (`cone/snapshot.rs`) + `.3` (`cone/semantic.rs`, ~1360 lines) done; frontier `.4` (extract `cone/primitives.rs`), then `.5` terminals / `.6` flops / `.7` motifs+closeout. Pure structural refactor; every leaf byte-identical (lib + snapshots; full suite at milestones). | [docs/tasks/CONE-DECOMPOSITION.md](tasks/CONE-DECOMPOSITION.md) |
