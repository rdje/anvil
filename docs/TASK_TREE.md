...
| `SV-VERSION-TARGETING` | `active` | Capability / breadth — version-targeted SV emission | `.1` **done** (decision `0009`): opt-in `--sv-version <2012\|2017\|2023>` gate (`Config::sv_version`) — down-gating (never emit a construct newer than the target → standard-validity guarantee) + up-opting (deliberately emit a higher standard's distinctive synthesizable constructs, each proven downstream-clean in the matching tool mode), default byte-identical, rules-first, per-version downstream acceptance axis. Owner-directed highest-leverage lane (`2026-06-15`). `.2a` **done** (design detail in `DEVELOPMENT_NOTES.md`: resolved decision `0009`'s five open questions — `SvVersion {Sv2012<Sv2017<Sv2023}` default `Sv2012`, emitter-parameter capability bound, serde-automatic introspection + schema MINOR bump `1.1→1.2`, per-version Verilator `--language` axis; split `.2`→`.2a`/`.2b`, pre-split `.2b`→`.2b.1`/`.2b.2`). Frontier = `.2b.1` (`proposed`, knob plumbing + emitter capability bound + cross-version byte-identity proof, default byte-identical) | [docs/tasks/SV-VERSION-TARGETING.md](tasks/SV-VERSION-TARGETING.md) |
