...
| `INSTA-SNAPSHOTS` | `active` | Quality — reproducibility regressions | `INSTA-SNAPSHOTS.2` (`.1` done — `insta` pinned `=1.47.2`, `tests/snapshots.rs` baseline [canonical leaf + bounded recursive library], byte-stable on re-run, full suite green; `.2` expands to ≥5 shapes) | [docs/tasks/INSTA-SNAPSHOTS.md](tasks/INSTA-SNAPSHOTS.md) |
