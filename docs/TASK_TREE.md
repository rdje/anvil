...
| `PHASE-7-ORACLE-MICRODESIGN` | `active` | Phase 7 — Oracle-backed micro-design artifacts | `PHASE-7-ORACLE-MICRODESIGN.2c.2a` (unblocked, code-bearing) — `.2c.1` done (parity comparator core + 9 cargo-portable proofs + tool-gated `#[ignore]` scaffold); empirical probe today: yosys 0.64 `write_json` exposes 4 of 7 manifest fact categories (seed/top/params/generate + partial widths["sig"]) — localparams + package_constants folded. **`.2c.2` split (`2026-05-20`)** on the discovered lower-level dependency into `.2c.2a` (`FactCategory`+`ParityScope`+scoped comparator + yosys-specific extractor + end-to-end-runnable `#[ignore]` harness; cargo stays green tool-less; no ROADMAP advance) + `.2c.2b` (real `--ignored` run + verify + bank + ROADMAP Phase 7 → done; gate-blocked); mirrors memory `.2.1`→`.2.1a`/`.2.1b` precedent | [docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md](tasks/PHASE-7-ORACLE-MICRODESIGN.md) |
