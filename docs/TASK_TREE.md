...
| `AGENT-MCP-EXPANSION` | `active` | Capability — agent/MCP interface breadth | `.1` done (decision `0005`); `.2` done (coverage_gaps MCP tool); `.3` done (`.3a`+`.3b` non-DUT lanes over MCP); `.4` done (`.4a` framing design + `.4b` hand-rolled `--http <addr>` transport over the same `handle_line`, loopback default, no new dep, DUT byte-identical); **frontier → `.5`** (closeout: book/USER_GUIDE/README sync, close tree); lane order `2 → 3 → 1` | [docs/tasks/AGENT-MCP-EXPANSION.md](tasks/AGENT-MCP-EXPANSION.md) |
| `SIGNOFF-AUTOMATION-EXPANSION` | `proposed` | Quality — downstream signoff automation breadth | `.1` design (`pending`); promotes to `active` after `AGENT-MCP-EXPANSION` reaches handoff | [docs/tasks/SIGNOFF-AUTOMATION-EXPANSION.md](tasks/SIGNOFF-AUTOMATION-EXPANSION.md) |
| `IDENTITY-DEEPENING` | `proposed` | NodeId as identity / full-factorization deepening | `.1` design (`pending`); promotes to `active` after `SIGNOFF-AUTOMATION-EXPANSION` reaches handoff | [docs/tasks/IDENTITY-DEEPENING.md](tasks/IDENTITY-DEEPENING.md) |
