...
| `DIFFERENTIAL-SIMULATION` | `active` | Quality — signoff-level downstream consistency | `DIFFERENTIAL-SIMULATION.2b.2` (real-tool alignment fix + verified run) — **`.2b.1` done (`2026-05-20`)**: new `tests/diff_sim.rs` carries the IR-driven testbench harness + dual-sim orchestration + 5 cargo-portable helper proofs + 2 tool-gated `#[ignore]` scaffolds; portable `cargo test` stays green tool-less. **`.2b` split (`2026-05-20`)** per `PHASE-7-ORACLE-MICRODESIGN.2c.2a`/`.2c.2b` precedent on a discovered iverilog↔verilator trace-alignment issue surfaced by the first real-tool run (iverilog samples one extra time at vector[0]; underlying sequences match, just shifted). `.2b.2` fixes the alignment + runs the real-tool gate end-to-end + records verified-clean byte-equal evidence
