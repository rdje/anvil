...
| `IDENTITY-DEEPENING` | `active` | NodeId as identity / full-factorization deepening | `.1` **done** (decision `0007` — first extension = bounded bisimulation-based sequential flop equivalence: greatest-fixpoint partition refinement reusing the bounded combinational endpoint proof up to a state correspondence, default-off knob + node-id/e-graph, sound via reset-base-case coinduction; lifts the recorded mutually-recursive-register / non-exact-feedback no-merge boundary). Frontier = `.2` (impl, `pending`); `.3` (module-level sequential equivalence) `proposed`/future | [docs/tasks/IDENTITY-DEEPENING.md](tasks/IDENTITY-DEEPENING.md) |
