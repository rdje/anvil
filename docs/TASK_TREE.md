...
| `DIFFERENTIAL-SIMULATION` | `active` | Quality — signoff-level downstream consistency | `DIFFERENTIAL-SIMULATION.3b.2` (wire `tool_matrix --diff-sim`) — **`.3b.1` done (`2026-05-24`)** pure refactor extracting harness helpers to `src/diff_sim/mod.rs` (~325 lines: `pub fn baked_input_vectors`/`mask_to_width`/`fmt_sv_hex`/`is_sequential`/`emit_testbench`/`run_iverilog`/`run_verilator`/`normalize_trace`/`tools_present`); `src/lib.rs` `pub mod diff_sim` with full-factorization-doctrine docstring; `tests/diff_sim.rs` reduced from ~556 to ~175 lines (`use anvil::diff_sim::{…}` + 2 `#[ignore]`-gated focused proofs + 2 cargo-portable smoke proofs round-tripping through real `Module`); `cargo test --lib` 247 (+3 migrated). **Real-tool gate clean against extracted helpers** — `differential_simulation_combinational` + `differential_simulation_sequential` both ok against iverilog 13.0 + verilator 5.046; pure refactor — byte-identical behavior verified end-to-end. `.3b` further split per the proven Phase-7 `.2c.2a`/`.2c.2b` discipline. `.3a` design landed `2026-05-24` (docs-only): module-extraction + `--diff-sim` CLI + per-axis subset selector + `saw_design_with_cross_simulator_agreement` coverage fact + `DiffSimReport` with retained `mismatch_excerpt` + `tools_present()`-friendly no-op. `.2b.2` closed `.2b` + `.2` container `2026-05-24` — first gate to assert downstream-tool *semantic* agreement on ANVIL output (`project_anvil_north_star.md`). | [docs/tasks/DIFFERENTIAL-SIMULATION.md](tasks/DIFFERENTIAL-SIMULATION.md) |
