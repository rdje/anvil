...
| `PHASE-7-ORACLE-MICRODESIGN` | `active` | Phase 7 — Oracle-backed micro-design artifacts | `PHASE-7-ORACLE-MICRODESIGN.2c.2b.2` (gate-blocked, real-tool re-run) — **`.2c.2b.1` done (`2026-05-20`)**: `width_expr` SV text flipped from `(<last> % 8) + 1` to the standard SV non-negative-modulo idiom `((<last> % 8) + 8) % 8 + 1` so oracle ≡ SV for any `last.value` (the oracle's `rem_euclid` was already correct; only the SV text needed to catch up). New lib regression proof pins seed 7 P4=-1 → bits=8 + seed 0 collapse + cross-seed structural assertion. Full `cargo test` green: lib 229 (+1 regression), microdesign 8/8, parity 15+1, pipeline 121, snapshots 6, bin 5+29+3, doc 0. `.2c.2b.2` runs `cargo test -- --ignored parity_against_real_yosys_write_json` against real yosys — the seed-7 counterexample is structurally impossible after `.2c.2b.1`'s fix — verifies exact-agreement on yosys-supported categories across the corpus, banks the verified-clean artifact, records **ROADMAP Phase 7 → done** with the explicit scope caveat + reconciles book/README/CODEBASE | [docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md](tasks/PHASE-7-ORACLE-MICRODESIGN.md) |
