...
| `PHASE-9-MULTI-ARTIFACT-UMBRELLA` | `active` | Phase 9 — Multi-artifact ANVIL umbrella | `PHASE-9-MULTI-ARTIFACT-UMBRELLA.2b` (unblocked, code-bearing) — **`.2a` done (`2026-05-20`)**: new top-level module `src/umbrella/` with `ArtifactLane` trait (`name`/`validate_knobs`/`generate(seed)`/`check_plan`) + `LaneArtifact` carrier + `CheckPlan{SynthAccept, ParityVsManifest}` + `LaneError` + `DutLane{base_config: Config}` wrapping today's DUT path (zero behavioural change); 4 unit proofs incl. **the load-bearing byte-identical regression proof** comparing trait-dispatch vs direct-`Generator` output across reproducibility seeds + a `dyn`-dispatch byte-identical proof. Full `cargo test` green (lib **240** ← 236+4). `.2b` adds `MicrodesignLane` + `FrontendLane` impls of the trait + cross-lane byte-identical regression proof. `.2c` then adds the `--artifact` CLI flag + book/CI byte-identical verification + ROADMAP Phase 9 → done | [docs/tasks/PHASE-9-MULTI-ARTIFACT-UMBRELLA.md](tasks/PHASE-9-MULTI-ARTIFACT-UMBRELLA.md) |
