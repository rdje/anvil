...
| `PHASE-8-FRONTEND-ACCEPT` | `active` | Phase 8 — Frontend/elaboration accept corpora | `PHASE-8-FRONTEND-ACCEPT.2c` (unblocked; expected to split per Phase 7 precedent) — **`.2b` done (`2026-05-20`)**: `src/frontend/mod.rs` extended with `emit_sv()` (un-elaborated SV) + manifest types (`PackageFacts`/`ParamFact`/`InstanceFact`/`GenerateFact`/`Manifest`, all `pub`+serde+`BTreeMap`) + `build_manifest()`/`emit_manifest()` (reads `.2a` oracle directly; byte-stable); cross-tree reuse of `microdesign::expr_to_sv`; 3 new lib proofs (`emit_sv_is_valid_unresolved_shape`/`manifest_mirrors_the_oracle`/`sv_and_manifest_are_byte_reproducible`); full `cargo test` green (lib 236 ← 233 + 3; `frontend::tests` 10/10 was 7). `.2c` adds the hierarchy-aware parity harness + repo-owned gate (reuses Phase 7's scoped comparator with hierarchy-aware variants; tool-gated `#[ignore]`); ROADMAP Phase 8 → done on verified clean banked artifact (r87) | [docs/tasks/PHASE-8-FRONTEND-ACCEPT.md](tasks/PHASE-8-FRONTEND-ACCEPT.md) |
