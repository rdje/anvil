...
| `DIFFERENTIAL-SIMULATION` | `active` | Quality — signoff-level downstream consistency | `DIFFERENTIAL-SIMULATION.2` (`.1` done — iverilog 13.0 installed + proven zero-config-compatible all 4 ANVIL categories; pair Verilator↔iverilog; pre-reset 4-state divergence is a `.2` design constraint; `.2` = single-design differential harness) | [docs/tasks/DIFFERENTIAL-SIMULATION.md](tasks/DIFFERENTIAL-SIMULATION.md) |
