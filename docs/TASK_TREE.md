...
| `PHASE-7-ORACLE-MICRODESIGN` | `active` | Phase 7 — Oracle-backed micro-design artifacts | `PHASE-7-ORACLE-MICRODESIGN.2c.2b.1` (unblocked, code-bearing — small fix) — first real-tool `--ignored` run of `.2c.2a`'s gate against yosys 0.64 retained one counterexample: seed 7, `WidthMismatch { sig, expected bits=8, actual bits=2 }`. Root cause is an **ANVIL self-consistency bug** in `width_expr`: oracle uses Rust's `rem_euclid` (non-negative modulo, gives 8 for `(-1).rem_euclid(8) + 1`) but SV emits `%` (truncated toward zero, gives 0 for `(-1 % 8) + 1`); diverge for negative `last.value`. **`.2c.2b` split (`2026-05-20`)** per memory `.2.1`→`.2.1a`/`.2.1b` discovered-dependency precedent into `.2c.2b.1` (fix: change BOTH oracle + SV text to non-negative-modulo idiom `((x % 8) + 8) % 8 + 1`; regression proof) + `.2c.2b.2` (re-run + verify clean + bank + ROADMAP Phase 7 → done; gate-blocked). Per ANVIL's "valid-by-construction + downstream-acceptance-quality" north-star, fix must land before promotion (r87) | [docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md](tasks/PHASE-7-ORACLE-MICRODESIGN.md) |
