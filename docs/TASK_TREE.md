...
| `PHASE-7-ORACLE-MICRODESIGN` | `active` | Phase 7 — Oracle-backed micro-design artifacts | `PHASE-7-ORACLE-MICRODESIGN.2c.2b` (gate-blocked, real-tool run) — **`.2c.2a` done (`2026-05-20`)**: `FactCategory`+`ParityScope`+`compare_manifest_to_tool_report_in_scope` in `src/microdesign/` (strict comparator delegates to `ParityScope::all()`, backwards-compatible); yosys-specific `parse_yosys_binary_param`+`yosys_write_json_to_tool_report` extractor in `tests/microdesign_parity.rs` with 3 sanity proofs + 3 scoped-comparator proofs (15 portable + 1 `#[ignore]`); `#[ignore]` test is now end-to-end-runnable. `.2c.2b` runs `cargo test -- --ignored parity_against_real_yosys_write_json` against real yosys, verifies exact-agreement on yosys-supported categories (Seed/Top/Params/Widths["sig"]/Generate) across the full corpus, banks the verified-clean artifact, records **ROADMAP Phase 7 → done** with the explicit scope caveat (r87) + reconciles book/README/CODEBASE | [docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md](tasks/PHASE-7-ORACLE-MICRODESIGN.md) |
