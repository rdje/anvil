...
| `PHASE-6-ADVANCED-MOTIFS` | `active` | Phase 6 — Advanced motifs | `.2.4` (gate-blocked: memory `.2.1`/`.2.2`/`.2.3` done — IR+knob+proofs+scenario, bin 219/876; verify when running gate completes) ‖ `.3.4b` (LAST Phase 6 leaf, gate-blocked: FSM `.3.4a` done — `phase6_fsm` scenario + `num_fsm_modules` metric + `saw_fsm_design` gap + non-vacuity, bin 222/888, tool_matrix 29/29; `.3.4b` = fresh real gate verified clean → records FSM delivered + closes ROADMAP Phase 6 + the tree, one run covers memory+fsm) | [docs/tasks/PHASE-6-ADVANCED-MOTIFS.md](tasks/PHASE-6-ADVANCED-MOTIFS.md) |
| `PHASE-7-ORACLE-MICRODESIGN` | `active` | Phase 7 — Oracle-backed micro-design artifacts | `PHASE-7-ORACLE-MICRODESIGN.2` (`.1` design done — expected-facts JSON schema + oracle-by-construction generation + new parity harness + 4 rejected alternatives; `.2` = IR+evaluator/emitters/harness, expected to split) | [docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md](tasks/PHASE-7-ORACLE-MICRODESIGN.md) |
| `PHASE-8-FRONTEND-ACCEPT` | `active` | Phase 8 — Frontend/elaboration accept corpora | `PHASE-8-FRONTEND-ACCEPT.2` (`.1` design done — dedicated source-level AST IR + un-elaborated-SV/elaborated-facts-manifest oracle + instance-tree schema, reuses Phase 7 core; `.2` sequences after `PHASE-7-ORACLE-MICRODESIGN.2`, expected to split) | [docs/tasks/PHASE-8-FRONTEND-ACCEPT.md](tasks/PHASE-8-FRONTEND-ACCEPT.md) |
...
| `COVERAGE-INSTRUMENTATION` | `active` | Quality — test-discipline visibility | `COVERAGE-INSTRUMENTATION.3` (`.2` triage done — no confirmed dead code in the top-5; `.3` scoped to a handful of `gen/cone.rs` focused proofs + optional `config.rs` orphan-knob spot-audit) | [docs/tasks/COVERAGE-INSTRUMENTATION.md](tasks/COVERAGE-INSTRUMENTATION.md) |
