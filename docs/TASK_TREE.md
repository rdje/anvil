...
| `PHASE-7-ORACLE-MICRODESIGN` | `active` | Phase 7 — Oracle-backed micro-design artifacts | `PHASE-7-ORACLE-MICRODESIGN.2c` (`.2a`+`.2b` done — `src/microdesign/` IR+evaluator/oracle + un-resolved SV emitter + JSON manifest emitter from the oracle, 7 unit proofs, byte-reproducible; `.2c` = tool-gated parity harness + repo-owned gate → ROADMAP Phase 7) | [docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md](tasks/PHASE-7-ORACLE-MICRODESIGN.md) |
