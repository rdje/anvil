...
| `SIGNOFF-AUTOMATION-EXPANSION` | `active` | Quality — downstream signoff automation breadth | `.1` done (decision `0006`: first increment = richer adversarial knob-sweep coverage — promote unswept knobs to explicit matrix axes + coverage facts, closing ROADMAP gap 3 hidden bias); **frontier → `.2`** (implement the first knob-sweep batch); new-tool-column + non-DUT-acceptance paths preserved as future leaves | [docs/tasks/SIGNOFF-AUTOMATION-EXPANSION.md](tasks/SIGNOFF-AUTOMATION-EXPANSION.md) |
