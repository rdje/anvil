...
| `AGENT-MCP-EXPANSION` | `active` | Capability — agent/MCP interface breadth | `.1` done (decision `0005`); **frontier → `.2`** (coverage-gaps pure-projection tool), then `.3a` (non-DUT introspection design); `.3` split into `.3a`/`.3b`; lane order `2 → 3 → 1` | [docs/tasks/AGENT-MCP-EXPANSION.md](tasks/AGENT-MCP-EXPANSION.md) |
