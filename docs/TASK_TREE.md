...
| `PHASE-7-ORACLE-MICRODESIGN` | `active` | Phase 7 — Oracle-backed micro-design artifacts | `PHASE-7-ORACLE-MICRODESIGN.2c.2` (gate-blocked, real-tool run) — **`.2c.1` done (`2026-05-20`)**: `src/microdesign/` extended with the parity comparator core (`ToolReport`/`Divergence`×17/`compare_manifest_to_tool_report`/`synthetic_tool_report_from_manifest`) + `pub`/`Clone`/`PartialEq`/`Eq`/`Deserialize` on the fact records; new `tests/microdesign_parity.rs` 9 cargo-portable comparator proofs green (agreement + per-axis divergence) + 1 tool-gated `#[ignore]` `parity_against_real_yosys_write_json` scaffold; portable `cargo test` stays green tool-less. `.2c.2` runs the real `--ignored` test end-to-end, verifies exact-agreement, records **ROADMAP Phase 7 → done** (r87) + reconciles book/README/CODEBASE | [docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md](tasks/PHASE-7-ORACLE-MICRODESIGN.md) |
