...
| `PHASE-6-ADVANCED-MOTIFS` | `active` | Phase 6 — Advanced motifs | `.2.4` (gate-blocked: memory `.2.1`/`.2.2`/`.2.3` done — IR+knob+proofs+scenario, bin 219/876; verify when running gate completes) ‖ `.3.4` (next, LAST Phase 6 leaf: FSM `.3.3` done — cargo-portable proof 96 designs, exact per-encoding template + 1 `FsmOut`/0 `Gate` across every FactorizationLevel incl. EGraph + all 3 encodings; `.3.4` = `phase6_fsm` scenario+metric+gap → real-gate verify → closes ROADMAP Phase 6 + the tree) | [docs/tasks/PHASE-6-ADVANCED-MOTIFS.md](tasks/PHASE-6-ADVANCED-MOTIFS.md) |
