...
| `ACCEPTANCE-DIVERGENCE-HUNTING` | `active` | Usability — acceptance-divergence bug-finder (north star, idea 2) | **Registered `2026-06-17`** (owner-directed). A first-class "tool A accepts / tool B rejects" (and version-vs-version) accept/warn/reject divergence finder + report — a `tool_matrix` column + an MCP query — complementing `--diff-sim` (cross-sim trace agreement) with acceptance divergence, where real downstream bugs live. Binds decision `0017`. Frontier `.1` (design ADR). No code yet. | [docs/tasks/ACCEPTANCE-DIVERGENCE-HUNTING.md](tasks/ACCEPTANCE-DIVERGENCE-HUNTING.md) |
| `DOWNSTREAM-ADAPTER-EXPANSION` | `active` | Usability / breadth — more downstream tool reach (north star, idea 3) | **Registered `2026-06-17`** (owner-directed). A generic, API-selectable downstream-adapter interface so new tools (slang / sv2v / Surelog-UHDM / commercial wrappers) plug in as acceptance columns with little new core code, reusing the hardened `src/downstream` allow-list + the `tool_matrix` column model. Binds decision `0017`. Frontier `.1` (design ADR). No code yet. | [docs/tasks/DOWNSTREAM-ADAPTER-EXPANSION.md](tasks/DOWNSTREAM-ADAPTER-EXPANSION.md) |
| `KNOB-ERGONOMICS-AND-PRESETS` | `active` | Usability — knob ergonomics + presets + full API steerability (north star, idea 4) | **Registered `2026-06-17`** (owner-directed). Promote high-value config-file-only knobs to CLI flags + a curated `--profile` preset registry (arithmetic-heavy / deep-hierarchy / structured-emission-max / sv2023-upopts), AND (per decision `0017`) guarantee every knob is MCP-steerable with the knob catalog + preset registry API-queryable. Frontier `.1` (design ADR). No code yet. | [docs/tasks/KNOB-ERGONOMICS-AND-PRESETS.md](tasks/KNOB-ERGONOMICS-AND-PRESETS.md) |
| `CI-PACKAGING-DISTRIBUTION` | `active` | Usability — drop-in CI packaging (north star, idea 5) | **Registered `2026-06-17`** (owner-directed). Prebuilt per-platform release binaries + a drop-in GitHub Action so a downstream-tool maintainer can continuously fuzz their parser/elaborator against ANVIL, driven through the same API surface (decision `0017`). Soft dep on `BUG-HUNT-ORCHESTRATION` (the engine it wraps). Frontier `.1` (design ADR). No code yet. | [docs/tasks/CI-PACKAGING-DISTRIBUTION.md](tasks/CI-PACKAGING-DISTRIBUTION.md) |
