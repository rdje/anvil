...
| `AGENT-MCP-EXPANSION` | `active` | Capability — agent/MCP interface breadth | `.1` done (decision `0005`); `.2` done (coverage_gaps MCP tool); `.3` done (`.3a` design + `.3b` non-DUT lanes microdesign/frontend over MCP, manifest inlined §6.5 + resource §4, DUT byte-identical); **frontier → `.4`** (HTTP transport), then `.5` closeout; lane order `2 → 3 → 1` | [docs/tasks/AGENT-MCP-EXPANSION.md](tasks/AGENT-MCP-EXPANSION.md) |
