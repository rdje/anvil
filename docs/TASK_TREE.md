...
| `WORKLOAD-MEMORY-SAFETY` | `active` | Quality / signoff — resource-safe generation (ANVIL's own runtime) | `.1` (audit + bounded-memory design) done; frontier `.2` (stream directory-output manifest so huge `--count` does not accumulate all metadata in RAM). `.3` node-budget governor, `.4` internal RAM/RSS self-governor, `.5` closeout pending. All mechanisms default-off / byte-identical. | [docs/tasks/WORKLOAD-MEMORY-SAFETY.md](tasks/WORKLOAD-MEMORY-SAFETY.md) |
