...
| `AGENT-INTROSPECTION-MCP` | `active` | Capability — agent-drivable introspection + MCP interface | frontier `.5.2` (controlled `validate` tool). Owner **accepted** the `.1`/`.2` design (`2026-06-14`), unblocking the code leaves. `.1` done — decision `0004`. `.2` done — `docs/AGENT_INTROSPECTION_SCHEMA.md` (versioned introspection envelope; zero new computed truth). `.3` done — `src/introspect/` + default-off `--introspect` flag (DUT byte-identical). `.4` done — `src/mcp/` + `anvil-mcp` bin (stdio JSON-RPC; generate/introspect/dump_config + resources over a content-addressed cache; no external-tool exec). `.5` split into `.5.1`/`.5.2`/`.5.3`; `.5.1` done — hardened downstream-tool invocation surface (`verilator`/`yosys`/`iverilog` acceptance command lines + warning-as-failure + `ToolInvocation`) extracted from the `tool_matrix` binary into the shared library module `src/downstream/`, behavior-preserving (snapshots 6/6). `.5.2`/`.5.3` (validate/minimize), `.6`–`.7` (prompts, book/USER_GUIDE closeout) proceed in order under PNT; user-facing docs deferred to `.7`. | [docs/tasks/AGENT-INTROSPECTION-MCP.md](tasks/AGENT-INTROSPECTION-MCP.md) |
