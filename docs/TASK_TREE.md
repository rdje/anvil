...
| `AGENT-INTROSPECTION-MCP` | `active` | Capability — agent-drivable introspection + MCP interface | frontier `.2` (introspection schema spec, docs). `.1` done — design + decision record `0004` (MCP as a thin read-mostly adapter beside the deterministic core; schema derived from existing metrics/manifest/config; no stateful simulator-style session; security + determinism→content-addressed-cache guardrails). `.3`–`.7` (code: emission surface, read-only MCP server, controlled validate/minimize, prompts, closeout) pending owner acceptance of `.1`/`.2`. | [docs/tasks/AGENT-INTROSPECTION-MCP.md](tasks/AGENT-INTROSPECTION-MCP.md) |
