...
| `PHASE-8-FRONTEND-ACCEPT` | `active` | Phase 8 — Frontend/elaboration accept corpora | `PHASE-8-FRONTEND-ACCEPT.2c.1` (unblocked, code-bearing) — **`.2c` split (`2026-05-20`)** into `.2c.1` (build the hierarchy-aware parity harness — cargo-portable comparator + tool-gated `#[ignore]` real-tool scaffold; no real run, no ROADMAP advance) + `.2c.2` (real `--ignored` run + verify + bank + ROADMAP Phase 8 → done; gate-blocked, r87; may further split per Phase 7 `.2c.2` → `.2c.2a`/`.2c.2b` precedent on a discovered tool-capability dependency). Exactly mirrors the proven `PHASE-7-ORACLE-MICRODESIGN.2c` → `.2c.1`/`.2c.2` decomposition that closed Phase 7 on 2026-05-20. `.2c.1` is unblocked and is the next code-bearing slice | [docs/tasks/PHASE-8-FRONTEND-ACCEPT.md](tasks/PHASE-8-FRONTEND-ACCEPT.md) |
