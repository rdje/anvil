...
| `PHASE-9-MULTI-ARTIFACT-UMBRELLA` | `active` | Phase 9 — Multi-artifact ANVIL umbrella | `PHASE-9-MULTI-ARTIFACT-UMBRELLA.2c` (unblocked; expected to split per Phase 7/8 `.2c` precedent) — **`.2b` done (`2026-05-20`)**: `src/umbrella/mod.rs` extended with `MicrodesignLane{n_params}` + `FrontendLane{n_params, n_children}` + `impl ArtifactLane` for each (wrapping Phase 7's microdesign + Phase 8's frontend; both `check_plan=ParityVsManifest`). 4 new lib proofs (lib **244** ← 240+4): per-lane byte-identical regression proofs (L2 + L3) + cross-lane heterogeneous dispatch proof. Full `cargo test` green. `.2c` adds the `--artifact <lane>` top-level CLI flag (default `dut`) + book/CI byte-identical verification + ROADMAP Phase 9 → done | [docs/tasks/PHASE-9-MULTI-ARTIFACT-UMBRELLA.md](tasks/PHASE-9-MULTI-ARTIFACT-UMBRELLA.md) |
