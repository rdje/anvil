...
| `AGGREGATE-ARRAY-PACKING` | `active` | Phase 5b follow-on — synthesizable aggregates (packed array) | `.1` done (`AggregateKind::ArrayPacked` variant); frontier `.2` emitter → `.3` knob+selection → `.4` matrix proof → `.5` book/docs+close. Default-off byte-identical. | [docs/tasks/AGGREGATE-ARRAY-PACKING.md](tasks/AGGREGATE-ARRAY-PACKING.md) |
