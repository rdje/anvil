...
| `PHASE-8-FRONTEND-ACCEPT` | `active` | Phase 8 — Frontend/elaboration accept corpora | `PHASE-8-FRONTEND-ACCEPT.2a` (unblocked, code-bearing) — `.1` design done; **`.2` split (`2026-05-20`)** into `.2a` (source-level AST IR + construction-time elaboration-evaluator) + `.2b` (un-elaborated-SV + elaborated-facts manifest emitters) + `.2c` (hierarchy-aware parity harness + ROADMAP Phase 8) exactly mirroring the proven `PHASE-7-ORACLE-MICRODESIGN.2`→`.2a`/`.2b`/`.2c` decomposition that closed Phase 7 on 2026-05-20; unblocked now that Phase 7 closed (`src/microdesign/` in-tree; Phase 7's `ConstExpr` set ready to be cross-tree-imported as the Expr layer of Phase 8's source IR) | [docs/tasks/PHASE-8-FRONTEND-ACCEPT.md](tasks/PHASE-8-FRONTEND-ACCEPT.md) |
