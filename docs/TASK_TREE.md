...
| `PHASE-6-ADVANCED-MOTIFS` | `active` | Phase 6 — Advanced motifs | `.2.4` (gate-blocked: memory `.2.1`/`.2.2`/`.2.3` done — IR+knob+proofs+scenario, bin 219/876; verify when running gate completes) ‖ `.3.2b` (next: FSM `.3.2a` done — IR core + opaque `Node::FsmOut` through every exhaustive match + load-bearing `compact.rs` reachability + emitter + validator 5c + 3 unit proofs, default-off byte-identical; `.3.2b` = `fsm_prob` knob + rules-first `build_fsm_block`) | [docs/tasks/PHASE-6-ADVANCED-MOTIFS.md](tasks/PHASE-6-ADVANCED-MOTIFS.md) |
