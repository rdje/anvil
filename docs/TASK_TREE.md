...
| `SIGNOFF-AUTOMATION-EXPANSION` | `proposed` | Quality — downstream signoff automation breadth | `.1` design (`pending`); next active lane now that `AGENT-MCP-EXPANSION` is closed — promote to `active` and start `.1` | [docs/tasks/SIGNOFF-AUTOMATION-EXPANSION.md](tasks/SIGNOFF-AUTOMATION-EXPANSION.md) |
