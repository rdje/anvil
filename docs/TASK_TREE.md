...
| `CONE-DECOMPOSITION` | `active` | Code quality / maintainability — generator core readability | `.1`–`.5` done (`cone/snapshot.rs`, `cone/semantic.rs` ~1360 lines, `cone/primitives.rs`, `cone/terminals.rs` ~537 lines); frontier `.6` (extract `cone/flops.rs`), then `.7` motifs+closeout. cone.rs 5551→3511. Pure structural refactor; every leaf byte-identical (lib + snapshots; full suite at milestones). | [docs/tasks/CONE-DECOMPOSITION.md](tasks/CONE-DECOMPOSITION.md) |
