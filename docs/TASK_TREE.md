...
| `PHASE-7-ORACLE-MICRODESIGN` | `active` | Phase 7 — Oracle-backed micro-design artifacts | `PHASE-7-ORACLE-MICRODESIGN.2a` (`.1` design done; `.2` split → `.2a` const-expr/parameter IR + evaluator/oracle / `.2b` SV+manifest emitters / `.2c` parity harness+gate→ROADMAP) | [docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md](tasks/PHASE-7-ORACLE-MICRODESIGN.md) |
