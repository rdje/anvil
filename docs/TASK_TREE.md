...
| `PHASE-6-ADVANCED-MOTIFS` | `active` | Phase 6 — Advanced motifs | `.2.4` (gate-blocked: memory `.2.1`/`.2.2`/`.2.3` done — IR+knob+proofs+scenario, bin 219/876; verify when running gate completes) ‖ `.3.3` (next: FSM `.3.2` container done — `.3.2a` IR core + `.3.2b` `fsm_prob` knob + rules-first `build_fsm_block`, default-off byte-identical, forced-on 1-`Fsm` leaf all 3 encodings reachable; `.3.3` = cargo-portable structural + CSE/EGraph-opacity proof) | [docs/tasks/PHASE-6-ADVANCED-MOTIFS.md](tasks/PHASE-6-ADVANCED-MOTIFS.md) |
