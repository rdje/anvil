...
| `CONE-DECOMPOSITION` | `active` | Code quality / maintainability — generator core readability | `.1` (design) + `.2` (`cone/snapshot.rs`) + `.3` (`cone/semantic.rs`, ~1360 lines) + `.4` (`cone/primitives.rs`) done; frontier `.5` (extract `cone/terminals.rs`), then `.6` flops / `.7` motifs+closeout. cone.rs 5551→4048. Pure structural refactor; every leaf byte-identical (lib + snapshots; full suite at milestones). | [docs/tasks/CONE-DECOMPOSITION.md](tasks/CONE-DECOMPOSITION.md) |
