...
| `WORKLOAD-MEMORY-SAFETY` | `active` | Quality / signoff — resource-safe generation (ANVIL's own runtime) | `.1` (design) + `.2` (stream directory-output manifest) + `.3` (real per-module node budget — `max_nodes_per_module` ghost knob wired up, sentinel-`0`-unlimited default) done; frontier `.4` (internal RAM/RSS self-governor), `.5` closeout — but a separate `CONE-DECOMPOSITION` tree is prioritized ahead of `.4` per owner request (2026-06-14). All mechanisms default-off / byte-identical. | [docs/tasks/WORKLOAD-MEMORY-SAFETY.md](tasks/WORKLOAD-MEMORY-SAFETY.md) |
| `CONE-DECOMPOSITION` | `active` | Code quality / maintainability — generator core readability | `.1`–`.6` done (`cone/snapshot.rs`, `cone/semantic.rs` ~1360 lines, `cone/primitives.rs`, `cone/terminals.rs` ~537 lines, `cone/flops.rs` ~270 lines); frontier `.7` (extract `cone/motifs.rs` + closeout: CODEBASE_ANALYSIS map, full suite). cone.rs 5551→3260. Pure structural refactor; every leaf byte-identical (lib + snapshots; full suite at milestones). | [docs/tasks/CONE-DECOMPOSITION.md](tasks/CONE-DECOMPOSITION.md) |
