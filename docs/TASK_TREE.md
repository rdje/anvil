...
| `IDENTITY-DEEPENING` | `active` | NodeId as identity / full-factorization deepening | `.1` **done** (decision `0007` — first extension = bounded bisimulation-based sequential flop equivalence). `.2` split: `.2a` **done** (grounded the `merge_bisimilar_flops` algorithm / shared `finalize_flop_merge` refactor / quotient D-signature / bucket cap / gate in real `compact.rs` code). Frontier = `.2b` (impl, `pending`); `.3` (module-level sequential equivalence) `proposed`/future | [docs/tasks/IDENTITY-DEEPENING.md](tasks/IDENTITY-DEEPENING.md) |
