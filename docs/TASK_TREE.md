...
| `PHASE-8-FRONTEND-ACCEPT` | `active` | Phase 8 — Frontend/elaboration accept corpora | `PHASE-8-FRONTEND-ACCEPT.2c.2a` (unblocked, code-bearing) — `.2c.1` done; **`.2c.2` split (`2026-05-20`)** on a discovered tool-capability dependency: empirical probe of `yosys hierarchy -top acc_0; write_json` confirmed yosys exposes 5 of 7 manifest fact categories (top params + **instances + per-instance per-binding values via `.cells[<inst>].{type, parameters}`** + generate-branch; localparams + package-constants folded) AND that `proc; opt` collapses empty-bodied child instances (so the yosys invocation pattern omits `proc; opt`). Mirrors `PHASE-7-ORACLE-MICRODESIGN.2c.2` → `.2c.2a`/`.2c.2b` precedent. `.2c.2a` (unblocked) lands the yosys-specific extractor + end-to-end-runnable `#[ignore]` + proof; `.2c.2b` (gate-blocked) runs the real `--ignored` gate + verifies + banks + ROADMAP Phase 8 → done (r87) | [docs/tasks/PHASE-8-FRONTEND-ACCEPT.md](tasks/PHASE-8-FRONTEND-ACCEPT.md) |
