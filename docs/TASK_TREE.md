...
| `BUG-HUNT-ORCHESTRATION` | `active` | Usability — turnkey bug-finder (north star, idea 1) | **Registered `2026-06-17`** (owner-directed). A turnkey, MCP-driven fuzz → detect (reject/warning/mismatch) → auto-minimize → self-contained reproducer-bundle loop (CLI `anvil hunt` + an `hunt` MCP tool) composing the existing `tool_matrix` / `src/downstream` validate+minimize / `--diff-sim` / introspect surfaces — making ANVIL directly usable as a bug-finder. Binds decision `0017` (fully MCP-driveable + queryable). Frontier `.1` (design ADR). No code yet. | [docs/tasks/BUG-HUNT-ORCHESTRATION.md](tasks/BUG-HUNT-ORCHESTRATION.md) |
