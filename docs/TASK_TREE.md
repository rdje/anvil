...
| `PHASE-8-FRONTEND-ACCEPT` | `active` | Phase 8 — Frontend/elaboration accept corpora | `PHASE-8-FRONTEND-ACCEPT.2c.2` (gate-blocked, real-tool run) — **`.2c.1` done (`2026-05-20`)**: `src/frontend/` extended with the Phase-8-specific parity comparator core (`ToolReport`/`InstanceToolReport`/`Divergence` with 23 variants incl. hierarchy-aware `Instance*` additions/`FactCategory`/`ParityScope`/`compare_manifest_to_tool_report` + `_in_scope`/`synthetic_tool_report_from_manifest`); new `tests/frontend_parity.rs` with 10 cargo-portable comparator proofs + 1 tool-gated `#[ignore]` `parity_against_real_downstream_elaborator` scaffold (any-of-yosys/slang/verilator presence guard); full `cargo test` green (frontend_parity 10+1; lib 236 unchanged; everything else unchanged). `.2c.2` runs the real `--ignored` gate (tool choice TBD; may further split per Phase 7 `.2c.2` → `.2c.2a`/`.2c.2b` precedent) + verifies exact-agreement + banks artifact + promotes ROADMAP Phase 8 → done (r87) + reconciles book/README/CODEBASE | [docs/tasks/PHASE-8-FRONTEND-ACCEPT.md](tasks/PHASE-8-FRONTEND-ACCEPT.md) |
