...
| `WORKLOAD-MEMORY-SAFETY` | `active` | Quality / signoff — resource-safe generation (ANVIL's own runtime) | `.1` (design) + `.2` (stream directory-output manifest) + `.3` (real per-module node budget) + `.4` (opt-in internal RAM/RSS self-governor — `src/mem_guard.rs`, `--max-rss-mb`/`--ram-abort-pct`, between-unit `--out` checkpoint, clean exit 99) done; frontier `.5` (closeout: deferred boundaries + book safe-envelope narrative + roadmap status + close). All mechanisms default-off / byte-identical. | [docs/tasks/WORKLOAD-MEMORY-SAFETY.md](tasks/WORKLOAD-MEMORY-SAFETY.md) |
