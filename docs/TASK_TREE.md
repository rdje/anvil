...
| `AGENT-INTROSPECTION-MCP` | `active` | Capability — agent-drivable introspection + MCP interface | frontier `.4` (read-only in-process MCP server). Owner **accepted** the `.1`/`.2` design (`2026-06-14`), unblocking the code leaves. `.1` done — design + decision record `0004`. `.2` done — `docs/AGENT_INTROSPECTION_SCHEMA.md` (versioned introspection envelope, `schema_version = "1.0"`, payload sections = exact serde projections of `Config`/`Metrics`/`DesignMetrics`/`CoverageSummary`/lane manifests; zero new computed truth). `.3` done — `src/introspect/` emission surface + default-off `--introspect` CLI flag (DUT byte-identical, snapshots 6/6). `.4`–`.7` (MCP server, controlled validate/minimize, prompts, book/closeout) proceed in order under PNT. | [docs/tasks/AGENT-INTROSPECTION-MCP.md](tasks/AGENT-INTROSPECTION-MCP.md) |
