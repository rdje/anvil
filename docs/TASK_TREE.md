...
| `CONE-DECOMPOSITION` | `active` | Code quality / maintainability — generator core readability | `.1` (design) + `.2` (extract `cone/snapshot.rs` — mechanic validated by full suite) done; frontier `.3` (extract `cone/semantic.rs`), then `.4` primitives / `.5` terminals / `.6` flops / `.7` motifs+closeout. Pure structural refactor; every leaf byte-identical (snapshots + full tests). | [docs/tasks/CONE-DECOMPOSITION.md](tasks/CONE-DECOMPOSITION.md) |
