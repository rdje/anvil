...
| `PHASE-6-ADVANCED-MOTIFS` | `active` | Phase 6 — Advanced motifs | `.3.4b` (the single remaining Phase 6 leaf: **`.2.4` done** — memory gate verified clean [219/876, `coverage_gaps=[]`, 876/0 Verilator+both-Yosys, `saw_inferrable_memory_design=true`], ROADMAP/book reconciled "memory delivered", `.2` container closed. `.3.4b` = run a fresh real gate with the post-`.3.4a` binary [expected 222/888], verify clean → records FSM delivered + **closes ROADMAP Phase 6 + this tree**) | [docs/tasks/PHASE-6-ADVANCED-MOTIFS.md](tasks/PHASE-6-ADVANCED-MOTIFS.md) |
| `PHASE-7-ORACLE-MICRODESIGN` | `active` | Phase 7 — Oracle-backed micro-design artifacts | `PHASE-7-ORACLE-MICRODESIGN.2b` (`.2a` done — `src/microdesign/` const-expr/parameter IR + construction-time evaluator/oracle, 4 unit proofs incl. oracle-no-drift, reproducible, full `cargo test` green; `.2b` = SV + JSON-manifest emitters from the evaluated IR / `.2c` parity harness+gate→ROADMAP) | [docs/tasks/PHASE-7-ORACLE-MICRODESIGN.md](tasks/PHASE-7-ORACLE-MICRODESIGN.md) |
