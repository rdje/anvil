...
| `AGENT-INTROSPECTION-MCP` | `active` | Capability — agent-drivable introspection + MCP interface | frontier `.5` (controlled `validate` + `minimize`). Owner **accepted** the `.1`/`.2` design (`2026-06-14`), unblocking the code leaves. `.1` done — decision `0004`. `.2` done — `docs/AGENT_INTROSPECTION_SCHEMA.md` (versioned introspection envelope; zero new computed truth). `.3` done — `src/introspect/` + default-off `--introspect` flag (DUT byte-identical). `.4` done — `src/mcp/` + `anvil-mcp` bin (stdio JSON-RPC; generate/introspect/dump_config + resources over a content-addressed cache; no external-tool exec). `.5`–`.7` (validate/minimize, prompts, book/USER_GUIDE closeout) proceed in order under PNT; user-facing docs deferred to `.7`. | [docs/tasks/AGENT-INTROSPECTION-MCP.md](tasks/AGENT-INTROSPECTION-MCP.md) |
