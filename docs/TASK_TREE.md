...
| `AGENT-MCP-EXPANSION` | `active` | Capability — agent/MCP interface breadth | `.1` done (decision `0005`); `.2` done (coverage_gaps MCP tool); `.3a` done (non-DUT introspection design: reuse manifest ResourceRef, serve as resource, no schema bump); **frontier → `.3b`** (umbrella lane dispatch), then `.4`; lane order `2 → 3 → 1` | [docs/tasks/AGENT-MCP-EXPANSION.md](tasks/AGENT-MCP-EXPANSION.md) |
