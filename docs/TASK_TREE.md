...
| `PHASE-9-MULTI-ARTIFACT-UMBRELLA` | `active` | Phase 9 — Multi-artifact ANVIL umbrella | `PHASE-9-MULTI-ARTIFACT-UMBRELLA.2a` (unblocked, code-bearing) — **UNBLOCKED 2026-05-20** by Phase 7 closure at `20a7b4a` + Phase 8 closure at `997b0a6` (all 3 delivered lanes now exist: DUT + microdesign + frontend). **`.2` split (`2026-05-20`)** per Phase 7/8 `.2` → `.2a`/`.2b`/`.2c` precedent into `.2a` (`ArtifactLane` trait + shared plumbing + L1 DUT wrap + byte-identical regression proof — load-bearing for `BOOK-EXAMPLES-RUNNABLE` + CI gates), `.2b` (L2 microdesign + L3 frontend lane impls under the trait + cross-lane byte-identical proof), `.2c` (`--artifact` top-level CLI flag + book/CI byte-identical verification + ROADMAP Phase 9 → done). `.2a` unblocked; `.2c` may further split per Phase 7/8 `.2c` → `.2c.1`/`.2c.2` precedent if a discovered regression surfaces during book/CI verification | [docs/tasks/PHASE-9-MULTI-ARTIFACT-UMBRELLA.md](tasks/PHASE-9-MULTI-ARTIFACT-UMBRELLA.md) |
