...
| `SIGNOFF-AUTOMATION-EXPANSION` | `active` | Quality — downstream signoff automation breadth | `.1` done (decision `0006`); `.2` split into `.2a` design + `.2b` impl; `.2a` done (fixed the four genuinely-unswept knobs — `mux_arm`/`operand` duplication, `aggregate_array_prob`, memory×fsm interplay — one focused scenario each, four `saw_*` facts, dedicated `--signoff-knob-sweep-gate`); **frontier → `.2b`** (implement + bank clean); new-tool-column + non-DUT-acceptance paths preserved as future leaves | [docs/tasks/SIGNOFF-AUTOMATION-EXPANSION.md](tasks/SIGNOFF-AUTOMATION-EXPANSION.md) |
