...
| `DIFFERENTIAL-SIMULATION` | `active` | Quality — signoff-level downstream consistency | `DIFFERENTIAL-SIMULATION.3b` (implement `--diff-sim` wiring + extract `src/diff_sim/mod.rs`) — **`.3a` design landed (`2026-05-24`)** docs-only: `DEVELOPMENT_NOTES.md` "Tool-matrix `--diff-sim` wiring + representative-subset selector + coverage fact design" records (1) module-extraction decision — extract `tests/diff_sim.rs` helpers into a new `src/diff_sim/mod.rs` library module (full-factorization doctrine), (2) `--diff-sim` opt-in CLI flag gated on Verilator+Yosys clean per scenario, (3) per-axis subset selector (combinational/sequential-flop/hierarchy/memory/fsm; capped K=5), (4) `saw_design_with_cross_simulator_agreement` coverage fact, (5) `DiffSimReport` with retained `mismatch_excerpt` per Phase-7 counterexample doctrine, (6) `tools_present()`-friendly no-op. 5 rejected alternatives. `.3b` proof shape specified. **`.3` split + Phase 7/8/9 design-first discipline applied.** `.2b.2` closed `.2b` + `.2` container (`2026-05-24`) — first gate to assert downstream-tool *semantic* agreement on ANVIL output passed first try after the targeted alignment fix (`project_anvil_north_star.md`). | [docs/tasks/DIFFERENTIAL-SIMULATION.md](tasks/DIFFERENTIAL-SIMULATION.md) |
