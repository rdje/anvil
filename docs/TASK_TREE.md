...
| `CONE-DECOMPOSITION` | `active` | Code quality / maintainability — generator core readability | `.1` (decomposition design — seam map + root-glob re-export strategy + extraction order) done; frontier `.2` (extract `cone/snapshot.rs`), then `.3` semantic / `.4` primitives / `.5` terminals / `.6` flops / `.7` motifs+closeout. Pure structural refactor; every leaf byte-identical (snapshots + full tests). | [docs/tasks/CONE-DECOMPOSITION.md](tasks/CONE-DECOMPOSITION.md) |
