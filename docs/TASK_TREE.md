...
| `AGENT-INTROSPECTION-MCP` | `active` | Capability — agent-drivable introspection + MCP interface | frontier `.7` (book/USER_GUIDE/README/CODEBASE_ANALYSIS closeout). Owner **accepted** the `.1`/`.2` design (`2026-06-14`), unblocking the code leaves. `.1` done — decision `0004`. `.2` done — `docs/AGENT_INTROSPECTION_SCHEMA.md` (versioned introspection envelope; zero new computed truth). `.3` done — `src/introspect/` + default-off `--introspect` flag (DUT byte-identical). `.4` done — `src/mcp/` + `anvil-mcp` bin (stdio JSON-RPC; generate/introspect/dump_config + resources over a content-addressed cache). `.5` split into `.5.1`/`.5.2`/`.5.3`, **all done → `.5` container closed**: `.5.1` extracted the hardened downstream-tool invocation surface into `src/downstream/` (behavior-preserving, snapshots 6/6); `.5.2` landed the controlled `validate` tool (`downstream::validate`: sandboxed temp dir + fixed `verilator`/`yosys`/`iverilog` allow-list + ram-guard decline-before-spawn) + MCP `validate` tool + `anvil://audit/log`, e2e clean vs real Verilator+Yosys; `.5.3` landed the controlled `minimize` delta-debugger (`downstream::minimize`: deterministic coordinate-descent over size bounds + optional-motif probs, seed fixed, budget-bounded, `validate` as oracle) + MCP `minimize` tool + audit log, e2e clean vs real Verilator 5.046 + Yosys 0.64 (`reproduced_initial=false` on valid-by-construction output). **`.6` done** — the five agent-workflow prompts (`find_downstream_bug`, `close_coverage_gap`, `minimize_reproducer`, `triage_tool_failures`, `explain_artifact`) shipped as first-class MCP prompts (`prompts/list`/`prompts/get`) in `src/mcp/`, each rendering its ordered tool chain with sample-arg substitution; all five chains proven runnable end-to-end (snapshots 6/6, lib 370/370). `.7` (book/USER_GUIDE/README/CODEBASE_ANALYSIS closeout) proceeds next under PNT; user-facing docs were deferred to `.7` by design. | [docs/tasks/AGENT-INTROSPECTION-MCP.md](tasks/AGENT-INTROSPECTION-MCP.md) |
