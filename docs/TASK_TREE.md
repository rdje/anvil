...
| `AGENT-INTROSPECTION-MCP` | `active` | Capability — agent-drivable introspection + MCP interface | design-complete, parked on owner acceptance. `.1` done — design + decision record `0004` (MCP as a thin read-mostly adapter beside the deterministic core; schema derived from existing metrics/manifest/config; no stateful simulator-style session; security + determinism→content-addressed-cache guardrails). `.2` done — `docs/AGENT_INTROSPECTION_SCHEMA.md` (versioned introspection envelope, `schema_version = "1.0"`, payload sections = exact serde projections of `Config`/`Metrics`/`DesignMetrics`/`CoverageSummary`/lane manifests; zero new computed truth; versioning policy). `.3`–`.7` (code: emission surface, read-only MCP server, controlled validate/minimize, prompts, closeout) pending owner acceptance of the `.1`/`.2` design. | [docs/tasks/AGENT-INTROSPECTION-MCP.md](tasks/AGENT-INTROSPECTION-MCP.md) |
