...
| `INSTA-SNAPSHOTS` | `active` | Quality — reproducibility regressions | `INSTA-SNAPSHOTS.3` (`.1`/`.2` done — `insta` pinned `=1.47.2`, `tests/snapshots.rs` 6 byte-stable shapes spanning leaf/library/on-demand/sibling/parent-composed/dedup; `.3` = `COMMIT.md` checklist + book acceptance-protocol, closes the tree) | [docs/tasks/INSTA-SNAPSHOTS.md](tasks/INSTA-SNAPSHOTS.md) |
