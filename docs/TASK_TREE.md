...
| `AGENT-MCP-EXPANSION` | `active` | Capability — agent/MCP interface breadth | `.1` done (decision `0005`); `.2` done (coverage_gaps MCP tool); `.3` done (`.3a`+`.3b` non-DUT lanes over MCP); `.4` split into `.4a` design + `.4b` impl; `.4a` done (hand-rolled HTTP/1.1 framing pinned); **frontier → `.4b`** (HTTP transport impl), then `.5` closeout; lane order `2 → 3 → 1` | [docs/tasks/AGENT-MCP-EXPANSION.md](tasks/AGENT-MCP-EXPANSION.md) |
