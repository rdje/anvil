...
| `PHASE-6-ADVANCED-MOTIFS` | `active` | Phase 6 — Advanced motifs | `.2.4` (gate-blocked: memory `.2.1`/`.2.2`/`.2.3` done — IR+knob+proofs+scenario, bin 219/876; verify when running gate completes) ‖ `.3.2a` (next: FSM `.3.1` design done; `.3.2` split → `.3.2a` IR-core+opaque-`FsmOut`+`compact.rs` reachability+emitter+validator+unit-proofs / `.3.2b` knob+rules-first) | [docs/tasks/PHASE-6-ADVANCED-MOTIFS.md](tasks/PHASE-6-ADVANCED-MOTIFS.md) |
