...
- Created: `2026-05-16`
- Last updated: `2026-05-20` (**`.2c.2b` real-tool gate clean — `PHASE-8-FRONTEND-ACCEPT` tree CLOSED; ROADMAP Phase 8 → done**; verified-clean banked artifact `anvil-frontend-parity-phase8-yosys-p1/` — `cargo test -- --ignored parity_against_real_yosys_hierarchy_write_json` against yosys 0.64 reports "parity gate clean across 5 seeds" with `test result: ok. 1 passed; 0 failed`; per-seed fact agreement verified across the corpus including both generate branches exercised — 4 seeds take `g_taken`, seed 12345 takes `g_else`; r87 no-aspirational-claims observed; explicit yosys-supported-categories scope caveat recorded — richer-AST coverage via slang/verilator-with-debug is the recorded post-Phase-8 follow-up that does NOT block closure since ANVIL's by-construction oracle already covers all 7 categories)
...
- ID: `PHASE-8-FRONTEND-ACCEPT`
  ...
  Children: `PHASE-8-FRONTEND-ACCEPT.1` (done), `PHASE-8-FRONTEND-ACCEPT.2` (done container: `.2a`, `.2b`, `.2c`)
...
- ID: `PHASE-8-FRONTEND-ACCEPT.2`
  ...
  Children: `PHASE-8-FRONTEND-ACCEPT.2a` (done), `PHASE-8-FRONTEND-ACCEPT.2b` (done), `PHASE-8-FRONTEND-ACCEPT.2c` (done — 2026-05-20)
...
- ID: `PHASE-8-FRONTEND-ACCEPT.2c`
  ...
  Children: `PHASE-8-FRONTEND-ACCEPT.2c.1` (done), `PHASE-8-FRONTEND-ACCEPT.2c.2` (done — 2026-05-20)
...
- ID: `PHASE-8-FRONTEND-ACCEPT.2c.2`
  ...
  Children: `PHASE-8-FRONTEND-ACCEPT.2c.2a` (done), `PHASE-8-FRONTEND-ACCEPT.2c.2b` (done — 2026-05-20)
...
- ID: `PHASE-8-FRONTEND-ACCEPT.2c.2b`
  ...
  Verification: `Real tool-equipped run of the .2c.2a #[ignore] parity gate against yosys 0.64 (no preceding fix needed — UNLIKE Phase 7 .2c.2b.1, this gate came back clean on first try; the lesson from Phase 7's width_expr non-negative-modulo-idiom fix carried forward via cross-tree reuse of crate::microdesign::expr_to_sv): cargo test --test frontend_parity -- --ignored parity_against_real_yosys_hierarchy_write_json --nocapture exited 0 with stdout "parity gate clean across 5 seeds; artifacts in target.cache/anvil-sandbox/frontend-parity-phase8-yosys" and "test result: ok. 1 passed; 0 failed; 0 ignored; 0 measured; 13 filtered out; finished in 0.04s". Per-seed fact agreement verified BEFORE promotion (r87): seed 0 — manifest {top_params: P0=57/P1=38/P2=36/P3=54; u_0_0 bindings CP0=57/CP1=40/CP2=38/CP3=57; u_0_1 bindings CP0=60/CP1=59/CP2=38/CP3=55; gen=true} matches yosys cells {u_0_0→child_0, u_0_1→child_0 with all 4 bindings each} + parameter_default_values + g_taken.gflag netname; seed 1 — {P0=59/P1=46/P2=46/P3=12; u_1_0/u_1_1 each with 4 bindings; gen=true} matches; seed 7 — {P0=39/P1=16/P2=6/P3=17; u_7_0/u_7_1 each with 4 bindings; gen=true} matches; seed 42 — {P0=48/P1=19/P2=42/P3=21; u_42_0/u_42_1 each with 4 bindings; gen=true} matches; seed 12345 — {P0=15/P1=22/P2=43/P3=12; u_12345_0/u_12345_1 each with 4 bindings; gen=false} matches with yosys netname g_else.gflag — the else-branch case is also covered. **Both generate branches** are exercised by the corpus (4 seeds take g_taken; seed 12345 takes g_else). **The hierarchy-aware Phase-8 axis is exercised** (every seed has 2 instances × 4 per-instance per-binding values verified against yosys's .cells[<inst>].parameters). Banked artifact copied to anvil-frontend-parity-phase8-yosys-p1/ per established convention (15 files — 5 × {acc_<seed>.sv, acc_<seed>.json, acc_<seed>.yosys.json}). Promotion strictly followed the verified artifact: ROADMAP Phase 8 (not started)→(done) with the verified-clean artifact citation + the explicit "yosys-supported categories" scope caveat (Seed/Top/TopParams/Instances/GenerateBranches; top_localparams + package_constants are folded by yosys's hierarchy elaboration and remain visible only to richer-AST tools — slang/verilator-with-debug — recorded as a post-Phase-8 follow-up that does NOT retract closure since ANVIL's by-construction oracle already covers all 7 categories); book/src/ir.md gains a Phase 8 frontend-accept lane delivered note citing the artifact; README phase narrative Phase 8 → done; CODEBASE_ANALYSIS phase-coverage-map Phase-8 row → done. No src/ or tests/ change in this slice (the code landed in .2a/.2b/.2c.1/.2c.2a); mdbook build book clean; cargo unchanged-green. **Closes the .2c.2b container; closes the .2c.2 container; closes the .2c container; closes the .2 container; closes the PHASE-8-FRONTEND-ACCEPT tree; closes ROADMAP Phase 8.** Notable: Phase 8's gate came back clean on the FIRST try (contrast with Phase 7's .2c.2b.1 width_expr fix), validating that cross-tree reuse of Phase 7's expr_to_sv carried forward the non-negative-modulo-idiom fix for free at the expression layer.`
  Commit: `Phase 8: PHASE-8-FRONTEND-ACCEPT.2c.2b parity gate clean against yosys (first try) — closes Phase 8 + tree`

## Current Frontier

| Order | Leaf | Status | Why next |
| --- | --- | --- | --- |
| — | (closed) | (tree done) | **`PHASE-8-FRONTEND-ACCEPT` tree CLOSED (2026-05-20).** `.2c.2b`'s real-tool parity gate against yosys 0.64 came back clean across all 5 reproducibility-set seeds on first try (`test result: ok. 1 passed; 0 failed`); verified-clean artifact banked at `anvil-frontend-parity-phase8-yosys-p1/` BEFORE promotion (r87 no-aspirational-claims); per-seed fact agreement verified incl. both generate branches exercised (seed 12345 `g_else`, others `g_taken`) AND the load-bearing hierarchy-aware Phase-8 axis (every seed has 2 instances × 4 per-instance per-binding values matched). **ROADMAP Phase 8 closed** with the explicit yosys-supported-categories scope caveat — richer-AST coverage via slang or verilator-with-debug remains a recorded post-Phase-8 follow-up that does NOT block closure. Notable: the gate came back clean on FIRST try (contrast with Phase 7's `.2c.2b.1` width_expr fix), validating that cross-tree reuse of Phase 7's `expr_to_sv` carried forward the non-negative-modulo-idiom fix for free at the expression layer. Continuous-PNT continues on the still-open trees: `PHASE-9-MULTI-ARTIFACT-UMBRELLA` (now unblocked with two delivered lanes — microdesign + frontend) + `DIFFERENTIAL-SIMULATION`. |
...
| `PHASE-8-FRONTEND-ACCEPT.2c.2b` | `Phase 8: PHASE-8-FRONTEND-ACCEPT.2c.2b parity gate clean against yosys (first try) — closes Phase 8 + tree` | Real-tool `--ignored` parity gate against yosys 0.64 came back clean on FIRST TRY across all 5 seeds; verified-clean artifact banked at `anvil-frontend-parity-phase8-yosys-p1/`; per-seed fact agreement verified incl. both generate branches + the load-bearing hierarchy-aware axis (instances + per-instance bindings). **Closes the `.2c.2b` + `.2c.2` + `.2c` + `.2` containers + `PHASE-8-FRONTEND-ACCEPT` tree; closes ROADMAP Phase 8** with the explicit yosys-supported-categories scope caveat. No code change in this slice (docs/ROADMAP/book only). |
...
| `2026-05-20` | `PHASE-8-FRONTEND-ACCEPT.2c.2b` | Real tool-equipped run of the `.2c.2a` `#[ignore]` parity gate against yosys 0.64 (no preceding fix needed — UNLIKE Phase 7 `.2c.2b.1`, came back clean on first try): `cargo test --test frontend_parity -- --ignored parity_against_real_yosys_hierarchy_write_json --nocapture` exited 0 with stdout `"parity gate clean across 5 seeds"` and `"test result: ok. 1 passed; 0 failed; 0 ignored; 0 measured; 13 filtered out; finished in 0.04s"`. Per-seed fact agreement verified BEFORE promotion (r87): seed 0 — manifest `{top_params: P0=57/P1=38/P2=36/P3=54; u_0_0 bindings CP0=57/CP1=40/CP2=38/CP3=57; u_0_1 bindings CP0=60/CP1=59/CP2=38/CP3=55; gen=true}` matches yosys `.cells {u_0_0→child_0, u_0_1→child_0 with all 4 bindings each}` + `.parameter_default_values` + `g_taken.gflag` netname; seed 1 — `{P0=59, P1=46, P2=46, P3=12; u_1_0/u_1_1 each with 4 bindings; gen=true}` matches; seed 7 — `{P0=39, P1=16, P2=6, P3=17; 4 bindings each; gen=true}` matches; seed 42 — `{P0=48, P1=19, P2=42, P3=21; 4 bindings each; gen=true}` matches; seed 12345 — `{P0=15, P1=22, P2=43, P3=12; 4 bindings each; **gen=false**}` matches with yosys netname `g_else.gflag` — the else-branch case is also covered. **Both generate branches** exercised (4 seeds take `g_taken`, seed 12345 takes `g_else`). **The load-bearing hierarchy-aware Phase-8 axis** is exercised (every seed has 2 instances × 4 per-instance per-binding values verified against yosys's `.cells[<inst>].parameters`). Banked artifact copied to `anvil-frontend-parity-phase8-yosys-p1/` (15 files: 5 × `{.sv, .json, .yosys.json}`). Promotion strictly followed the artifact: ROADMAP Phase 8 `(not started)`→`(done)` + verified-clean citation + explicit **yosys-supported-categories scope caveat** (top_localparams + package_constants folded by yosys's hierarchy elaboration; richer-AST coverage via slang/verilator-with-debug is recorded post-Phase-8 follow-up that does NOT retract closure since ANVIL's by-construction oracle already covers all 7 categories); `book/src/ir.md` gains the Phase 8 frontend-accept lane delivered note; README + CODEBASE phase-coverage-map Phase-8 row done. **Closes `.2c.2b` + `.2c.2` + `.2c` + `.2` containers + `PHASE-8-FRONTEND-ACCEPT` tree + ROADMAP Phase 8.** **Notable**: Phase 8's gate came back clean on the FIRST try (contrast with Phase 7's `.2c.2b.1` width_expr fix), validating that cross-tree reuse of Phase 7's `expr_to_sv` carried forward `.2c.2b.1`'s non-negative-modulo-idiom fix for free at the expression layer. No `src/`/`tests/` change in this slice (code landed in `.2a`/`.2b`/`.2c.1`/`.2c.2a`); `mdbook build book` clean; `cargo` unchanged-green. | Done — Phase 8 closed; tree closed. |
...
- `2026-05-20`: **`.2c.2b` landed — `PHASE-8-FRONTEND-ACCEPT`
  tree CLOSED; ROADMAP Phase 8 → done.** Real tool-equipped
  run of the `.2c.2a` `#[ignore]` parity gate against yosys
  0.64 (no preceding fix needed — UNLIKE Phase 7
  `.2c.2b.1`, came back clean on first try): `cargo test
  --test frontend_parity -- --ignored
  parity_against_real_yosys_hierarchy_write_json
  --nocapture` exited 0 with stdout `"parity gate clean
  across 5 seeds"` and `"test result: ok. 1 passed; 0
  failed"`. Per-seed fact agreement verified BEFORE any
  promotion (r87 no-aspirational-claims): seed 0 — manifest
  `{P0=57, P1=38, P2=36, P3=54; u_0_0 with 4 bindings;
  u_0_1 with 4 bindings; gen=true}` matches yosys `.cells`
  + `.parameter_default_values` + `g_taken.gflag` netname;
  seeds 1/7/42 similarly clean with `gen=true`; **seed
  12345** — `{P0=15, …; gen=false}` matches with yosys
  netname `g_else.gflag` — the else-branch case is
  exercised. **Both generate branches** in the corpus (4
  seeds take `g_taken`, seed 12345 takes `g_else`). **The
  load-bearing hierarchy-aware Phase-8 axis** is exercised
  (every seed has 2 instances × 4 per-instance per-binding
  values verified against yosys's `.cells[<inst>].parameters`).
  Banked artifact copied to
  `anvil-frontend-parity-phase8-yosys-p1/` per
  established convention (15 files: 5 × `{acc_<seed>.sv,
  acc_<seed>.json, acc_<seed>.yosys.json}`). Promotion
  strictly followed the artifact: ROADMAP Phase 8 `(not
  started)`→`(done)` + verified-clean citation + explicit
  **yosys-supported-categories scope caveat**
  (Seed/Top/TopParams/Instances/GenerateBranches;
  top_localparams + package_constants folded — richer-AST
  coverage via slang/verilator-with-debug is the recorded
  post-Phase-8 follow-up that does NOT retract closure
  since ANVIL's by-construction oracle already covers all 7
  categories); `book/src/ir.md` gains the Phase 8
  frontend-accept lane delivered note; README phase
  narrative Phase 8 done; `CODEBASE_ANALYSIS.md`
  phase-coverage-map Phase-8 row done. **Notable**: the
  gate came back clean on the FIRST try (contrast with
  Phase 7's `.2c.2b.1` `width_expr` non-negative-modulo
  fix). Cross-tree reuse of Phase 7's `expr_to_sv`
  carried forward Phase 7's hard-won fix at the expression
  layer for free — the kind of factorization payoff the
  user's `full-factorization` doctrine targets. No
  `src/`/`tests/` change in this slice (code landed in
  `.2a`/`.2b`/`.2c.1`/`.2c.2a`); `mdbook build book`
  clean; `cargo` unchanged-green. **Closes the `.2c.2b`
  container; closes the `.2c.2` container; closes the
  `.2c` container; closes the `.2` container; closes the
  `PHASE-8-FRONTEND-ACCEPT` tree; closes ROADMAP Phase
  8.** ANVIL now ships THREE complementary lanes (DUT,
  microdesign, frontend) with cross-tree-shared
  ConstExpr/eval/expr_to_sv at the expression layer.
  Continuous-PNT continues on still-open trees:
  `PHASE-9-MULTI-ARTIFACT-UMBRELLA` (now unblocked with
  two delivered post-DUT lanes — microdesign + frontend)
