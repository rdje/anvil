...
- Created: `2026-07-31`
- Last updated: `2026-07-31` (`.2` **done** — decision [`0040`](../decisions/0040-overflow-destination-classification-and-the-unmeasured-axis.md): **three** classes not two, and the pressure went to the **unmeasured AXIS** of `MEMORY.md`; `.5a` **done** — the owner authorised relocation `2026-07-31`; decision [`0041`](../decisions/0041-owner-standing-directives-recorded-in-layer-c.md) records the three directives that had **no layer-C home**, and points at `0031` for the four that already did; frontier **`.5b`**)
...
- ID: `OVERFLOW-DESTINATION-INSTRUMENTATION.5b`
  ...
  Acceptance: `Gotchas go to docs/knowledge/ fact cards, which KNOWLEDGE_MAP_ARCHITECTURE.md §4 names EXPLICITLY as card material ("a root cause, an architectural constraint, a GOTCHA, where-something-lives"), question-keyed on what an agent would actually search for so the lesson is retrieved rather than re-derived — one atomic fact per card (§3.3), each POINTING at the decision record that holds the full story where one exists, never copying it. Probe each lesson against the durable layers FIRST and open every hit individually rather than trusting a coarse key — .5a's own probe reported "has a home" for entries whose only hits cited MEMORY.md back. Nothing is lost, proven by the same whole-range token sweep (0036 §(iii)) with the residue reported. State the resulting byte count against the 6,144 cap; if it still does not fit, report the gap rather than raising the cap (0040 non-license 3) — and note that ## Current state is itself 5,028 B of layer-A prose that should POINT at the decision records rather than summarise them, which is the same defect one level in. Scope note to record: 0040 §(d) scoped .5 to the two named sections, but removing only those leaves 6,698 B, still over the cap — so the widening to Lane invariants / Validation policy / Current state is REQUIRED BY THE LEAF'S OWN GOAL and must be stated as a measured widening, the DATED-COUNT-SWEEP-EXEMPTION.2 precedent.`
...
| 4 | `OVERFLOW-DESTINATION-INSTRUMENTATION.5b` | `pending` | **Next.** The remainder: `## Operating gotchas` (**10,491 B in 14 lines = 749 B/line**) → Knowledge Map cards, which `KNOWLEDGE_MAP_ARCHITECTURE.md` §4 names explicitly as card material; then `## Lane invariants`, `## Validation policy`, and the tightening of `## Current state` (itself **5,028 B** that should *point* rather than summarise). **12,013 B still over the 6,144 cap** — reported, not met by raising it. |
| 5 | `OVERFLOW-DESTINATION-INSTRUMENTATION.3` | `pending` | Apply the derived **6,144-byte** cap as a second assertion inside `check_memory_architecture.sh` (not a new doctrine — `MEMORY-ARCH` already owns that file's size). Control proven capable of failing before it is trusted. |
...
| --- | --- | --- |
| `OVERFLOW-DESTINATION-INSTRUMENTATION.5a` | `pending` — `OVERFLOW-DESTINATION-INSTRUMENTATION.5a — the directive against parking things in MEMORY.md was parked in MEMORY.md` | Decision `0041` records 3 directives verbatim; 4 more repaired to pointers at `0033` R1. Owner-authorised. |
