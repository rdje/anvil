...
- ID: `OVERFLOW-DESTINATION-INSTRUMENTATION`
  ...
  Children: `.1` (audit + register), `.2` (classify + decide the instrument), `.5a` (the owner's standing directives -> decision 0041), `.5b` (the operating gotchas + the remaining layer-C sections -> Knowledge Map cards) — both ordered BEFORE `.3`, which cannot land while the file is over the derived cap, `.3` (apply the instrument), `.6` (own 0040 §(g)'s unrepaired long-line finding), `.7` (repair the malformed table rows .6 found, and gate the class), `.8` (classify docs/TASK_TREE.md — the destination 0040 never classified and neither enumeration routes to), `.4` (feed the correction back into the portable policy)
...
- ID: `OVERFLOW-DESTINATION-INSTRUMENTATION.6`
  ...
  Verification: `done — MEASURED TREE-WIDE, AND THE ANSWER TO THE LEAF'S OWN QUESTION IS "NO, REFLOW IS NOT THE REPAIR" — for a reason stronger than cost/benefit: FOR THE LINES THAT MOTIVATED THIS LEAF, REFLOW IS STRUCTURALLY UNAVAILABLE. The sweep that acceptance mandated then found a DIFFERENT AND WORSE DEFECT, which is why the leaf closes with two new ones registered rather than with a shrug. (1) THE PREMISE WAS WRONG ABOUT WHICH LINE IS WORST. 0040 §(g) and this leaf's goal name CODEBASE_ANALYSIS.md's 24,990-byte line as the offender. Over all 243 tracked *.md it is THIRD: docs/TASK_TREE.md carries 39,679 and docs/tasks/SEMANTIC-INTROSPECTION-EXPANSION.md 33,890. Longest-line distribution (denominator 243): >=20,000 = 3; 10,000-19,999 = 0; 5,000-9,999 = 7; 2,000-4,999 = 21; 1,000-1,999 = 33; 500-999 = 57; <500 = 122. (2) THE CLASS, BY SHAPE. Every line >=1,000 chars tree-wide: 626 total = 315 PROSE + 257 TABLE-ROW + 54 LIST-ITEM. Split by 0040 class: 531 sit in append-only records (CHANGES.md, DEVELOPMENT_NOTES.md, docs/tasks/, docs/decisions/, docs/evidence/) and are OUT OF SCOPE by the tree Non-Goals + 0031; IN SCOPE are 95 lines over 15 files = 54 TABLE-ROW + 33 LIST-ITEM + 8 PROSE. (3) WHY REFLOW IS UNAVAILABLE, NOT MERELY UNATTRACTIVE. A Markdown table row CANNOT carry a newline — a newline terminates the row — so the acceptance-sanctioned repair (REFLOW ONLY, content byte-identical, diff provably whitespace-only) cannot be applied to 54 of the 95 in-scope long lines (56.8 %), INCLUDING BOTH TOP OFFENDERS, which are table rows. Of the 33 in-scope LIST-ITEMs, 26 are in KNOWLEDGE_MAP.md, whose own header reads "AUTO-GENERATED — DO NOT EDIT": a hand reflow is reverted by the next gen_knowledge_map.sh run, and changing the generator is a different change needing its own leaf. The genuinely reflowable residue is therefore 15 lines — 8 PROSE + 7 non-KM LIST-ITEM — the largest being book/src/architecture.md at 6,327 and ROADMAP.md at 3,653, the rest ~1.0-1.5 k, i.e. nothing near the headline number. THAT IS THE MEASURED "NO" the acceptance licensed. (4) IS LENGTH A DEFECT AT ALL, AND FOR WHOM — MEASURED PER COST, NOT ASSERTED. Rendering: NO. A long but WELL-FORMED row renders correctly; verified with the project's own renderer. Git: YES, and quantified from real history rather than simulated — the last five commits touching docs/TASK_TREE.md are each +1/-1 LINES and produce diff payloads of 37,132 / 42,134 / 37,199 / 30,116 / 37,092 BYTES; a one-row status update costs ~37 KB of diff. Agent context: YES, and in the same currency 0040 protects — a single grep -n hit on docs/TASK_TREE.md:131 injects 39,679 bytes (~9,900 tokens) into a reader, CODEBASE_ANALYSIS.md:2270 injects 24,990 (~6,250). Regex backtracking: SHAPE-DEPENDENT, not inherent — the sibling timeout this leaf cites came from a bounded-context regex; every linear scan run here handled all 243 files in seconds. (5) THE REAL DEFECT, WHICH IS NOT LENGTH AND IS WORSE. Sweeping all 243 files for GFM table well-formedness — 394 tables, 2,310 data rows (fence-aware; a fence-blind scan reports 395/2,308 and the SAME 36 malformed, so the finding is stable under both) — found 36 MALFORMED ROWS whose cell count EXCEEDS their header. GFM splits a row on unescaped | BEFORE inline parsing, so a pipe inside a code span still splits, and the spec then IGNORES the excess cells: that content is in the source and ABSENT FROM EVERY RENDERED PAGE. Tree-wide 57,283 characters are dropped, including 5 cells that were a row's LINK TO ITS OWN DETAIL FILE. And the convergence that settles this leaf: CODEBASE_ANALYSIS.md:2270 — the exact line 0040 §(g) flagged and this leaf was opened to own — is MALFORMED, 5 cells against a 4-column header, DROPPING 24,229 OF ITS 24,990 BYTES = 97.0 %. 0040 called it "a formatting defect, not a size defect" and was right, but about the wrong axis: it is not ugly, it is INVISIBLE. In-scope files affected: docs/TASK_TREE.md 5 rows, CODEBASE_ANALYSIS.md 1, USER_GUIDE.md 1; the other 29 are in docs/tasks/ (append-only, so their disposition is .7's call, not this leaf's). (6) VERIFIED EMPIRICALLY WITH A NEGATIVE CONTROL, NOT ASSERTED FROM THE SPEC. A 3-column probe table was built and rendered with mdbook (pulldown-cmark, the same engine that builds book/). BROKEN FORM: row "| b1 | see `x | y` here | [LINKB](y.md) |" rendered 3 cells — b1 / "see `x" / "y` here" — with [LINKB] ABSENT FROM THE HTML. NEGATIVE CONTROL: the same row with the pipe written \| rendered 3 cells — c1 / "see x | y here" / LINKC — the link PRESENT and the pipe literal. So the fix is one backslash, and the repo already uses it correctly elsewhere in the very same table. (7) THE FIRST DETECTOR WAS WRONG AND IS RECORDED AS SUCH. A naive gsub(/\|/) count flagged 8 malformed rows in docs/TASK_TREE.md; the escape-aware count flags 5. Rows 103, 130 and 136 were FALSE POSITIVES that use the correct \| idiom — the naive key cannot tell the escaped form from the bare one, which is this repo's standing lesson that an extractor unable to distinguish its cases reports something plausible instead of dying. Recorded rather than quietly corrected, per 0039 rule (b). Two further precision notes in the same spirit: the initial scoped-row audit ran past the table into a later 2-column table and was re-bounded; and the headline line is 24,990 BYTES but 24,970 CHARACTERS (20 multi-byte), stated so the two numbers cannot later read as a discrepancy. (8) A SECOND INSTANCE OF 0040 §(g), FOUND BY THE SAME SWEEP. docs/TASK_TREE.md is classified by NEITHER — 0040 §(a) classifies docs/tasks/ (a different path; the index is not under it), and neither routing enumeration names it (both say docs/tasks/). It is now the worst-profile non-append-only file in the repo: 243,983 B over 376 lines = 648 B/line, against the 406 B/line that made MEMORY.md the tree's central finding. 232,019 B (95.1 %) is one table, and the "Current frontier" COLUMN ALONE is 192,806 chars = 79.0 % of the file, mean 2,605 per status cell, max 39,095. Whether that is the tree's own MIXED-SURFACE category is NOT settled here and is deliberately not claimed: the distinct-date probe fires on 8 of 74 rows, and 0039 rule (a) holds that past-tense closure facts inside a present-tense document are CORRECT — settling it requires reading each cell, exactly as .2 read all 15 ROADMAP dates. That is .8. (9) TWO LEAVES REGISTERED, NOT PARKED IN PROSE. .7 owns the malformed rows + a mechanical check; .8 owns the unclassified index. This is the standing directive applied to this leaf's own output — 0040 §(g) recorded a finding "for a future leaf" and it took a dedicated leaf to notice, which is the failure mode 0039 exists for. NOTHING WAS REPAIRED HERE, deliberately: the repair is .7's, and widening a measurement leaf into a sweep is how the /tmp sweep damaged 0030. Knowledge Map card gfm-table-unescaped-pipe-drops-content written (KM 106 -> 107 facts / 1,022 -> 1,029 keys, check_knowledge_map OK) — and it was itself render-probed, because it contains the construct it documents. Checks: check_knowledge_map OK; check_doctrines.sh green after git add. Docs-only => DUT byte-identical.`
  Commit: `pending`

- ID: `OVERFLOW-DESTINATION-INSTRUMENTATION.7`
  Status: `done`
  Goal: `Repair the 36 malformed GFM table rows .6 measured — rows whose cell count exceeds their header, so every cell past the header count is silently dropped from every rendered page (57,283 characters tree-wide, including 5 cells that were a row's own link to its detail file) — and gate the class so it cannot recur.`
  Acceptance: `The repair is ONE BACKSLASH PER OFFENDING PIPE (\| inside the cell) plus the removal of row 131's duplicated File link — a rendering-fidelity repair, NOT a content sweep: no claim, number, date or word changes, and the tree's Non-Goals still forbid trimming anything. Prove it: for every repaired row, the cell text after removing the escapes must be byte-identical to the source before, and the RENDERED cell count must equal the header's. Decide explicitly whether the 29 rows in docs/tasks/ are in scope — they are append-only layer-B history that is not retro-edited, but an unrendered cell is a RENDERING defect rather than a restatement of a claim, so the two doctrines pull in different directions and the answer must be argued rather than assumed. The check must be escape-aware (.6 proved a naive pipe count yields 3 false positives out of 8 on one file), must SKIP FENCED CODE BLOCKS (the fact card .6 wrote demonstrates the broken row inside a ```markdown fence and would otherwise be the gate's first false positive — .6 ran the sweep both fence-aware and fence-blind and reports 394/2,310 vs 395/2,308 tables/rows with the SAME 36 malformed, so the two agree today and the gate must not depend on that continuing), must fail closed, must be negative-controlled BOTH ways and vacuity-probed per decision 0037, and must state where it sits: MEMORY-ARCH/README-GROWTH own file SIZE and none of the 8 registered doctrines owns markdown WELL-FORMEDNESS, so registering a 9th needs the feedback_full_factorization test applied explicitly rather than waved through.`
  Verification: `done — 36 MALFORMED ROWS REPAIRED, 0 REMAIN, AND THE GATE IS LIVE AS THE NINTH DOCTRINE. (1) THE 36 ARE THREE DEFECT CLASSES, NOT ONE — .6 characterised only the first. Class B, 10 rows: an unescaped pipe inside cell content (CODEBASE_ANALYSIS.md:2270, USER_GUIDE.md:294, docs/TASK_TREE.md:104/143/147/149, STRUCTURED-EMISSION-EXPANSION.md:1800/1814, PHASE-8-FRONTEND-ACCEPT.md:209, BOOK-TEST-COUNT-SHADOWS.md:148) — repaired with one backslash each, 15 pipes total. Class C, 1 row: docs/TASK_TREE.md:131 carries its File link TWICE, an 88-char duplicate — deleted. Class A, 25 rows and the BULK of the loss: SEMANTIC-INTROSPECTION-EXPANSION.md:696-720 each begin with a stray leading em-dash cell against a 3-column header, so every one of them was dropping its ENTIRE Summary column; the same table holds 10 rows WITHOUT the prefix, which is what proves the prefix an artifact rather than an intent. (2) THE docs/tasks/ SCOPE QUESTION WAS ARGUED, NOT ASSUMED, as acceptance required — and the answer is IN SCOPE. The rule that task files are layer-B history and are not retro-edited protects a recorded CLAIM from revision; it does not protect a rendering defect. Decision 0031 states the purpose in the owner words keep it raw, keep honest, so that people can follow the whole history — and a Summary that reaches no rendered page defeats exactly that. The repair changes no claim, no number, no date and no word: classes B and C are proven byte-identical modulo escaping, and class A deletes only a table-structure placeholder. Excluding them would have left 29 of 36 rows broken to honour the letter of a doctrine against its stated purpose. (3) REPAIRED BY SEPARATOR INDEX, NOT BY STRING MATCH. Each escape names the row and the 0-based index of the internal separator to escape, applied right-to-left so earlier offsets stay valid — so the edit cannot hit a lookalike elsewhere in a 39,000-character row, which a global substitution demonstrably could. (4) THREE PROOFS, THE SECOND ONE AGAINST git show RATHER THAN THE WORKTREE. Proof 1, tree-wide re-scan: 2,310 data rows, MALFORMED 36 -> 0. Proof 2, content equality against HEAD: every differing line classified — 10 escape-only rows BYTE-IDENTICAL after removing the inserted backslashes, 1 duplicate-cell removal with the remainder identical, 25 stray-cell removals where prepending the stray cell to the new line reproduces the old line exactly, and UNEXPLAINED = 0. Proof 3, render: rebuilt with mdbook and read the HTML — the class-B and class-C rows now emit 5 cells WITH their File link href present, and the class-A row emits 3 cells with a 687-character Summary that previously rendered as nothing at all. (5) FIVE CONTROLS ON THE GATE, AND EACH ONE STATES WHAT IT PROVES. Baseline: pass, 2,310 rows / 394 tables / 244 files. NEGATIVE CONTROL: un-escaped a single repaired pipe; the gate failed naming docs/TASK_TREE.md:104, 1 excess cell, 75 characters dropped — then restoration was proven by sha256 identity, not by re-reading. VACUITY PROBE (decision 0037): the comparison neutered to if false and the SAME breached tree re-checked; it PASSED, which is what proves the negative control fired from THIS assertion. SPECIFICITY CONTROL: a row with FEWER cells than the header does NOT fail — the spec pads those and nothing is lost, and a gate that also cried wolf on untidiness gets argued with instead of obeyed. FENCE CONTROL: a fenced pseudo-table was injected; the fence-aware gate passed and the fence-blind variant failed on it, proving the fence handling is load-bearing as a mechanism — while the honest limit is recorded too, that it has ZERO live subjects today. (6) THE VERIFIER WAS WRONG THREE TIMES AND THE REPAIR NEVER WAS — recorded because the pattern is the reusable half. (i) The reconstruction predicate dropped the leading pipe-space when rebuilding the old line. (ii) perl -CD ALREADY decodes backtick output, so an explicit utf8::decode double-decoded and mangled the em-dash — invisible on the ASCII rows and fatal on exactly the 25 rows whose defect is an em-dash. (iii) A source literal containing an em-dash was compared against decoded text without use utf8, so it was 3 bytes against 1 character; fixed with an explicit codepoint escape. All three failed CLOSED, reporting UNEXPLAINED rather than passing, which is the right direction for a checker to be wrong in. (7) AND THE FOURTH LANDING OF THE STANDING GOTCHA, IN THIS LEAF: the first fence probe reported the fence-blind gate as clean — but the neutering substitution had SILENTLY NOT APPLIED, matching 2 spaces of indent against the script actual 4. A probe that cannot fail proves nothing; the repair was to diff the neutered script against its backup and refuse the result unless it differed. (8) REGISTERED AS THE NINTH DOCTRINE, WITH THE TEST APPLIED RATHER THAN WAVED THROUGH. feedback_full_factorization forbids a SECOND registered mechanism for a job that already has one — which is precisely why .3 put the MEMORY.md byte cap INSIDE check_memory_architecture.sh. Here the subject is unowned: MEMORY-ARCH and README-GROWTH own the SIZE of two named files, ENUMERATION-PARITY owns declared list pairs, and the remaining five own staging, paths and citations; none owns markdown WELL-FORMEDNESS. So TABLE-RENDER-FIDELITY registers on its own, and all FOUR ENUMERATION-PARITY-gated doctrine-id lists were updated with it (README.md, book/src/architecture.md, docs/knowledge/doctrine-enforcement.md, CODEBASE_ANALYSIS.md). (9) THE GATE CAUGHT ITS OWN REGISTRATION, WHICH IS THE BEST EVIDENCE IT WORKS. The DOCTRINE_ENFORCEMENT.md registry row written to DESCRIBE this defect CONTAINED it — a bare pipe inside a code span inside a table cell — and the first full run reported DOCTRINE_ENFORCEMENT.md:304 dropping 1,347 characters. The doctrine failed its own author within a minute of existing. Checks: scripts/check_doctrines.sh 9/9 after git add; mdbook build clean; cargo check --all-targets clean. No src/, tests/ or examples/ change => DUT byte-identical.`
  Commit: `pending`

- ID: `OVERFLOW-DESTINATION-INSTRUMENTATION.8`
  Status: `done`
  Goal: `Classify docs/TASK_TREE.md — the destination decision 0040 never classified and neither routing enumeration names — and decide whether its "Current frontier" column is this tree's own MIXED-SURFACE category.`
  Acceptance: `0040 §(a) classifies docs/tasks/; the INDEX is a different file on a different path, and unlike the per-tree files it is REWRITTEN (frontier rows change), so class A cannot simply be inherited — the classification must be argued from what the file is FOR. Measured by .6: 243,983 B over 376 lines = 648 B/line (against the 406 B/line that made MEMORY.md this tree's central finding), 95.1 % of it one table, and the frontier COLUMN alone 192,806 chars = 79.0 % of the file, mean 2,605 per status cell, max 39,095. FIRST READ, THEN CLASSIFY: the distinct-date probe fires on 8 of 74 rows, and .2 retired exactly that instrument for crying wolf on ROADMAP.md, so every flagged cell must be READ and judged against 0039 rule (a) — past-tense closure facts inside a present-tense document are CORRECT and are not evidence of a log. If it IS mixed, the recorded repair is SEPARATION BEFORE INSTRUMENTATION and 0040 §(f)'s "ANVIL has no mixed surface today" needs superseding rather than editing. If it is not, record the measured no and classify it B1 or B2 with the reasoning. Either way the routing gap is real and cheap to close, and 0040 §(g)'s lesson now has TWO instances, which is what .4 should carry.`
  Verification: `done — CLASSIFIED MIXED, recorded as decision 0042, which SUPERSEDES 0040 §(f). (1) THE ANSWER IS THE ONE 0040 SAID DID NOT EXIST. docs/TASK_TREE.md is a MIXED surface — the first measured in ANVIL. A column named Current frontier has a contract of ONE value: the leaf that is next. Measured, it carries a PER-LEAF JOURNAL, and that journal is a LOSSY COPY OF LAYER B. Authoritative set and match count per 0039 rule (b): the 74 owning docs/tasks/<TREE>.md files declare 508 leaf IDs between them; the index frontier column RE-STATES 416 OF THEM = 81.9 %, with 22 of 74 cells re-stating >=5 of their own tree leaves and the worst — STRUCTURED-EMISSION-EXPANSION — re-stating 75 OF 75 in a SINGLE 39,095-character cell alongside 60 completion words. That is the 0036 lossy-copy signature inside the exact shape 0040 named and could not detect. (2) CLASS A COULD NOT BE INHERITED, AND THAT IS WHY THE LEAF EXISTED. 0040 §(a) classifies docs/tasks/; the index is a DIFFERENT FILE ON A DIFFERENT PATH and, unlike the per-tree files, it is REWRITTEN — a frontier row changes every time a leaf lands — so append-only cannot be assumed and the classification had to be argued from what the file is FOR. Profile: 246,172 B over 376 lines = 654 B/LINE, against the 406 B/line that made MEMORY.md this tree central finding; 95 % of the file is one table; the frontier column alone is 219,585 chars, mean 2,967 per status cell, max 39,095. (3) THE VERDICT WAS EARNED BY READING, AND COUNTING WOULD HAVE GOT IT WRONG. .2 retired the distinct-date instrument for crying wolf on ROADMAP.md, and that retirement binds here, so every dated occurrence in all 8 flagged cells was READ INDIVIDUALLY. Of roughly 27 dates: 6 are REGISTRATION facts (Registered 2026-06-17, Activated 2026-06-16) — one per tree and exactly what a status row should carry; 4 are dates INSIDE QUOTED EVIDENCE, including DATED-COUNT-SWEEP-EXEMPTION quoting the very strings it exists to repair, and citing a date is not being a log; the remaining ~15 are PER-LEAF COMPLETION ENTRIES and THOSE are the journal. So the date count is NOT the signal — the accretion of per-leaf entries is, and a leaf that had trusted the count would have reported the same verdict for a reason that does not survive inspection. (4) A THIRD INSTRUMENT, OFFERED AS A FINDING AND NOT AS A DOCTRINE. Take the leaf IDs the OWNING TREE FILE ITSELF DECLARES as the authoritative set, then count how many the status cell re-states. It needs no baseline, no cap and no judgement; it cannot fire on ROADMAP.md, which has no leaf IDs to re-state; and it is keyed on what the surface DUPLICATES rather than on how many dates it holds. NOT registered as a doctrine: one surface is not a class, and DOCTRINE_ENFORCEMENT.md §9 is followed rather than quoted. (5) MY FIRST KEY WAS WRONG AND IS RECORDED. A naive first probe took every backticked .N token in a cell as a leaf of that row tree, and reported 95.4 % duplication over a denominator of 439. Opening the misses showed the key was MIS-ATTRIBUTING: in LIVE-DOC-HYGIENE-BACKFILL the tokens .15 and .22 are SCHEMA VERSIONS and .10b.2 / .5b.2 belong to a DIFFERENT tree. The key was inverted to the tree file own declared set — unambiguous by construction — giving the 416/508 above. Same failure shape as .6 naive pipe count and .7 silent no-op substitution: an extractor that cannot tell its cases apart returns something plausible. (6) NOTHING WAS SWEPT, AND .9 IS REGISTERED. 0040 rule for a mixed surface is SEPARATION BEFORE INSTRUMENTATION, never a cap — capping the index would pressure an author into deleting recorded facts, and the facts are not the problem, their LOCATION is. This tree Non-Goals forbid a content sweep, so .9 owns the separation. And 0042 explicitly does NOT license deleting anything: duplicate is a claim about location, and .9 must prove it PER ROW, per the .5a lesson that a coarse probe reports has-a-home for entries whose only hits point back at the source. (7) 0040 §(f) IS SUPERSEDED, NOT EDITED, per MEMORY_ARCHITECTURE.md §10 — and only its FACTUAL CLAIM. Its reasoning stands: it was right that BOTH candidate detectors were disqualified, and 0042 resurrects neither. The general lesson recorded for .4: a NO MIXED SURFACE finding is only as good as the instrument that looked. (8) THE ROUTING GAP IS CLOSED IN BOTH ENUMERATIONS, and deliberately WITHOUT collapsing the difference .2 recorded between them: the check hint routes NEW content by kind, README table routes a READER, and each gains docs/TASK_TREE.md beside docs/tasks/ with the qualifier that it routes to the ROW-LEVEL contract — one frontier — and is NOT a licence to append a journal there. 0040 §(g) shape now has TWO instances (CODEBASE_ANALYSIS.md and docs/TASK_TREE.md), both unclassified and both unrouted, which turns it from an anecdote into the pattern .4 carries: a routing enumeration must be checked against the set of files it can send you to, or the busiest destinations are the ones nobody declared. Checks: scripts/check_doctrines.sh 9/9 after git add; check_knowledge_map OK; mdbook build clean; cargo check --all-targets clean. Docs + scripts/ only => DUT byte-identical.`
  Commit: `pending`

- ID: `OVERFLOW-DESTINATION-INSTRUMENTATION.9`
  Status: `pending`
  Goal: `Apply decision 0042's repair to the mixed surface .8 measured: SEPARATE the per-leaf journal out of docs/TASK_TREE.md's "Current frontier" column, back to the layer-B tree files that already own 81.9 % of it, leaving each row with the one value its column name promises.`
  Acceptance: `SEPARATION BEFORE INSTRUMENTATION, in that order — no cap is proposed until the mixture is gone, per 0040's rule and 0042. PROVE DUPLICATION PER ROW BEFORE MOVING ANYTHING: 0042's 81.9 % is an aggregate, and the .5a lesson is that a coarse probe reports "has a home" for entries whose only hits point back at the source — so every sentence removed from a cell must be shown present in the owning docs/tasks/<TREE>.md, and anything that is NOT must be MOVED there first, never deleted. Whole-range token sweep with the residue reported and every residue item opened (0036 §(iii)), exactly as .5a/.5b proved their moves. The surviving row must satisfy a STATED contract — one frontier, one status, one link — so that the row becomes B1 and a derived cap becomes available; state that contract in words before writing the rows, and derive any cap from it rather than from the resulting size (0036 §(c), and 0040's non-license 3 that a gap is reported rather than met by raising a cap). NON-GOALS: not a content sweep of the tree files themselves; not a re-wording of any recorded claim; and the 25 class-A rows .7 repaired must stay byte-identical modulo the repair. Expect the git-diff cost measured at .6 (30-42 KB for a one-row edit) to fall out as a side effect — record it as evidence, not as the goal.`
...
| 6 | `OVERFLOW-DESTINATION-INSTRUMENTATION.6` | `done` | **Measured "no" on reflow — and the reason is structural, not economic.** A table row **cannot carry a newline**, so the acceptance-sanctioned repair is *unavailable* for **54 of the 95** in-scope long lines including **both** top offenders; 26 of the 33 in-scope list-items live in `KNOWLEDGE_MAP.md`, which is auto-generated. Reflowable residue: **15 lines**, none near the headline. The premise was also wrong about *which* line is worst — `CODEBASE_ANALYSIS.md` is **third** behind `docs/TASK_TREE.md` (39,679) and `SEMANTIC-INTROSPECTION-EXPANSION.md` (33,890). **And the sweep found a worse defect than length:** **36 malformed table rows drop 57,283 rendered characters** tree-wide (394 tables / 2,310 rows), including **5 links to detail files** — and the headline line is one of them, with **97.0 % of its 24,990 bytes never rendered**. Verified with `mdbook` + a negative control; the first, naive detector's 3 false positives are recorded rather than quietly fixed. |
| 7 | `OVERFLOW-DESTINATION-INSTRUMENTATION.7` | `done` | **36 → 0, and `TABLE-RENDER-FIDELITY` is the ninth doctrine.** The 36 turned out to be **three** classes, not one: 10 unescaped content pipes, 1 duplicated File cell, and **25 rows carrying a stray leading `— ` cell** that was dropping each row's **entire Summary** — the bulk of the loss, and a class `.6` had not characterised. `docs/tasks/` ruled **in scope**, argued: *not retro-edited* protects a recorded **claim**, not a rendering defect, and `0031`'s stated purpose — *"so that people can follow the whole history"* — is what an unrendered cell defeats. Proofs: tree-wide re-scan **36 → 0**; content-equality against `git show HEAD:` with **0 unexplained** differences; and a **render** proof showing the links and a 687-char Summary back. Five controls, incl. a `0037` vacuity probe and a specificity control proving short rows stay silent. **The gate caught its own registry row** — the sentence describing the defect contained it, dropping 1,347 chars. |
| 8 | `OVERFLOW-DESTINATION-INSTRUMENTATION.8` | `done` | **Classified MIXED — decision [`0042`](../decisions/0042-task-tree-index-is-a-mixed-surface.md), which supersedes `0040` §(f)'s *"ANVIL has no mixed surface today"*.** A column named *Current frontier* has a contract of **one value**; measured, it re-states **416 of the 508 leaves (81.9 %)** the owning tree files already hold, **22 of 74** cells re-stating ≥5 of their own, the worst **75 of 75** in one 39,095-char cell. **The verdict was earned by reading:** of ~27 dates, 6 are registration facts and 4 are quoted evidence — both correct under `0039` rule (a) — and only the ~15 per-leaf completion entries are the journal, so the *date count is not the signal*. A **third instrument** is contributed (count how many of the tree file's own declared leaves the status cell re-states) and deliberately **not** registered as a doctrine — one surface is not a class. Repair is **separation before instrumentation** ⇒ `.9`. Routing gap closed in **both** enumerations without collapsing their intended difference. |
| 9 | `OVERFLOW-DESTINATION-INSTRUMENTATION.9` | `pending` | Apply `0042`'s repair: **separate** the per-leaf journal out of the *Current frontier* column back to the layer-B tree files that already own **81.9 %** of it, leaving each row the one value its column name promises. **Prove duplication per row before moving anything** — `0042`'s 81.9 % is an aggregate, and the `.5a` lesson is that a coarse probe reports *"has a home"* for entries whose only hits point back at the source. Separation **before** any cap, in that order. |
| 10 | `OVERFLOW-DESTINATION-INSTRUMENTATION.4` | `pending` | Push the corrected rule back into the portable policy — the owner's point was that this is a policy hole, so fixing only ANVIL leaves every other adopter exposed. `.2` sharpened what must travel: not just *classify your destinations* but **instrument every axis of a bounded surface, or the cap measures the one thing that is not growing.** |
...
- `2026-07-31` (`.6`): **Reflow is rejected as unavailable, not as unattractive — and that
  distinction is the reusable half.** `.6`'s own acceptance named reflow as the only sanctioned
  repair. Measured, **54 of the 95** in-scope long lines are table rows, and a table row **cannot
  carry a newline** — inserting one terminates the row, so the "provably whitespace-only" proof the
  acceptance demanded is *unobtainable by construction* for both top offenders. A leaf that had
  reasoned about cost/benefit alone would have reached the same verdict for the wrong reason and
  left the door open to "reflow it later".
- `2026-07-31` (`.6`): **The finding the tree opened on was real but mis-diagnosed on the axis.**
  `0040` §(g) called `CODEBASE_ANALYSIS.md`'s 24,990-byte line *"a formatting defect, not a size
  defect"*. Correct, and understated: the line is **malformed**, 5 cells against a 4-column header,
  so GFM drops the excess and **24,229 bytes (97.0 %) never render**. It is not ugly, it is
  **invisible** — and no size instrument, byte-first or otherwise, would ever have found it.
- `2026-07-31` (`.6`): **The class is bigger than the file, which is why the sweep was mandated.**
  394 tables / 2,310 data rows across 243 tracked `*.md`: **36 malformed rows**, **57,283
  characters** dropped from every rendered page, **5** of them a row's own link to its detail file.
  Measuring only the file named in the goal would have found one row of thirty-six.
- `2026-07-31` (`.6`): **Verified against a renderer with a negative control, not against the
  spec.** `mdbook`/`pulldown-cmark` — the engine that builds `book/` — was run on a probe table:
  the unescaped form drops the trailing link from the HTML; the `\|` form restores it. Reading the
  GFM spec would have supported the same claim with none of the evidence.
- `2026-07-31` (`.6`): **The first detector was wrong, and it is recorded rather than corrected in
  silence.** A naive `gsub(/\|/)` count flagged 8 malformed rows in `docs/TASK_TREE.md`; the
  escape-aware count flags **5**, because rows 103/130/136 use the correct `\|` idiom. This is the
  standing lesson — *an extractor that cannot distinguish its cases reports something plausible
  instead of dying* — landing for the third time, and `0039` rule (b) requires the denominator and
  the disposition, not just the finds.
- `2026-07-31` (`.6`): **Nothing was repaired in the measuring leaf, deliberately.** The repair is
  four backslashes and a duplicate-link deletion — small enough to be tempting. Folding it in would
  have merged a measurement with a sweep, which is how the `/tmp` sweep damaged `0030`, and would
  have left the *gate* unowned. `.7` and `.8` are registered instead, per the standing directive
  that a defect is only handled once a task-tree leaf owns it.

- `2026-07-31` (`.7`): **`docs/tasks/` is IN SCOPE, and the argument is the leaf's main decision.**
  *Task files are layer-B history and are not retro-edited* protects a recorded **claim** from
  revision. It does not protect a **rendering** defect, and `0031` states its own purpose in the
  owner's words — *"keep it raw, keep honest, so that people can follow the whole history"* — which
  is precisely what a Summary that reaches no rendered page defeats. Excluding them would have left
  **29 of 36** rows broken to honour the letter of a doctrine against its stated intent.
- `2026-07-31` (`.7`): **The 36 were three defect classes, and the biggest one was invisible to
  `.6`.** `.6` characterised the unescaped content pipe (10 rows). Repairing them surfaced a
  duplicated File cell (1 row) and — the bulk — **25 rows carrying a stray leading `— ` cell**, each
  dropping its **entire Summary**. Ten rows of the same table lack the prefix, which is what proves
  it an artifact rather than an intent. **Measuring a class is not the same as characterising it;
  the repair is where the sub-classes appear.**
- `2026-07-31` (`.7`): **Registered as a ninth doctrine, with the factorization test applied rather
  than waved through.** `feedback_full_factorization` forbids a *second* mechanism for a job that
  already has one — which is why `.3` put `MEMORY.md`'s byte cap **inside** `MEMORY-ARCH`. Here the
  subject is unowned: two doctrines own the *size* of two named files, one owns declared list pairs,
  five own staging/paths/citations, and **none owns markdown well-formedness**.
- `2026-07-31` (`.7`): **The gate failed its own author within a minute of existing.** The
  `DOCTRINE_ENFORCEMENT.md` §10 row written to *describe* the defect **contained** it — a bare `|`
  in a code span inside a table cell — and the first full run reported it dropping **1,347**
  characters. Recorded rather than quietly fixed: it is the strongest evidence the check earns its
  place, and it shows the defect is a *typing reflex*, not a legacy artifact.

## Blockers

- None. `.8` is next; `.4` is last by design, since it carries the accumulated correction into the portable policy and
  `.6` added a second instance of `0040` §(g) for it to cite.
- *(historical)* `.3` was **blocked on `.5`** by design, not by an external dependency: the
  instrument it installs could not pass at HEAD. `.6` was opened in the same turn that surfaced its
  finding, per the standing directive that **a defect is only handled if a task-tree owns it** —
  `0040` §(g) had recorded it as *"for a future leaf"*, which is exactly the prose-only parking the
  directive forbids. `.6` then did the same for its own output: `.7` and `.8` were registered in
  the turn that measured them.

## Verification Log

| Date | Leaf | Checks | Result |
| --- | --- | --- | --- |
| `2026-07-31` | `OVERFLOW-DESTINATION-INSTRUMENTATION.8` | `CLASSIFIED MIXED; recorded as decision 0042, superseding 0040 §(f). Authoritative set and match count per 0039 rule (b): the 74 owning docs/tasks/<TREE>.md files declare 508 leaf IDs; the index Current-frontier column re-states 416 = 81.9 %, with 22 of 74 cells re-stating >=5 of their own tree leaves and STRUCTURED-EMISSION-EXPANSION re-stating 75 of 75 in ONE 39,095-char cell with 60 completion words. Profile: 246,172 B / 376 lines = 654 B/line (vs the 406 that made MEMORY.md the tree finding), 95 % of the file one table, frontier column 219,585 chars, mean 2,967, max 39,095. ALL 8 date-flagged cells READ INDIVIDUALLY rather than counted, because .2 retired the distinct-date instrument for crying wolf on ROADMAP.md: of ~27 dates, 6 are registration facts and 4 are dates inside quoted evidence — both CORRECT under 0039 rule (a) — and only ~15 per-leaf completion entries are the journal. FIRST KEY WAS WRONG AND IS RECORDED: a naive probe treating every backticked .N as a leaf of that row tree reported 95.4 % over a denominator of 439; opening the misses showed .15 and .22 are SCHEMA VERSIONS and .10b.2/.5b.2 belong to another tree, so the key was inverted to the tree file own declared set. Nothing swept: 0040 rule for a mixed surface is separation before instrumentation, so .9 is registered and 0042 explicitly licenses no deletion. Routing gap closed in BOTH enumerations without collapsing the difference .2 recorded between them. check_doctrines.sh 9/9 after git add; check_knowledge_map OK; mdbook build clean; cargo check --all-targets clean` | `mixed surface confirmed; 0040 §(f) superseded; .9 owns the separation` |
| `2026-07-31` | `OVERFLOW-DESTINATION-INSTRUMENTATION.7` | `36 malformed rows repaired, 0 remain (tree-wide re-scan, 2,310 data rows). THREE classes, not one: B = 10 rows / 15 unescaped content pipes escaped by SEPARATOR INDEX (not string match, so the edit cannot hit a lookalike in a 39,000-char row); C = 1 row, docs/TASK_TREE.md:131, an 88-char duplicated File cell deleted; A = 25 rows, SEMANTIC-INTROSPECTION-EXPANSION.md:696-720, a stray leading em-dash cell that was dropping each row ENTIRE Summary — the bulk of the 57,283 characters, and a class .6 had not characterised. docs/tasks/ ruled IN SCOPE with the argument stated: not-retro-edited protects a recorded CLAIM, not a rendering defect, and 0031 own purpose (keep it raw, keep honest, so that people can follow the whole history) is exactly what an unrendered cell defeats. CONTENT-EQUALITY PROOF against git show HEAD: rather than the worktree — 10 rows byte-identical after removing the inserted backslashes, 1 duplicate removal with remainder identical, 25 stray-cell removals where re-prepending the stray cell reproduces the old line exactly, UNEXPLAINED = 0. RENDER PROOF via mdbook HTML: class-B/C rows now emit 5 cells with their File link href present; the class-A row emits a 687-char Summary that previously rendered as nothing. FIVE CONTROLS: baseline pass (2,310 rows / 394 tables / 244 files); negative control (one escape removed => FAIL naming docs/TASK_TREE.md:104, 1 excess cell, 75 chars dropped; restore proven by sha256 identity); VACUITY PROBE per 0037 (assertion neutered to if-false, SAME breached tree PASSES, proving which assertion fired); SPECIFICITY control (a row with FEWER cells does NOT fail — the spec pads it, nothing lost); FENCE control (fenced pseudo-table: fence-aware passes, fence-blind fails, so the handling is load-bearing as a mechanism — honest limit recorded: 0 live subjects today). THE VERIFIER WAS WRONG THREE TIMES AND THE REPAIR NEVER WAS: a predicate dropping the leading pipe-space; perl -CD already decoding backtick output so an explicit utf8::decode double-decoded and mangled the em-dash (invisible on ASCII rows, fatal on exactly the 25 em-dash rows); and a source em-dash literal compared to decoded text without use utf8 — all three failed CLOSED. FOURTH LANDING OF THE STANDING GOTCHA: the first fence probe reported clean because the neutering substitution silently did not apply (2-space pattern against 4-space indent) — fixed by diffing the neutered script against its backup and refusing the result unless it differed. Registered as the NINTH doctrine with feedback_full_factorization applied explicitly (no registered doctrine owns markdown well-formedness; contrast .3, where MEMORY-ARCH already owned the subject), and all FOUR ENUMERATION-PARITY-gated doctrine-id lists updated. check_doctrines.sh 9/9 after git add; mdbook build clean; cargo check --all-targets clean` | `36 -> 0; the gate caught its own registry row, which contained the very defect it describes (1,347 chars dropped)` |
| `2026-07-31` | `OVERFLOW-DESTINATION-INSTRUMENTATION.6` | `Every sweep records its authoritative set and match count per 0039 rule (b). Sweep 1 (longest line per tracked *.md, set = 243 files): >=20,000 = 3, 10,000-19,999 = 0, 5,000-9,999 = 7, 2,000-4,999 = 21, 1,000-1,999 = 33, 500-999 = 57, <500 = 122; top three docs/TASK_TREE.md 39,679, docs/tasks/SEMANTIC-INTROSPECTION-EXPANSION.md 33,890, CODEBASE_ANALYSIS.md 24,990 — so the file the leaf was opened on is THIRD, not first. Sweep 2 (every line >=1,000 chars, set = 243 files): 626 matched = 315 PROSE + 257 TABLE-ROW + 54 LIST-ITEM; by 0040 class 531 append-only (out of scope) and 95 in-scope over 15 files = 54 TABLE-ROW + 33 LIST-ITEM + 8 PROSE. Sweep 3 (GFM table well-formedness, set = 243 files / 394 tables / 2,310 data rows): 36 MALFORMED rows dropping 57,283 chars and 5 link cells; in-scope docs/TASK_TREE.md 5, CODEBASE_ANALYSIS.md 1, USER_GUIDE.md 1, remainder 29 in append-only docs/tasks/. Sweep 4 (git cost, set = last 5 commits touching docs/TASK_TREE.md): all +1/-1 LINES, diff payloads 37,132 / 42,134 / 37,199 / 30,116 / 37,092 BYTES. Sweep 5 (docs/TASK_TREE.md profile): 243,983 B / 376 lines = 648 B/line; 232,019 B (95.1%) is one table; the Current-frontier column alone 192,806 chars = 79.0%, mean 2,605, max 39,095; 8 of 74 rows carry >=3 distinct dates — NOT claimed as mixed-surface, since 0039 rule (a) requires each cell be read first. RENDER PROOF, both directions, with mdbook/pulldown-cmark: unescaped pipe in a code span => row splits, trailing [LINKB](y.md) ABSENT from the HTML; the same row with \| => 3 cells, link PRESENT, pipe literal. DETECTOR CORRECTION RECORDED: naive gsub(/\|/) flagged 8 rows on docs/TASK_TREE.md, escape-aware flags 5 — rows 103/130/136 are false positives using the correct idiom. UNIT PRECISION: the headline line is 24,990 BYTES / 24,970 CHARACTERS (20 multi-byte), stated so the pair cannot later read as a discrepancy. check_knowledge_map OK (106 -> 107 facts, 1,022 -> 1,029 keys); check_doctrines.sh green after git add; docs-only => DUT byte-identical` | `reflow REJECTED as structurally unavailable, not merely uneconomic; a worse defect found and handed to .7; the unclassified index handed to .8` |
...
| --- | --- | --- |
| `OVERFLOW-DESTINATION-INSTRUMENTATION.8` | `pending` — `OVERFLOW-DESTINATION-INSTRUMENTATION.8 — the index is the mixed surface` | Decision **`0042`** supersedes `0040` §(f). **416/508 = 81.9 %** duplication measured; routing gap closed in both enumerations; `.9` registered for the separation. |
| `OVERFLOW-DESTINATION-INSTRUMENTATION.7` | `pending` — `OVERFLOW-DESTINATION-INSTRUMENTATION.7 — every cell renders` | **36 → 0.** Ninth doctrine `TABLE-RENDER-FIDELITY`. Content-equality proved against `git show HEAD:`; **0** unexplained differences. Five controls incl. a `0037` vacuity probe. |
| `OVERFLOW-DESTINATION-INSTRUMENTATION.6` | `pending` — `OVERFLOW-DESTINATION-INSTRUMENTATION.6 — the long line was invisible, not just long` | Measurement only; **nothing repaired, deliberately**. Reflow rejected as *structurally unavailable*. 36 malformed rows / 57,283 dropped chars handed to `.7`; the unclassified index handed to `.8`. KM card `gfm-table-unescaped-pipe-drops-content` (**106 → 107** facts). |
...
- `2026-07-31` (`.6`): The long-line question came back **"no"**, and then came back with
  something else entirely. **No**, because reflow is not merely a poor trade on these files — it
  is **unavailable**: a table row cannot carry a newline, and table rows are **54 of the 95**
  in-scope long lines including both worst cases. But the sweep that answering it required found
  that the 24,990-byte line the tree has cited since `.1` is not simply long — it is **malformed**,
  and **97.0 % of it never reaches a rendered page**. Tree-wide, **36 rows silently drop 57,283
  characters**, five of them the row's own link to its detail file. Three leaves have now cited
  that line as a *size* exhibit; it took the leaf that finally measured it to notice that most of
  it was never visible. **A number quoted often enough starts to feel verified** — this tree's own
  rule 0 (*never write "currently correct" without measuring it*) applied to the tree itself.
- `2026-07-31` (`.7`): Repaired — and the repair found a class the measurement had not.
  **36 → 0**, with `TABLE-RENDER-FIDELITY` registered as the **ninth** doctrine. The rows split
  three ways: 10 unescaped content pipes, 1 duplicated cell, and **25 stray leading cells** that
  were dropping whole Summary columns — so the bulk of the loss belonged to a class `.6` never saw.
  **Measuring a class is not the same as characterising it; the sub-classes appear in the repair.**
  `docs/tasks/` was ruled **in scope** on the argument that *not retro-edited* protects a recorded
  claim, not a rendering defect. Every difference was proved against `git show HEAD:` (**0**
  unexplained), every control's restoration proved by **sha256 identity**, and the gate's
  specificity, vacuity and fence handling each controlled separately. **And the gate immediately
  failed its own registry row**, which contained the very defect that row describes — evidence the
  defect is a live typing reflex rather than a legacy artifact, which is the argument for a gate
  over a review habit.
- `2026-07-31` (`.8`): **The index is the mixed surface — the category the tree named at `.1` and
  `0040` §(f) recorded as absent.** `docs/TASK_TREE.md`'s *"Current frontier"* column, a field whose
  contract is **one value**, re-states **416 of the 508 leaves (81.9 %)** the owning tree files
  already hold in full — the `0036` lossy-copy signature inside `0040`'s own shape. Recorded as
  decision [`0042`](../decisions/0042-task-tree-index-is-a-mixed-surface.md), which **supersedes**
  §(f)'s factual claim while leaving its reasoning standing. **The lesson is about the instrument,
  not the file:** `0040` disqualified two detectors honestly and then recorded the absence as fact.
  The surface was there; it took a **third** instrument — keyed on what the surface *duplicates*
  rather than on how many dates it holds — to see it. **A "none found" is only as good as the
