...
- Created: `2026-07-31`
- Last updated: `2026-07-31` (`.2` **done** — decision [`0040`](../decisions/0040-overflow-destination-classification-and-the-unmeasured-axis.md): **three** classes not two, and the pressure went to the **unmeasured AXIS** of `MEMORY.md`; `.5b` **done** — 12 Knowledge Map fact cards + pointer blocks; **`MEMORY.md` 19,885 → 6,037 B, UNDER the derived 6,144 cap**, so `.3` is unblocked; frontier **`.3`**)
...
| 5 | `OVERFLOW-DESTINATION-INSTRUMENTATION.3` | `pending` | **Next — now unblocked.** Apply the derived **6,144-byte** cap as a second assertion inside `check_memory_architecture.sh` (not a new doctrine — `MEMORY-ARCH` already owns that file's size). Control proven capable of failing before it is trusted. |
...
| --- | --- | --- |
| `OVERFLOW-DESTINATION-INSTRUMENTATION.5b` | `pending` — `OVERFLOW-DESTINATION-INSTRUMENTATION.5b — MEMORY.md under its derived cap` | 12 KM fact cards + pointer blocks; **19,885 → 6,037 B**. Loss proof: 706 tokens swept, residue 7, all composites. |
