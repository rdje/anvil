...
| 7 | `OVERFLOW-DESTINATION-INSTRUMENTATION.7` | `pending` | Repair the 36 malformed rows (one backslash per offending pipe) and gate the class escape-aware. Must argue, not assume, whether the 29 rows inside append-only `docs/tasks/` are in scope — an unrendered cell is a *rendering* defect, not a restatement of a claim, so `0031` and rendering fidelity pull in different directions. |
...
- None. `.7` is next and needs only `.6`'s measurement — the offending rows, their line numbers and
  the exact repair are all recorded. `.8` is independent of `.7` and may follow it in either order;
  `.4` is last by design, since it carries the accumulated correction into the portable policy and
