...
| --- | --- | --- |
| `OVERFLOW-DESTINATION-INSTRUMENTATION.2` | `pending` — `OVERFLOW-DESTINATION-INSTRUMENTATION.2 — the pressure went to the unmeasured axis` | Decision `0040`. **Three** classes, not two; the one real gap is `MEMORY.md`'s missing byte axis; both candidate instruments retired on measurement; tree grows to five leaves with `.5` inserted **before** `.3`. |
