...
| 8 | `OVERFLOW-DESTINATION-INSTRUMENTATION.8` | `pending` | Classify `docs/TASK_TREE.md` — **`0040` never classified it** (it classified `docs/tasks/`, a different path) and **neither routing enumeration names it**, the second instance of `0040` §(g)'s shape. It is now the worst-profile non-append-only file: **648 B/line** against the **406** that made `MEMORY.md` this tree's central finding, **79.0 %** of it inside the *"Current frontier"* column. Whether that column is the tree's own **mixed-surface** category is deliberately **not** claimed by `.6` — the distinct-date probe fires on 8/74 rows and `.2` retired that instrument for crying wolf, so each cell must be read. |
| 9 | `OVERFLOW-DESTINATION-INSTRUMENTATION.4` | `pending` | Push the corrected rule back into the portable policy — the owner's point was that this is a policy hole, so fixing only ANVIL leaves every other adopter exposed. `.2` sharpened what must travel: not just *classify your destinations* but **instrument every axis of a bounded surface, or the cap measures the one thing that is not growing.** |
