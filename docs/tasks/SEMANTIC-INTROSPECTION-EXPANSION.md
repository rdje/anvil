...
**Active frontier: `.7b.2`** (the `memory_provenance` MCP surface). **Five** derived queries are delivered end-to-end —
`output_support` (`.1`/`.2`), `input_reach` (`.3`), `flop_reset_provenance` (`.4`),
`module_reachability` (`.5`), and `flop_dependencies` (`.6`, the register→register
dependency graph) — at introspection schema `1.18`, DUT byte-identical. The fifth
query (`.6`) is the first beyond decision `0011`'s four named kinds, delivered under
the lane's documented "further derived-query kinds are open-ended breadth" clause.
The tree stays `active` (nothing retired). **`.7` `memory_provenance` is now the
active frontier** — the **sixth** derived query (the second beyond decision
`0011`'s four named kinds): per-inferrable-memory port provenance, opening the
documented opaque-`MemRead`-leaf boundary. `.7a` design-detail landed; frontier →
`.7b.1` (pure core) → `.7b.2` (surface, schema `1.18 → 1.19`).

| Order | Leaf | Status | Why next |
| --- | --- | --- | --- |
| 1 | `SEMANTIC-INTROSPECTION-EXPANSION.7b.2` | `active` | **NEXT.** Surface: add `memory_provenance` to `supported_query_kinds()` + branch `run_analyze` (same commit) + the vec-aware `-32602` guard + schema `1.18 → 1.19` + the `analyze_schema` enum (incl. the `"mem:<id>"` target form) + schema-doc/book/USER_GUIDE/README/KM + e2e `anvil-mcp` smoke. Closes `.7b`/`.7`. DUT byte-identical. |
