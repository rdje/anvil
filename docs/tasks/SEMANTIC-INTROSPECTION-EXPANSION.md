...
- Status: `active` (**SEVEN derived queries delivered end-to-end** at schema `1.20`: `output_support` `.1`/`.2` + `input_reach` `.3` + `flop_reset_provenance` `.4` + `module_reachability` `.5` + `flop_dependencies` `.6` + `memory_provenance` `.7` + **`fsm_provenance` `.8`** (per-generated-encoding-FSM provenance — the third beyond decision `0011`'s four named kinds, and **the second to open a documented opaque-leaf boundary**, the `Node::FsmOut` sibling of `.7`'s `MemRead`). **No active frontier**; nothing retired — further derived-query kinds (a per-FSM reach; per-child-module provenance) are open-ended breadth)
...
- ID: `SEMANTIC-INTROSPECTION-EXPANSION`
  ...
  Children: `SEMANTIC-INTROSPECTION-EXPANSION.1`, `SEMANTIC-INTROSPECTION-EXPANSION.2`, `SEMANTIC-INTROSPECTION-EXPANSION.3`, `SEMANTIC-INTROSPECTION-EXPANSION.4`, `SEMANTIC-INTROSPECTION-EXPANSION.5`, `SEMANTIC-INTROSPECTION-EXPANSION.6`, `SEMANTIC-INTROSPECTION-EXPANSION.7`
...
**No active frontier.** **Seven** derived queries are now delivered end-to-end —
...
breadth" clause. The tree stays `active` (nothing retired): further kinds — a per-FSM
