...
- Status: `active` (**four named query kinds from decision `0011` delivered** at schema `1.7`: `output_support` `.1`/`.2` + `input_reach` `.3` + `flop_reset_provenance` `.4` + `module_reachability` `.5`; **a fifth query — `flop_dependencies` (the register→register dependency graph) — is now in progress**: `.6a` design-detail + `.6b.1` pure core landed; `.6b.2` (surface) pending, schema `1.17 → 1.18`. The fifth query exercises the lane's documented "further derived-query kinds are open-ended breadth" clause; nothing retired)
- Roadmap lane: `Capability — deeper agent/introspection surface (extends AGENT-INTROSPECTION-MCP / AGENT-MCP-EXPANSION)`
- Created: `2026-06-15`
- Last updated: `2026-06-23` (**`.6b.1` landed — the pure `flop_dependencies` core** in `src/introspect/analyze.rs`: `QUERY_FLOP_DEPENDENCIES` + `FlopDependencies { flop, depends_on_flops, driven_flops, self_dependent }` + the fifth `DerivedAnalysis.flop_dependencies` `skip_serializing_if` vec + `module_flop_dependencies`/`design_flop_dependencies` + the `flop_dependencies_with` driver (reuse `build_cone`: predecessors = each flop's D-cone `support_flops`, successors = the transpose, `self_dependent` = `flop ∈ depends_on_flops`); the 8 existing `DerivedAnalysis` literals gained `flop_dependencies: Vec::new()`. Not in `supported_query_kinds()` yet (joins with the `run_analyze` dispatch + schema `1.17 → 1.18` in `.6b.2`). 6 in-crate proofs; `cargo test --lib introspect::analyze` 32/0; snapshots 6/6 byte-identical; clippy/fmt clean. DUT byte-identical. Frontier → `.6b.2`. Prior: `.6a` design-detail.)
...
**Active frontier: `SEMANTIC-INTROSPECTION-EXPANSION.6b.2`** (wire the
`flop_dependencies` surface). The four named derived queries from decision `0011`
are delivered end-to-end — `output_support` (`.1`/`.2`), `input_reach` (`.3`),
`flop_reset_provenance` (`.4`), and `module_reachability` (`.5`) — at introspection
schema `1.7`, DUT byte-identical. A **fifth** query, `flop_dependencies` (the
register→register dependency graph — the first beyond decision `0011`'s four named
kinds, under the lane's "open-ended breadth" clause), is in progress: `.6a`
design-detail + `.6b.1` pure core landed; `.6b.2` (surface, schema `1.17 → 1.18`) is
next. Nothing retired.

| Order | Leaf | Status | Why next |
| --- | --- | --- | --- |
| 1 | `SEMANTIC-INTROSPECTION-EXPANSION.6b.2` | `pending` | **Next.** Surface: add `flop_dependencies` to `supported_query_kinds()` + branch `run_analyze` (same commit) + schema `1.17 → 1.18` + the `analyze_schema` enum + schema-doc/book/USER_GUIDE/README/KM + e2e `anvil-mcp` smoke. Closes `.6b`/`.6`. DUT byte-identical. |
