...
**Frontier: `SEMANTIC-INTROSPECTION-EXPANSION.8b.1`** (the pure `fsm_provenance`
core). **Six** derived queries are delivered end-to-end —
...
| --- | --- | --- | --- |
| 1 | `SEMANTIC-INTROSPECTION-EXPANSION.8b.1` | `not started` | The pure `fsm_provenance` core in `analyze.rs`: `QUERY_FSM_PROVENANCE` + `FsmProvenance` (reusing `SupportCone` for the `sel` cone) + the seventh `fsm_provenance` vec + `module_fsm_provenance`/`design_fsm_provenance` (`build_cone` over `Fsm::sel`; structural projection incl. `FsmEncoding::state_width`). Not in `supported_query_kinds()` yet (joins with dispatch in `.8b.2`). Lib-tested; DUT byte-identical. |
| 2 | `SEMANTIC-INTROSPECTION-EXPANSION.8b.2` | `not started` | Surface: add `fsm_provenance` to `supported_query_kinds()` + branch `run_analyze` (same commit) + the vec-aware `-32602` guard + schema `1.19 → 1.20` + the `analyze_schema` enum (the `"fsm:<id>"` target form) + schema-doc/book/USER_GUIDE/README/KM + e2e `anvil-mcp` smoke. Closes `.8b`/`.8`. DUT byte-identical. |
