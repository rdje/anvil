...
**Active frontier: `.7b.1`** (the pure `memory_provenance` core). **Five** derived queries are delivered end-to-end —
...
| --- | --- | --- | --- |
| 1 | `SEMANTIC-INTROSPECTION-EXPANSION.7b.1` | `active` | **NEXT.** The pure `memory_provenance` core in `analyze.rs`: `QUERY_MEMORY_PROVENANCE` + `MemoryProvenance` (reusing `SupportCone` per port) + the sixth `memory_provenance` vec + `module_memory_provenance`/`design_memory_provenance` (`build_cone` per `raddr`/`waddr`/`wdata`/`we`). Not in `supported_query_kinds()` yet (joins with dispatch in `.7b.2`). Lib proofs; snapshots 6/6; DUT byte-identical. |
| 2 | `SEMANTIC-INTROSPECTION-EXPANSION.7b.2` | `pending` | Surface: add `memory_provenance` to `supported_query_kinds()` + branch `run_analyze` (same commit) + schema `1.18 → 1.19` + the `analyze_schema` enum + schema-doc/book/USER_GUIDE/README/KM + e2e `anvil-mcp` smoke. Closes `.7b`/`.7`. DUT byte-identical. |
