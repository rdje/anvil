...
- ID: `SEMANTIC-INTROSPECTION-EXPANSION.9`
  ...
  Result: `(in progress — .9a design + .9b.1 pure core landed; .9b.2 surface pending)`
...
- ID: `SEMANTIC-INTROSPECTION-EXPANSION.9b`
  ...
  Result: `(in progress — .9b.1 pure core done; .9b.2 surface pending)`
...
**Active frontier: `.9b.2`** (the `node_drivers` MCP surface + schema `1.20 → 1.21`),
after `.9a` (design) + `.9b.1` (pure core) landed. **Seven** derived queries are already delivered end-to-end —
...
provenance), and `fsm_provenance` (`.8`, per-generated-encoding-FSM provenance) — at
introspection schema `1.20`, DUT byte-identical. The seventh query (`.8`) is the
third beyond decision `0011`'s four named kinds, and **the second to open a
documented opaque-leaf boundary** — the `Node::FsmOut` sibling of `.7`'s `MemRead` —
delivered under the lane's documented "further derived-query kinds are open-ended
breadth" clause. `.9` opens the **eighth** query, `node_drivers` (per-node immediate
1-hop driver adjacency + each node's `GateOp` — the atomic node-level primitive
complementing the transitive `output_support` cone, the **fourth** beyond decision
`0011`'s four named kinds). `.9a` (design) is done; `.9b.1`/`.9b.2` (impl) are the
frontier. The tree stays `active` (nothing retired): further kinds — `node_drivers`'s
dual `node_readers`, a per-FSM reach, per-child-module provenance — remain open-ended
breadth (none named as a blocker).
