...
- ID: `SEMANTIC-INTROSPECTION-EXPANSION.8b`
  ...
  Result: `In progress — .8b.1 (pure core) done; .8b.2 (surface) is the frontier.`
...
**Frontier: `SEMANTIC-INTROSPECTION-EXPANSION.8b.2`** (the `fsm_provenance`
surface). **Six** derived queries are delivered end-to-end —
`output_support` (`.1`/`.2`), `input_reach` (`.3`), `flop_reset_provenance` (`.4`),
`module_reachability` (`.5`), `flop_dependencies` (`.6`, the register→register
dependency graph), and `memory_provenance` (`.7`, per-inferrable-memory port
provenance) — at introspection schema `1.19`, DUT byte-identical. The **seventh**
query, `fsm_provenance` (`.8`), is now in progress: `.8a` design-detail landed (per-
generated-encoding-FSM structural shape + the support cone of its one generated
input cone `sel`, opening the documented opaque-`Node::FsmOut`-leaf boundary — the
direct sibling of `.7`'s `MemRead` boundary; the table *values* are deliberately out
of scope). `.8` is the third query beyond decision `0011`'s four named kinds and the
second to open an opaque-leaf boundary, delivered under the lane's documented
"further derived-query kinds are open-ended breadth" clause. The tree stays `active`
(nothing retired): after `.8`, further kinds (a per-FSM reach; per-child-module
provenance) remain open-ended breadth.

| Order | Leaf | Status | Why next |
| --- | --- | --- | --- |
| 1 | `SEMANTIC-INTROSPECTION-EXPANSION.8b.2` | `not started` | Surface: add `fsm_provenance` to `supported_query_kinds()` + branch `run_analyze` (same commit) + the vec-aware `-32602` guard + schema `1.19 → 1.20` + the `analyze_schema` enum (the `"fsm:<id>"` target form) + schema-doc/book/USER_GUIDE/README/KM + e2e `anvil-mcp` smoke. Closes `.8b`/`.8`. DUT byte-identical. |
