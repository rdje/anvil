...
**No active frontier.** **Six** derived queries are now delivered end-to-end —
...
provenance) — at introspection schema `1.19`, DUT byte-identical. The sixth query
(`.7`) is the second beyond decision `0011`'s four named kinds, and **the first to
open the documented opaque-`MemRead`-leaf boundary**, delivered under the lane's
documented "further derived-query kinds are open-ended breadth" clause. The tree
stays `active` (nothing retired): further derived-query kinds — including the FSM
analog `fsm_provenance` and a per-memory reach — remain open-ended breadth (none
named as a blocker). The next lane is owner-directed / PNT.
