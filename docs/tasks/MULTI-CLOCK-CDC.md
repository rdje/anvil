...
- Created: `2026-05-24`
- Last updated: `2026-05-24` (**`.3b.1` done** — `Config.multi_clock_prob: f64` knob + `multi_clock::promote_to_multi_clock` post-construction pass wired into `Generator::generate_design` per the `aggregate_prob` pattern; default 0.0 byte-identical; 8 new cargo-portable proofs incl. 2 end-to-end Generator integration tests; lib 256 → 264)
...
- ID: `MULTI-CLOCK-CDC.3b.2`
  Status: `pending`
  Goal: `tool_matrix wiring: add a multi-clock scenario (--multi-clock-prob > 0 + sequential-favoring config) + saw_multi_clock_design + saw_cdc_2_flop_synchronizer coverage facts on CoverageSummary + merge_coverage / summarize_coverage updates + Verilator downstream-tool gate (--cdc=metastable or equivalent) + #[ignore] end-to-end proof. Closes MULTI-CLOCK-CDC.3.`
  Acceptance: `cargo fmt/clippy(-D warnings)/check/test all green; tool_matrix has a multi-clock scenario in build_default_scenarios (or its own scenario set); CoverageSummary gains saw_multi_clock_design + saw_cdc_2_flop_synchronizer; merge_coverage + summarize_coverage updates; cargo-portable proofs of the coverage facts + scenario; #[ignore] end-to-end proof exits 0 with both coverage facts true on a machine with Verilator.`
