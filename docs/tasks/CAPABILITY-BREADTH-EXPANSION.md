...
| --- | --- | --- | --- |
| 1 | `CAPABILITY-BREADTH-EXPANSION.4b.2a` | `pending` | The metric + the additive schema bump, with the surface already **live and downstream-proven** at `.4b.1`. Nothing about RTL moves here: a post-hoc `Metrics` field is structural, so snapshots stay byte-identical. The one thing to get right is *which* `1.27` references are current-value (bump) and which are **history** — a per-metric "landed at schema `1.16`" attribution or a `1.26 → 1.27` changelog line is a fact about the past and must not be rewritten. |
| 2 | `CAPABILITY-BREADTH-EXPANSION.4b.2b` | `pending` | The gate. Two things here are already decided and must not be re-derived: detection is **metric-keyed**, not a text scan (decision `0028`); and the `unique` tool plan needs a **new** `scenario_emits_unique_case_qualifier` predicate beside `scenario_emits_soft_union_overlay` — a separate boolean, **not** a widening of `verilator_only`, which drops Yosys *and* Icarus where this drops Icarus only. The gate's 13th scenario is the co-occurrence one; without it the pass's non-vacuous exclusion is never exercised end-to-end. |
| 3 | `CAPABILITY-BREADTH-EXPANSION.4b.3` | `pending` | The user docs. Note for whoever writes them: the book section must **not** be called "the tenth surface" — the chapter's "Combining the surfaces" section is about the mutual-exclusion chain over *projections*, and a qualifier is not one of them. |
