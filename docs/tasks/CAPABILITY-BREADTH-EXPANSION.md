...
- ID: `CAPABILITY-BREADTH-EXPANSION.4`
  Status: `done` (container — `.4a` design-detail + the whole `.4b` impl chain landed `2026-08-01`)
...
- ID: `CAPABILITY-BREADTH-EXPANSION.4b`
  Status: `done` (container — `.4b.1` live surface, `.4b.2a` metric+schema, `.4b.2b` gate, `.4b.3` docs all landed)
...
- ID: `CAPABILITY-BREADTH-EXPANSION.4b.2`
  Status: `done` (container — `.4b.2a` metric+schema and `.4b.2b` gate both landed)
...
- ID: `CAPABILITY-BREADTH-EXPANSION.4b.3`
  ...
  Verification: `done — book/src/structured-emission.md gained "A different kind of thing: case qualifiers", deliberately NOT "the tenth surface" and placed AFTER "Combining the surfaces" so the projection/decoration boundary reads in the table of contents rather than only in the prose; it covers the assertion argument (the FULL/PARALLEL table + why the default: arm is never conditional), the lane's one non-vacuous exclusion, the two knobs, and the gate + its per-qualifier tool-plan table. book/src/knobs.md gained a "Case-qualifier knobs" section (its own heading, not a tenth bullet under structured emission). USER_GUIDE.md gained the prose knob entry, TWO CLI-table rows, and the --case-qualifier-gate entry (the latter landed at .4b.2b). New KM card docs/knowledge/case-qualifiers-unique-priority.md with a WORKING reverify (measured: prints 9). EVERY CLAIM RE-MEASURED AGAINST REAL OUTPUT BEFORE SHIPPING, and one was WRONG: the draft cited .4b.1'"'"'s "8 modules / 30 qualified blocks" beside a DIFFERENT command, which actually yields 176 — a reader running the book'"'"'s own command would have seen a number the book did not predict. Corrected to 8 modules / 176 statements, and the strip-the-token diff re-run at that shape (empty diff, byte-identical). THE DIGEST COMMIT BACKFILLED: docs/evidence/anvil-case-qualifier-gate-r1.md said c1cc6a1ff1f9 (the commit the run executed AGAINST — a gate banked by its own introducing commit cannot know its hash) and now says 955e387, with a note recording why, so the re-verification instruction is followable. Checks: mdbook build clean; cargo test --test book_examples 4/4; KM regenerated 113 → 114 facts; check_doctrines.sh 9/9. Docs-only ⇒ DUT byte-identical.`
  Commit: `CAPABILITY-BREADTH-EXPANSION.4b.3`

## Current Frontier

| Order | Leaf | Status | Why next |
| --- | --- | --- | --- |
| 1 | `CAPABILITY-BREADTH-EXPANSION.1` | `pending` | **The only remaining child, and it is deferred-not-retired.** The whole `.3`/`.4` case-qualifier strand closed `2026-08-01` (`.4b.3` shipped the docs; digest `commit` backfilled). SV up-opt breadth — **deferred-not-retired**, and deferred on evidence twice (the `.2a` and pre-`.1` probes): the named candidates (enum/typedef, packed multidim arrays) are accepted at every Verilator `--language` mode + Yosys + Icarus ⇒ not version-distinctive, no down-gating teeth, and post-2012 SV additions are overwhelmingly verification features a synthesizable generator cannot emit. The genuinely-2023 clean space with the installed tools is essentially `union soft`, shipped. Unblocks when a tool gains real 2023 synthesizable coverage. Its question was **not** folded into `.3` — see decision `0044` Context. |
...
| `2026-08-01` | `CAPABILITY-BREADTH-EXPANSION.4b.3` | `book/src/structured-emission.md "A different kind of thing: case qualifiers" (NOT "the tenth surface", and placed AFTER "Combining the surfaces" so the projection/decoration boundary shows in the ToC) + book/src/knobs.md "Case-qualifier knobs" + USER_GUIDE prose entry and two CLI-table rows + the new KM card docs/knowledge/case-qualifiers-unique-priority.md (reverify measured: prints 9). Every claim re-measured against real output first, and ONE WAS WRONG: the draft cited .4b.1'"'"'s "8 modules / 30 blocks" beside a DIFFERENT command that actually yields 176 — corrected, and the strip-the-token diff re-run at that shape (empty diff). Digest commit backfilled c1cc6a1ff1f9 → 955e387 with a note recording why. mdbook build clean; cargo test --test book_examples 4/4; KM 113 → 114 facts; check_doctrines.sh 9/9` | `done` (docs-only; book in sync; DUT byte-identical) — **`.4b`, `.4` and the whole case-qualifier strand `.3`/`.4` are closed** |
...
| `CAPABILITY-BREADTH-EXPANSION.4b.3` | `CAPABILITY-BREADTH-EXPANSION.4b.3 — case-qualifier user docs (book + knobs + USER_GUIDE + KM card)` | Docs-only. The book's *"A different kind of thing: case qualifiers"* section (deliberately not "the tenth surface", and placed after "Combining the surfaces"), a `knobs.md` "Case-qualifier knobs" section, the `USER_GUIDE` prose entry + two CLI-table rows, and a new KM card with a working reverify. Also backfills the evidence digest's `commit` (`c1cc6a1ff1f9` → `955e387`). One drafted number was measured wrong and corrected before shipping. **Closes `.4b`, `.4`, and the whole case-qualifier strand.** |
...
- `2026-08-01`: `.4b.3` done — **the user docs, and with them `.4b`, `.4`, and the whole
  case-qualifier strand `.3`/`.4`**. `book/src/structured-emission.md` gained *"A different
  kind of thing: case qualifiers"* — not *"the tenth surface"*, and placed **after**
  "Combining the surfaces" so the projection-vs-decoration boundary is visible in the table
  of contents rather than only in the prose; `book/src/knobs.md` gained its own
  "Case-qualifier knobs" section rather than a tenth bullet under structured emission;
  `USER_GUIDE.md` gained the prose knob entry plus **two CLI-table rows**; and
  `docs/knowledge/case-qualifiers-unique-priority.md` is a new KM card whose `reverify` was
  run (prints `9`). The digest's `commit` was backfilled `c1cc6a1ff1f9` → `955e387` with a
  note recording why a gate banked by its own introducing commit cannot know its hash.
  **One drafted number was wrong and was caught by re-measuring**: the section cited
  `.4b.1`'s *"8 modules / 30 qualified blocks"* next to a **different** command, which
  actually yields **176** — so a reader running the book's own command would have got a
  number the book did not predict. Corrected, and the strip-the-token `diff` re-run at that
  shape (empty diff, byte-identical). `mdbook build` clean, `tests/book_examples` 4/4, KM
  113 → 114 facts. Docs-only; DUT byte-identical. The tree stays `active` with its one
  remaining child `.1` (SV up-opt) deferred-not-retired.
