...
- ID: `CAPABILITY-BREADTH-EXPANSION.4b`
  Status: `active` (container — `.4b.1` **done**; `.4b.2` metric+gate and `.4b.3` docs pending)
...
| --- | --- | --- | --- |
| 1 | `CAPABILITY-BREADTH-EXPANSION.4b.2` | `pending` | The metric + gate, with the surface already **live and downstream-proven** at `.4b.1` (Verilator/Yosys/Icarus clean, cell counts identical 8/8, 30/30 eligible gates qualified, snapshots byte-identical). Two things here are already decided and must not be re-derived: detection is **metric-keyed**, not a text scan (decision `0028`); and the `unique` tool plan needs a **new** `scenario_emits_unique_case_qualifier` predicate beside `scenario_emits_soft_union_overlay` — a separate boolean, **not** a widening of `verilator_only`, which drops Yosys *and* Icarus where this drops Icarus only. The gate's 13th scenario is the co-occurrence one; without it the pass's non-vacuous exclusion is never exercised end-to-end. |
| 2 | `CAPABILITY-BREADTH-EXPANSION.4b.3` | `pending` | The user docs. Note for whoever writes them: the book section must **not** be called "the tenth surface" — the chapter's "Combining the surfaces" section is about the mutual-exclusion chain over *projections*, and a qualifier is not one of them. |
...
- `2026-08-01`: `.4b.1` done — **the case-qualifier surface is live**. `src/ir/case_qualifier.rs`
  ...
  `unique_case_prob` / `priority_case_prob` knobs and their CLI flags + two `emission` `KnobId`
  rows + the tenth guarded roll in both `gen/mod.rs` paths + the emitter keyword prefix on the two
