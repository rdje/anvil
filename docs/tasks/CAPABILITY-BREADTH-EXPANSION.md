...
- ID: `CAPABILITY-BREADTH-EXPANSION.4`
  ...
  Acceptance: `cargo check/test/clippy/fmt green; snapshots 6/6 byte-identical with both knobs at 0.0; each knob at 1.0 emits the qualifier on every eligible gate and is downstream-clean on its pinned tool plan; the gate lights saw_unique_case_qualifier / saw_priority_case_qualifier with coverage_gaps = [].`
