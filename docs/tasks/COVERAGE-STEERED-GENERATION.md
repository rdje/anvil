...
| 7 | `COVERAGE-STEERED-GENERATION.4c` | `COVERAGE-STEERED-GENERATION.4c — steering-width docs + close .4` | The docs/close slice: six live-doc taxonomy copies updated, the per-gate measurement chapter, a fourth `ENUMERATION-PARITY` pair so they cannot rot, and a hand-written count deleted. **Closes `.4`.** |
