...
| 7 | `COVERAGE-STEERED-GENERATION.4b.2` | `COVERAGE-STEERED-GENERATION.4b.2 — nine emission surfaces, measurable per gate` | The second feature slice of `.4`: the nine emit-projections gain a `KnobId`, the `emission` category, and per-gate telemetry. Byte-identical across 16 configurations. |
