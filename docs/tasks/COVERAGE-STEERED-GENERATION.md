...
| 11 | `COVERAGE-STEERED-GENERATION.4b.1` | `pending` | **Next.** The 7 module-level motif knobs. **Not purely mechanical** (decision `0035` Correction): 3 of the 7 roll *before any `Module` exists* — the roll chooses which builder to call — so they need a `Generator::pending_knob_rolls` buffer drained into the module once built, with the drain inside `ir::knob_roll`. Also carries the `KnobId::all()` guard (38 entries). |
