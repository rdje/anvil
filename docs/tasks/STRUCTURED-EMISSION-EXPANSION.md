...
- Created: `2026-06-15`
- Last updated: `2026-06-23` (**`.19b.3` landed — the ninth-surface user docs (DOCS-ONLY ⇒
  DUT byte-identical, no `src/`): a `book/src/structured-emission.md` "## The ninth surface: a
  masked if/else if priority chain" section with a byte-verified seed-1 before/after (the
  `casez (slice_0)` block vs the masked `if ((slice_0 & 2'h2) == …)` chain) + the chapter-intro
  surface list; the 4 deferred book example-JSON `schema_version` `1.16 → 1.17` (`agent-mcp.md`
  ×1 + `api-tools.md` ×3); the `casez_mux_if_emit_prob` entry in `book/src/knobs.md` /
  `USER_GUIDE.md` / README + a `--casez-mux-if-gate` matrix-gate entry; a
  `docs/knowledge/casez-mux-if-emit.md` KM card, `KNOWLEDGE_MAP.md` regenerated (69 → 70 facts /
  672 keys). `mdbook build` clean + `book_examples` 3/3 + `check_knowledge_map` OK. **`.19b.3` /
  `.19b` / `.19` close ⇒ the NINTH structured surface (the wildcard `CasezMux` masked `if`/`else
  if` priority chain, decision `0029`) is delivered end-to-end; NINE surfaces total.** The tree
  stays `active` at a no-frontier boundary (future `.20+`: nested/multi-level `generate`,
  `interface`/`modport`).**) Prior: **`.19b.2b` landed — the ninth-surface repo-owned downstream
  gate `tool_matrix --casez-mux-if-gate` (a CODE change, task-tree-owned by `.19b.2b`),
  templated 1:1 on `--case-mux-if-gate` (~30 touch points in `src/bin/tool_matrix.rs`):
  the `--casez-mux-if-gate` flag + `ScenarioSet::CasezMuxIfSweep` +
  `build_casez_mux_if_sweep_scenarios` + `casez_mux_if_focus_config` (comb-only node-id +
  e-graph, `casez_mux_if_emit_prob=1.0`, `casez_mux_prob=0.9`, and BOTH `comb_mux_prob` AND
  `case_mux_prob` forced to `0.0` — both roll before `casez_mux` in `cone.rs` and
  short-circuit it) × 3 strategies + METRIC-KEYED `ModuleReport.emitted_casez_mux_if`
  (`= num_emitted_casez_mux_if_chains > 0`, no new text token) +
  `CoverageSummary.saw_casez_mux_if_emit` + merge + `MatrixReport.casez_mux_if_gate` +
  run-plan/dispatch/mutual-exclusion/gap-arm/3 exhaustive match arms/2 slug arms/`test_cli`
  default + 8 `ModuleReport` fixtures + 5 cargo proofs. Co-located: a 1-line blank-line fix
  resolved a pre-existing rustfmt-1.9.0 fmt drift in the `.19b.2a` `metrics.rs` test (HEAD
  itself failed `cargo fmt --all --check`). `cargo check/clippy(-D warnings)/fmt` clean;
  `cargo test --bin tool_matrix` green incl. 5 new proofs; `--lib casez_mux_if` 10/10;
  snapshots 6/6 byte-identical. Banked clean `anvil-casez-mux-if-gate-r1`: 3 scenarios /
  12 modules / 12 emitting a chain / 108 chains / `coverage_gaps = []` /
  `saw_casez_mux_if_emit = true` / 12/0 Verilator + both Yosys modes + Icarus. `.19b.2`
  closes; frontier → `.19b.3` (docs). Default-off / DUT byte-identical. The lane stays
  `active`.**) Prior: **`.19b.2a` landed — the ninth-surface metric + introspection
  schema bump (`1.16 → 1.17`): `Metrics::num_emitted_casez_mux_if_chains` (`=
  m.casez_mux_if_gates.len()`, `#[serde(default)]`) + a lib proof
  (`metrics_count_emitted_casez_mux_if_chains`), surfaced at introspection schema `1.17` —
  all current-output refs bumped (`introspect/mod.rs` const + doc-comment history + 3
  assertions; 8 `mcp/mod.rs` assertions; the schema doc current-version ×3 + a new `1.16 →
  1.17` changelog entry; README envelope ×2 + USER_GUIDE envelope ×1 + the CODEBASE envelope
  line). Per-metric "landed at schema X" attributions (`num_emitted_case_mux_if_chains @
  1.16` etc.) left intact; the book example JSONs deferred to `.19b.3` (the `.17b.2a`/`.17b.3`
  precedent). The metric is **exact** (static-selector `CasezMux` excluded). `cargo test
  --lib` 635→636 (the new proof; all schema assertions at `"1.17"`); `clippy -D warnings` +
  `fmt` clean; snapshots 6/6 byte-identical; live `--introspect` shows `1.17` + the metric `0`
  default (seed 2) / `1` forced (seed 4, `casez_mux_if_emit_prob=1.0`). `.19b.2` pre-split →
  `.19b.2a` + `.19b.2b` (the repo-owned `tool_matrix --casez-mux-if-gate`); frontier →
  `.19b.2b`. Default-off / DUT byte-identical.**) Prior: **`.19b.1` landed — the LIVE ninth structured surface (the
  first source change of the ninth surface). The procedural `always_comb` `if`/`else if`
  **masked** priority-chain emit-projection of a dynamic-selector `CasezMux` gate (decision
  `0029`) goes live behind the opt-in `casez_mux_if_emit_prob` knob: new
  `src/ir/casez_mux_if_emit.rs` (`annotate_casez_mux_if_gates` — the `GateOp::CasezMux`
  predicate requiring a non-`Constant` selector + `operands.len() >= 4`, excluding `CaseMux` +
  `case_mux_if` + all sibling marks, run **last** after `case_mux_if`; 9 lib proofs) +
  `Module.casez_mux_if_gates: BTreeSet<NodeId>` + `ir/mod.rs` registration +
  `Config::casez_mux_if_emit_prob` + `--casez-mux-if-emit-prob` flag + two guarded
  `gen/mod.rs` rolls (after `case_mux_if`) + the `emit/sv.rs` `GateOp::CasezMux` body branch
  (`casez…endcase` → masked `if`/`else if` computing per-arm `care_mask = ~wildcard_mask &
  sel_mask` + `value_masked = pattern_value & care_mask` via the existing
  `constant_value`/`bitmask` helpers, emitting `(sel & SW'h{care}) == SW'h{val}` + the
  trailing `else W'h0`; the `:769` trailing-default suppression also excludes marked casez
  gates). Implemented exactly per `.19a` (no deviations). **Simpler than the seventh surface
  / like the eighth** — a `CasezMux` is already an `always_comb`-written `logic` var, so no
  `__cv` passthrough, only the body swaps. No metric/schema bump (the `.19b.2` metric bumps
  `1.16 → 1.17`; the knob rides the version). `cargo check`/`clippy -D warnings`/`fmt` clean;
  `cargo test --lib` 626→635 (9 new proofs); snapshots 6/6 byte-identical. Forced
  `casez_mux_if_emit_prob=1.0` sweep (`scratchpad/sweep19`, 6 seeds): every `casez` block →
  a masked chain (0 residual `casez`); Verilator `-Wall` Δ=0 vs OFF (2012/2017/2023, 18
  comparisons) + both Yosys modes (0 issues) + Icarus (0 msg lines) clean; ON-vs-OFF
  sim-equiv (seed 1 8/8 + seed 4 128/128 with a real signal data arm, 0 mismatches).
  Default-off / DUT byte-identical. Frontier → `.19b.2`. The lane stays `active`.**) Prior: **`.19a` landed — the ninth-surface impl design-detail
  (no source change ⇒ DUT byte-identical): a `DEVELOPMENT_NOTES.md` entry grounding decision
  `0029` in a fresh read of the real `src/ir/case_mux_if_emit.rs` (the annotation-pass
  template), `src/emit/sv.rs` (the `GateOp::CasezMux` structured-case arm `:724`,
  `render_casez_pattern` `:2171`, the `constant_value`/`bitmask`/`casez_pattern_matches`
  helpers, the trailing-default suppression `:769`), `src/gen/cone/motifs.rs:832`
  (`build_casez_patterns`, `wildcard_bits=1`), `src/metrics.rs:2940` / `src/ir/compact.rs:603`
  (the `care_mask = (~wildcard_mask) & sel_mask` idiom), `src/gen/mod.rs` (the eighth-pass
  roll chain), `src/config.rs`, `src/main.rs`, `src/ir/types.rs`, `src/ir/mod.rs`,
  `src/introspect/mod.rs`. Pins all impl points: the **dynamic-selector** `CasezMux`
  candidate predicate (`operands[0]` not a `Node::Constant`, `operands.len() >= 4`; `CaseMux`
  + `case_mux_if`/sibling marks + `param_env` excluded); `Module.casez_mux_if_gates:
  BTreeSet<NodeId>`; the **in-place** emitter body branch `casez…endcase` → masked
  `if`/`else if` computing per-arm `care_mask = !wildcard_mask & sel_mask` + `value_masked =
  pattern_value & care_mask` from the arm's `(value, mask)` constants and emitting `(sel &
  SW'h{care}) == SW'h{val}` (**no `__cv`** — a `CasezMux` is already an `always_comb` var) +
  the trailing-default suppression tweak (`:769` also excludes marked casez gates); run
  `annotate_casez_mux_if_gates` last; the `casez_mux_if_emit_prob` knob + flag; the
  `num_emitted_casez_mux_if_chains` metric (schema `1.16 → 1.17`, exact); the **metric-keyed**
  `--casez-mux-if-gate` / `saw_casez_mux_if_emit` gate + a `casez_mux_prob`-biased focus
  config (zero `comb_mux_prob` + `case_mux_prob` — both roll earlier in `cone.rs`); the
  byte-identical/RNG argument. Pre-split `.19` → `.19a` (done) + `.19b` (impl, itself `.19b.1`
  live / `.19b.2` metric+gate / `.19b.3` docs); frontier → `.19b.1`. No source change ⇒ DUT
  byte-identical. The lane stays `active`.**) Prior: **`.18` landed — picked the NINTH structured surface (a
  ...
  before/after: a dynamic 1-bit-selector `CaseMux` parallel `case` → the `if (slice_0 ==
  1'd0) … else if … else …` chain, **no `__cv`**) + the chapter-intro surface list + the
  `case_mux_if_emit_prob` entry in `book/src/knobs.md` + USER_GUIDE + README "Current CLI
  truth" + the deferred book example-JSON `schema_version` `1.15 → 1.16` refresh (`api-tools`
  ×3 + `agent-mcp` ×1, plus the stale seed-42 `run_id` `3f1cad…` → `ee39c1e3df8192dd` — the
  `run_id` is an FNV hash over `lane+seed+knobs_json`, which gained `case_mux_if_emit_prob`
  at `.17b.1`; verified live: seed 42 → `ee39c1e3df8192dd` / schema `1.16` / 80383 bytes) +
  a `docs/knowledge/case-mux-if-emit.md` KM card (`KNOWLEDGE_MAP.md` regenerated). `mdbook
  build` clean; `cargo test --test book_examples` **3/3**; `check_knowledge_map.sh` OK; the
  card reverify runs CLEAN. Observed pre-existing drift NOT owned here: `agent-mcp.md`'s
  `analyze`-tool example JSONs sit at schema `1.11` (frozen across `.12`/`.13`/`.15`/`.17` —
  flagged for a dedicated docs pass). Lane returns to a no-frontier boundary; next surface
  (`CasezMux` masked chain / nested `generate` / `interface`) PNT-selected per
  `feedback_pick_and_roll_at_no_frontier`. The lane stays `active`.**) Prior: **`.17b.2b` landed — the eighth-surface repo-owned
  downstream gate `tool_matrix --case-mux-if-gate`, templated 1:1 on `--mux-if-gate` across
  ~30 touch points in `src/bin/tool_matrix.rs`: `CASE_MUX_IF_SWEEP_MIN_UNITS_PER_SCENARIO`
  + the `--case-mux-if-gate` flag + `ScenarioSet::CaseMuxIfSweep` +
  `build_case_mux_if_sweep_scenarios` + `case_mux_if_focus_config` + **METRIC-KEYED**
  `ModuleReport.emitted_case_mux_if` (`= prepared.metrics.num_emitted_case_mux_if_chains >
  0`, NOT a text token — a marked `CaseMux` emits no new identifier; a text scan for
  `if(==)` would also hit FSM decode blocks) + `CoverageSummary.saw_case_mux_if_emit` +
  merge + `MatrixReport.case_mux_if_gate` + run-plan/dispatch/mutual-exclusion/summarize
  block/gap arm/three exhaustive match arms/slug arms/`test_cli` default + 5 cargo-portable
  proofs + 8 fixtures. Focus config diverges from `--mux-if-gate` in ONE load-bearing way:
  `comb_mux_prob = 0.0` (not 0.1) + `case_mux_prob = 0.9`, because `comb_mux` rolls before
  `case_mux` in `cone.rs` and would otherwise starve the `case`-mux roll; no
  `comb_mux_encoding_prob` steering (a `CaseMux` selector is a dynamic cone by
  construction). `cargo check/clippy(-D warnings)/fmt` clean (one `doc_lazy_continuation`
  fixed — a wrapped doc line began with `+`); `cargo test --bin tool_matrix` 89→94 (5 new
  proofs); snapshots 6/6 byte-identical. Banked clean `anvil-case-mux-if-gate-r1`
  (3 scenarios / 12 modules / 12 emitting a chain / 83 chains / `coverage_gaps = []` /
  `saw_case_mux_if_emit = true` / 12/0 Verilator + both Yosys + Icarus). Default-off / DUT
  byte-identical. Frontier → `.17b.3` (user docs). The lane stays `active`.**) Prior: **`.17b.2a` landed — the eighth-surface metric +
  introspection schema bump (`1.15 → 1.16`): `Metrics::num_emitted_case_mux_if_chains` (`=
  m.case_mux_if_gates.len()`, `#[serde(default)]`) + a lib proof, surfaced at introspection
  schema `1.16` (all current-output refs bumped — `introspect` const + 3 assertions, 8
  `mcp` assertions, the schema doc current-version + a `1.15→1.16` changelog entry, README
  ×2 + USER_GUIDE ×1 + CODEBASE envelope; historical `num_emitted_mux_if_blocks @ 1.15`
  attributions intact; book JSONs deferred to `.17b.3`). The metric is **exact** (static
  selectors excluded). `cargo test --lib` 625→626 (the new proof; all schema assertions at
  `"1.16"`); snapshots 6/6 byte-identical; live `--introspect` shows `1.16` + the metric `0`
  default / `53` forced (seed 2). `.17b.2` pre-split → `.17b.2a` (this) + `.17b.2b` (the
  repo-owned `tool_matrix --case-mux-if-gate`); frontier → `.17b.2b`. Default-off / DUT
  byte-identical. The lane stays `active`.**) Prior: **`.17b.1` landed — the LIVE eighth structured surface (the
  first source change of the eighth surface). The procedural `always_comb` `if`/`else if`
  **priority chain** emit-projection of a dynamic-selector `CaseMux` gate (decision `0028`)
  goes live behind the opt-in `case_mux_if_emit_prob` knob: new `src/ir/case_mux_if_emit.rs`
  (`annotate_case_mux_if_gates` — the `GateOp::CaseMux` predicate requiring a non-`Constant`
  selector, excluding `CasezMux` + all sibling marks, run **last**; 9 lib proofs) +
  `Module.case_mux_if_gates: BTreeSet<NodeId>` + `ir/mod.rs` registration +
  `Config::case_mux_if_emit_prob` + `--case-mux-if-emit-prob` flag + two guarded
  `gen/mod.rs` rolls (after `mux_if`) + the `emit/sv.rs` structured-case body branch
  (`case…endcase` → `if`/`else if` over the same operand refs + `W'h0` default; parallel
  `case` suppressed for a marked gate). Implemented exactly per `.17a` (no deviations).
  **Simpler than the seventh surface** — a `CaseMux` is already an `always_comb`-written
  `logic` var, so no `__cv` passthrough, only the body swaps. No metric/schema bump (the
  `.17b.2` metric bumps `1.15 → 1.16`; the knob rides the version). `cargo check`/`clippy
  -D warnings`/`fmt` clean; `cargo test --lib` 616→625 (9 new proofs); `--bin tool_matrix`
  89 (unaffected); snapshots 6/6 byte-identical. Forced `case_mux_if_emit_prob=1.0` sweep
  (seed 2: 7 `CaseMux` → 7 chains): Verilator `-Wall` Δ=0 vs OFF (2012/2017/2023) + both
  Yosys + Icarus clean; generated-RTL ON-vs-OFF sim-equiv over 20000 vectors. Default-off /
  DUT byte-identical. Frontier → `.17b.2`. The lane stays `active`.**) Prior: **`.17a` landed — the eighth-surface impl design-detail
  (no source change): a `DEVELOPMENT_NOTES.md` entry grounding decision `0028` in a fresh
  read of the real `src/ir/mux_if_emit.rs` (the annotation-pass template),
  `src/emit/sv.rs` (`render_static_structured_gate`'s `CaseMux` static-collapse test
  `constant_value(operands[0]).is_some()` `:2077`; the structured-case `always_comb` loop
  `:667`–`:753`), `src/gen/mod.rs` (the seven-pass roll chain), `src/config.rs`,
  `src/main.rs`, `src/ir/types.rs`. Pins all seven impl points: the **dynamic-selector**
  `CaseMux` candidate predicate (`operands[0]` not a `Node::Constant`, the inverse of the
  static-collapse test; `CasezMux` + sibling marks + `param_env` excluded);
  `Module.case_mux_if_gates: BTreeSet<NodeId>`; the **in-place** emitter body branch
  `case…endcase`→`if…else if` reusing the same operand refs + `W'h0` default (**no `__cv`
  passthrough** — a `CaseMux` is already an `always_comb` var, the simpler-than-seventh
  case); run `annotate_case_mux_if_gates` last; the `case_mux_if_emit_prob` knob + flag;
  the `num_emitted_case_mux_if_chains` metric (schema `1.15 → 1.16`, exact because static
  selectors are excluded); the **metric-keyed** `--case-mux-if-gate` /
  `saw_case_mux_if_emit` gate + `case_mux_prob`-biased focus config; the byte-identical/RNG
  argument. Pre-split `.17` → `.17a` (done) + `.17b` (impl, itself `.17b.1` live / `.17b.2`
  metric+gate / `.17b.3` docs); frontier → `.17b.1`. No source change ⇒ DUT byte-identical.
  The lane stays `active`.**) Prior: **`.16` landed — picked the EIGHTH structured surface
  (a procedural `always_comb` `if`/`else if` **priority chain** emit-projection of a
  `CaseMux` gate, decision
  [`0028`](../decisions/0028-structured-emission-eighth-surface-case-mux-priority-chain.md))
  autonomously at the no-frontier boundary per `feedback_pick_and_roll_at_no_frontier`;
  design/decision leaf, no source change; frontier → `.17` (impl, pre-split
  `.17a`/`.17b`). The N-way `CaseMux` — rendered today as the parallel `case (sel) …
  default` statement — is projected into an `if`/`else if` priority chain writing the
  gate's existing `always_comb`-written `logic` var, **no `__cv` passthrough** (the
  simpler-than-seventh case: the `CaseMux` is already an `always_comb` var). It is
  genuinely distinct from the seventh surface (the N-way `CaseMux` chain vs the single
  2:1 `Mux` conditional) and from the existing `case` render (sequential-priority vs
  parallel-match). Reuses the structured-case `always_comb` section + the operand-ref
  rendering. Fresh probe this session (`scratchpad/probe8/`): Verilator `-Wall`
  2012/2017/2023 + both Yosys modes (32 cells, no warnings) + Icarus clean, iverilog
  `vvp` sim-equiv to the parallel `case` over 20000 vectors + an exhaustive selector
  sweep (`EQUIV OK`). Own `case_mux_if_emit_prob` knob + `num_emitted_case_mux_if_chains`
  metric (schema `1.15 → 1.16` at impl) + `--case-mux-if-gate` / `saw_case_mux_if_emit`
  (metric-keyed, no new text token); the `CasezMux` masked chain recorded `.18+`. Decision
  `0028` + `INDEX.md` row + regenerated `KNOWLEDGE_MAP.md`; the stale parent `.15`/`.15b`
  flipped `active`→`done` (children all done; the `.5`/`.6`/`.6b`/`.13` closed-parent
  convention); ROADMAP owner-directed-lane #2 + `docs/TASK_TREE.md` row updated.
  Default-off / DUT byte-identical. The lane stays `active`.**) Prior: **`.15b.3` landed — the user docs for the seventh
  ...
  `introspect/mod.rs` const + 3 lib assertions, 8 `mcp/mod.rs` assertions,
  `docs/AGENT_INTROSPECTION_SCHEMA.md` §7 current-version + a `1.14 → 1.15` changelog
  entry, README + USER_GUIDE envelope refs, `CODEBASE_ANALYSIS.md` envelope changelog;
  historical attributions [`num_emitted_multi_output_tasks` @ `1.14`] intact; book
  example JSONs deferred to `.15b.3` per the `.12b.2a`/`.12b.3` precedent) + the
  repo-owned `tool_matrix --mux-if-gate` (`ScenarioSet::MuxIfSweep` + a Mux-biased
  `mux_if_focus_config` [comb-only, `comb_mux_prob = 0.9` + `comb_mux_encoding_prob = 1.0`
  forcing the encoded chained-ternary path that builds plain `GateOp::Mux` gates] × the
  three strategies + `ModuleReport.emitted_mux_if` [`__cv` detection] + `saw_mux_if_emit`
  + `MatrixReport.mux_if_gate` + the gap arm + `MUX_IF_SWEEP_MIN_UNITS_PER_SCENARIO` units
  floor + 5 cargo-portable proofs + 8 fixture extensions), banked clean at
  `anvil-mux-if-gate-r1` (3 scenarios / 12 modules, `coverage_gaps = []`,
  `saw_mux_if_emit = true`, 12/12 modules emit `__cv` / 215 blocks, Verilator 12/0 +
  both Yosys 12/0 + Icarus 12/0). Templated on `--multi-output-task-gate`. Default-off /
  DUT byte-identical (`cargo test --lib` 615→616, snapshots 6/6, the `0.0` path never
  marks). Frontier → `.15b.3` (user docs: book/knobs/USER_GUIDE/README/KM). The lane
  stays `active`.**) Prior: **`.15b.1` landed — the LIVE seventh structured surface
  (the first source change). The procedural `always_comb` `if`/`else` emit-projection
  of a 2:1 `Mux` gate (decision `0027`) goes live: new `src/ir/mux_if_emit.rs`
  (`annotate_mux_if_gates` — the `GateOp::Mux` candidate predicate excluding all six
  sibling marks, run **last**; 10 lib proofs) + `Module.mux_if_gates: BTreeSet<NodeId>`
  + `ir/mod.rs` registration + `Config::mux_if_emit_prob` knob + `--mux-if-emit-prob`
  flag + two guarded `gen/mod.rs` rolls (after cone_function, single + design) +
  the `emit/sv.rs` procedural-block section (`logic [w-1:0] <name>__cv;` +
  `always_comb begin if (sel) <name>__cv = a; else <name>__cv = b; end`, operand refs
  via `node_ref`) + the gate-assign-loop `<name>__cv` passthrough. Implemented exactly
  per the `.15a` design (no deviations). No metric/schema bump (the `.15b.2` metric
  bumps `1.14 → 1.15`; the knob rides the version). Default-off / DUT byte-identical
  (`cargo test --lib` 605→615 incl. 10 new proofs; snapshots 6/6). Forced
  `mux_if_emit_prob=1.0` sweep (`anvil-muxif-genproof.*`): every plain mux
  (121–195/seed across 5 seeds) projects to a `__cv` block; Verilator `-Wall` Δ=0 vs
  OFF (2012/2017/2023) + Yosys both modes (no warnings) + Icarus clean; a comb-only
  ON-vs-OFF run is iverilog-sim-equiv over 20000 vectors (3 outputs). Frontier →
  `.15b.2` (metric + `--mux-if-gate`). The lane stays `active`.** Prior: **`.15a` landed — the seventh-surface impl
  design-detail (no source): a `DEVELOPMENT_NOTES.md` entry grounding decision `0027`
  in a fresh read of the real `src/ir/task_emit.rs` (the candidate-predicate +
  one-roll-per-candidate template), `src/emit/sv.rs` (the `node_ref` operand
  resolver at `~644`, the task decl/call section at `~444`, the gate-assign-loop
  passthrough at `~578`, the procedural-block section at `~620`), `src/gen/mod.rs`
  (the six-pass roll chain at `~95`/`~330`), `src/config.rs` (the `*_emit_prob`
  knobs), `src/ir/types.rs` (the `*_gates` carriers). Pinned all seven impl points:
  (0) the `GateOp::Mux` candidate predicate excluding all six sibling marks (pass
  runs last); (1) `Module.mux_if_gates: BTreeSet<NodeId>`; (2) the new
  procedural-block emit section (`logic [w-1:0] <name>__cv;` + `always_comb begin if
  (sel) <name>__cv = a; else <name>__cv = b; end`, operand refs via the existing
  `node_ref`) + the gate-assign-loop `<name>__cv` passthrough branch; (3) run
  `annotate_mux_if_gates` **last** (minimal blast radius — no other pass changes);
  (4) `mux_if_emit_prob` knob + `--mux-if-emit-prob` flag; (5)
  `num_emitted_mux_if_blocks` metric (schema `1.14 → 1.15`); (6)
  `tool_matrix --mux-if-gate` / `saw_mux_if_emit` (`__cv` detection) + gate-shape
  calibration; (7) the byte-identical/RNG argument. Pre-split `.15` → `.15b` (impl,
  itself `.15b.1` live / `.15b.2` metric+gate / `.15b.3` docs); frontier → `.15b.1`.
  No source change. The lane stays `active`.** Prior: **`.14` landed — picked the SEVENTH structured surface
  ...
  Docs-only / DUT byte-identical: a `### Wider groups (k > 2)` subsection in
  `book/src/structured-emission.md` with a real seed-22 three-member task + the
  connected-co-support / all-member-independence rules + the 8-member cap; the
  sixth-surface intro + "What gets wrapped" + "How anvil proves it" generalized
  pair → group; the `multi_output_task_emit_prob` knob entry updated in
  `book/src/knobs.md` / `USER_GUIDE.md` / README; the KM card `multi-output-task-emit`
  refreshed (4 new answers, body pair → k>=2; KM 589 → 593 keys). `mdbook build` +
  `check_knowledge_map` + `check_memory_architecture` + `book_examples` 3/3 green.**
  Prior: **`.13b` landed — the LIVE k>2 widening of the sixth
  surface (the multi-output combinational `task automatic`). The whole change is in
  `src/ir/multi_output_task_emit.rs`: a `const MAX_MULTI_OUTPUT_TASK_GROUP_MEMBERS = 8`
  + two helpers (`connected_co_support` / `independent_of_all`) + the partner loop
  rewritten "find first + break" → "greedily admit every eligible partner up to the
  cap" (a group forms iff `>= 1` partner ⇒ `k=2` is the exact subset). **No
  emitter / config / metric / schema / knob change** (the emitter was already
  k-agnostic). 5 new k>2 lib proofs + all 10 pair proofs green (15 total); doc
  comments + `CODEBASE_ANALYSIS.md` brought current. Default-off / DUT
  byte-identical (`cargo test --lib` 600→605, snapshots 6/6). Gate re-bank
  `anvil-mo-k3-gate-r1` `coverage_gaps = []` / `12/0` all tools with a k=3
  group present; forced sweep seed 22 emits a genuine k=3 task, Verilator `-Wall`
  Δ=0 vs OFF + iverilog sim-equiv over 20000 vectors. Frontier → `.13c` (user
  docs).** Prior: **`.13a` landed — the design-detail (the greedy group-extension,
  connected co-support, generalized mutual fan-in independence, the const cap, the
  no-new-knob/metric/schema decision, the byte-identical/RNG argument).** Prior:
  **`.12b.3` landed — the multi-output task user-facing
  docs; `.12b.3` / `.12b` / `.12` all close — the SIXTH structured surface (the
  multi-output combinational `task automatic`, decision `0025`) is **delivered
  end-to-end**; the lane returned to no current frontier (open-ended; future
  surfaces `.13+`). Docs-only / DUT byte-identical: a `## The sixth surface` section
  in `book/src/structured-emission.md` (byte-verified seed-3 before/after) + the
  `multi_output_task_emit_prob` knob entry in `book/src/knobs.md` / `USER_GUIDE.md` /
  README + the KM card `multi-output-task-emit` (62 facts / 565 keys) + the deferred
  book example-JSON `schema_version` `1.13 → 1.14` refresh; `mdbook build` +
  `check_knowledge_map` + `check_memory_architecture` + `book_examples` 3/3 green.**
  Prior: **`.12b.2b` landed — the repo-owned `tool_matrix
  --multi-output-task-gate` (templated on `--cone-function-gate`):
  `ScenarioSet::MultiOutputTaskSweep` + `build_multi_output_task_sweep_scenarios`/
  `multi_output_task_focus_config` (comb-only `multi_output_task_emit_prob=1.0` × 3
  strategies; high `terminal_reuse_prob=0.6` + shallow `max_depth=2` +
  `min_outputs>=2`) + `ModuleReport.emitted_multi_output_task` (`__mt(` detection) +
  `saw_multi_output_task_emit` + `MatrixReport.multi_output_task_gate` + early-return
  gap arm + units floor + mutual-exclusion + 5 proofs + 8 ModuleReport + 1 Cli
  fixtures + README/USER_GUIDE/CODEBASE gate entries. **Banked clean**
  `anvil-multi-output-task-gate-r1` (3 scenarios / 12 modules / 6 emitting /
  `coverage_gaps=[]` / `12/0` all tools). `cargo test --bin tool_matrix` 84 +
  snapshots 6/6. `.12b.2b` / `.12b.2` close; frontier → `.12b.3` (docs).** Prior:
  **`.12b.2a` landed — the multi-output task metric +
  introspection schema `1.13 → 1.14`: `Metrics::num_emitted_multi_output_tasks`
  (= `m.multi_output_task_groups.len()`) in `module_metrics` ⇒ `SCHEMA_VERSION`
  `1.13→1.14` (doc comment + const + 3 introspect + 8 mcp assertions + schema doc +
  README + USER_GUIDE current refs + the drifted CODEBASE envelope brought current;
  book JSONs deferred to `.12b.3`) + a lib proof; `cargo test --lib` 600 +
  snapshots 6/6; end-to-end `--introspect` default 0 / forced 1 at schema 1.14.
  `.12b.2` split `.12b.2a` (done) + `.12b.2b` (the `tool_matrix` gate); frontier →
  `.12b.2b`.** Prior: **`.12b.1` landed — the multi-output combinational
  `task automatic` LIVE surface (first source change): `multi_output_task_emit_prob`
  knob + `--multi-output-task-emit-prob` flag + `Module.multi_output_task_groups` +
  new `src/ir/multi_output_task_emit.rs` (one-roll-per-leader pairing on a shared
  non-constant operand + the `in_fanin` bounded-DFS soundness check) + two
  `gen/mod.rs` rolls (after `task_emit`, before `cone_function`) +
  `cone_function` sibling exclusion + `src/emit/sv.rs` decl/call section (reusing
  `render_cone_gate_expr` with an empty interior_set) + per-gate passthrough + 11
  lib proofs. Default-off / DUT byte-identical (lib 599, snapshots 6/6); forced
  sweep `anvil-mo-sweep/` Verilator -Wall Δ=0 + Yosys both + Icarus clean +
  exhaustive sim-equiv. No metric/schema bump (knob rides the version; metric+gate
  at `.12b.2`). `.12b` split `.12b.1` (done) + `.12b.2` + `.12b.3`; frontier →
  `.12b.2`.** Prior: **`.12a` landed — the multi-output task impl
  design-detail (no source): a `DEVELOPMENT_NOTES.md` entry grounding decision `0025`
  in the real `task_emit.rs`/`cone_function_emit.rs`/`sv.rs`/`gen/mod.rs`/`config.rs`
  and pinning all seven `.12b` impl choices (replicate predicate;
  `multi_output_task_groups` BTreeMap; one-roll-per-leader pairing on a shared
  non-constant operand; the `in_fanin` bounded-DFS soundness check; dedup-formal
  rendering reusing the cone primitives + `__mt`/`__mtv`; run after `task_emit`
  before `cone_function`; knob/flag/metric-`1.13→1.14`/gate). `.12` split `.12a`
  (done) + `.12b` (impl pending, pre-split `.12b.1`/`.12b.2`/`.12b.3`); frontier →
  `.12b`.** Prior: **`.11` landed — picked the SIXTH structured surface
  ...
  `## The fifth surface: a multi-gate-cone function automatic` section in
  `book/src/structured-emission.md` (byte-verified seed-4 before/after — the
  inline `xor_0 = i_1 ^ i_3; sub_0 = i_2 - xor_0;` cone becomes one `function
  automatic sub_0__cf(a0,a1,a2)` with a local `xor_0 = a0 ^ a2;` + return + the
  call, interior wire+assign suppressed; the single-use-interior rule; the
  separate `cone_function_emit_prob` knob; the five-way mutual exclusion; metric @
  schema `1.11` + the gate; a skip-sentinelled repro) + the
  `cone_function_emit_prob` knob entry in `book/src/knobs.md` / `USER_GUIDE.md` /
  README + the KM how-to card `multi-gate-cone-function-emit` (KM 43→44 facts /
  351→364 keys). `mdbook build` + `check_knowledge_map` + `check_memory_architecture`
  + `book_examples` 3/3 green; book before/after byte-verified + downstream-clean.
  FIVE structured surfaces delivered end-to-end. Prior: **`.10b.2` landed — the
  cone-function metric
  (`Metrics::num_emitted_cone_functions`, introspection schema `1.10 → 1.11`) +
  the repo-owned `tool_matrix --cone-function-gate`; `.10b.2` closes; frontier →
  `.10b.3` (user docs).** Metric is the new derived `Metrics` field
  (`= m.cone_function_gates.len()`, `#[serde(default)]`) surfaced in
  `module_metrics` ⇒ `SCHEMA_VERSION` `1.10 → 1.11` (the `.6b.2a` precedent: the
  metric bumps; the `.10b.1` knob rode the version); bumped all current-output
  schema refs (9 assertions + schema doc + README + USER_GUIDE + 5 book example
  JSONs + CODEBASE envelope/changelog), historical attributions intact. Gate is
  ...
  strategies; `terminal_reuse_prob=0.3` keeps single-use cone interiors plentiful)
  + `ModuleReport.emitted_cone_function` (`__cf(` detection, distinct from the
  single-gate `__f(`) + `saw_cone_function_emit` + `MatrixReport.cone_function_gate`
  + early-return gap arm + 5 proofs + 6 fixtures + `test_cli` default, templated on
  `--task-emit-gate`. Banked clean `anvil-cone-function-gate-r1` (3 scenarios
  / 12 modules / 12 emitting a cone function / 148 cone functions /
  `coverage_gaps=[]` / `12/0` Verilator + both Yosys + Icarus). `cargo test --lib`
  502 / `--bin tool_matrix` 73 / snapshots 6/6 byte-identical; end-to-end
  `--introspect` default `0` / forced `14` at schema `1.11`. Default-off / DUT
  byte-identical. Prior: **`.10b.1` landed — the multi-gate-cone `function
  automatic` LIVE surface (the first source change of the fifth surface); `.10b`
  split into `.10b.1` (done) + `.10b.2` (metric+gate) + `.10b.3` (docs); frontier
  → `.10b.2`.** Live emitter change: `Config::cone_function_emit_prob` (separate
  from `function_emit_prob`) + `Module.cone_function_gates: BTreeMap<NodeId,
  Vec<NodeId>>` + new `src/ir/cone_function_emit.rs` (use-count + single-use
  post-order cone-walk + greedy selection + interior reservation, run last) + two
  `gen/mod.rs` rolls + the `src/emit/sv.rs` cone-decl section + interior-suppression
  (net-decl + gate-assign) + 4 render helpers + 8 lib proofs. Two impl wrinkles:
  `node_ref` intrinsic names ⇒ a dedicated `render_cone_gate_expr`; an absorbed
  interior suppresses its module **wire** + assign (`use_count==1` safe). No metric
  / no schema bump (those at `.10b.2`). Default-off / DUT byte-identical (`cargo
  test --lib` 493→501; snapshots 6/6). Forced sweep `anvil-cf-sweep/` (8 seeds,
  18 cone functions): Verilator `-Wall` Δ=0 vs OFF + Yosys both + Icarus clean.
  Prior: **`.10a` landed — the multi-gate-cone `function
  automatic` impl design-detail; `.10` split into `.10a` (done) + `.10b` (impl
  pending); frontier → `.10b`.** Design-detail leaf, no source change (a
  `DEVELOPMENT_NOTES.md` entry). Grounded decision `0016` in the real
  `src/ir/function_emit.rs` (`gate_qualifies`), `src/introspect/analyze.rs`
  (`build_cone`/`visit` — the post-order walk + support-leaf boundary),
  `src/emit/sv.rs` (`render_gate_function_decl`/`_body`/`_call`), `src/gen/mod.rs`
  (run cone-function last in the roll chain), and `src/config.rs` (the knob list),
  resolving all five open questions: separate `cone_function_emit_prob` knob;
  walk-roots/roll-per-cone selection with interior reservation; absorb only
  admissible (non-structured/non-`Slice`), non-sibling-marked,
  single-use-within-cone interior gates (else boundary param; `Constant` folds
  inline); post-order topo body + module-wire-name locals + ascending-NodeId
  params; run last + `Module.cone_function_gates: BTreeMap<NodeId, Vec<NodeId>>` +
  emitter interior-suppression; `num_emitted_cone_functions` (schema `1.10 →
  1.11`) + `--cone-function-gate` / `saw_cone_function_emit`. Prior: **`.9` landed
  — picked the FIFTH structured surface (the multi-gate-cone `function
  automatic`); decision `0016`; design/decision leaf, no source change; frontier →
  `.10` (impl, pre-split `.10a`/`.10b`).** At a no-active-frontier boundary, autonomously selected per
  `feedback_pick_and_roll_at_no_frontier`. A fresh empirical probe
  (`anvil-se9-probe/`, Verilator 5.046 `-Wall` + Yosys 0.64 both modes +
  ...
  sweep (8 seeds, `anvil-gl8b/`) emits 9 wider-lane part-selects with
  ...
  proven**: a 300-module comb-only sweep (`anvil-widelane-probe/`) emits 20
  ...
  entries. Banked clean `anvil-task-emit-gate-r1` (3 scenarios / 12 modules /
  ...
  (`anvil-te-r1/`, 5 seeds, 4–39 tasks each). Prior: **`.6a` landed — the
  ...
  (`-Wall` Δ=0 vs OFF) + both Yosys + Icarus (`anvil-gl-r1/`, 5 seeds,
  ...
  **`.3` landed — picked the SECOND structured
  ...
  5 cargo-portable proofs; banked clean `anvil-function-emit-gate-r1` (3
...
- ID: `STRUCTURED-EMISSION-EXPANSION`
  Status: `active`
  Goal: `Richer structured synthesizable SV surfaces (functions / generate / tasks / interfaces), valid-by-construction. NINE surfaces delivered end-to-end: combinational function automatic (.1+.2), generate for loop (.3+.4), combinational task automatic (.5+.6), the wider-lane generate for part-select (.7+.8, decision 0015), the multi-gate-cone function automatic (.9+.10, decision 0016), the multi-output combinational task automatic (.11+.12 incl. the wider k>2 deepening .13, decision 0025), the procedural always_comb if/else projection of a 2:1 Mux (.14+.15, decision 0027), the procedural always_comb if/else if priority chain of a plain CaseMux (.16+.17, decision 0028), and the procedural always_comb if/else if MASKED priority chain of a wildcard CasezMux (.18+.19, decision 0029). Further future surfaces (nested-multi-level generate / interface-modport) remain .20+, each its own decision; the tree stays active with no current frontier.`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.2`
  ...
  Result: `Done (closed by .2b.2c, 2026-06-16). The combinational function automatic emit-projection is delivered end-to-end: the function_emit_prob knob + Module.function_emit_gates + the gen-time annotate_function_emit_gates selection + the to_sv_with_modules <wire>__f decl/positional-body/call rendering (.2b.1), the num_emitted_combinational_functions metric + introspection schema 1.8 (.2b.2a), the repo-owned tool_matrix --function-emit-gate downstream-clean gate (.2b.2b, banked anvil-function-emit-gate-r1), and the book/USER_GUIDE/README/KM user-facing closeout (.2b.2c). Default-off / DUT byte-identical throughout (snapshots 6/6). Nothing retired.`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.4`
  ...
  Result: `Done (closed by .4b.3, 2026-06-16). The second structured surface — the generate for loop emit-projection of a {N{x}} 1-bit-lane replication (decision 0013) — is delivered end-to-end: .4a (design-detail) + .4b.1 (live surface: generate_loop_emit_prob knob + Module.generate_loop_gates + src/ir/generate_loop.rs + the to_sv_with_modules rendering + 9 lib proofs) + .4b.2a (num_emitted_generate_loops metric + introspection schema 1.9) + .4b.2b (the repo-owned tool_matrix --generate-loop-gate, banked anvil-generate-loop-gate-r1) + .4b.3 (book/knobs/USER_GUIDE/README/KM closeout). Default-off / DUT byte-identical throughout (snapshots 6/6). Nothing retired.`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.4b`
  ...
  Result: `Done (closed by .4b.3, 2026-06-16). All of .4b.1 (live surface), .4b.2 (.4b.2a metric + schema 1.9 + .4b.2b the tool_matrix gate), and .4b.3 (docs closeout) complete. The generate for loop emit-projection is delivered end-to-end and downstream-clean (banked anvil-generate-loop-gate-r1). Default-off / DUT byte-identical.`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.4b.1`
  ...
  Verification: `cargo check --all-targets clean; cargo clippy --all-targets -- -D warnings clean; cargo fmt --all --check clean; cargo test --lib 477 passed / 2 ignored (incl. 9 new generate_loop proofs; introspect schema_version 1.8 + umbrella DUT-byte-identical still green); cargo test --test snapshots 6/6 byte-identical (default-off). Forced generate_loop_emit_prob=1.0 sweep (5 seeds 1-5, 62-168 loops each, banked anvil-gl-r1/): Verilator --lint-only 5/5 rc=0 / 0 warnings (repo bar), -Wall ON-vs-OFF delta = 0 (change adds no new warnings; residual -Wall UNUSEDSIGNAL is pre-existing, identical ON and OFF), Yosys without-abc 5/5, with-abc 5/5, Icarus iverilog -g2012 5/5.`
  Commit: `done`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.4b.2`
  ...
  Result: `Done (closed by .4b.2b, 2026-06-16). .4b.2a (metric + introspection schema 1.9) + .4b.2b (the repo-owned tool_matrix --generate-loop-gate, banked clean anvil-generate-loop-gate-r1) both complete. Default-off / DUT byte-identical.`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.6b.1`
  ...
  Verification: `cargo check --all-targets clean; cargo clippy --all-targets -- -D warnings clean; cargo fmt --all --check clean; cargo test --lib 489 passed / 2 ignored (incl. 11 new task_emit proofs; introspect schema_version 1.9 + umbrella DUT-byte-identical still green); cargo test --test snapshots 6/6 byte-identical (default-off). Forced task_emit_prob=1.0 sweep (5 seeds: 1/7/42/100/2024, 4-39 tasks each, banked anvil-te-r1/): Verilator --lint-only 5/5 CLEAN (repo bar) + -Wall ON-vs-OFF delta = 0 (the task projection adds no new warnings), Yosys without-abc 5/5 + with-abc 5/5, Icarus iverilog -g2012 5/5 CLEAN. (Tools: Verilator 5.046, Yosys 0.64, iverilog.)`
  Commit: `done`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.6b.2`
  ...
  Result: `Done (closed by .6b.2b, 2026-06-16). .6b.2a (the num_emitted_combinational_tasks metric + introspection schema 1.10) + .6b.2b (the repo-owned tool_matrix --task-emit-gate, banked clean anvil-task-emit-gate-r1) both complete. Default-off / DUT byte-identical.`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.10a`
  ...
  Acceptance: `A DEVELOPMENT_NOTES design-detail entry resolving the five points grounded in real code; tree split recorded; no source change; docs/workflow self-checks clean.`
  Result: `Done. DEVELOPMENT_NOTES design-detail entry resolves all five open questions grounded in a fresh read of src/ir/function_emit.rs (gate_qualifies + annotate_function_emit_gates), src/introspect/analyze.rs (build_cone ~729 + visit ~769 — the post-order fan-in walk + support-leaf boundary rules), src/emit/sv.rs (render_gate_function_decl/_body/_call ~1293-1399 + the function-decl section ~371 + the gate-assign inline-suppression ~440-500), src/gen/mod.rs (the soft_union->function_emit->generate_loop->task_emit roll chain ~91-125 + the design mirror ~293-327), src/config.rs (the *_emit_prob knob defaults/validation/dump-config). RESOLVED: (1) new Config::cone_function_emit_prob (default 0.0, config-file-only, separate from function_emit_prob); selection = walk root candidates (admissible op, not sibling-marked, cone has >= 1 absorbed interior gate) and gen_bool(p) per candidate in node order, reserving absorbed interiors so a later root cannot re-absorb them; param_env modules skipped. (2) Purpose-built post-order walk (mirrors analyze::visit's structure, does NOT call it — private + doc-shaped); a Gate is absorbed iff admissible (non-structured, non-Slice) AND not sibling-marked/reserved AND single-use-within-the-cone; otherwise it is a boundary param (keeps its inline assign); structural leaves (input/flopQ/instanceOutput) are params, Constant folds inline as a literal, the root is the return. Guarantees -Wall clean (every param used; multi-use gates stay external). (3) post-order DFS gives topo order; one `logic <w> <wire>;` local per absorbed interior gate (reuse the module wire name), assigns in topo order, then `<root>__cf = <root-expr>;`; body maps each operand NodeId -> in-function name (boundary leaf -> param aI [params ordered by ascending NodeId, one per distinct boundary], interior gate -> local wire name, Constant -> literal). (4) run annotate_cone_function_gates LAST (after task_emit) in both call-site chains, guarded on prob > 0.0 (default byte-identical); sibling-marked gates are never a root and never absorbed; IR change Module.cone_function_gates: BTreeMap<NodeId, Vec<NodeId>> (root -> topo-ordered absorbed interior gate ids); the emitter renders render_cone_function_decl + the call and suppresses every absorbed interior gate's inline assign. (5) Metrics::num_emitted_cone_functions (= cone_function_gates.len()) => schema 1.10 -> 1.11; tool_matrix --cone-function-gate + ScenarioSet::ConeFunctionSweep + ModuleReport.emitted_cone_function (detect __cf() ) + saw_cone_function_emit, templated on --function-emit-gate, full Verilator + both Yosys + Icarus. .10b impl shape recorded (new src/ir/cone_function_emit.rs + Config + Module + emitter + lib proofs + metric/schema + gate + book/USER_GUIDE/KM; pre-split .10b.1 live / .10b.2 metric+gate / .10b.3 docs if warranted). No source change.`
  Verification: `done`
  Commit: `done`

- ID: `STRUCTURED-EMISSION-EXPANSION.10b`
  Status: `active`
  Goal: `Implement the .10a design: the cone_function_emit_prob knob + the new annotate_cone_function_gates pass + the multi-statement function rendering + lib proofs + the num_emitted_cone_functions metric + schema 1.10 -> 1.11 + the --cone-function-gate proof + book/USER_GUIDE/KM closeout. Default-off / DUT byte-identical. Pre-split into .10b.1 (live surface), .10b.2 (metric + gate), .10b.3 (user docs).`
  Children: `STRUCTURED-EMISSION-EXPANSION.10b.1`, `STRUCTURED-EMISSION-EXPANSION.10b.2`, `STRUCTURED-EMISSION-EXPANSION.10b.3`

- ID: `STRUCTURED-EMISSION-EXPANSION.10b.1`
  Status: `done`
  Goal: `The live cone-function emit surface: Config::cone_function_emit_prob (separate from function_emit_prob) + Module.cone_function_gates + new src/ir/cone_function_emit.rs (annotate_cone_function_gates — use-count, single-use post-order cone-walk, greedy node-order selection with interior reservation, run last) + two guarded gen/mod.rs call-site rolls (single + design, after task_emit) + the src/emit/sv.rs cone-decl section + interior-suppression (net-decl + gate-assign loops) + the render helpers + lib proofs. Default-off / DUT byte-identical. No metric / no schema bump (those land at .10b.2).`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --lib green incl. new cone-function proofs + the single-gate function_emit proofs unchanged; cargo test --test snapshots 6/6 byte-identical (default-off); a forced cone_function_emit_prob=1.0 sweep emits cone functions accepted warning-clean by Verilator -Wall + both Yosys + Icarus (-Wall delta=0 vs OFF); committed through COMMIT.md with the leaf id.`
  Result: `Done. Live emitter change: src/config.rs cone_function_emit_prob knob (default 0.0, config-file-only, #[serde(default)], 0.0..=1.0 validated, in dump-config) + src/ir/types.rs Module.cone_function_gates: BTreeMap<NodeId, Vec<NodeId>> (root -> topo-ordered absorbed interiors) + new src/ir/cone_function_emit.rs (annotate_cone_function_gates: compute_use_counts over ALL value-consumer sites [gate operands + drives + flop d/mux OneHot+Encoded + instance inputs]; admissible/sibling_marked/should_absorb predicates; post-order absorb_children single-use walk; greedy node-order selection reserving committed roots+interiors) + src/ir/mod.rs registration + src/gen/mod.rs two guarded rolls (after task_emit, single + design paths) + src/emit/sv.rs (a cone_interior set built once; the net-decl loop and gate-assign loop both skip absorbed interiors; a cone-decl section; the root call; four new helpers render_cone_function_decl/render_cone_function_call/render_cone_gate_expr/cone_function_params + cone_operand_ref) + 8 lib proofs. TWO impl wrinkles handled (recorded in DEVELOPMENT_NOTES): (1) node_ref resolves PrimaryInput/Constant/FlopQ intrinsically (ignores names[]), so the body needs a dedicated resolver-based render_cone_gate_expr, NOT a render_gate names-override; (2) an absorbed interior must suppress its module WIRE decl AND inline assign (use_count==1 makes this safe). No metric / no schema bump (the metric num_emitted_cone_functions + schema 1.10->1.11 land at .10b.2; the knob rides the version, the .6b.1/.4b.1 precedent). Default-off / DUT byte-identical (snapshots 6/6; cargo test --lib 493 -> 501). Forced cone_function_emit_prob=1.0 sweep (anvil-cf-sweep/, 8 seeds): 18 cone functions emitted, Verilator --lint-only -Wall delta=0 vs OFF on every seed, Yosys without-abc + with-abc + Icarus all clean. Nothing retired.`
  Verification: `done`
  Commit: `done`

- ID: `STRUCTURED-EMISSION-EXPANSION.10b.2`
  Status: `done`
  Goal: `Metric + repo-owned gate: Metrics::num_emitted_cone_functions (= m.cone_function_gates.len(), #[serde(default)]) surfaced in introspection module_metrics => SCHEMA_VERSION 1.10 -> 1.11 (bump all current-output schema refs: test assertions + schema doc + README + USER_GUIDE + book example JSONs + CODEBASE_ANALYSIS envelope line; historical attributions intact) + a lib proof; the tool_matrix --cone-function-gate (ScenarioSet::ConeFunctionSweep + build_cone_function_sweep_scenarios [comb-only cone_function_emit_prob=1.0 x 3 strategies] + ModuleReport.emitted_cone_function [__cf( SV-text detection] + saw_cone_function_emit + MatrixReport.cone_function_gate + early-return gap arm + proofs + fixture updates) + README/USER_GUIDE/CODEBASE_ANALYSIS gate entries. Banked clean (Verilator + both Yosys + Icarus). Default-off / DUT byte-identical (snapshots untouched).`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --lib + --bin tool_matrix green incl. the metric proof + the new gate proofs; snapshots 6/6 byte-identical; the --cone-function-gate bank clean and proves a cone function emitted + accepted; end-to-end --introspect default 0 / forced N at schema 1.11; committed through COMMIT.md with the leaf id.`
  Result: `Done. Metric: Metrics::num_emitted_cone_functions: usize (#[serde(default)]) added to src/metrics.rs, computed in metrics::compute() as m.cone_function_gates.len() (a post-hoc structural count of an emitter-surface annotation; 0 by default, the configured count when cone_function_emit_prob fired); separate from num_emitted_combinational_functions (the single-gate surface). Surfaced in introspection module_metrics (Metrics is the exact serde projection), so SCHEMA_VERSION bumped 1.10 -> 1.11 in src/introspect/mod.rs (the new derived Metrics field BUMPS the schema — the .6b.2a/.4b.2a/.2b.2a precedent; the .10b.1 knob rode the version via #[serde(default)]). MINOR is an integer ⇒ 1.10 -> 1.11 (eleven), not a decimal — recorded in the doc comment + schema-doc changelog. Bumped all current-output schema refs to 1.11: the 9 schema_version assertions (2 in src/introspect/mod.rs + 7 in src/mcp/mod.rs), the schema doc (new 1.10->1.11 changelog entry + the early-example/defines/checklist lines), README (--introspect + analyze current refs), USER_GUIDE (--sv-version --introspect row), the 5 book agent-mcp.md example JSONs, and the CODEBASE_ANALYSIS envelope line + changelog list. Historical "landed at schema X" attributions left intact (num_emitted_combinational_tasks @ 1.10; num_emitted_generate_loops @ 1.9; num_emitted_combinational_functions @ 1.8; sv-version @ 1.2). Lib proof metrics_count_emitted_cone_functions (unmarked 0, marked 1). Gate: src/bin/tool_matrix.rs gains --cone-function-gate + ScenarioSet::ConeFunctionSweep + build_cone_function_sweep_scenarios/cone_function_focus_config (comb-only cone_function_emit_prob=1.0 x 3 construction strategies; terminal_reuse_prob=0.3 keeps single-use cone interiors plentiful — the one deliberate deviation from the task/function focus shape, since the cone surface only absorbs single-use interiors and high reuse drives CSE sharing that turns them multi-use) + ModuleReport.emitted_cone_function ("__cf(" SV-text detection, distinct from the single-gate "__f(" surface) + CoverageSummary.saw_cone_function_emit (lit on Verilator + Yosys acceptance) + MatrixReport.cone_function_gate + the early-return gap arm + mutual-exclusion + units floor (CONE_FUNCTION_SWEEP_MIN_UNITS_PER_SCENARIO=4) + 5 cargo-portable proofs + 6 ModuleReport fixture updates + the test_cli default. README/USER_GUIDE/CODEBASE_ANALYSIS gate entries added. Default-off / DUT byte-identical (snapshots 6/6).`
  Verification: `cargo check --lib --bin tool_matrix clean; cargo clippy --all-targets -- -D warnings clean; cargo fmt --all --check clean; cargo test --lib 502 passed / 2 ignored (501 + the new metric proof; all schema_version assertions green at 1.11); cargo test --bin tool_matrix 73 passed / 1 ignored (68 + 5 new cone-function gate proofs); cargo test --test snapshots 6/6 byte-identical (default-off). End-to-end --introspect: default (seed 42) ⇒ schema_version 1.11 + num_emitted_cone_functions 0; forced cone_function_emit_prob=1.0 (seed 42) ⇒ 1.11 + 14. Banked clean anvil-cone-function-gate-r1 (--cone-function-gate --yosys-mode both --iverilog-compile): 3 scenarios / 12 modules / 12 emitting a cone function / 148 cone functions / coverage_gaps=[] / 12/0 Verilator + 12/0 Yosys without-abc + 12/0 Yosys with-abc + 12/0 Icarus compile; saw_cone_function_emit=true. mdbook build book OK; check_memory_architecture + check_knowledge_map OK.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.10b.2 commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.10b.3`
  Status: `done`
  Goal: `User-facing closeout (docs-only): a ## The fifth surface: a multi-gate-cone function automatic section in book/src/structured-emission.md (byte-verified before/after) + the cone_function_emit_prob knob entry in book/src/knobs.md / USER_GUIDE.md / README "Current CLI truth" (config-file-only knob) + a KM how-to card. mdbook build + check_knowledge_map + check_memory_architecture + book_examples 3/3 green. Docs-only / DUT byte-identical.`
  Acceptance: `mdbook build clean; KM gen+check OK; check_memory_architecture OK; cargo test --test book_examples 3/3; the new repro block skip-sentinelled if non-byte-stable; committed through COMMIT.md with the leaf id.`
  Result: `Done. book/src/structured-emission.md gains a "## The fifth surface: a multi-gate-cone function automatic" section: a BYTE-VERIFIED seed-4 before/after (the inline xor_0 = i_1 ^ i_3; sub_0 = i_2 - xor_0; cone becomes one function automatic sub_0__cf(a0,a1,a2) with a function-local xor_0 = a0 ^ a2; + return sub_0__cf = a1 - xor_0; + the call assign sub_0 = sub_0__cf(i_1,i_2,i_3);, the interior xor_0 wire + assign suppressed); the single-use-interior absorption rule; the separate cone_function_emit_prob knob vs single-gate function_emit_prob; the five-way mutual exclusion; combinational-only; the metric @ schema 1.11 + the tool_matrix --cone-function-gate proof; a skip-sentinelled repro bash block; the chapter intro updated to list the whole-cone function as a current surface. The cone_function_emit_prob knob entry added to book/src/knobs.md (the ### Structured emission subsection, beside function_emit_prob/generate_loop_emit_prob/task_emit_prob), USER_GUIDE.md (after the task_emit_prob config-knob bullet), and the README "Current CLI truth" (a config-file knob bullet after task_emit_prob). New Knowledge Map how-to card docs/knowledge/multi-gate-cone-function-emit.md (id multi-gate-cone-function-emit) with how-to question keys distinct from decision 0016's conceptual keys + a validated reverify (dump-config -> set cone_function_emit_prob=1.0 + comb-only -> generate seed 4 -> grep "__cf(" -> verilator --lint-only) + cross-links to [[structured-emission-fifth-surface-cone-function]] and [[combinational-function-emit]]. KM regenerated (43 -> 44 facts, 351 -> 364 question keys). The book example is byte-verified against anvil --seed 4 output and downstream-clean (Verilator -Wall rc=0 + both Yosys rc=0 + Icarus rc=0). Docs-only / DUT byte-identical (no source touched). With this leaf, .10b.3 / .10b / .10 all close: the fifth structured surface (the multi-gate-cone function automatic emit-projection) is delivered end-to-end. The tree stays active as an open-ended lane with no current frontier; future surfaces (multi-output tasks / nested-multi-level generate / interface-modport) are .11+, each its own decision when picked. Nothing retired.`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.12`
  ...
  Result: `Done (closed by .12b.3, 2026-06-22). The sixth structured surface (the multi-output combinational task automatic emit-projection, decision 0025) is delivered end-to-end: .12a (design-detail) + .12b.1 (live surface: knob + flag + Module field + src/ir/multi_output_task_emit.rs + emitter + 11 proofs + forced sweep) + .12b.2a (metric + introspection schema 1.13->1.14) + .12b.2b (the repo-owned tool_matrix --multi-output-task-gate, banked clean anvil-multi-output-task-gate-r1) + .12b.3 (book/USER_GUIDE/README/KM docs + the deferred book schema-JSON refresh). Default-off / DUT byte-identical throughout (snapshots 6/6). Nothing retired.`

- ID: `STRUCTURED-EMISSION-EXPANSION.12a`
  Status: `done`
  Goal: `Design-detail leaf (no source): ground decision 0025 in the real src/ir/task_emit.rs (gate_qualifies), src/ir/cone_function_emit.rs (admissible/sibling_marked), src/emit/sv.rs (render_gate_task_decl/_call + the cone primitives cone_function_params/cone_operand_ref/render_cone_gate_expr + the per-gate assign loop), src/gen/mod.rs (the roll chain, both call sites), src/config.rs, src/metrics.rs, src/ir/types.rs. Pin: (1) the candidate predicate (replicate, the codebase convention); (2) the Module carrier Module.multi_output_task_groups: BTreeMap<NodeId, Vec<NodeId>> (leader -> partners); (3) the selection + one-roll-per-leader; (4) the mutual-fan-in-independence in_fanin bounded backward DFS soundness mechanism; (5) the deduplicated-formal rendering reusing render_cone_gate_expr with an empty interior_set + the __mt/__mtv naming + one always_comb per group; (6) the ordering (after task_emit, before cone_function) + the cone_function sibling exclusion; (7) the knob + metric (schema 1.13->1.14) + the tool_matrix gate + the gate-shape calibration. DEVELOPMENT_NOTES design-detail entry + the .12b impl shape.`
  Acceptance: `A DEVELOPMENT_NOTES design-detail entry resolving the seven points grounded in real code; tree split recorded (.12b pre-split .12b.1/.12b.2/.12b.3); no source change; docs/workflow self-checks clean.`
  Result: `Done. DEVELOPMENT_NOTES "Multi-output task surface — impl design-detail — .12a" entry resolves all seven points grounded in a fresh read of the real code. (1) Replicate the 4-line admissibility matches! locally (each emit-projection pass already carries its own; not a full-factorization violation — the reuse is the body rendering + the candidate concept). (2) Module.multi_output_task_groups: BTreeMap<NodeId, Vec<NodeId>> keyed by leader (lowest NodeId) -> partner members, mirroring cone_function_gates; emitter-surface annotation only (flat IR/validators/CSE/canonical_signature untouched) + a member_set() helper. (3) src/ir/multi_output_task_emit.rs annotate_multi_output_task_groups(m, rng, prob): skip param_env; collect admissible non-sibling-marked candidates ascending NodeId; walk with a used set; ONE gen_bool(prob) roll per leader; on fire, pair with the next ungrouped candidate sharing a non-constant operand AND mutually fan-in-independent. (4) in_fanin(m, target, root) bounded backward DFS over Node::Gate operands with a visited set; the operand-topological NodeId invariant (intern_gate appends after operands) makes one direction automatic, but both checked for robustness. (5) decl reuses render_cone_gate_expr(op_j, operands_j, m, {}, params, names) with params = the deduplicated non-constant union of members' operands (ascending NodeId, cone_function_params adapted) ⇒ a shared non-constant operand becomes ONE a{i} feeding multiple o{j}; one always_comb per group; per-member <name>__mtv vars + passthrough assigns; members keep their wires (co-equal roots, no use-count rule); __mt/__mtv names (no __t( substring collision). (6) run after task_emit, before cone_function; extend cone_function_emit::sibling_marked to exclude multi-output members. (7) Config::multi_output_task_emit_prob (default 0.0) + --multi-output-task-emit-prob CLI flag; Metrics::num_emitted_multi_output_tasks ⇒ schema 1.13->1.14; tool_matrix --multi-output-task-gate + ScenarioSet::MultiOutputTaskSweep + ModuleReport.emitted_multi_output_task (__mt( detection) + saw_multi_output_task_emit; gate-shape calibration = high terminal_reuse_prob (~0.6) + shallow max_depth so co-supported independent sibling pairs exist. Pre-split .12b -> .12b.1 (live) / .12b.2 (metric+schema+gate) / .12b.3 (docs); frontier -> .12b. No source change. Rejected at this leaf: making task_emit::gate_qualifies pub (the convention is per-pass replication); a use-count suppression of member wires (members are co-equal roots, not absorbed interiors).`
  Verification: `done`
  Commit: `done`

- ID: `STRUCTURED-EMISSION-EXPANSION.12b`
  Status: `active`
  Goal: `Implement the .12a design: the multi_output_task_emit_prob knob + --multi-output-task-emit-prob CLI flag + Module.multi_output_task_groups + new src/ir/multi_output_task_emit.rs (annotate_multi_output_task_groups with the leader-roll pairing + the in_fanin soundness DFS) + two guarded gen-time rolls (after task_emit, before cone_function) + the cone_function sibling exclusion + the emitter rendering (deduplicated-formal multi-output task decl reusing the cone primitives + per-member always_comb call + passthrough assigns) + lib proofs + the num_emitted_multi_output_tasks metric (schema 1.13->1.14) + the tool_matrix --multi-output-task-gate downstream-clean gate + book/USER_GUIDE/README/KM closeout. Default-off / DUT byte-identical (snapshots untouched).`
  Children: `STRUCTURED-EMISSION-EXPANSION.12b.1`, `STRUCTURED-EMISSION-EXPANSION.12b.2`, `STRUCTURED-EMISSION-EXPANSION.12b.3`
  Result: `Done (closed by .12b.3, 2026-06-22). .12b.1 (live surface) + .12b.2 (.12b.2a metric + introspection schema 1.13->1.14, .12b.2b the repo-owned tool_matrix --multi-output-task-gate banked clean) + .12b.3 (book/USER_GUIDE/README/KM docs + the deferred book schema-JSON refresh) all complete. Default-off / DUT byte-identical throughout. The sixth structured surface is delivered end-to-end.`

- ID: `STRUCTURED-EMISSION-EXPANSION.12b.1`
  Status: `done`
  Goal: `The live multi-output task surface (first source change): Config::multi_output_task_emit_prob (default 0.0, validation, dump-config) + --multi-output-task-emit-prob CLI flag + Module.multi_output_task_groups: BTreeMap<NodeId,Vec<NodeId>> + new src/ir/multi_output_task_emit.rs (annotate_multi_output_task_groups: leader-roll pairing on a shared non-constant operand + the in_fanin bounded backward DFS soundness check) + src/ir/mod.rs registration + two guarded gen/mod.rs rolls (after task_emit, before cone_function) + extend cone_function_emit::sibling_marked + src/emit/sv.rs rendering (the multi-output task decl/call section reusing render_cone_gate_expr with an empty interior_set + the per-gate passthrough) + lib proofs + a forced multi_output_task_emit_prob=1.0 Verilator/Yosys/Icarus sweep. NO metric/schema bump (the knob rides the version; metric+gate at .12b.2). Default-off / DUT byte-identical (snapshots 6/6).`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --lib green incl. the new multi_output_task proofs; cargo test --test snapshots 6/6 byte-identical (default-off); a forced multi_output_task_emit_prob=1.0 sample emits __mt( and lints clean (Verilator -Wall Δ=0 vs OFF + both Yosys + Icarus) + is sim-equiv to the inline reference. Committed through COMMIT.md with the leaf id.`
  Result: `Done. Config::multi_output_task_emit_prob (default 0.0, default_multi_output_task_emit_prob() serde default; added to the Default impl + the 0.0..=1.0 validation list + dump-config) + the --multi-output-task-emit-prob Option<f64> CLI flag (src/main.rs clap + the Overrides struct + apply_cli_overrides). Module.multi_output_task_groups: BTreeMap<NodeId, Vec<NodeId>> (Default-empty; emitter-surface annotation only — flat IR / validators / CSE / canonical_module_signature untouched, disjoint from the sibling gate sets). New src/ir/multi_output_task_emit.rs annotate_multi_output_task_groups(m, rng, prob): skip param_env; collect admissible (non-structured/non-Slice/>=1-operand) non-sibling-marked candidates ascending NodeId; one gen_bool(prob) roll per ungrouped leader; pair with the next ungrouped candidate sharing a non-constant operand AND mutually fan-in-independent (in_fanin bounded backward DFS over Node::Gate operands with a visited set); keyed leader→[partner]. src/ir/mod.rs registration. Two guarded gen/mod.rs rolls (single + design) AFTER task_emit, BEFORE cone_function. cone_function_emit::sibling_marked extended to exclude multi-output members (leader or partner). src/emit/sv.rs: the mo_task_member set + the multi-output task decl/call section (render_multi_output_task_decl + render_multi_output_task_call + multi_output_task_params, the body reusing render_cone_gate_expr with an EMPTY interior_set so a shared non-constant operand becomes one a{i} feeding multiple o{j}) + the per-gate assign-loop passthrough assign <m> = <m>__mtv; (members keep their module wires — co-equal roots, no use-count rule). 11 lib proofs (group/skip/no-shared-operand/shared-constant/fan-in-dependent/structured/slice/sibling/param-env exclusions + identity-and-node-count-untouched + end-to-end emit). NO schema bump (default-off prob-knob precedent; the .12b.2 metric bumps 1.13->1.14).`
  Verification: `cargo check --all-targets clean; cargo clippy --all-targets -- -D warnings clean; cargo fmt --all --check clean; cargo test --lib 599 passed / 2 ignored (incl. 10 new multi_output_task_emit proofs; introspect schema_version 1.13 + umbrella DUT-byte-identical still green); cargo test --test snapshots 6/6 byte-identical (default-off). Forced multi_output_task_emit_prob=1.0 sweep (anvil-mo-sweep/, comb-only terminal_reuse_prob=0.6 max_depth=2 min_outputs=2, seeds 1/7/42/100): __mt( fires on all four (2-6 tasks each), Verilator --lint-only -Wall delta=0 vs OFF across 1800-2012/2017/2023, Yosys without-abc + with-abc OK, Icarus iverilog -g2012 OK; seed-7 module exhaustively sim-equivalent to the inline OFF reference over all 128 input values (GEN-RTL SIM-EQUIV OK); seed-100's lone -Wall warning is a pre-existing UNUSEDSIGNAL on an unrelated concat_0 wire (identical ON/OFF, delta=0).`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.12b.1 commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.12b.2`
  Status: `active`
  Goal: `The metric + schema bump + repo-owned gate. Pre-split (2026-06-22) into .12b.2a (metric + schema 1.13->1.14) + .12b.2b (the tool_matrix --multi-output-task-gate) — the metric is a Metrics field surfaced in introspection (a careful current-output schema-ref bump, the .6b.2a precedent); the tool_matrix gate is a large, fragile change (flag + ScenarioSet + config builder + coverage fact + detection + merge + gap enforcement + many ModuleReport/Cli fixtures) warranting its own focused slice.`
  Children: `STRUCTURED-EMISSION-EXPANSION.12b.2a`, `STRUCTURED-EMISSION-EXPANSION.12b.2b`
  Result: `Done. .12b.2a (metric + introspection schema 1.13->1.14) + .12b.2b (the repo-owned tool_matrix --multi-output-task-gate, banked clean anvil-multi-output-task-gate-r1) both complete. Default-off / DUT byte-identical. Frontier → .12b.3 (docs).`

- ID: `STRUCTURED-EMISSION-EXPANSION.12b.2a`
  Status: `done`
  Goal: `The metric + schema bump: add Metrics::num_emitted_multi_output_tasks (usize, #[serde(default)]) computed in metrics::compute() as m.multi_output_task_groups.len(); it surfaces in introspection module_metrics (the SCHEMA-DERIVED projection), so bump the introspection schema MINOR 1.13 -> 1.14 (SCHEMA_VERSION const + doc comment + the 3 introspect + 8 mcp schema_version assertions + the docs/AGENT_INTROSPECTION_SCHEMA.md changelog/defines/checklist + README + USER_GUIDE current refs + CODEBASE_ANALYSIS envelope; historical attributions + the book example JSONs deferred to .12b.3 per the Mealy .2b.2a/.2b.3 precedent). A lib proof. Default-off / DUT byte-identical.`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --lib green incl. the metric proof + the schema_version 1.14 assertions; cargo test --test snapshots 6/6 byte-identical; the schema doc records the 1.13 -> 1.14 additive MINOR bump; end-to-end --introspect default 0 / forced N at schema 1.14. Committed through COMMIT.md with the leaf id.`
  Result: `Done. Metrics::num_emitted_multi_output_tasks: usize (#[serde(default)]) added to src/metrics.rs, computed in metrics::compute() as m.multi_output_task_groups.len() (a post-hoc structural count of an emitter-surface annotation; 0 by default, the configured group count when multi_output_task_emit_prob fired); separate from num_emitted_combinational_tasks (the single-gate surface). Surfaced in introspection module_metrics (Metrics is the exact serde projection), so SCHEMA_VERSION bumped 1.13 -> 1.14 in src/introspect/mod.rs (the new derived Metrics field BUMPS the schema — the .10b.2/.6b.2a/.4b.2a/.2b.2a precedent; the .12b.1 knob rode the version via #[serde(default)]). MINOR is an integer ⇒ 1.13 -> 1.14 (fourteen). Bumped all current-output schema refs to 1.14: the doc comment + SCHEMA_VERSION const + the 3 introspect schema_version assertions + the 8 mcp schema_version assertions, the schema doc (new 1.13->1.14 changelog entry + the early-example/defines/checklist lines), README (--introspect + analyze current refs), USER_GUIDE (--sv-version --introspect row), and the CODEBASE_ANALYSIS envelope line (which had drifted, frozen at "1.12" — brought current to "1.14" + the 1.12->1.13 + 1.13->1.14 list entries added). Historical "landed at schema X" attributions left intact (num_mealy_fsm_modules @ 1.13 in README/CODEBASE; the 1.12->1.13 changelog entry). The book/src example JSON schema_version refresh is deferred to .12b.3 (the Mealy .2b.2a/.2b.3 precedent). Lib proof metrics_count_emitted_multi_output_tasks (unmarked 0, marked 1).`
  Verification: `cargo check --all-targets clean; cargo clippy --all-targets -- -D warnings clean; cargo fmt --all --check clean; cargo test --lib 600 passed / 2 ignored (599 + the new metric proof; all schema_version assertions green at 1.14); cargo test --test snapshots 6/6 byte-identical (default-off; metric changes no RTL). End-to-end --introspect: default (seed 42) ⇒ schema_version "1.14" + introspection.module_metrics.num_emitted_multi_output_tasks 0; forced multi_output_task_emit_prob=1.0 (seed 42) ⇒ "1.14" + 1. Frontier → .12b.2b.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.12b.2a commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.12b.2b`
  Status: `done`
  Goal: `The repo-owned tool_matrix gate: a saw_multi_output_task_emit coverage fact + a --multi-output-task-gate flag + ScenarioSet::MultiOutputTaskSweep + build_multi_output_task_sweep_scenarios/focus config (comb-only multi_output_task_emit_prob=1.0 across the three construction strategies; terminal_reuse_prob high + shallow max_depth + min_outputs>=2 so co-supported independent pairs exist) + a ModuleReport.emitted_multi_output_task detection (__mt( SV-text token, distinct from __t( and __cf() + MatrixReport.multi_output_task_gate + coverage-gap enforcement, proving Verilator + both Yosys modes + Icarus accept the emitted tasks warning-clean. Bank a clean report. Default-off / DUT byte-identical. Template: --cone-function-gate.`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --bin tool_matrix green incl. the new gate proofs; cargo test --test snapshots 6/6 byte-identical; the --multi-output-task-gate bank clean and proves a multi-output task emitted + accepted (saw_multi_output_task_emit, coverage_gaps=[]); README/USER_GUIDE/CODEBASE gate entries. Committed through COMMIT.md with the leaf id.`
  Result: `Done. src/bin/tool_matrix.rs gains --multi-output-task-gate + ScenarioSet::MultiOutputTaskSweep + build_multi_output_task_sweep_scenarios/multi_output_task_focus_config (comb-only multi_output_task_emit_prob=1.0 x 3 construction strategies; node-id + e-graph; high terminal_reuse_prob=0.6 + shallow max_depth=2 + min_outputs>=2 so co-supported fan-in-independent sibling pairs exist) + ModuleReport.emitted_multi_output_task (prepared.sv_text.contains("__mt("), distinct from the single-gate "__t(" and the cone "__cf(") + CoverageSummary.saw_multi_output_task_emit (lit on Verilator success + Yosys clean) + MatrixReport.multi_output_task_gate + the early-return gap arm + the units floor (MULTI_OUTPUT_TASK_SWEEP_MIN_UNITS_PER_SCENARIO=4) + mutual-exclusion + 5 cargo-portable proofs + the 8 ModuleReport + 1 Cli fixture updates + the test_cli/MatrixReport-fixture defaults. README/USER_GUIDE/CODEBASE_ANALYSIS gate entries added. Default-off / DUT byte-identical (snapshots 6/6).`
  Verification: `cargo check --all-targets clean; cargo clippy --bin tool_matrix -- -D warnings clean; cargo fmt --all --check clean; cargo test --bin tool_matrix 84 passed / 1 ignored (79 + 5 new gate proofs); cargo test --test snapshots 6/6 byte-identical. Banked clean anvil-multi-output-task-gate-r1 (--multi-output-task-gate --yosys-mode both --iverilog-compile): 3 scenarios / 12 modules / 6 emitting a multi-output task / coverage_gaps=[] / saw_multi_output_task_emit=true / 12/0 Verilator + 12/0 Yosys without-abc + 12/0 Yosys with-abc + 12/0 Icarus compile. Closes .12b.2b / .12b.2; frontier → .12b.3.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.12b.2b commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.12b.3`
  Status: `done`
  Goal: `User-facing closeout (docs-only): a ## The sixth surface: a multi-output task automatic section in book/src/structured-emission.md (byte-verified before/after) + the multi_output_task_emit_prob knob entry in book/src/knobs.md / USER_GUIDE.md / README "Current CLI truth" + a KM how-to card + the deferred book example-JSON schema 1.13->1.14 refresh. mdbook build + check_knowledge_map + check_memory_architecture + book_examples green. Docs-only / DUT byte-identical.`
  Acceptance: `mdbook build clean; KM gen+check OK; check_memory_architecture OK; cargo test --test book_examples green; the new repro block skip-sentinelled; the before/after byte-verified vs the release binary + downstream-clean; committed through COMMIT.md with the leaf id.`
  Result: `Done. book/src/structured-emission.md gains a "## The sixth surface: a multi-output task automatic" section: a BYTE-VERIFIED seed-3 before/after (the inline xor_0 = i_1 ^ i_2; not_0 = ~i_2; pair becomes one task automatic xor_0__mt(output o0, o1, input a0, a1) where the shared operand i_2 is the single formal a1 feeding BOTH outputs — the co-supported sink — + per-member <wire>__mtv vars + one always_comb + per-member passthrough assigns); the shared-non-constant-operand rule; the fan-in-independence soundness rule; members-keep-their-wires; the mutual exclusion + pass ordering; combinational-only; the metric @ schema 1.14 + the tool_matrix --multi-output-task-gate proof; a skip-sentinelled repro bash block; the chapter intro updated to list the multi-output task as a current surface. The multi_output_task_emit_prob knob entry added to book/src/knobs.md (the ### Structured emission subsection, after cone_function_emit_prob), USER_GUIDE.md (after the cone_function_emit_prob knob bullet), and the README "Current CLI truth" (after the cone_function_emit_prob bullet). New Knowledge Map how-to card docs/knowledge/multi-output-task-emit.md (id multi-output-task-emit) with how-to question keys + a validated reverify (dump-config -> set multi_output_task_emit_prob=1.0 + the seed-3 comb-only shape -> grep "__mt(" -> verilator --lint-only) + cross-links to [[structured-emission-sixth-surface-multi-output-task]], [[combinational-task-emit]], [[multi-gate-cone-function-emit]]. The deferred book/src example-JSON schema_version 1.13 -> 1.14 refresh landed too (api-tools/agent-mcp/api-introspection/api-reference current refs + the per-version changelog narrative; the historical num_mealy_fsm_modules @ 1.13 refs in sequential.md/knobs.md stay). KM regenerated (61 -> 62 facts, 551 -> 565 question keys). The book before/after is byte-verified against anvil --seed 3 output and downstream-clean (Verilator -Wall 2012/2017/2023 rc=0 + both Yosys rc=0 + Icarus rc=0). Docs-only / DUT byte-identical (no source touched). With this leaf, .12b.3 / .12b / .12 all close: the sixth structured surface (the multi-output task automatic emit-projection) is delivered end-to-end. The tree stays active as an open-ended lane with no current frontier; future surfaces (wider k>2 task groups / nested-multi-level generate / interface-modport) are .13+, each its own decision when picked. Nothing retired.`
  Verification: `mdbook build book OK; bash knowledge-map/scripts/gen_knowledge_map.sh (62 facts / 565 keys) + check_knowledge_map.sh OK; bash scripts/check_memory_architecture.sh OK; cargo test --test book_examples 3/3 passed (the new repro block carries the <!-- book-test: skip --> sentinel, preserving the byte-identical book-runnable contract). Docs-only => cargo check/clippy/fmt/test --lib unaffected (no src/ touched). The book before/after is byte-verified against anvil --seed 3 output and the ON form is downstream-clean (Verilator -Wall 2012/2017/2023 rc=0 / both Yosys rc=0 / Icarus rc=0).`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.12b.3 commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.13`
  Status: `done`
  Result: `Done (closed by .13c, 2026-06-22). The first deepening of the sixth structured surface (the multi-output combinational task automatic, decision 0025) — wider k>2 co-supported task groups — is delivered end-to-end: .13a (design-detail) + .13b (the live widening of annotate_multi_output_task_groups: const cap + connected_co_support/independent_of_all + greedy partner extension; no emitter/config/metric/schema/knob change since the emitter was already k-agnostic; 5 new k>2 lib proofs; gate re-bank anvil-mo-k3-gate-r1 coverage_gaps=[] with a k=3 group + forced seed-22 k=3 task Verilator -Wall Δ=0 + sim-equiv) + .13c (user docs: book sixth-surface "Wider groups (k>2)" subsection with a byte-verified seed-22 k=3 example + knob entries + KM card). Default-off / DUT byte-identical throughout (snapshots 6/6). Nothing retired. The lane returns to no current frontier; further surfaces (nested/multi-level generate, interface/modport) are .14+.`
  Goal: `Deepen the sixth structured surface (the multi-output combinational task automatic, decision 0025) from the first-cut PAIR (k=2) to a bounded k>2 co-supported group — the recorded .13 follow-up. The emitter is already k-agnostic (multi_output_task_params / render_multi_output_task_decl / render_multi_output_task_call iterate members of any length; the carrier is BTreeMap<NodeId,Vec<NodeId>>), so the only source change is widening annotate_multi_output_task_groups to greedily collect all eligible partners (connected co-support + generalized mutual fan-in independence) up to a const cap. No new knob (reuse multi_output_task_emit_prob) / no new metric (num_emitted_multi_output_tasks counts groups, valid for any k) / no schema bump. Default-off / DUT byte-identical (snapshots untouched).`
  Children: `STRUCTURED-EMISSION-EXPANSION.13a`, `STRUCTURED-EMISSION-EXPANSION.13b`, `STRUCTURED-EMISSION-EXPANSION.13c`

- ID: `STRUCTURED-EMISSION-EXPANSION.13a`
  Status: `done`
  Goal: `Design-detail leaf (no source): ground the k>2 widening in the real src/ir/multi_output_task_emit.rs (annotate_multi_output_task_groups / admissible / sibling_marked / nonconst_operands / shares_nonconst_operand / in_fanin), src/emit/sv.rs (verify multi_output_task_params / render_multi_output_task_decl / render_multi_output_task_call are k-agnostic), src/gen/mod.rs (the two guarded call sites), src/config.rs. Pin: (1) the widening locus is annotate_multi_output_task_groups only — no emitter/knob/metric/schema change; (2) the greedy group-extension algorithm keeping the per-leader inline roll; (3) the connectivity rule = connected co-support (shares a non-constant operand with at least one current member); (4) the generalized soundness rule = mutual fan-in independence with all current members (inductively maintained); (5) a named const cap MAX_MULTI_OUTPUT_TASK_GROUP_MEMBERS; (6) the byte-identical + RNG-stream argument (default 0.0 not called; prob=1.0 gen_bool short-circuits ⇒ zero draws); (7) the .13b proof plan + rejected alternatives. DEVELOPMENT_NOTES design-detail entry + the .13b/.13c shape.`
  Acceptance: `A DEVELOPMENT_NOTES design-detail entry resolving the points grounded in real code; tree split recorded (.13b impl + .13c docs); no source change; docs/workflow self-checks clean.`
  Result: `Done. DEVELOPMENT_NOTES "Wider (k>2) multi-output task groups — impl design-detail — .13a" entry resolves all points grounded in a fresh read of the real code. (0) The emitter is already k-agnostic (verified): the emit loop builds members = [leader] ++ partners, sorts, and the three render helpers iterate members of arbitrary length (o0..o{k-1} outputs, deduped a0..a{m-1} inputs, k __mtv vars + k passthroughs); the carrier already holds k-1 partners — so the ONLY source change is annotate_multi_output_task_groups (no emitter change, no new knob, no new metric, no schema bump). (1) Keep the one-gen_bool(prob)-roll-per-leader mechanism; on a fire, greedily collect ALL eligible partners in ascending NodeId up to the cap (a group forms iff >= 1 partner ⇒ k=2 is the exact subset, every pair proof holds). (2)+(3) Connectivity = connected co-support: gc joins iff it shares >= 1 non-constant operand with at least one current member (leader-anchored star rejected — narrower, no soundness gain). (4) Soundness = mutual fan-in independence with ALL current members (both in_fanin directions for robustness), inductively maintained ⇒ cycle-free for any k. (5) const MAX_MULTI_OUTPUT_TASK_GROUP_MEMBERS = 8 (leader + up to 7 partners) — bounded/reviewable, lets multiple groups form per module; raising/removing is a one-const follow-up. (6) Default 0.0 ⇒ pass not called (guarded > 0.0 both gen sites) ⇒ byte-identical; no snapshot sets the knob; at prob=1.0 gen_bool(1.0) short-circuits (rand 0.8 Bernoulli ALWAYS_TRUE, no draw) ⇒ zero RNG draws ⇒ no cone_function stream shift; the only change is wider groups at prob>0 (intended; gate re-banked). (7) .13b proofs = co-supported triple / connectivity-bounded / fan-in-dependent-excluded-at-k>2 / cap-respected / 3-output emit; all pair proofs stay green; + a forced k>=3 sweep + a gate re-bank. Pre-split .13 → .13b (live impl) + .13c (docs); frontier → .13b. No source change. Rejected: leader-anchored star co-support; an unbounded group; a new max-group-size config knob; an up-front roll-once-per-candidate refactor.`
  Verification: `this STRUCTURED-EMISSION-EXPANSION.13a commit`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.13a commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.13b`
  Status: `done`
  Goal: `Implement the .13a design: widen src/ir/multi_output_task_emit.rs annotate_multi_output_task_groups to greedily collect all eligible partners (connected co-support via shares_nonconst_operand against any current member + generalized mutual fan-in independence via in_fanin against all current members) up to const MAX_MULTI_OUTPUT_TASK_GROUP_MEMBERS = 8; keep the per-leader inline gen_bool(prob) roll. Add k>2 lib proofs (co-supported triple, connectivity-bounded extension, fan-in-dependent exclusion at k>2, cap respected, 3-output emit) and keep all existing pair proofs green. NO emitter / knob / metric / schema change. Default-off / DUT byte-identical (snapshots 6/6). Forced multi_output_task_emit_prob=1.0 sweep proving k>=3 groups fire and are Verilator -Wall Δ=0 + both Yosys + Icarus clean; re-bank the tool_matrix --multi-output-task-gate (still coverage_gaps=[], saw_multi_output_task_emit).`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --lib green incl. the new k>2 proofs + all pair proofs; cargo test --test snapshots 6/6 byte-identical (default-off); a forced multi_output_task_emit_prob=1.0 sample emits a >=3-output __mt( task and lints clean (Verilator -Wall Δ=0 vs OFF + both Yosys + Icarus) + is sim-equiv to the inline reference; the --multi-output-task-gate re-bank stays clean. Committed through COMMIT.md with the leaf id.`
  Result: `Done. The whole change is in src/ir/multi_output_task_emit.rs: const MAX_MULTI_OUTPUT_TASK_GROUP_MEMBERS = 8 + two helpers (connected_co_support = shares a non-constant operand with at least one current member; independent_of_all = mutually fan-in-independent with every current member via in_fanin both directions) + the partner loop rewritten from "find the first partner and break" to "greedily admit every eligible ungrouped higher-NodeId candidate up to the cap"; a group forms iff >= 1 partner is admitted (k=2 is the exact subset). NO emitter/config/metric/schema/knob change — the emitter (render_multi_output_task_decl/_call/multi_output_task_params) was already k-agnostic, verified by the new 3-output emit proof. Doc comments updated (file header, module doc, the per-leader-roll + soundness paragraphs); CODEBASE_ANALYSIS.md pass description brought current (pair → k>=2). 5 new lib proofs (prob_one_groups_a_co_supported_triple, group_extends_only_to_connected_co_support, group_excludes_fan_in_dependent_member_when_widening, group_respects_the_member_cap, grouped_triple_emits_three_output_task) + all 10 existing pair proofs stay green (15 total). Default-off / DUT byte-identical (snapshots 6/6; the default 0.0 path never calls the pass).`
  Verification: `cargo fmt --all --check clean; cargo clippy --all-targets -- -D warnings clean; cargo test --lib 605 passed / 2 ignored (600 + 5 new k>2 proofs; introspect schema_version 1.14 + umbrella DUT-byte-identical still green); cargo test --test snapshots 6/6 byte-identical (default-off). Repo-owned gate re-bank anvil-mo-k3-gate-r1 (--multi-output-task-gate --yosys-mode both --iverilog-compile): 3 scenarios / 12 modules / 6 emitting a multi-output task / coverage_gaps = [] / saw_multi_output_task_emit = true / Verilator 12/0 / Yosys without-abc 12/0 / Yosys with-abc 12/0 / Icarus compile 12/0; a k=3 group (output formal o2) present among the emitted modules. Forced multi_output_task_emit_prob=1.0 comb-only sweep (terminal_reuse_prob=0.6, max_depth=2, min_outputs=2): seed 22 emits a genuine k=3 task shr_0__mt(o0,o1,o2, a0,a1,a2) co-emitting shr_0/mux_0/mux_1 over the deduplicated inputs i_4/slice_0/concat_0; Verilator -Wall (1800-2012) 0 real warnings ON and OFF (Δ=0; the lone line is the filename≠module DECLFILENAME artifact, identical both ways); iverilog+vvp prove the k=3 task bit-equal to the inline OFF reference over 20000 random vectors (SIM-EQUIV OK). Frontier → .13c.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.13b commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.13c`
  Status: `done`
  Goal: `User-facing closeout (docs-only): extend the "## The sixth surface: a multi-output task automatic" section in book/src/structured-emission.md to note the now-k>2 groups (a byte-verified before/after of a 3-member group; the connected-co-support + all-member-independence rules; the bounded cap) + a note in the multi_output_task_emit_prob knob entry (book/src/knobs.md / USER_GUIDE.md / README) that groups can be k>2 + update the KM card multi-output-task-emit + a decision 0025 cross-ref noting the .13 follow-up landed. mdbook build + check_knowledge_map + check_memory_architecture + book_examples green. Docs-only / DUT byte-identical.`
  Acceptance: `mdbook build clean; KM gen+check OK; check_memory_architecture OK; cargo test --test book_examples green; the before/after byte-verified vs the release binary + downstream-clean; committed through COMMIT.md with the leaf id.`
  Result: `Done. book/src/structured-emission.md: the sixth-surface intro reframed (co-supported PAIR → co-supported GROUP k>=2, up to 8) + a new "### Wider groups (k > 2)" subsection with a real seed-22 three-member task (shr_0/mux_0/mux_1; the shared select a0 feeds both o1 and o2, a2 feeds both o0 and o1) + the connected-co-support + all-member-independence rules + the 8-member cap + the Δ=0/sim-equiv note; the "What gets wrapped" bullets generalized pair → group / every member / the delivered k>=2 + cap; "How anvil proves it" updated (k=3 group in the gate bank + the new library proofs at every size). The multi_output_task_emit_prob knob entry updated in book/src/knobs.md + USER_GUIDE.md + README "Current CLI truth" (pairs → greedy co-supported group k>=2 up to 8, checked against every member, cycle-free at any size). KM card docs/knowledge/multi-output-task-emit.md updated (title, 4 new answers incl. "can a multi-output task have more than two outputs" / "what is the group size cap" / "what is connected co-support", evidence, body pair → k>=2 greedy extension + cap, bank path → anvil-mo-k3-gate-r1 with k=3); KM regenerated 64 facts / 589 → 593 keys. No decision-0025 mutation (the ADR already records the .13 follow-up; the tree Decisions section cross-refs). Docs-only / DUT byte-identical (no src/ touched). Closes .13c / .13 — the first deepening of the sixth surface delivered end-to-end; the lane returns to no current frontier.`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.15`
  Status: `done`
  Goal: `Implement the seventh structured surface (decision 0027): the procedural always_comb if/else emit-projection of a Mux gate. mux_if_emit_prob knob + Module.mux_if_gates carrier + new src/ir/mux_if_emit.rs gen-time mark + the emitter always_comb if/else block (<name>__cv output var) + the gate-assign-loop passthrough + num_emitted_mux_if_blocks metric (schema 1.14 -> 1.15) + tool_matrix --mux-if-gate / saw_mux_if_emit + book/USER_GUIDE/KM. Default-off / DUT byte-identical. Pre-split .15a (design-detail) + .15b (impl, itself .15b.1 live / .15b.2 metric+gate / .15b.3 docs).`
  Children: `STRUCTURED-EMISSION-EXPANSION.15a`, `STRUCTURED-EMISSION-EXPANSION.15b`

- ID: `STRUCTURED-EMISSION-EXPANSION.15a`
  Status: `done`
  Goal: `Design-detail leaf (no source): ground decision 0027 in the real src/ir/task_emit.rs (candidate predicate + one-roll-per-candidate template), src/emit/sv.rs (node_ref operand resolver, the task decl/call section, the gate-assign-loop passthrough, the procedural-block section), src/gen/mod.rs (the six-pass roll chain), src/config.rs (the *_emit_prob knobs), src/ir/types.rs (the *_gates carriers). Pin all seven impl points (candidate predicate excluding the six sibling marks; Module.mux_if_gates: BTreeSet<NodeId>; the emitter always_comb if/else block + <name>__cv var + passthrough; run annotate_mux_if_gates last; the mux_if_emit_prob knob + flag; the num_emitted_mux_if_blocks metric schema 1.14->1.15; the --mux-if-gate gate + saw_mux_if_emit + gate-shape calibration; the byte-identical/RNG argument). DEVELOPMENT_NOTES design-detail entry + the .15b shape + rejected alternatives. No source change.`
  Acceptance: `A DEVELOPMENT_NOTES design-detail entry resolving the points grounded in real code; tree split recorded (.15b impl, pre-split .15b.1/.15b.2/.15b.3); no source change; docs/workflow self-checks clean.`
  Result: `Done. DEVELOPMENT_NOTES "Procedural if/else mux surface — impl design-detail — .15a" entry resolves all seven points grounded in a fresh read of the real code. (0) The candidate predicate (new src/ir/mux_if_emit.rs, mirror task_emit.rs): a GateOp::Mux gate (3 operands, 1-bit selector by IR invariant) not already marked by any sibling projection — because the pass runs LAST the exclusion is the union of function_emit_gates / generate_loop_gates / task_emit_gates / soft_union_slice_gates / multi_output_task_groups (keys + values) / cone_function_gates (keys + flattened values); only function/task/multi-output/cone can ever mark a Mux but all six excluded for robustness; collect-then-roll one gen_bool per candidate into m.mux_if_gates; skip param_env modules. (1) Module.mux_if_gates: BTreeSet<NodeId> beside task_emit_gates, serde-default-empty (a set, not a map — a marked Mux carries no payload, its operands are read straight from the node). (2) The emitter: a new procedural-block section (logic [w-1:0] <name>__cv; + always_comb begin if (sel) <name>__cv = a; else <name>__cv = b; end, operand refs via the same node_ref resolver the CaseMux block uses at sv.rs:644; 1-bit drops [w-1:0]) + the gate-assign-loop passthrough branch (assign <name> = <name>__cv; continue;) beside task_emit at sv.rs:578, BEFORE render_gate's inline ternary — keeps <name> a net (decision-0014 minimal-blast-radius). The Mux is NOT in the existing CaseMux/CasezMux/ForFold procedural loop so no change there. (3) Run annotate_mux_if_gates LAST (seventh guarded call site after cone_function in both gen/mod.rs paths, guarded > 0.0) so it only EXCLUDES already-marked gates — NO other pass changes (minimal blast radius); rejected: run it among the single-gate projections + teach multi_output_task/cone_function to exclude it (more edits, no benefit). (4) mux_if_emit_prob knob (default 0.0) + --mux-if-emit-prob flag (the task/cone knob pattern). (5) num_emitted_mux_if_blocks metric (= m.mux_if_gates.len()) ⇒ schema 1.14 -> 1.15 at .15b.2. (6) tool_matrix --mux-if-gate + ScenarioSet::MuxIfSweep + ModuleReport.emitted_mux_if (__cv detection) + saw_mux_if_emit, templated on --task-emit-gate, with a Mux-biased focus config (calibration risk recorded). (7) default 0.0 ⇒ pass not entered ⇒ zero draws ⇒ byte-identical; prob=1.0 gen_bool short-circuits ⇒ no extra draws; the mark leaves the flat IR / validators / CSE / canonical_module_signature untouched. .15b proof plan (6 lib proofs + snapshots 6/6 + forced sweep Δ=0 + sim-equiv) + the .15b pre-split (.15b.1/.15b.2/.15b.3) recorded. No source change. Rejected alternatives: <name>-as-var (drops passthrough, rewrites consumers); a 2-arm case(sel) (duplicates CaseMux); the N-way if/else-if chain in this cut (the .16+ follow-up).`
  Verification: `this STRUCTURED-EMISSION-EXPANSION.15a commit. Docs-only ⇒ cargo check/clippy/fmt/test --lib unaffected (no src/ touched); DUT byte-identical. bash scripts/check_doctrines.sh green (4 doctrines); check_knowledge_map.sh + check_memory_architecture.sh green (no card change — 0027 already carries answers:); mdbook build clean. Grounded line refs verified against current HEAD: node_ref sv.rs:1242, the CaseMux always_comb block sv.rs:644, the task_emit passthrough sv.rs:578, the gen/mod.rs roll chain 95-150 (single) / 330-383 (design), the Module *_gates fields types.rs:417-466, the *_emit_prob config defaults config.rs:106-114.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.15a commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.15b`
  Status: `done`
  Goal: `Implement the seventh structured surface per the .15a design. Pre-split .15b.1 (live surface: knob + carrier + pass + emitter + lib proofs + forced-sweep downstream proof) + .15b.2 (num_emitted_mux_if_blocks metric @ schema 1.14->1.15 + tool_matrix --mux-if-gate + saw_mux_if_emit) + .15b.3 (user docs: book/knobs/USER_GUIDE/README/KM). Default-off / DUT byte-identical.`
  Children: `STRUCTURED-EMISSION-EXPANSION.15b.1`, `STRUCTURED-EMISSION-EXPANSION.15b.2`, `STRUCTURED-EMISSION-EXPANSION.15b.3`

- ID: `STRUCTURED-EMISSION-EXPANSION.15b.1`
  Status: `done`
  Goal: `The live seventh-surface source change: Config::mux_if_emit_prob (default 0.0) + --mux-if-emit-prob flag + Module.mux_if_gates: BTreeSet<NodeId> + new src/ir/mux_if_emit.rs (annotate_mux_if_gates, the GateOp::Mux candidate predicate excluding all six sibling marks, run last) + src/ir/mod.rs registration + two guarded gen/mod.rs rolls (after cone_function, single + design) + src/emit/sv.rs procedural-block section (logic [w-1:0] <name>__cv; + always_comb if/else, operand refs via node_ref) + the gate-assign-loop <name>__cv passthrough + ~6 lib proofs. No metric/schema bump (the .15b.2 metric bumps 1.14->1.15; the knob rides the version). Default-off / DUT byte-identical (snapshots 6/6); a forced mux_if_emit_prob=1.0 sweep proving the always_comb if/else fires + Verilator -Wall Δ=0 vs OFF + both Yosys + Icarus + iverilog sim-equiv to the inline ternary.`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --lib green incl. the new mux_if proofs; cargo test --test snapshots 6/6 byte-identical (default-off); a forced mux_if_emit_prob=1.0 sample emits an always_comb if/else <name>__cv block + lints clean (Verilator -Wall Δ=0 vs OFF + both Yosys + Icarus) + is sim-equiv to the inline reference; CODEBASE_ANALYSIS + DEVELOPMENT_NOTES updated. Committed through COMMIT.md with the leaf id.`
  Result: `Done, implemented exactly per the .15a design (no deviations). New src/ir/mux_if_emit.rs: gate_qualifies (a GateOp::Mux with exactly 3 operands, not in function_emit_gates / generate_loop_gates / task_emit_gates / soft_union_slice_gates / multi_output_task_groups[keys|values] / cone_function_gates[keys|interiors]) + annotate_mux_if_gates (collect-then-roll one gen_bool(p) per candidate into m.mux_if_gates; param_env skipped) + 10 lib proofs (qualifies; prob-zero byte-identical; non-Mux excluded; function_emit/task_emit/cone-root-or-interior/mo-leader-or-partner excluded; param_env skipped; identity/node-count untouched; the end-to-end emit proof: a marked Mux emits logic [3:0] mux_0__cv; + always_comb begin if (sel) mux_0__cv = a; else mux_0__cv = b; end + assign mux_0 = mux_0__cv;, and unmarked is the inline assign mux_0 = (sel) ? (a) : (b);). Module.mux_if_gates: BTreeSet<NodeId> (ir/types.rs, serde-default-empty); ir/mod.rs registration. Config::mux_if_emit_prob (config.rs: default_mux_if_emit_prob=0.0 + #[serde(default)] field + Default-construction entry + the validate probs-list entry + the Overrides Option<f64> + the apply); --mux-if-emit-prob clap flag + overlay wiring (main.rs). Two guarded gen/mod.rs rolls AFTER cone_function (single generate_module + design generate_design). emit/sv.rs: a new procedural-block section (after the cone-function decl section) emitting logic + the always_comb if/else over node_ref operand refs (reusing param_width_decl_w for the __cv width) + the gate-assign-loop <name>__cv passthrough branch beside task_emit (intercepts the marked Mux before render_gate's inline ternary; <name> stays a net). No metric/schema bump (rides the version; .15b.2 bumps 1.14->1.15). CODEBASE_ANALYSIS.md gains the ir/mux_if_emit.rs module-tree entry; DEVELOPMENT_NOTES.md gains the .15b.1 impl-time note. Default-off / DUT byte-identical. Frontier → .15b.2.`
  Verification: `cargo check --all-targets clean; cargo fmt --all --check clean; cargo clippy --all-targets -- -D warnings clean; cargo test --lib 615 passed / 2 ignored (605 + 10 new mux_if proofs; umbrella DUT-byte-identical proofs green); cargo test --test snapshots 6/6 byte-identical (default-off — the 0.0 path never calls the pass). Forced mux_if_emit_prob=1.0 sweep anvil-muxif-genproof.*: across seeds 1/2/3/7/11 every plain Mux (121/143/132/144/195) projects to a __cv always_comb if/else block (ON __cv == OFF mux count exactly); Verilator --lint-only -Wall warning count ON == OFF (Δ=0) on all five (the pre-existing UNUSEDSIGNAL/etc warnings are identical ON/OFF — the projection adds none); Yosys synth -noabc and abc -fast; opt -fast; check both report 0 warnings/errors; iverilog -g2012 compiles (rc=0). Comb-only ON-vs-OFF sim-equiv (seed 3, --flop-prob 0 --comb-mux-prob 0.9, 128 muxes): iverilog+vvp prove the ON module bit-equal to OFF across 20000 random vectors on all 3 outputs (GEN-RTL SIM-EQUIV OK). The standalone construct probe (this session) already proved the if/else === (sel)?(a):(b) ternary over 20000 vectors. dump-config surfaces mux_if_emit_prob.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.15b.1 commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.15b.2`
  Status: `done`
  Goal: `The seventh-surface metric + repo-owned downstream gate. Add Metrics::num_emitted_mux_if_blocks (= m.mux_if_gates.len(), #[serde(default)]) in metrics::compute() + a lib proof ⇒ bump introspection SCHEMA_VERSION 1.14 → 1.15 across all current-output refs (introspect const + assertions, mcp assertions, docs/AGENT_INTROSPECTION_SCHEMA.md current-version + a 1.14→1.15 changelog entry, README + USER_GUIDE + CODEBASE envelope; historical attributions intact; book example JSONs deferred to .15b.3) + the repo-owned tool_matrix --mux-if-gate (ScenarioSet::MuxIfSweep + a Mux-biased focus config × the three strategies + ModuleReport.emitted_mux_if [__cv detection] + saw_mux_if_emit + MatrixReport.mux_if_gate + the gap arm + units floor + 5 proofs + fixtures), banked clean. Templated on --multi-output-task-gate. Default-off / DUT byte-identical.`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --lib green incl. the new metric proof; cargo test --bin tool_matrix green incl. 5 new mux-if-gate proofs; cargo test --test snapshots 6/6 byte-identical (default-off); a banked clean --mux-if-gate report (coverage_gaps=[], saw_mux_if_emit=true, N/0 Verilator + both Yosys + Icarus); schema-version assertions (introspect + mcp) updated to 1.15; live docs (CHANGES/MEMORY/CODEBASE/README/USER_GUIDE/schema-doc) amended. Committed through COMMIT.md with the leaf id.`
  Result: `Done. Metric: Metrics::num_emitted_mux_if_blocks (= m.mux_if_gates.len(), #[serde(default)]) added beside num_emitted_multi_output_tasks + assigned in metrics::compute() + the metrics_count_emitted_mux_if_blocks lib proof (unmarked ⇒ 0, marked ⇒ 1). Schema: introspect/mod.rs SCHEMA_VERSION "1.14" → "1.15" + the const doc-comment rewritten to attribute 1.15 to num_emitted_mux_if_blocks (decision 0027) and demote 1.14 to the prior num_emitted_multi_output_tasks; 3 introspect lib assertions + 8 mcp lib assertions bumped to "1.15"; docs/AGENT_INTROSPECTION_SCHEMA.md §7 current-version statements (×3) → 1.15 + a new 1.14→1.15 changelog entry; README (×2 envelope refs) + USER_GUIDE (×1 envelope ref) + CODEBASE_ANALYSIS.md (envelope schema_version + a 1.14→1.15 list entry) bumped; the num_emitted_multi_output_tasks @ 1.14 historical attributions (metrics.rs comment, README/USER_GUIDE metric lines) left intact. Gate: tool_matrix gains const MUX_IF_SWEEP_MIN_UNITS_PER_SCENARIO = 4, the --mux-if-gate CLI flag, ScenarioSet::MuxIfSweep, ModuleReport.emitted_mux_if (sv_text.contains("__cv")), CoverageSummary.saw_mux_if_emit + merge, MatrixReport.mux_if_gate, the run-plan units+fail-on-gap branches, the mutual-exclusion count+message, select_scenario_set + build_scenarios dispatch, build_mux_if_sweep_scenarios + mux_if_focus_config (comb-only node-id+egraph, mux_if_emit_prob=1.0, comb_mux_prob=0.9, comb_mux_encoding_prob=1.0 forcing the encoded chained-ternary path that yields plain GateOp::Mux gates), the summarize-coverage block, the early-return gap arm, the slug arms, test_cli default, 5 cargo-portable proofs, and 8 ModuleReport fixture extensions. Banked clean anvil-mux-if-gate-r1 (3 scenarios, 12 modules, coverage_gaps=[], saw_mux_if_emit=true, 12/12 emitting __cv / 215 blocks, Verilator 12/0 + Yosys both 12/0 + Icarus 12/0, exit 0). Book example JSONs + knob user docs deferred to .15b.3. Default-off / DUT byte-identical. Frontier → .15b.3.`
  Verification: `cargo check --all-targets clean; cargo fmt --all --check clean; cargo clippy --all-targets -- -D warnings clean; cargo test --lib 616 passed / 2 ignored (615 + 1 new metrics_count_emitted_mux_if_blocks proof; umbrella DUT-byte-identical proofs green); cargo test --bin tool_matrix 89 passed / 1 ignored (84 + 5 new mux-if-gate proofs); cargo test --test snapshots 6/6 byte-identical (default-off — the 0.0 path never marks). Banked --mux-if-gate (--yosys-mode both --iverilog-compile) at anvil-mux-if-gate-r1: 3 scenarios, 12 modules, mux_if_gate=true, scenario_set=mux-if-sweep, coverage_gaps=[], saw_mux_if_emit=true, 12/12 modules emit a __cv block (215 total num_emitted_mux_if_blocks across the report — the metric flows through the per-module Metrics into the report), Verilator pass/fail 12/0, Yosys without-abc 12/0, with-abc 12/0, Icarus compile 12/0; gate exit code 0 (coverage-gap gate satisfied). All schema-version assertions read "1.15".`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.16`
  Status: `done`
  Goal: `Pick the EIGHTH structured surface at the no-frontier boundary (feedback_pick_and_roll_at_no_frontier) and record it as a decision + design leaf (no source change). The surface = a default-off, valid-by-construction procedural always_comb if/else if PRIORITY CHAIN emit-projection of a CaseMux gate (the N-way selection it renders today as a parallel case (sel) ... default statement) — the recorded decision-0027 follow-up, distinct from the seventh surface (N-way chain vs single 2:1 Mux conditional) and from the case render (sequential-priority vs parallel-match). Run a fresh empirical probe (Verilator -Wall 2012/2017/2023 + both Yosys modes + Icarus + iverilog sim-equiv) before authoring the decision. Own case_mux_if_emit_prob knob + num_emitted_case_mux_if_chains metric (schema 1.15 -> 1.16 at impl) + --case-mux-if-gate / saw_case_mux_if_emit (metric-keyed detection). Split .16 (design) + .17 (impl, pre-split .17a/.17b) + future .18+.`
  Acceptance: `A decision record docs/decisions/0028-*.md (with answers: front-matter) + an INDEX row + KNOWLEDGE_MAP.md regenerated in sync; the .16 leaf recorded (tree split, current frontier -> .17, decisions/verification/commit/changelog entries); the stale parent .15/.15b flipped done; ROADMAP owner-directed-lane #2 + TASK_TREE.md row updated; docs/workflow self-checks clean (check_doctrines.sh, check_knowledge_map.sh, check_memory_architecture.sh, mdbook build); no source change (DUT byte-identical). Committed through COMMIT.md with the leaf id.`
  Result: `Done. Decision 0028 picks the eighth structured surface — a default-off, valid-by-construction procedural always_comb if/else if priority-chain emit-projection of a CaseMux gate. The CaseMux renders today (the structured-case always_comb block) as a parallel case (sel) SW'dk: g = arm_k; default: g = W'h0; endcase; the projection re-expresses that case body as if (sel == SW'd0) g = arm_0; else if (sel == SW'd1) g = arm_1; ... else g = W'h0; — a priority chain of selector-equality tests, behaviour-preserving because the case labels SW'd0..SW'd{k-1} are distinct constants by construction (so priority == parallel) and the trailing else covers exactly the case default. Simpler than the seventh surface: the CaseMux is ALREADY an always_comb-written logic var, so NO <g>__cv output var + passthrough is needed (rejected as unnecessary) — only the always_comb body swaps case...endcase -> if...else. It is genuinely distinct from the seventh surface (N-way CaseMux chain vs single 2:1 Mux conditional; GateOp::CaseMux vs GateOp::Mux) and from the existing case render (sequential-priority if/else if vs parallel-match case — a different frontend/synth code path). Candidate = a CaseMux that renders as the dynamic always_comb case (render_static_structured_gate returns None — non-constant selector); CasezMux excluded (its casez ? wildcards need a masked comparison — the recorded follow-up); first cut = the plain equality CaseMux, mirroring the seventh surface scoping to the 2:1 Mux. Own case_mux_if_emit_prob knob (separate from mux_if_emit_prob; reusing one rejected) + num_emitted_case_mux_if_chains metric (schema 1.15 -> 1.16 at impl) + --case-mux-if-gate / saw_case_mux_if_emit (keyed on the metric > 0, since this surface introduces no new identifier token — strictly more robust than a text scan). Chosen over nested/multi-level generate (no by-construction source) and interface/modport (empirically disqualified since .7). Default-off / DUT byte-identical. Split .16 (design, this leaf) + .17 (impl, pre-split .17a design-detail + .17b impl) + future .18+. No source change. Also flipped the stale parent .15/.15b active -> done (all their children were done; the prior-surface convention marks closed surface parents done — .5/.6/.6b/.13).`
  Verification: `Fresh empirical probe this session (Verilator 5.046 + Yosys 0.64 + Icarus 13.0 + iverilog 13.0): the procedural always_comb if/else if priority chain (sel-width 3, 5 arms + default, 4-bit data) is accepted warning-clean by Verilator --lint-only -Wall under --language 1800-2012/2017/2023, by Yosys synth -noabc (32 cells: 20 $_MUX_ / 3 $_NOT_ / 9 $_OR_, no warnings) and synth -noabc; abc -fast; opt -fast; check (no warnings/errors), and by iverilog -g2012 (compile rc=0); iverilog+vvp prove the chain bit-equal to the parallel case (sel) ... default form over 20000 random vectors PLUS an exhaustive 8-value selector sweep (EQUIV OK). Probe files: scratchpad/probe8/{probe_chain.sv,probe_case.sv,tb_equiv.sv}. Docs-only => cargo check/clippy/fmt/test --lib unaffected (no src/ touched); DUT byte-identical. check_doctrines.sh green (4 doctrines); check_knowledge_map.sh green (0028 answers folded into the regenerated KNOWLEDGE_MAP.md); check_memory_architecture.sh green (0028 indexed); mdbook build clean.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.16 commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.17`
  Status: `done`
  Goal: `Implement the eighth structured surface (decision 0028): the procedural always_comb if/else if priority-chain emit-projection of a CaseMux gate. case_mux_if_emit_prob knob + Module.case_mux_if_gates carrier + new src/ir/case_mux_if_emit.rs gen-time mark (dynamic-selector CaseMux predicate) + the emitter structured-case body branch (case...endcase -> if...else if over the same operand refs + W'h0 default) + num_emitted_case_mux_if_chains metric (schema 1.15 -> 1.16) + tool_matrix --case-mux-if-gate / saw_case_mux_if_emit (metric-keyed) + book/USER_GUIDE/KM. Default-off / DUT byte-identical. Pre-split .17a (design-detail) + .17b (impl, itself .17b.1 live / .17b.2 metric+gate / .17b.3 docs).`
  Children: `STRUCTURED-EMISSION-EXPANSION.17a`, `STRUCTURED-EMISSION-EXPANSION.17b`

- ID: `STRUCTURED-EMISSION-EXPANSION.17a`
  Status: `done`
  Goal: `Design-detail leaf (no source): ground decision 0028 in the real src/ir/mux_if_emit.rs (the annotation-pass template), src/emit/sv.rs (the structured-case always_comb loop + render_static_structured_gate's CaseMux static-collapse test), src/gen/mod.rs (the seven-pass roll chain), src/config.rs (the *_emit_prob knob touch points), src/main.rs (the clap flag + overlay), src/ir/types.rs (the *_gates carriers + GateOp::CaseMux). Pin all seven impl points (the dynamic-selector CaseMux candidate predicate excluding constant selectors + sibling marks + CasezMux; Module.case_mux_if_gates: BTreeSet<NodeId>; the in-place emitter case->if/else-if body branch reusing the same operand refs + W'h0 default; run annotate_case_mux_if_gates last; the case_mux_if_emit_prob knob + flag; the num_emitted_case_mux_if_chains metric schema 1.15->1.16; the metric-keyed --case-mux-if-gate gate + saw_case_mux_if_emit + case_mux_prob-biased focus config; the byte-identical/RNG argument). DEVELOPMENT_NOTES design-detail entry + the .17b shape + rejected alternatives. No source change.`
  Acceptance: `A DEVELOPMENT_NOTES design-detail entry resolving the points grounded in real code; tree split recorded (.17b impl, pre-split .17b.1/.17b.2/.17b.3); no source change; docs/workflow self-checks clean.`
  Result: `Done. DEVELOPMENT_NOTES "CaseMux if/else if priority-chain surface — impl design-detail — .17a" entry resolves all seven points grounded in a fresh read of the real code. The pass mirrors src/ir/mux_if_emit.rs but is SIMPLER (a CaseMux is already an always_comb-written logic var, so no <g>__cv output var + passthrough — only the always_comb body swaps case...endcase -> if...else if). (0) Candidate = a Node::Gate op GateOp::CaseMux whose selector operands[0] is NOT a Node::Constant (the dynamic-selector test — the exact inverse of render_static_structured_gate's CaseMux arm constant_value(operands[0]).is_some() at sv.rs:2077-2086), checked locally in ir/ with matches!(m.nodes[operands[0]], Node::Constant{..}) (no emit/ dependency), plus the robustness exclusion of every sibling mark (vacuous — no other pass marks a CaseMux); CasezMux excluded (casez ? wildcards need a masked comparison — the follow-up); param_env skipped. (1) Module.case_mux_if_gates: BTreeSet<NodeId> beside mux_if_gates (types.rs:537), serde-default-empty. (2) In the structured-case always_comb loop (sv.rs:667-753) branch the GateOp::CaseMux arm (:690-702) on m.case_mux_if_gates.contains(&idx): marked => if (sel == SW'd0) name = data_0; else if ... else name = W'h0; reusing the SAME sel/sel_width/node_ref(data) the case arm computes + the same W'h0 default (the shared :747 tail becomes the trailing else; endcase omitted); unmarked => case...endcase verbatim. The static-collapse continue at :685-687 already skips constant-selector CaseMux before this arm, and the predicate excludes them, so the two agree. (3) Run annotate_case_mux_if_gates LAST (eighth guarded call site after mux_if at gen/mod.rs:160 / :409; ordering not load-bearing for exclusion but follows the convention; minimal blast radius). (4) case_mux_if_emit_prob knob (default 0.0) mirroring mux_if_emit_prob's six config.rs touch points (default fn :118, field :1032, Default :1281, validate-probs :1664, apply :1893, Overrides :2018) + --case-mux-if-emit-prob clap flag (main.rs :551/:1171). (5) num_emitted_case_mux_if_chains (= case_mux_if_gates.len()) => schema 1.15 -> 1.16 at .17b.2; because static selectors are excluded, len() == the true emitted-chain count (no over-count). (6) tool_matrix --case-mux-if-gate + ScenarioSet::CaseMuxIfSweep + case_mux_if_focus_config (comb-only, node-id+egraph, case_mux_if_emit_prob=1.0, case_mux_prob biased high) + ModuleReport.emitted_case_mux_if = module_metrics.num_emitted_case_mux_if_chains > 0 (METRIC-KEYED, no new text token — strictly more robust than scanning if(==), which FSM decode blocks also contain) + saw_case_mux_if_emit; templated on --mux-if-gate. (7) default 0.0 => pass skipped => zero draws => byte-identical; prob=1.0 gen_bool short-circuits; the mark leaves the flat IR / validators / CSE / canonical_module_signature untouched (the mux_if marking_leaves_identity proof, replicated). .17b proof plan (~8 lib proofs + snapshots 6/6 + forced ON-vs-OFF sweep Δ=0 + sim-equiv 20000 vectors + the banked gate) + the .17b pre-split (.17b.1/.17b.2/.17b.3) recorded. No source change. Rejected alternatives: marking all CaseMux incl. static + relying on the emitter continue (metric over-counts); a <g>__pc output var + passthrough (unnecessary — already an always_comb var); reusing mux_if_emit_prob (separate surface); unique/priority if qualifiers (the bare chain is maximally portable).`
  Verification: `this STRUCTURED-EMISSION-EXPANSION.17a commit. Docs-only => no src/ touched => cargo check/clippy/fmt/test --lib unaffected (the .15b.2 bank holds: cargo test --lib 616 / --bin tool_matrix 89 / snapshots 6/6); DUT byte-identical. bash scripts/check_doctrines.sh green (4 doctrines); check_knowledge_map.sh + check_memory_architecture.sh green (no card change — 0028 already carries answers:); mdbook build clean. Grounded line refs verified against current HEAD: render_static_structured_gate sv.rs:2069 (CaseMux arm :2077-2086), the structured-case always_comb loop sv.rs:667-753 (static-collapse continue :685, CaseMux arm :690, default/endcase tail :747-749), the gen/mod.rs roll chain :89-160 (single) / :323-409 (design), the config.rs mux_if_emit_prob touch points :118/:1032/:1281/:1664/:1893/:2018, the main.rs flag :551/:1171, types.rs mux_if_gates :537 + GateOp::CaseMux :2341.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.17a commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.17b`
  Status: `done`
  Goal: `Implement the eighth structured surface per the .17a design. Pre-split .17b.1 (live surface: case_mux_if_emit_prob knob + --case-mux-if-emit-prob flag + Module.case_mux_if_gates + new src/ir/case_mux_if_emit.rs (annotate_case_mux_if_gates, the dynamic-selector CaseMux predicate, run last) + src/ir/mod.rs registration + two guarded gen/mod.rs rolls (after mux_if) + the src/emit/sv.rs structured-case body branch (case->if/else-if) + ~8 lib proofs) + .17b.2 (num_emitted_case_mux_if_chains metric @ schema 1.15->1.16 + tool_matrix --case-mux-if-gate + saw_case_mux_if_emit (metric-keyed)) + .17b.3 (user docs: book/knobs/USER_GUIDE/README/KM). Default-off / DUT byte-identical.`
  Children: `STRUCTURED-EMISSION-EXPANSION.17b.1`, `STRUCTURED-EMISSION-EXPANSION.17b.2a`, `STRUCTURED-EMISSION-EXPANSION.17b.2b`, `STRUCTURED-EMISSION-EXPANSION.17b.3` (the `.17b.2` metric+gate leaf pre-split into `.17b.2a` metric+schema + `.17b.2b` the repo-owned gate, per the `.6b.2a`/`.6b.2b` / `.12b.2a`/`.12b.2b` convention)

- ID: `STRUCTURED-EMISSION-EXPANSION.17b.1`
  Status: `done`
  Goal: `The live eighth-surface source change: Config::case_mux_if_emit_prob (default 0.0) + --case-mux-if-emit-prob flag + Module.case_mux_if_gates: BTreeSet<NodeId> + new src/ir/case_mux_if_emit.rs (annotate_case_mux_if_gates, the dynamic-selector CaseMux candidate predicate excluding constant selectors + CasezMux + sibling marks, run last) + src/ir/mod.rs registration + two guarded gen/mod.rs rolls (after mux_if, single + design) + the src/emit/sv.rs structured-case always_comb body branch (case...endcase -> if...else if over the same operand refs + W'h0 default; the parallel case/endcase suppressed for a marked gate) + ~8 lib proofs. No metric/schema bump (the .17b.2 metric bumps 1.15->1.16; the knob rides the version). Default-off / DUT byte-identical (snapshots 6/6); a forced case_mux_if_emit_prob=1.0 ON-vs-OFF generated-RTL sweep proving the priority chain fires + Verilator -Wall Delta=0 vs OFF + both Yosys + Icarus + iverilog sim-equiv to the parallel case.`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --lib green incl. the new case_mux_if proofs; cargo test --bin tool_matrix green (unaffected); cargo test --test snapshots 6/6 byte-identical (default-off); a forced case_mux_if_emit_prob=1.0 sample emits an if/else-if priority chain + lints clean (Verilator -Wall Delta=0 vs OFF + both Yosys + Icarus) + is sim-equiv to the inline reference; CODEBASE_ANALYSIS + DEVELOPMENT_NOTES updated. Committed through COMMIT.md with the leaf id.`
  Result: `Done, implemented exactly per the .17a design (no deviations). New src/ir/case_mux_if_emit.rs: gate_qualifies (a GateOp::CaseMux with >= 1 arm whose selector operands[0] is NOT a Node::Constant — the dynamic-selector test; not in function_emit_gates / generate_loop_gates / task_emit_gates / soft_union_slice_gates / mux_if_gates / multi_output_task_groups[keys|values] / cone_function_gates[keys|interiors]) + annotate_case_mux_if_gates (collect-then-roll one gen_bool(p) per candidate into m.case_mux_if_gates; param_env skipped) + 9 lib proofs (prob_one marks a dynamic CaseMux; constant_selector excluded; prob_zero byte-identical; non-CaseMux excluded; CasezMux excluded; sibling-marked excluded; param_env skipped; identity/node-count untouched; the end-to-end emit proof: a marked dynamic CaseMux emits if (sel==W'd0) g = arm_0; else if ... else g = W'h0; and no case/endcase, unmarked emits case (sel) ... endcase). Module.case_mux_if_gates: BTreeSet<NodeId> (ir/types.rs, Module derives Default => empty); ir/mod.rs registration (alphabetical, rustfmt-sorted). Config::case_mux_if_emit_prob (config.rs: default_case_mux_if_emit_prob=0.0 + #[serde(default)] field + Default-construction entry + the validate probs-list entry + the Overrides Option<f64> + the apply); --case-mux-if-emit-prob clap flag + overlay wiring (main.rs). Two guarded gen/mod.rs rolls AFTER mux_if (single generate_module + design generate_design). emit/sv.rs: the GateOp::CaseMux match arm branches on m.case_mux_if_gates.contains(&(idx as NodeId)) (the if/else-if chain reusing the same sel/sel_width/node_ref(data) + W'h0 default vs the case body), and the shared default/endcase tail gains the same !contains(...) guard (a marked gate emits only its own trailing else). No metric/schema bump (rides the version; .17b.2 bumps 1.15->1.16). CODEBASE_ANALYSIS.md gains the ir/case_mux_if_emit.rs module-tree entry; DEVELOPMENT_NOTES.md gains the .17b.1 impl-time note. Default-off / DUT byte-identical. Frontier -> .17b.2.`
  Verification: `this STRUCTURED-EMISSION-EXPANSION.17b.1 commit. cargo check --all-targets clean (4.67s); cargo fmt --all --check clean (after rustfmt sorted the new ir/mod.rs decl alphabetically); cargo clippy --all-targets -- -D warnings clean; cargo test --lib 625 passed / 2 ignored (616 + 9 new case_mux_if proofs; umbrella DUT-byte-identical proofs green); cargo test --bin tool_matrix 89 passed / 1 ignored (unaffected — the new Config field defaults 0.0); cargo test --test snapshots 6/6 byte-identical (default-off — the 0.0 path never calls the pass). Forced case_mux_if_emit_prob=1.0 sweep (scratchpad/probe8/, comb-only --case-mux-prob 0.9, 8 seeds): every dynamic CaseMux projects to an if/else-if chain (seed 2: 7 CaseMux -> 7 chains, 14 else if, 0 residual case; seeds 1-5/11/13 similar; seed 7 has no CaseMux). Seed 2: Verilator --lint-only -Wall warning count ON == OFF (Delta=0) on all three standards (1800-2012/2017/2023 — the 3 pre-existing warnings are identical ON/OFF; the projection adds none, incl. for exhaustive selectors whose trailing else mirrors the case default); Yosys synth -noabc and abc -fast; opt -fast; check both 0 warnings/errors; iverilog -g2012 compiles (rc=0). Generated-RTL ON-vs-OFF sim-equiv (seed 2, both modules in one tb with the OFF module renamed mod_ref): iverilog+vvp prove the ON module bit-equal to OFF across 20000 random vectors on all 3 outputs (GEN-RTL SIM-EQUIV OK). The standalone construct probe (.16) already proved the chain === the parallel case over 20000 vectors + an exhaustive selector sweep. dump-config surfaces case_mux_if_emit_prob.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.17b.1 commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.17b.2a`
  Status: `done`
  Goal: `The eighth-surface metric + introspection schema bump. Add Metrics::num_emitted_case_mux_if_chains (= m.case_mux_if_gates.len(), #[serde(default)]) in metrics::compute() + a lib proof ⇒ bump introspection SCHEMA_VERSION 1.15 -> 1.16 across all current-output refs (introspect const + doc-comment + 3 assertions, 8 mcp assertions, docs/AGENT_INTROSPECTION_SCHEMA.md current-version + a 1.15->1.16 changelog entry, README + USER_GUIDE + CODEBASE envelope; historical num_emitted_mux_if_blocks @ 1.15 attributions intact; book example JSONs deferred to .17b.3). Default-off / DUT byte-identical. Split .17b.2 into .17b.2a (this, metric+schema) + .17b.2b (the repo-owned tool_matrix --case-mux-if-gate).`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --lib green incl. the new metric proof + all schema-version assertions at 1.16; cargo test --test snapshots 6/6 byte-identical (default-off); live --introspect shows schema_version 1.16 + num_emitted_case_mux_if_chains 0 default / > 0 forced; live docs amended. Committed through COMMIT.md with the leaf id.`
  Result: `Done. Metric: Metrics::num_emitted_case_mux_if_chains (= m.case_mux_if_gates.len(), #[serde(default)]) added beside num_emitted_mux_if_blocks + assigned in metrics::compute() + the metrics_count_emitted_case_mux_if_chains lib proof (unmarked => 0, marked dynamic CaseMux => 1). Exact because the annotation pass excludes constant-selector CaseMux, so every counted gate emits one chain. Schema: introspect/mod.rs SCHEMA_VERSION "1.15" -> "1.16" + the const doc-comment rewritten to attribute 1.16 to num_emitted_case_mux_if_chains (decision 0028) and demote 1.15 to the prior num_emitted_mux_if_blocks; 3 introspect lib assertions + 8 mcp lib assertions bumped to "1.16"; docs/AGENT_INTROSPECTION_SCHEMA.md "today both are early" example + "This document defines" + the §8 checklist line -> 1.16 + a new 1.15->1.16 changelog entry; README (×2: the --introspect envelope + the analyze schema) + USER_GUIDE (×1: the --sv-version row envelope ref) + CODEBASE_ANALYSIS.md (the envelope schema_version "1.15"->"1.16" + a 1.15->1.16 bump-list entry) bumped; the num_emitted_mux_if_blocks @ 1.15 historical landing attributions (README/USER_GUIDE knob-desc lines, CODEBASE metric entry + bump-list) left intact. Book example JSONs deferred to .17b.3 (the .15b.2/.15b.3 precedent). Default-off / DUT byte-identical. Frontier -> .17b.2b.`
  Verification: `this STRUCTURED-EMISSION-EXPANSION.17b.2a commit. cargo check --all-targets clean; cargo fmt --all --check clean; cargo clippy --all-targets -- -D warnings clean; cargo test --lib 626 passed / 2 ignored (625 + 1 new metrics_count_emitted_case_mux_if_chains proof; all introspect + mcp schema_version assertions green at "1.16"; umbrella DUT-byte-identical proofs green); cargo test --test snapshots 6/6 byte-identical (default-off — the metric is a post-hoc RTL-invisible count). Live --introspect: default (seed 42) => schema_version "1.16" + module_metrics.num_emitted_case_mux_if_chains 0; forced (seed 2, --flop-prob 0 --case-mux-prob 0.9 --case-mux-if-emit-prob 1.0) => "1.16" + 53. Historical "landed at schema X" attributions left intact (num_emitted_mux_if_blocks @ 1.15).`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.17b.2a commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.17b.2b`
  Status: `done`
  Goal: `The eighth-surface repo-owned downstream gate. Add tool_matrix --case-mux-if-gate (ScenarioSet::CaseMuxIfSweep + a case_mux_prob-biased case_mux_if_focus_config × the three strategies + ModuleReport.emitted_case_mux_if = module_metrics.num_emitted_case_mux_if_chains > 0 [METRIC-KEYED, no new text token] + CoverageSummary.saw_case_mux_if_emit + MatrixReport.case_mux_if_gate + the gap arm + a units floor + ~5 proofs + fixtures), templated on --mux-if-gate, banked clean. Default-off / DUT byte-identical.`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --bin tool_matrix green incl. the new gate proofs; cargo test --test snapshots 6/6 byte-identical; a banked clean --case-mux-if-gate report (coverage_gaps=[], saw_case_mux_if_emit=true, N/0 Verilator + both Yosys + Icarus); README + USER_GUIDE + CODEBASE gate entries. Committed through COMMIT.md with the leaf id.`
  Result: `Done. Repo-owned tool_matrix --case-mux-if-gate landed, templated 1:1 on --mux-if-gate across ~30 touch points in src/bin/tool_matrix.rs: CASE_MUX_IF_SWEEP_MIN_UNITS_PER_SCENARIO = 4 + the --case-mux-if-gate CLI flag + ScenarioSet::CaseMuxIfSweep + build_case_mux_if_sweep_scenarios + case_mux_if_focus_config + ModuleReport.emitted_case_mux_if (METRIC-KEYED: = prepared.metrics.num_emitted_case_mux_if_chains > 0, NOT a text token — a marked CaseMux emits no new identifier, only the always_comb body swaps case→if/else-if; a text scan for if(==) would also match FSM decode blocks) + CoverageSummary.saw_case_mux_if_emit + merge + MatrixReport.case_mux_if_gate + run-plan units/fail-on-gap + select/build dispatch + mutual-exclusion count/msg + the summarize-module-coverage block + the early-return gap arm + the three exhaustive match arms (no-op / required_categories / required_knobs) + scenario_set_slug "case-mux-if-sweep" + artifact_kind_slug + test_cli default + 5 cargo-portable proofs + 8 fixture extensions. The focus config diverges from --mux-if-gate's Mux-biasing in ONE load-bearing way: it sets comb_mux_prob = 0.0 (not the default 0.1) and case_mux_prob = 0.9, because in cone.rs comb_mux rolls BEFORE case_mux and short-circuits — a high comb_mux_prob would starve the case-mux roll. No comb_mux_encoding_prob steering is needed (a CaseMux selector is a generated dynamic cone by construction; no encoding-path trap, the inverse of the seventh surface). Default-off (case_mux_if_emit_prob = 0.0) ⇒ DUT byte-identical. Banked clean anvil-case-mux-if-gate-r1.`
  Verification: `this STRUCTURED-EMISSION-EXPANSION.17b.2b commit. cargo check --all-targets clean; cargo fmt --all --check clean; cargo clippy --all-targets -- -D warnings clean (one doc_lazy_continuation fixed: reworded a wrapped doc line that began with "+" so it is not parsed as a markdown list bullet); cargo test --bin tool_matrix 89→94 passed / 1 ignored (5 new proofs: case_mux_if_gate_flag_defaults_false_and_parses, case_mux_if_gate_selects_set_and_raises_units, case_mux_if_gate_is_mutually_exclusive_with_other_gates, case_mux_if_sweep_scenarios_force_the_knob, case_mux_if_sweep_gaps_require_the_fact); cargo test --test snapshots 6/6 byte-identical (default-off). Banked clean ./target/release/tool_matrix --case-mux-if-gate --yosys-mode both --iverilog-compile --out anvil-case-mux-if-gate-r1: 3 scenarios / 12 modules / 12 emitting a chain / 83 chains, coverage_gaps = [], saw_case_mux_if_emit = true, case_mux_if_gate = true, Verilator 12/0 + Yosys without-abc 12/0 + Yosys with-abc 12/0 + Icarus compile 12/0, exit 0. Spot-checked emitted SV (int_nodeid_egraph_case_mux_if/mod_2_0000.sv): always_comb begin if (slice_0 == 1'd0) … else if (slice_0 == 1'd1) … end — exactly the designed priority chain.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.17b.2b commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.17b.3`
  Status: `done`
  Goal: `The eighth-surface user docs (DOCS-ONLY ⇒ DUT byte-identical, NOT code-gated). A book/src/structured-emission.md "## The eighth surface: a procedural if/else if priority chain" section (byte-verified before/after example) + the chapter-intro surface list + the case_mux_if_emit_prob entry in book/src/knobs.md + USER_GUIDE.md + README.md "Current CLI truth" + the deferred book example-JSON schema_version 1.15 -> 1.16 refresh (api-tools.md ×3 + agent-mcp.md ×1, also the stale seed-42 run_id 3f1cad578805bd04 -> ee39c1e3df8192dd from the .17b.1 Config-field addition) + a docs/knowledge/case-mux-if-emit.md KM card (KNOWLEDGE_MAP.md regenerated). On completion .17b / .17 close ⇒ EIGHTH surface delivered end-to-end.`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.19`
  ...
  Children: `STRUCTURED-EMISSION-EXPANSION.19a`, `STRUCTURED-EMISSION-EXPANSION.19b`
  Result: `Done 2026-06-23. .19a (impl design-detail, DEVELOPMENT_NOTES.md, no source) + .19b (the live impl: .19b.1 live source / .19b.2 metric+gate [.19b.2a schema 1.17 / .19b.2b tool_matrix --casez-mux-if-gate, banked clean anvil-casez-mux-if-gate-r1] / .19b.3 docs) all done. The NINTH structured surface — the procedural always_comb if/else if MASKED priority chain (sel & care_mask) == value_masked projecting a wildcard CasezMux (decision 0029) — is delivered end-to-end: live behind casez_mux_if_emit_prob, metered at introspection schema 1.17, proven downstream-clean, and documented in the book/knobs/USER_GUIDE/README/KM.`

- ID: `STRUCTURED-EMISSION-EXPANSION.19a`
  Status: `done`
  Goal: `Design-detail leaf (no source change): ground decision 0029 (the ninth surface) in a fresh read of the real code — src/ir/case_mux_if_emit.rs (the annotation-pass template), src/emit/sv.rs (the GateOp::CasezMux structured-case arm + render_casez_pattern + the constant_value/bitmask/casez_pattern_matches helpers + the trailing-default suppression), src/gen/cone/motifs.rs (build_casez_patterns), src/metrics.rs / src/ir/compact.rs (the care_mask idiom), src/gen/mod.rs (the roll chain), src/config.rs, src/main.rs, src/ir/types.rs, src/ir/mod.rs, src/introspect/mod.rs. Pin every .19b impl point (the dynamic-selector CasezMux candidate predicate, the carrier, the in-place masked-chain emitter branch + the trailing-default suppression tweak, run last, the knob/flag, the metric+schema 1.16→1.17, the metric-keyed gate + focus config, the byte-identical/RNG argument) + the .19b proof plan. DEVELOPMENT_NOTES.md design-detail entry.`
  Acceptance: `A DEVELOPMENT_NOTES.md design-detail entry resolving every impl point grounded in real code (with cited line numbers); the empirical form-selection result recorded (==? disqualified, masked-AND ships); tree split recorded; no source change; docs/workflow self-checks clean.`
  Result: `Done. A DEVELOPMENT_NOTES.md "## 2026-06-23 — CasezMux masked if/else if priority-chain surface — impl design-detail — STRUCTURED-EMISSION-EXPANSION.19a" entry, mirroring the eighth surface's .17a, grounding decision 0029 in the real code with cited line numbers and resolving impl points 0–7: (0) candidate = a GateOp::CasezMux with a non-Constant selector + operands.len() >= 4, excluding CaseMux/case_mux_if + sibling marks + param_env; (1) Module.casez_mux_if_gates: BTreeSet<NodeId> + ir/mod.rs registration; (2) the in-place emit/sv.rs CasezMux arm branch computing per-arm care_mask = !wildcard_mask & sel_mask + value_masked = pattern_value & care_mask from the (value, mask) constants via the existing constant_value/bitmask helpers, emitting (sel & SW'h{care}) == SW'h{val} chained + trailing else W'h0, NO __cv (a CasezMux is already an always_comb var), plus the :769 trailing-default suppression tweak to also exclude marked casez gates; (3) run annotate_casez_mux_if_gates LAST (after case_mux_if) in both gen/mod.rs paths; (4) Config::casez_mux_if_emit_prob + --casez-mux-if-emit-prob flag at all six config.rs touch points + Overrides; (5) Metrics::num_emitted_casez_mux_if_chains = m.casez_mux_if_gates.len() ⇒ schema 1.16→1.17, exact (static selectors excluded); (6) the metric-keyed tool_matrix --casez-mux-if-gate / saw_casez_mux_if_emit + a casez_mux_prob-biased focus config that must ZERO comb_mux_prob AND case_mux_prob (both roll before casez_mux in cone.rs — the eighth surface's short-circuit-chain trap generalized); (7) the byte-identical/RNG argument. The empirical form selection is recorded: the sel ==? pattern wildcard-equality form is DISQUALIFIED (Yosys 0.64 rejects ==? in both repo modes), the lowered masked-AND form ships (clean across Verilator -Wall 2012/2017/2023 + both Yosys + Icarus + exhaustive iverilog sim-equivalence). The .19b proof plan (~9 lib proofs mirroring case_mux_if_emit's + the live source change shape) is pinned. No source change ⇒ DUT byte-identical. Pre-split .19 → .19a (done) + .19b (impl, .19b.1 live / .19b.2 metric+gate / .19b.3 docs); frontier → .19b.1.`
  Verification: `done`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.19a commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.19b`
  Status: `done`
  Goal: `Implement the .19a design: the live casez_mux_if surface (a new src/ir/casez_mux_if_emit.rs annotate_casez_mux_if_gates + Module.casez_mux_if_gates + ir/mod.rs registration + the casez_mux_if_emit_prob knob/flag + two gen/mod.rs rolls after case_mux_if + the emit/sv.rs CasezMux masked-chain body branch + the trailing-default suppression tweak + ~9 lib proofs), the num_emitted_casez_mux_if_chains metric (introspection schema 1.16→1.17), the metric-keyed tool_matrix --casez-mux-if-gate / saw_casez_mux_if_emit gate (casez_mux_prob-biased focus config, banked clean), and book/USER_GUIDE/KM. Default-off / DUT byte-identical. Pre-split into .19b.1 (live) + .19b.2 (metric+gate) + .19b.3 (docs).`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test green incl. the new casez_mux_if proofs + the schema 1.17 assertions; cargo test --test snapshots byte-identical (default-off); a repo-owned tool_matrix --casez-mux-if-gate banked clean (coverage_gaps = [] / saw_casez_mux_if_emit / Verilator + both Yosys + Icarus). Default-off / DUT byte-identical. Committed through COMMIT.md per slice with the leaf id.`
  Children: `STRUCTURED-EMISSION-EXPANSION.19b.1`, `STRUCTURED-EMISSION-EXPANSION.19b.2`, `STRUCTURED-EMISSION-EXPANSION.19b.3`
  Result: `Done. .19b.1 (the live source change) done 2026-06-23; .19b.2 (metric + schema 1.16→1.17 [.19b.2a] + the repo-owned tool_matrix --casez-mux-if-gate [.19b.2b, banked clean anvil-casez-mux-if-gate-r1]) done 2026-06-23; .19b.3 (book ninth-surface section + chapter-intro list + knobs/USER_GUIDE/README entries + the deferred book example-JSON 1.16→1.17 + a casez-mux-if-emit KM card / KNOWLEDGE_MAP.md regenerated) done 2026-06-23. .19b closes ⇒ .19 closes ⇒ the NINTH structured surface is delivered end-to-end.`

- ID: `STRUCTURED-EMISSION-EXPANSION.19b.1`
  Status: `done`
  Goal: `The live ninth-surface source change: Config::casez_mux_if_emit_prob (default 0.0, serde default) + --casez-mux-if-emit-prob flag + Module.casez_mux_if_gates: BTreeSet<NodeId> + new src/ir/casez_mux_if_emit.rs annotate_casez_mux_if_gates (the dynamic-selector GateOp::CasezMux candidate, run last) + ir/mod.rs registration + two guarded gen/mod.rs call-site rolls (after case_mux_if) + the emit/sv.rs CasezMux masked-chain body branch + the trailing-default suppression tweak + ~9 lib proofs + a forced-knob downstream spot-check. Default-off / DUT byte-identical (snapshots 6/6).`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --lib green incl. the new casez_mux_if proofs; cargo test --test snapshots 6/6 byte-identical (default-off); a forced casez_mux_if_emit_prob=1.0 sweep emits masked chains accepted warning-clean by Verilator + both Yosys + Icarus and sim-equiv to the casez. Committed through COMMIT.md with the leaf id.`
  Result: `Done. Config::casez_mux_if_emit_prob (default 0.0, default_casez_mux_if_emit_prob() serde default; added to the Default impl + the 0.0..=1.0 validation list + the apply + the Overrides Option<f64>) + the --casez-mux-if-emit-prob clap flag + overlay (main.rs) + Module.casez_mux_if_gates: BTreeSet<NodeId> (Default-empty; emitter-surface annotation only — flat IR / validators / CSE / canonical_module_signature untouched) + new src/ir/casez_mux_if_emit.rs annotate_casez_mux_if_gates (gen-time mark; the case_mux_if_emit.rs precedent; gate_qualifies = a GateOp::CasezMux with a non-Constant selector + operands.len() >= 4, excluding CaseMux + case_mux_if + all sibling marks; rolls gen_bool(prob) per candidate; skips param_env modules) + ir/mod.rs registration + two call-site rolls in BOTH generate_module and generate_design guarded on casez_mux_if_emit_prob > 0.0, run AFTER case_mux_if + src/emit/sv.rs rendering: the GateOp::CasezMux arm branches on m.casez_mux_if_gates.contains(&idx) — a marked gate emits the masked chain (per arm: care_mask = !wildcard_mask & sel_mask, value_masked = pattern_value & care_mask via the existing constant_value/bitmask helpers, kw (sel & SW'h{care}) == SW'h{val}) g = data; + a trailing else g = W'h0;), an unmarked gate the verbatim casez; the shared trailing-default/endcase tail (:769) now also excludes marked casez gates. The masked-AND form ships (the .18 probe disqualified sel ==? pattern — Yosys rejects ==?). NO schema bump (the .19b.2 metric bumps 1.16→1.17). 9 lib proofs (prob_one_marks_a_dynamic_casez_mux; constant_selector_casez_mux_is_excluded; prob_zero_marks_nothing_byte_identical; case_mux_gate_does_not_qualify; plain_mux_gate_does_not_qualify; sibling_marked_casez_mux_is_excluded; param_env_module_is_skipped; marking_leaves_identity_and_node_count_untouched; marked_casez_mux_emits_masked_priority_chain_unmarked_is_casez).`
  Verification: `cargo check --all-targets clean; cargo clippy --all-targets -- -D warnings clean; cargo fmt --all --check clean; cargo test --lib 635 passed / 2 ignored (incl. 9 new casez_mux_if_emit proofs); cargo test --test snapshots 6/6 byte-identical (default-off). Forced casez_mux_if_emit_prob=1.0 sweep (scratchpad/sweep19, 6 seeds, comb-only casez_mux_prob=1.0): every casez block → a masked if/else if chain (3 masked arms each, 0 residual casez); Verilator -Wall ON-vs-OFF Δ=0 across 1800-2012/2017/2023 (18 comparisons); Yosys without-abc + abc-fast 0 issues; Icarus iverilog -g2012 0 msg lines; ON-vs-OFF iverilog sim-equiv seed 1 (8/8) + seed 4 (128/128, a real i_1 data arm), 0 mismatches.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.19b.1 commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.19b.2`
  Status: `done`
  Goal: `The metric + repo-owned gate. Pre-split into .19b.2a (metric + introspection schema 1.16 → 1.17) + .19b.2b (the repo-owned tool_matrix --casez-mux-if-gate) — the metric is a small Metrics-field schema MINOR bump, the gate a large/fragile harness change (the .17b.2a / .17b.2b precedent). Default-off / DUT byte-identical.`
  Children: `STRUCTURED-EMISSION-EXPANSION.19b.2a`, `STRUCTURED-EMISSION-EXPANSION.19b.2b`
  Result: `Done. .19b.2a (metric + schema 1.17) done 2026-06-23; .19b.2b (the repo-owned tool_matrix --casez-mux-if-gate, banked clean anvil-casez-mux-if-gate-r1: 12/12 emitting / 108 chains / coverage_gaps = [] / 12/0 Verilator + both Yosys + Icarus) done 2026-06-23. .19b.2 closes; frontier → .19b.3 (docs).`

- ID: `STRUCTURED-EMISSION-EXPANSION.19b.2a`
  Status: `done`
  Goal: `The num_emitted_casez_mux_if_chains metric: add Metrics::num_emitted_casez_mux_if_chains (usize, #[serde(default)]) computed as m.casez_mux_if_gates.len(); it surfaces in introspection module_metrics, so bump introspection schema MINOR 1.16 → 1.17 (SCHEMA_VERSION const + doc-comment history + all current-output assertions in introspect/mcp + the schema-doc current-version + a 1.16→1.17 changelog entry + README/USER_GUIDE/CODEBASE envelope refs). A lib proof. Per-metric landing attributions + book JSONs left for later. Default-off / DUT byte-identical (a post-hoc Metrics field changes no emitted RTL; snapshots untouched).`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --lib green incl. the metric proof + the schema_version 1.17 assertions; cargo test --test snapshots 6/6 byte-identical; the schema doc records the 1.16 → 1.17 additive MINOR bump; live --introspect shows 1.17 + the metric. Committed through COMMIT.md with the leaf id.`
  Result: `Done. Metrics::num_emitted_casez_mux_if_chains: usize (#[serde(default)]) added to src/metrics.rs, computed in metrics::compute() as m.casez_mux_if_gates.len() (a post-hoc structural count of an emitter-surface annotation; reads 0 by default, the configured count when casez_mux_if_emit_prob fired; EXACT because static-selector CasezMux are excluded). Surfaced in introspection module_metrics (Metrics is the exact serde projection), so SCHEMA_VERSION bumped 1.16 → 1.17 in src/introspect/mod.rs (const + the doc-comment history note). Bumped all current-output schema refs to 1.17: the 3 introspect assertions + 8 mcp assertions, the schema doc (current-version ×3 + a new 1.16→1.17 changelog entry above the 1.15→1.16 one), README envelope ×2 (--introspect + analyze), USER_GUIDE envelope ×1 (--sv-version), and the CODEBASE_ANALYSIS envelope line. Per-metric "landed at schema X" attributions (num_emitted_case_mux_if_chains @ 1.16 etc.) left intact (the .17b.2a convention); the 5 book agent-mcp.md example JSONs deferred to .19b.3. Lib proof metrics_count_emitted_casez_mux_if_chains (unmarked 0, marked 1). The metric BUMPS the schema (new derived Metrics field — the 1.0→1.1 bisimulation_flops_merged precedent) whereas the .19b.1 knob rode the version.`
  Verification: `cargo check --all-targets clean; cargo clippy --all-targets -- -D warnings clean; cargo fmt --all --check clean; cargo test --lib 636 passed / 2 ignored (the new metric proof + all schema_version assertions green at 1.17); cargo test --test snapshots 6/6 byte-identical (default-off; metric changes no RTL); end-to-end --introspect: default seed 2 ⇒ schema_version 1.17 + num_emitted_casez_mux_if_chains 0; forced casez_mux_if_emit_prob=1.0 (seed 4) ⇒ 1.17 + 1. No stray current "1.16" left in src.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.19b.2a commit`

- ID: `STRUCTURED-EMISSION-EXPANSION.19b.2b`
  Status: `done`
  Goal: `The repo-owned tool_matrix gate: a saw_casez_mux_if_emit coverage fact + a --casez-mux-if-gate flag + ScenarioSet::CasezMuxIfSweep forcing casez_mux_if_emit_prob=1.0 over comb-only DUTs × 3 strategies (casez_mux_if_focus_config with casez_mux_prob biased high and BOTH comb_mux_prob and case_mux_prob zeroed — both roll before casez_mux in cone.rs) + METRIC-KEYED ModuleReport.emitted_casez_mux_if (= module_metrics.num_emitted_casez_mux_if_chains > 0, no new text token) + CoverageSummary.saw_casez_mux_if_emit + merge + MatrixReport.casez_mux_if_gate + run-plan/dispatch/mutual-exclusion/summarize/gap arm/match arms/slug arms/test_cli default + proofs + fixtures, proving Verilator + both Yosys modes + Icarus accept the masked chains warning-clean. Bank clean anvil-casez-mux-if-gate-r1. Templated on --case-mux-if-gate. Default-off / DUT byte-identical.`
  Acceptance: `cargo check/clippy(-D warnings)/fmt clean; cargo test --bin tool_matrix green incl. the new gate proofs; snapshots 6/6 byte-identical; banked clean tool_matrix --casez-mux-if-gate (coverage_gaps = [] / saw_casez_mux_if_emit / Verilator + both Yosys + Icarus). Committed through COMMIT.md with the leaf id.`
  Result: `Done. The full --casez-mux-if-gate wiring landed in src/bin/tool_matrix.rs (~30 touch points mirroring --case-mux-if-gate 1:1): CASEZ_MUX_IF_SWEEP_MIN_UNITS_PER_SCENARIO const + the --casez-mux-if-gate CLI flag + ScenarioSet::CasezMuxIfSweep + build_casez_mux_if_sweep_scenarios (one comb-only DUT × 3 construction strategies) + casez_mux_if_focus_config (node-id + e-graph, casez_mux_if_emit_prob=1.0, casez_mux_prob=0.9, flop_prob=0.0, and BOTH comb_mux_prob AND case_mux_prob forced to 0.0 — both roll before casez_mux in src/gen/cone.rs and short-circuit it, so the single-zero of --case-mux-if-gate generalizes to a double-zero) + METRIC-KEYED ModuleReport.emitted_casez_mux_if (= prepared.metrics.num_emitted_casez_mux_if_chains > 0; NO new text token — a marked CasezMux is already an always_comb logic var) + CoverageSummary.saw_casez_mux_if_emit + the merge_coverage |= + MatrixReport.casez_mux_if_gate + the cli→report mapping + run-plan units + fail_on_coverage_gap + the mutual-exclusion count + message + select_scenario_set + build_scenarios dispatch + the early-return gap arm + the 3 exhaustive "unreachable" match arms (categories/knobs/return) + scenario_set_slug "casez-mux-if-sweep" + artifact_kind_slug "module" + the test_cli default + the 8 ModuleReport fixtures + 5 cargo-portable proofs (flag parse / set-select+units / mutual-exclusion / knob-forcing / gap-requires-fact). Co-located hygiene: a 1-line blank-line fix to the .19b.2a metrics.rs test resolved a pre-existing rustfmt-1.9.0 fmt drift (a standalone comment vertically aligned to the preceding trailing-comment column — HEAD itself failed cargo fmt --all --check). Default-off / DUT byte-identical.`
  Verification: `cargo check --all-targets clean; cargo clippy --all-targets -- -D warnings clean; cargo fmt --all --check clean (the .19b.2a drift fixed); cargo test --bin tool_matrix green incl. the 5 new casez_mux_if_* proofs; cargo test --lib casez_mux_if 10/10 (metrics test intact after the whitespace edit); cargo test --test snapshots 6/6 byte-identical (default-off ⇒ DUT byte-identical). Banked downstream-clean anvil-casez-mux-if-gate-r1 (release tool_matrix --casez-mux-if-gate --yosys-mode both --iverilog-compile): 3 scenarios / 12 modules / 12 emitting a chain / 108 chains / coverage_gaps = [] / saw_casez_mux_if_emit = true / 12/0 Verilator + Yosys without-abc + Yosys with-abc + Icarus compile. bash scripts/check_doctrines.sh green (4 doctrines; code-scoped checks satisfied).`
  Commit: `STRUCTURED-EMISSION-EXPANSION.19b.2b — tool_matrix --casez-mux-if-gate` (`925c103`)

- ID: `STRUCTURED-EMISSION-EXPANSION.19b.3`
  Status: `done`
  Goal: `The ninth-surface user docs (DOCS-ONLY ⇒ DUT byte-identical): a book/src/structured-emission.md "## The ninth surface: a masked if/else if priority chain" section (byte-verified before/after) + the chapter-intro surface list + the casez_mux_if_emit_prob entry in book/src/knobs.md + USER_GUIDE.md + README.md "Current CLI truth" + the deferred book example-JSON schema_version 1.16 → 1.17 refresh + a docs/knowledge/casez-mux-if-emit.md KM card (KNOWLEDGE_MAP.md regenerated). On completion .19b / .19 close ⇒ the NINTH surface delivered end-to-end.`
  Acceptance: `mdbook build book clean; cargo test --test book_examples green; check_knowledge_map.sh OK; the byte-verified before/after example matches real emitter output; no src/ touched ⇒ DUT byte-identical. Committed through COMMIT.md with the leaf id.`
  Result: `Done. DOCS-ONLY ⇒ DUT byte-identical (no src/ touched). (1) book/src/structured-emission.md: a "## The ninth surface: a masked if/else if priority chain" section (a byte-verified seed-1 before/after captured from the real emitter — the casez (slice_0) 2'b0?/2'b1? block vs the masked if ((slice_0 & 2'h2) == 2'h0)/(== 2'h2) chain — plus What-gets-wrapped / How-anvil-proves-it / Reproducing-it subsections mirroring the eighth surface), and the chapter-intro surface list gains ", and a masked if/else if priority chain". (2) The 4 deferred book example-JSON schema_version "1.16" → "1.17" (book/src/agent-mcp.md ×1 + book/src/api-tools.md ×3). (3) The casez_mux_if_emit_prob knob entry in book/src/knobs.md + USER_GUIDE.md + a --casez-mux-if-gate matrix-gate entry in USER_GUIDE.md, plus README.md "Current CLI truth" casez_mux_if_emit_prob knob para + --casez-mux-if-gate gate para. (4) A docs/knowledge/casez-mux-if-emit.md KM card (templated on case-mux-if-emit.md, linking [[case-mux-if-emit]] + [[structured-emission-ninth-surface-casez-mux-masked-priority-chain]]); KNOWLEDGE_MAP.md regenerated (69 → 70 facts / 672 question keys). The before/after example was regenerated against a freshly-rebuilt anvil binary (a stale release binary initially lacked the .19b.1 --casez-mux-if-emit-prob flag — rebuilt --bin anvil before capturing).`
  Verification: `mdbook build book clean (HTML written, no warnings); cargo test --test book_examples 3/3 green (skip_sentinels_have_reasons + harness_detects_a_broken_command + every_runnable_book_bash_block_succeeds — the new Reproducing-it bash block carries a book-test: skip sentinel with a reason); bash knowledge-map/scripts/check_knowledge_map.sh OK (facts valid, ids unique, map in sync, casez-mux-if-emit indexed); the seed-1 before/after example byte-matches real emitter output (anvil --seed 1 --config <casez-focused> with/without --casez-mux-if-emit-prob 1.0; off == 0.0 byte-identical confirmed). No src/ touched ⇒ DUT byte-identical.`
  Commit: `this STRUCTURED-EMISSION-EXPANSION.19b.3 commit`

## Current Frontier

**No current frontier — the NINTH structured surface is DELIVERED END-TO-END (`.18` + `.19a` +
`.19b.1` + `.19b.2a` + `.19b.2b` + `.19b.3` all done); `.19b`/`.19` closed `2026-06-23`.** The
tree stays `active` at a no-frontier boundary: future surfaces (nested/multi-level `generate`,
`interface`/`modport`) are the recorded `.20+` candidates, each picked on its own merits with its
own decision (per `feedback_pick_and_roll_at_no_frontier`, the next lane is self-selected when
work resumes). **`.19b.3` (done)** landed the ninth-surface user docs (DOCS-ONLY ⇒ DUT
byte-identical): a `book/src/structured-emission.md` "## The ninth surface: a masked if/else if
priority chain" section with a byte-verified seed-1 before/after (the `casez (slice_0)` block vs
the masked `if ((slice_0 & 2'h2) == …)` chain) + the chapter-intro surface list + the
`casez_mux_if_emit_prob` entries in `book/src/knobs.md` / `USER_GUIDE.md` / README + a
`--casez-mux-if-gate` matrix-gate entry + the deferred book example-JSON `schema_version`
`1.16 → 1.17` (`agent-mcp.md` ×1 + `api-tools.md` ×3) + a `docs/knowledge/casez-mux-if-emit.md`
KM card (KNOWLEDGE_MAP.md regenerated, 69 → 70 facts / 672 keys); `mdbook build` clean +
`book_examples` 3/3 + `check_knowledge_map` OK. **`.19b.2b` (done)** landed the repo-owned
`tool_matrix --casez-mux-if-gate`
(metric-keyed, `ScenarioSet::CasezMuxIfSweep` + `casez_mux_if_focus_config` × 3 strategies with
BOTH `comb_mux_prob` and `case_mux_prob` zeroed) — banked clean
`anvil-casez-mux-if-gate-r1`: 3 scenarios / 12 modules / 12 emitting a chain / 108 chains /
`coverage_gaps = []` / `saw_casez_mux_if_emit = true` / 12/0 Verilator + both Yosys modes +
Icarus; a co-located 1-line blank-line fix resolved a pre-existing rustfmt-1.9.0 fmt drift in
the `.19b.2a` `metrics.rs` test. `.19b.2` closes; only the user docs at `.19b.3` remain before
`.19b`/`.19` close ⇒ the ninth surface delivered end-to-end. `.19b.2a` (done) added
`Metrics::num_emitted_casez_mux_if_chains` (`= m.casez_mux_if_gates.len()`, exact) ⇒
introspection schema `1.16 → 1.17` (all current-output refs; per-metric landing attributions +
book JSONs left intact/deferred); `cargo test --lib` 636, snapshots 6/6 byte-identical; live
`--introspect` `1.17` + `0` default / `1` forced. The repo-owned `tool_matrix
--casez-mux-if-gate` (metric-keyed) is next at `.19b.2b`; the user docs at `.19b.3`. `.19b.1` (done)
landed the live source change: the procedural `always_comb` `if`/`else if` **masked**
priority-chain emit-projection of a dynamic-selector `CasezMux` goes live behind the opt-in
`casez_mux_if_emit_prob` knob — new `src/ir/casez_mux_if_emit.rs` + `Module.casez_mux_if_gates`
+ `ir/mod.rs` registration + the knob/flag + two `gen/mod.rs` rolls (after `case_mux_if`) +
the `emit/sv.rs` `CasezMux` masked-chain body branch (`(sel & SW'h{care}) == SW'h{val}` via
the existing `constant_value`/`bitmask` helpers, no `__cv`) + the `:769` trailing-default
suppression tweak + 9 lib proofs. `cargo test --lib` 635, snapshots 6/6 byte-identical;
forced sweep (6 seeds) Verilator `-Wall` Δ=0 (2012/2017/2023) + both Yosys + Icarus clean +
ON-vs-OFF sim-equiv (seed 1 8/8 + seed 4 128/128). No metric/schema bump (the `.19b.2` metric
bumps `1.16 → 1.17`). The metric + the repo-owned `tool_matrix --casez-mux-if-gate` are next
at `.19b.2`; the user docs at `.19b.3`. `.19a`
(done) landed the impl design-detail — a `DEVELOPMENT_NOTES.md` entry grounding decision
`0029` in the real code (the `case_mux_if_emit.rs` template; the `emit/sv.rs` `CasezMux` arm
`:724` + `render_casez_pattern` `:2171` + the `constant_value`/`bitmask`/`casez_pattern_matches`
helpers + the `:769` trailing-default suppression; `build_casez_patterns` `wildcard_bits=1`;
the `metrics.rs`/`compact.rs` `care_mask` idiom; the roll chain; config/main/types/mod/introspect
touch points), pinning all impl points (the dynamic-selector `CasezMux` predicate; the
`casez_mux_if_gates` carrier; the in-place masked-chain emitter branch `(sel & SW'h{care}) ==
SW'h{val}` with no `__cv` + the trailing-default suppression tweak; run last; the knob/flag;
the metric + schema `1.16 → 1.17`; the metric-keyed `--casez-mux-if-gate` + a focus config
that zeros both `comb_mux_prob` and `case_mux_prob`; the byte-identical/RNG argument) + the
`.19b` proof plan. `.18` (done) — autonomously at
...
--case-mux-if-gate`), and `.17b.3` (user docs) are all `done`; `.17b` / `.17` close. The
N-way `CaseMux` (rendered today as the parallel `always_comb case (sel) … default`) is
projected — under the opt-in `case_mux_if_emit_prob` knob — into an `if`/`else if` priority
chain over the same operand refs, **no `__cv` passthrough** (a `CaseMux` is already an
`always_comb`-written `logic` var, the simpler-than-seventh case). The surface ships
`src/ir/case_mux_if_emit.rs` + `Module.case_mux_if_gates` + the `case_mux_if_emit_prob`
knob/flag + two `gen/mod.rs` rolls (after `mux_if`) + the `emit/sv.rs` structured-case body
branch + 9 lib proofs (`.17b.1`), the `num_emitted_case_mux_if_chains` metric @ introspection
schema `1.16` + the **metric-keyed** `tool_matrix --case-mux-if-gate` (banked clean
`anvil-case-mux-if-gate-r1`: 3 scenarios / 12 modules / 12 emitting a chain / 83 chains /
`coverage_gaps = []` / `saw_case_mux_if_emit = true` / `12/0` Verilator + both Yosys + Icarus)
(`.17b.2a`/`.17b.2b`), and the user docs (`book/src/structured-emission.md` eighth-surface
section with a byte-verified seed-1 example + the `case_mux_if_emit_prob` entries in
book/knobs + USER_GUIDE + README + the deferred book example-JSON `1.15 → 1.16` refresh + the
`case-mux-if-emit` KM card) (`.17b.3`). Default-off / DUT byte-identical throughout (`cargo
test --lib` 626, `--bin tool_matrix` 94, snapshots 6/6, book_examples 3/3). The next surface
(the `CasezMux` masked priority chain, nested/multi-level `generate`, or `interface` /
`modport` — empirically disqualified since `.7`) is PNT-selected at the next no-frontier
boundary per `feedback_pick_and_roll_at_no_frontier`; each lands as its own design+decision
leaf, none retired. The lane stays `active`. (Observed, NOT owned by this tree: the
`agent-mcp.md` `analyze`-tool example JSONs sit at schema `1.11`, frozen across the `.12`/`.13`/`.15`/`.17`
introspect-schema bumps — a separate long-standing staleness flagged for a dedicated docs pass.)

SEVEN structured surfaces are delivered end-to-end. The seventh surface (the procedural `always_comb`
...
gate (banked clean `anvil-mux-if-gate-r1`: 3 scenarios / 12 modules /
...
The eighth surface was **PNT-selected at the no-frontier boundary** per
`feedback_pick_and_roll_at_no_frontier` (`.16`, decision `0028`): the N-way `CaseMux` →
`if`/`else if` priority chain, the recorded follow-up of decision `0027`. The remaining
future candidates are the `CasezMux` masked priority chain, nested/multi-level
`generate`, and `interface`/`modport` (empirically disqualified since `.7`) — each lands
as its own design+decision leaf when picked, none retired. The lane stays `active`.
...
proofs; gate re-bank `anvil-mo-k3-gate-r1` `coverage_gaps = []` with a k=3
...
`anvil-cone-function-gate-r1`, introspection schema `1.11`), and the
**multi-output combinational `task automatic`** (`.11`+`.12`, decision `0025`:
`.11` design + `.12a` design-detail + `.12b.1` live surface + `.12b.2a` metric +
introspection schema `1.14` + `.12b.2b` repo-owned `--multi-output-task-gate` +
`.12b.3` user docs; banked clean `anvil-multi-output-task-gate-r1` — a
co-supported pair of mutually-fan-in-independent gates co-emitted into one `task
automatic` with deduplicated input formals + per-member output args, reusing the
cone-function dedup render primitives). Future surfaces — wider (`k > 2`) task
groups, nested/multi-level `generate`, `interface` / `modport` — are `.13+`, each
its own decision when picked at a no-frontier boundary per
...
| — | `STRUCTURED-EMISSION-EXPANSION.12b.3` | `done` | User-facing docs closeout — **the sixth surface delivered end-to-end**. `book/src/structured-emission.md` gains a `## The sixth surface: a multi-output task automatic` section (byte-verified seed-3 before/after — the inline `xor_0 = i_1 ^ i_2; not_0 = ~i_2;` pair becomes one `task automatic xor_0__mt(...)` where the shared `i_2` is the single formal `a1` feeding both outputs — the co-supported sink; the shared-non-constant-operand + fan-in-independence rules; members keep their wires; metric @ schema `1.14` + the gate; a skip-sentinelled repro) + the `multi_output_task_emit_prob` knob entry in `book/src/knobs.md` / `USER_GUIDE.md` / README "Current CLI truth" + the KM how-to card `multi-output-task-emit` (KM 61→62 facts / 551→565 keys) + the deferred book example-JSON `schema_version` `1.13 → 1.14` refresh. `mdbook build` + `check_knowledge_map` + `check_memory_architecture` + `book_examples` 3/3 green; book before/after byte-verified + downstream-clean. Docs-only / DUT byte-identical. Closes `.12b.3` / `.12b` / `.12` — the sixth structured surface delivered end-to-end; lane returns to no current frontier. |
| — | `STRUCTURED-EMISSION-EXPANSION.12b.2b` | `done` | The repo-owned `tool_matrix --multi-output-task-gate` (templated on `--cone-function-gate`): `ScenarioSet::MultiOutputTaskSweep` + `build_multi_output_task_sweep_scenarios`/`multi_output_task_focus_config` (comb-only `multi_output_task_emit_prob=1.0` × 3 strategies; high `terminal_reuse_prob=0.6` + shallow `max_depth=2` + `min_outputs≥2` so co-supported independent pairs exist) + `ModuleReport.emitted_multi_output_task` (`__mt(` detection) + `saw_multi_output_task_emit` + `MatrixReport.multi_output_task_gate` + the early-return gap arm + units floor + mutual-exclusion + 5 proofs + 8 ModuleReport + 1 Cli fixture updates + README/USER_GUIDE/CODEBASE gate entries. **Banked clean** `anvil-multi-output-task-gate-r1` (3 scenarios / 12 modules / **6 emitting a multi-output task** / `coverage_gaps=[]` / `saw_multi_output_task_emit=true` / `12/0` Verilator + both Yosys + Icarus compile). `cargo test --bin tool_matrix` 84 + snapshots 6/6 byte-identical. Default-off / DUT byte-identical. Closes `.12b.2b` / `.12b.2`; frontier → `.12b.3`. |
| — | `STRUCTURED-EMISSION-EXPANSION.12b.2a` | `done` | The metric + schema bump. `Metrics::num_emitted_multi_output_tasks` (= `m.multi_output_task_groups.len()`, `#[serde(default)]`) in `metrics::compute()` + lib proof; surfaced in introspection `module_metrics` ⇒ `SCHEMA_VERSION` `1.13 → 1.14` (doc comment + const + 3 introspect + 8 mcp `schema_version` assertions + the schema doc changelog/defines/checklist + README + USER_GUIDE current refs + the CODEBASE envelope line — brought current from a drifted "1.12"; book example JSONs deferred to `.12b.3` per the Mealy `.2b.2a`/`.2b.3` precedent; historical attributions intact). `cargo test --lib` 600 + snapshots 6/6 byte-identical; end-to-end `--introspect` default `0` / forced `1` at schema `1.14`. Default-off / DUT byte-identical. Frontier → `.12b.2b`. |
| — | `STRUCTURED-EMISSION-EXPANSION.12b.1` | `done` | The live multi-output task surface (first source change). `Config::multi_output_task_emit_prob` (default `0.0`) + `--multi-output-task-emit-prob` flag + `Module.multi_output_task_groups: BTreeMap<NodeId,Vec<NodeId>>` + new `src/ir/multi_output_task_emit.rs` (`annotate_multi_output_task_groups`: one-roll-per-leader pairing on a shared **non-constant** operand + the `in_fanin` bounded backward DFS soundness check) + `src/ir/mod.rs` registration + two guarded `gen/mod.rs` rolls (after `task_emit`, before `cone_function`) + `cone_function_emit::sibling_marked` extension + `src/emit/sv.rs` (the `mo_task_member` set + the decl/call section reusing `render_cone_gate_expr` with an **empty `interior_set`** + the per-gate passthrough; members keep their wires) + **11 lib proofs**. No metric/schema bump (the knob rides the version; `.12b.2` bumps `1.13→1.14`). Default-off / DUT byte-identical (`cargo test --lib` 589→599, snapshots 6/6). Forced `multi_output_task_emit_prob=1.0` sweep (`anvil-mo-sweep/`): `__mt(` fires 4/5 seeds (2–6 tasks), Verilator `-Wall` Δ=0 vs OFF (2012/2017/2023) + Yosys both + Icarus, seed-7 **exhaustively sim-equiv** to the inline reference (all 128 inputs). Frontier → `.12b.2`. |
| — | `STRUCTURED-EMISSION-EXPANSION.12a` | `done` | Design-detail (no source): a `DEVELOPMENT_NOTES.md` entry grounding decision `0025` in the real `task_emit.rs` / `cone_function_emit.rs` / `sv.rs` (the cone primitives `cone_function_params`/`cone_operand_ref`/`render_cone_gate_expr` — reused with an **empty `interior_set`** so every member operand resolves to a folded literal or its dedup param `a{i}`) / `gen/mod.rs` / `config.rs`. Pinned all seven points: (1) replicate the 4-line admissibility predicate (the per-pass convention); (2) `Module.multi_output_task_groups: BTreeMap<NodeId, Vec<NodeId>>` (leader → partners, the `cone_function_gates` shape); (3) `src/ir/multi_output_task_emit.rs` with **one `gen_bool(prob)` roll per leader** + forward pairing on a shared non-constant operand; (4) the `in_fanin(m, target, root)` **bounded backward DFS** soundness check (operand-topological `NodeId` invariant makes one direction free; both checked for robustness); (5) deduplicated-formal rendering + `__mt`/`__mtv` naming + one `always_comb` per group + per-member passthrough `assign` (members keep their wires — co-equal roots, no use-count rule); (6) run after `task_emit`, before `cone_function` (extend `sibling_marked`); (7) `multi_output_task_emit_prob` knob + `--multi-output-task-emit-prob` flag + `num_emitted_multi_output_tasks` metric (schema `1.13→1.14`) + `--multi-output-task-gate` / `saw_multi_output_task_emit` + gate-shape calibration (high `terminal_reuse_prob` + shallow `max_depth`). Pre-split `.12b` → `.12b.1`/`.12b.2`/`.12b.3`; frontier → `.12b`. No source change. |
| — | `STRUCTURED-EMISSION-EXPANSION.11` | `done` | Decision `0025`: picked the SIXTH structured surface — a default-off, valid-by-construction **multi-output combinational `task automatic`** (generalize the single-gate task from one `output` to several: a co-supported group of qualifying gates co-emitted into ONE task with deduplicated input formals + per-member output args, called once from `always_comb`). First cut = a **pair** (`k=2`) sharing a non-constant operand; **soundness rule** = mutual fan-in independence (no cycle through the shared task). Reuses the cone-function dedup render primitives (`cone_function_params`/`cone_operand_ref`/`render_cone_gate_expr`) + the single-gate task candidate predicate. Own `multi_output_task_emit_prob` knob (single-gate surface stays byte-identical; reusing `task_emit_prob` rejected) + `num_emitted_multi_output_tasks` metric (schema `1.13 → 1.14` at impl) + `--multi-output-task-gate` / `saw_multi_output_task_emit` (`__mt(` detection). Fresh probe (`anvil-se-motask-probe/`): a 2-output combinational task is Verilator `-Wall` 2012/2017/2023 + Yosys both modes + Icarus clean and iverilog-sim-equiv (5000 vectors). The recorded deferred runner-up (decision `0016` `.9` probe). Split `.11` (design) + `.12` (impl, pre-split `.12a`/`.12b`) + future `.13+`. No source change. |
...
- `2026-06-22` (`.13a`, no new decision record — covered by decision
  ...
  whose "Tree split → future (`.13`+)" + "Rejected alternatives → Wider groups
  (`k > 2`)" already enumerate this deepening): the first **deepening** of the
  sixth surface widens the multi-output `task automatic` group from the first-cut
  **pair** (`k = 2`) to a bounded **`k > 2`**. A deepening of an already-decided,
  proven surface needs no new ADR; the design-detail (the greedy group-extension
  algorithm, connected co-support, generalized mutual fan-in independence, the
  const cap, the no-new-knob/metric/schema decision, the byte-identical/RNG
  argument) is pinned in `DEVELOPMENT_NOTES.md` (the `.12a` design-detail
  precedent). The emitter was already written k-agnostic at `.12b.1`, so the only
  source change is `annotate_multi_output_task_groups`.
...
- `2026-06-22` (`.11`, decision [`0025`](../decisions/0025-structured-emission-sixth-surface-multi-output-task.md)):
  ...
  **own** opt-in `multi_output_task_emit_prob` knob (default `0.0` ⇒ byte-identical
  ...
  Yosys + Icarus. Empirically grounded this session (`anvil-se-motask-probe/`):
...
- `2026-06-17` (`.9`, decision [`0016`](../decisions/0016-structured-emission-fifth-surface-cone-function.md)):
  ...
  absent 2D source. Empirically grounded this session (`anvil-se9-probe/`):
...
- (`.4b`) Does the forced `generate_loop_emit_prob=1.0` comb-only gate corpus
  ...
  `anvil-generate-loop-gate-r1` shows 8/12 modules emitting a loop, fact lit.
...
| --- | --- | --- | --- |
| `2026-06-23` | `STRUCTURED-EMISSION-EXPANSION.19b.3` | **Ninth-surface user docs** (DOCS-ONLY ⇒ DUT byte-identical; no `src/` touched). `book/src/structured-emission.md` "## The ninth surface: a masked if/else if priority chain" section (byte-verified seed-1 before/after captured from the real emitter — the `casez (slice_0)` `2'b0?`/`2'b1?` block vs the masked `if ((slice_0 & 2'h2) == 2'h0)`/`(== 2'h2)` chain — + What-gets-wrapped / How-anvil-proves-it / Reproducing-it subsections) + the chapter-intro surface list (+ "a masked if/else if priority chain"); the 4 deferred book example-JSON `schema_version` `"1.16" → "1.17"` (`agent-mcp.md` ×1 + `api-tools.md` ×3); the `casez_mux_if_emit_prob` entry in `book/src/knobs.md` + `USER_GUIDE.md` + a `--casez-mux-if-gate` matrix-gate entry in `USER_GUIDE.md` + README "Current CLI truth" knob + gate paras; a `docs/knowledge/casez-mux-if-emit.md` KM card (links `[[case-mux-if-emit]]` + `[[structured-emission-ninth-surface-casez-mux-masked-priority-chain]]`), `KNOWLEDGE_MAP.md` regenerated (69 → 70 facts / 672 keys). **`mdbook build book` clean (HTML written, no warnings); `cargo test --test book_examples` 3/3 green (the new Reproducing-it bash block carries a `book-test: skip` sentinel with a reason); `bash knowledge-map/scripts/check_knowledge_map.sh` OK (facts valid / ids unique / map in sync / `casez-mux-if-emit` indexed).** The before/after byte-matches real emitter output (`anvil --seed 1 --config <casez-focused>` with/without `--casez-mux-if-emit-prob 1.0`; off == 0.0 byte-identical) — required rebuilding the stale `--bin anvil` release binary first (it lacked the `.19b.1` flag). `bash scripts/check_doctrines.sh` green (DOCS-ONLY exempt from the code-scoped checks). **`.19b.3` / `.19b` / `.19` close ⇒ the NINTH structured surface is delivered end-to-end.** Tree stays `active` at a no-frontier boundary. | `done` |
| `2026-06-23` | `STRUCTURED-EMISSION-EXPANSION.19b.2b` | **Ninth-surface repo-owned `tool_matrix --casez-mux-if-gate`** (CODE change — task-tree-owned by `.19b.2b`). The full gate wiring in `src/bin/tool_matrix.rs` (~30 touch points, 1:1 on `--case-mux-if-gate`): the flag + `ScenarioSet::CasezMuxIfSweep` + `build_casez_mux_if_sweep_scenarios` + `casez_mux_if_focus_config` (comb-only node-id+e-graph, `casez_mux_if_emit_prob=1.0`, `casez_mux_prob=0.9`, **BOTH** `comb_mux_prob` AND `case_mux_prob` zeroed — both short-circuit `casez_mux` in `cone.rs`) × 3 strategies + METRIC-KEYED `ModuleReport.emitted_casez_mux_if` (`= num_emitted_casez_mux_if_chains > 0`) + `CoverageSummary.saw_casez_mux_if_emit` + merge + `MatrixReport.casez_mux_if_gate` + run-plan/dispatch/mutual-exclusion/gap-arm/3 match arms/2 slug arms/`test_cli`/8 fixtures + 5 cargo proofs. Co-located: a 1-line blank-line fix to the `.19b.2a` `metrics.rs` test resolved a pre-existing rustfmt-1.9.0 fmt drift (HEAD failed `cargo fmt --all --check`). **`cargo check --all-targets` clean; `cargo clippy --all-targets -- -D warnings` clean; `cargo fmt --all --check` clean (drift fixed); `cargo test --bin tool_matrix` green incl. the 5 new `casez_mux_if_*` proofs; `cargo test --lib casez_mux_if` 10/10; `cargo test --test snapshots` 6/6 byte-identical (default-off ⇒ DUT byte-identical).** **Banked clean `anvil-casez-mux-if-gate-r1`** (release `tool_matrix --casez-mux-if-gate --yosys-mode both --iverilog-compile`): 3 scenarios / 12 modules / 12 emitting a chain / 108 chains / `coverage_gaps = []` / `saw_casez_mux_if_emit = true` / **12/0** Verilator + Yosys without-abc + Yosys with-abc + Icarus compile. `bash scripts/check_doctrines.sh` green (4 doctrines; code-scoped `CODE-CHANGE-EVIDENCE` + `TASK-TREE-OWNERSHIP` satisfied). Default-off / DUT byte-identical. `.19b.2` closes; frontier → `.19b.3`. | `done` |
| `2026-06-23` | `STRUCTURED-EMISSION-EXPANSION.19b.2a` | **Ninth-surface metric + introspection schema bump `1.16 → 1.17`** (CODE change — task-tree-owned by `.19b`). `Metrics::num_emitted_casez_mux_if_chains` (`= m.casez_mux_if_gates.len()`, `#[serde(default)]`, exact) + the `metrics_count_emitted_casez_mux_if_chains` lib proof ⇒ `SCHEMA_VERSION` `"1.16" → "1.17"` across all current-output refs (`introspect/mod.rs` const + doc-comment history + 3 assertions; 8 `mcp/mod.rs` assertions; `docs/AGENT_INTROSPECTION_SCHEMA.md` current-version ×3 + a `1.16→1.17` changelog entry; README envelope ×2 + USER_GUIDE envelope ×1 + `CODEBASE_ANALYSIS.md` envelope), with per-metric `@ 1.16` landing attributions intact and the book JSONs deferred to `.19b.3`. **`cargo check --all-targets` clean; `cargo clippy --all-targets -- -D warnings` clean; `cargo fmt --all --check` clean; `cargo test --lib` 635→636 (the new proof; all schema assertions at `"1.17"`); `cargo test --test snapshots` 6/6 byte-identical (default-off).** Live `--introspect`: default seed 2 ⇒ schema `1.17` + metric `0`; forced `casez_mux_if_emit_prob=1.0` seed 4 ⇒ `1.17` + `1`. No stray current `"1.16"` left in `src`. `bash scripts/check_doctrines.sh` green (4 doctrines; code-scoped checks satisfied). Default-off / DUT byte-identical. Frontier → `.19b.2b`. | `done` |
| `2026-06-23` | `STRUCTURED-EMISSION-EXPANSION.19b.1` | **The LIVE ninth structured surface** (CODE change — task-tree-owned by `.19b`). New `src/ir/casez_mux_if_emit.rs` (`annotate_casez_mux_if_gates`, the dynamic-selector `GateOp::CasezMux` predicate, run last; 9 lib proofs) + `Module.casez_mux_if_gates` + `ir/mod.rs` registration + `Config::casez_mux_if_emit_prob` + `--casez-mux-if-emit-prob` flag + two `gen/mod.rs` rolls (after `case_mux_if`) + the `emit/sv.rs` `GateOp::CasezMux` masked-chain body branch (`(sel & SW'h{care}) == SW'h{val}` via the existing `constant_value`/`bitmask` helpers, no `__cv`) + the `:769` trailing-default suppression tweak. **`cargo check --all-targets` clean; `cargo clippy --all-targets -- -D warnings` clean; `cargo fmt --all --check` clean; `cargo test --lib` 626→635 (9 new proofs); `cargo test --test snapshots` 6/6 byte-identical (default-off ⇒ DUT byte-identical).** Forced `casez_mux_if_emit_prob=1.0` sweep (`scratchpad/sweep19`, 6 seeds, comb-only `casez_mux_prob=1.0`): every `casez` block → a masked chain (0 residual `casez`); Verilator `-Wall` ON-vs-OFF Δ=0 across 1800-2012/2017/2023 (18 comparisons); Yosys without-abc + abc-fast 0 issues; Icarus `iverilog -g2012` 0 msg lines; ON-vs-OFF iverilog sim-equiv seed 1 (8/8) + seed 4 (128/128, a real signal data arm), 0 mismatches. `bash scripts/check_doctrines.sh` green (4 doctrines; code-scoped `CODE-CHANGE-EVIDENCE` + `TASK-TREE-OWNERSHIP` satisfied — `CHANGES.md`/`MEMORY.md` + the owning task file co-staged). No metric/schema bump (the `.19b.2` metric bumps `1.16 → 1.17`). Default-off / DUT byte-identical. Frontier → `.19b.2`. | `done` |
| `2026-06-23` | `STRUCTURED-EMISSION-EXPANSION.19a` | **Ninth-surface impl design-detail** (no source change ⇒ DUT byte-identical). A `DEVELOPMENT_NOTES.md` design-detail entry grounding decision `0029` in a fresh read of the real code (`src/ir/case_mux_if_emit.rs` template; `src/emit/sv.rs` `GateOp::CasezMux` arm `:724` + `render_casez_pattern` `:2171` + `constant_value`/`bitmask`/`casez_pattern_matches` + the `:769` trailing-default suppression; `src/gen/cone/motifs.rs:832` `build_casez_patterns`; `src/metrics.rs:2940`/`src/ir/compact.rs:603` care-mask idiom; the `gen/mod.rs` roll chain; config/main/types/mod/introspect touch points), resolving impl points 0–7 (dynamic-selector `CasezMux` predicate; `casez_mux_if_gates` carrier; in-place masked-chain emitter branch `(sel & SW'h{care}) == SW'h{val}` + trailing-`else W'h0`, **no `__cv`**, + the `:769` suppression tweak; run last; knob/flag; metric + schema `1.16 → 1.17`; metric-keyed `--casez-mux-if-gate` + a focus config zeroing both `comb_mux_prob` and `case_mux_prob`; byte-identical/RNG) + the `.19b` proof plan. The empirical form selection is recorded (`sel ==? pattern` DISQUALIFIED — Yosys rejects `==?`; the masked-AND form ships). `bash scripts/check_doctrines.sh` green (DOCS-ONLY exempt from the code-scoped checks); `git diff --check` clean. No source change ⇒ DUT byte-identical. Frontier → `.19b.1`. | `done` |
...
| `2026-06-22` | `STRUCTURED-EMISSION-EXPANSION.17b.1` | **Live emitter change** (new `src/ir/case_mux_if_emit.rs` + `src/ir/mod.rs` registration + `Module.case_mux_if_gates: BTreeSet<NodeId>` in `src/ir/types.rs` + `Config::case_mux_if_emit_prob` knob/default/validate-probs-list/Overrides/apply in `src/config.rs` + `--case-mux-if-emit-prob` flag + overlay wiring in `src/main.rs` + two guarded `gen/mod.rs` rolls after `mux_if` [single + design] + the `src/emit/sv.rs` structured-case body branch [`GateOp::CaseMux` arm branches on `case_mux_if_gates.contains(&(idx as NodeId))` → `if`/`else if` chain vs `case`; the shared `default`/`endcase` tail gains the same `!contains(...)` guard]; `CODEBASE_ANALYSIS.md` + `DEVELOPMENT_NOTES.md` updated). Implemented exactly per `.17a` (no deviations). `cargo check --all-targets` clean (4.67s); `cargo fmt --all --check` clean (after rustfmt sorted the new `ir/mod.rs` decl alphabetically); `cargo clippy --all-targets -- -D warnings` clean; `cargo test --lib` **625 passed** / 2 ignored (616 + 9 new `case_mux_if_emit` proofs incl. `constant_selector_case_mux_is_excluded` + the end-to-end emit proof; umbrella DUT-byte-identical proofs green); `cargo test --bin tool_matrix` **89 passed** / 1 ignored (unaffected — the new `Config` field defaults `0.0`); `cargo test --test snapshots` **6/6 byte-identical** (default-off — the `0.0` path never calls the pass). **Forced `case_mux_if_emit_prob=1.0` sweep** (`scratchpad/probe8/`, comb-only `--case-mux-prob 0.9`, 8 seeds): every dynamic `CaseMux` projects to an `if`/`else if` priority chain (seed 2: 7 `CaseMux` → 7 chains / 14 `else if` / **0 residual `case`**; seed 7 has no `CaseMux`). Seed 2: Verilator `--lint-only -Wall` warning count **ON == OFF (Δ=0)** across **1800-2012/2017/2023** (the 3 pre-existing warnings identical ON/OFF — the projection adds none, incl. for exhaustive selectors whose trailing `else` mirrors the `case` `default`); Yosys `synth -noabc` and `abc -fast; opt -fast; check` both **0 warnings/errors**; `iverilog -g2012` compiles (rc=0). **Generated-RTL ON-vs-OFF sim-equiv** (seed 2, both modules in one tb with the OFF module renamed `mod_ref`): `iverilog`+`vvp` prove the ON module **bit-equal to OFF across 20000 random vectors** on all 3 outputs (`GEN-RTL SIM-EQUIV OK`). The standalone construct probe (`.16`) already proved the chain **=== the parallel `case`** over 20000 vectors + an exhaustive selector sweep. `dump-config` surfaces `case_mux_if_emit_prob`. Default-off / DUT byte-identical. Frontier → `.17b.2`. | `done` |
| `2026-06-22` | `STRUCTURED-EMISSION-EXPANSION.17a` | **Design-detail leaf, no source change** (a `DEVELOPMENT_NOTES.md` "CaseMux if/else if priority-chain surface — impl design-detail — `.17a`" entry + the `.17`/`.17a`/`.17b` tree registration + the `.16`→`.17a`→`.17b.1` frontier move; no `src/` touched). Grounded in a fresh read of `src/ir/mux_if_emit.rs` (the whole annotation-pass template), `src/emit/sv.rs` (`render_static_structured_gate` `:2069`–`:2110` — the `CaseMux` static-collapse test is `constant_value(operands[0]).is_some()` `:2077`–`:2086`; the structured-case `always_comb` loop `:667`–`:753` — the static-collapse `continue` `:685`, the `CaseMux` arm `:690`–`:702`, the `default`/`endcase` tail `:747`–`:749`), `src/gen/mod.rs` (the seven-pass `soft_union→…→mux_if` guarded roll chain `:89`–`:160` single / `:323`–`:409` design), `src/config.rs` (the `mux_if_emit_prob` touch points `:118`/`:1032`/`:1281`/`:1664`/`:1893`/`:2018`), `src/main.rs` (the clap flag `:551` + overlay `:1171`), `src/ir/types.rs` (`mux_if_gates` `:537`, `GateOp::CaseMux` `:2341`). Resolved all seven impl points (the **dynamic-selector** `CaseMux` candidate predicate — `operands[0]` not a `Node::Constant`, the inverse of the static-collapse test — excluding `CasezMux` + sibling marks; `Module.case_mux_if_gates: BTreeSet<NodeId>`; the **in-place** `always_comb` body branch `case…endcase`→`if…else if` reusing the same operand refs + `W'h0` default, **no `__cv` passthrough** since a `CaseMux` is already an `always_comb` var; run `annotate_case_mux_if_gates` last; the `case_mux_if_emit_prob` knob + flag; the `num_emitted_case_mux_if_chains` metric schema `1.15→1.16`, exact because static selectors are excluded; the **metric-keyed** `--case-mux-if-gate`/`saw_case_mux_if_emit` gate + `case_mux_prob`-biased focus config; the byte-identical/RNG argument) + the `.17b` proof plan + the pre-split `.17b.1`/`.17b.2`/`.17b.3` + rejected alternatives. `bash scripts/check_doctrines.sh` ✅ (4 doctrines); `bash scripts/check_memory_architecture.sh` ✅; `bash knowledge-map/scripts/gen_knowledge_map.sh` + `check_knowledge_map.sh` ✅ (no card change — `0028` already carries `answers:`); `mdbook build book` ✅. No source touched ⇒ `cargo check/clippy/fmt/test --lib` unaffected (`cargo test --lib` 616 + `--bin tool_matrix` 89 + snapshots 6/6 per the `.15b.2` bank still hold). DUT byte-identical. Frontier → `.17b.1`. | `done` |
| `2026-06-22` | `STRUCTURED-EMISSION-EXPANSION.16` | **Design/decision leaf, no source change.** Decision `0028` (`docs/decisions/0028-structured-emission-eighth-surface-case-mux-priority-chain.md`, with `answers:` front-matter) + `INDEX.md` row + `KNOWLEDGE_MAP.md` regenerated in sync + tree split (`.16` done + `.17` impl pending, pre-split `.17a`/`.17b`; frontier → `.17`) + the stale parent `.15`/`.15b` flipped `active`→`done` + ROADMAP owner-directed-lane #2 + `docs/TASK_TREE.md` row. **Fresh empirical tool-acceptance + simulation-equivalence probe** (this session, `scratchpad/probe8/`): the **procedural `always_comb` `if`/`else if` priority chain** projection of a `CaseMux` (selector width 3, 5 arms + `default`, 4-bit data) accepted **warning-clean** by **Verilator 5.046 `-Wall --lint-only`** under `--language 1800-2012`, `1800-2017`, **and** `1800-2023`, by **Yosys 0.64 both modes** (`synth -noabc` → 32 cells [20 `$_MUX_` / 3 `$_NOT_` / 9 `$_OR_`], no warnings; and `synth -noabc; abc -fast; opt -fast; check` — no warnings/errors), and by **Icarus `iverilog -g2012`** (compile rc=0), and **`iverilog`+`vvp` prove the chain bit-equal to the parallel `case (sel) … default` form** over **20000 random vectors + an exhaustive 8-value selector sweep** (`EQUIV OK`). `bash scripts/check_doctrines.sh` ✅ (4 doctrines); `bash scripts/check_memory_architecture.sh` ✅ (`0028` indexed); `bash knowledge-map/scripts/gen_knowledge_map.sh` + `check_knowledge_map.sh` ✅ (decision `0028` carries `answers:` — folded into the map); `mdbook build book` ✅. No source touched ⇒ `cargo check/clippy/fmt/test --lib` unaffected (`cargo test --lib` 616 + `--bin tool_matrix` 89 + snapshots 6/6 per the `.15b.2` bank still hold). DUT byte-identical. Frontier → `.17`. | `done` |
| `2026-06-22` | `STRUCTURED-EMISSION-EXPANSION.15b.1` | **Live emitter change** (new `src/ir/mux_if_emit.rs` + `src/ir/mod.rs` registration + `Module.mux_if_gates: BTreeSet<NodeId>` in `src/ir/types.rs` + `Config::mux_if_emit_prob` knob/default/validate-probs-list/Overrides/apply in `src/config.rs` + `--mux-if-emit-prob` flag + overlay wiring in `src/main.rs` + two guarded `gen/mod.rs` rolls after `cone_function` [single + design] + the `src/emit/sv.rs` procedural-block section + the gate-assign-loop `<name>__cv` passthrough; `CODEBASE_ANALYSIS.md` + `DEVELOPMENT_NOTES.md` updated). `cargo check --all-targets` clean; `cargo fmt --all --check` clean; `cargo clippy --all-targets -- -D warnings` clean; `cargo test --lib` **615 passed** / 2 ignored (605 + 10 new `mux_if_emit` proofs incl. the end-to-end emit proof; umbrella DUT-byte-identical proofs green); `cargo test --test snapshots` **6/6 byte-identical** (default-off — the `0.0` path never calls the pass). **Forced `mux_if_emit_prob=1.0` sweep** `anvil-muxif-genproof.*` (seeds 1/2/3/7/11): every plain `Mux` (121/143/132/144/195) projects to a `__cv` `always_comb if/else` block (ON `__cv` count == OFF mux count exactly); Verilator `--lint-only -Wall` warning count **ON == OFF (Δ=0)** on all five (the pre-existing `UNUSEDSIGNAL`/etc warnings are identical ON/OFF — the projection adds none); Yosys `synth -noabc` and `abc -fast; opt -fast; check` both **0 warnings/errors**; `iverilog -g2012` compiles (rc=0). Comb-only **ON-vs-OFF sim-equiv** (seed 3, `--flop-prob 0 --comb-mux-prob 0.9`, 128 muxes): `iverilog`+`vvp` prove the ON module **bit-equal to OFF across 20000 random vectors** on all 3 outputs (`GEN-RTL SIM-EQUIV OK`). The standalone construct probe this session already proved `if/else === (sel)?(a):(b)` over 20000 vectors. `dump-config` surfaces `mux_if_emit_prob`. Default-off / DUT byte-identical. Frontier → `.15b.2`. | `done` |
| `2026-06-22` | `STRUCTURED-EMISSION-EXPANSION.15a` | **Design-detail leaf, no source change** (a `DEVELOPMENT_NOTES.md` "Procedural if/else mux surface — impl design-detail — `.15a`" entry + the `.15`/`.15a`/`.15b` tree registration + the `.15a`→`.15b.1` frontier move; no `src/` touched). Grounded in a fresh read of `src/ir/task_emit.rs` (`gate_qualifies` + `annotate_task_emit_gates` — the collect-then-roll template), `src/emit/sv.rs` (`node_ref` `:1242` used in the `CaseMux` `always_comb` block `:644`/`:648`; the task decl/call section `:444-455`; the gate-assign-loop projection passthroughs `:566-605` incl. `task_emit` `:578`; the `CaseMux`/`CasezMux`/`ForFold` procedural-block loop `:620-641`), `src/gen/mod.rs` (the `soft_union → function_emit → generate_loop → task_emit → multi_output_task → cone_function` roll chain `:91-150` single + `:330-383` design), `src/config.rs` (`default_task_emit_prob`/`default_cone_function_emit_prob`/`default_multi_output_task_emit_prob` `:106-114`; the knob defs `:962-1011`), `src/ir/types.rs` (the `*_gates`/`*_groups` carriers `:417-466`). Resolved all seven impl points (candidate predicate excluding the six sibling marks since the pass runs last; `Module.mux_if_gates: BTreeSet<NodeId>`; the emitter `always_comb if/else` `<name>__cv` block + the gate-assign-loop passthrough; run `annotate_mux_if_gates` last ⇒ no other pass changes; `mux_if_emit_prob` knob + flag; `num_emitted_mux_if_blocks` metric schema `1.14→1.15`; `--mux-if-gate`/`saw_mux_if_emit` + Mux-biased calibration; the byte-identical/RNG argument) + the `.15b` proof plan + the pre-split `.15b.1`/`.15b.2`/`.15b.3` + rejected alternatives. `bash scripts/check_doctrines.sh` ✅ (4 doctrines); `bash scripts/check_memory_architecture.sh` ✅; `bash knowledge-map/scripts/gen_knowledge_map.sh` + `check_knowledge_map.sh` ✅ (no card change — `0027` already carries `answers:`); `mdbook build book` ✅. No source touched ⇒ `cargo check/clippy/fmt/test --lib` unaffected (`cargo check --all-targets` clean at session start; `cargo test --lib` 605 + snapshots 6/6 per the `.13b` bank still hold). DUT byte-identical. Frontier → `.15b.1`. | `done` |
| `2026-06-22` | `STRUCTURED-EMISSION-EXPANSION.14` | **Design/decision leaf, no source change.** Decision `0027` (`docs/decisions/0027-structured-emission-seventh-surface-procedural-if-else.md`) + `INDEX.md` row + tree split (`.14` done + `.15` impl pending, pre-split `.15a`/`.15b`; frontier → `.15`) + ROADMAP owner-directed-lane #2 + `docs/TASK_TREE.md` row. **Fresh empirical tool-acceptance + simulation-equivalence probe** (this session, `anvil-ifelse-probe.*`): the **procedural `always_comb` `if`/`else`** projection of a 2:1 `Mux` into a `<wire>__cv` output var (`if (sel) cv = a; else cv = b;` + passthrough `assign`) accepted **warning-clean** by **Verilator 5.046 `-Wall --lint-only`** under `--language 1800-2012`, `1800-2017`, **and** `1800-2023`, by **Yosys 0.64 both modes** (`synth -noabc` and `synth -noabc; abc -fast; opt -fast; check` — an explicit `grep -iE warning\|error` scan returned **NO warnings/errors**), and by **Icarus `iverilog -g2012`**, and **`iverilog`+`vvp` prove the `if`/`else` block bit-equal to the inline `(sel)?(a):(b)` ternary** over **20000 random vectors** (`SIM-EQUIV OK`). `bash scripts/check_doctrines.sh` ✅ (4 doctrines); `bash scripts/check_memory_architecture.sh` ✅ (`0027` indexed); `bash knowledge-map/scripts/gen_knowledge_map.sh` + `check_knowledge_map.sh` ✅ (decision `0027` carries `answers:`); `mdbook build book` ✅. No source touched ⇒ `cargo check/clippy/fmt/test --lib` unaffected (`cargo check --all-targets` was clean at session start; `cargo test --lib` 605 + snapshots 6/6 per the `.13b` bank still hold). DUT byte-identical. Frontier → `.15`. | `done` |
...
| `2026-06-22` | `STRUCTURED-EMISSION-EXPANSION.12b.3` | **User-facing docs closeout (docs-only / DUT byte-identical)** (`book/src/structured-emission.md` sixth-surface section + intro; `book/src/knobs.md` + `USER_GUIDE.md` + README `multi_output_task_emit_prob` knob entries; new KM card `docs/knowledge/multi-output-task-emit.md`; the deferred `book/src/{api-tools,agent-mcp,api-introspection,api-reference}.md` example-JSON `schema_version` `1.13 → 1.14` refresh; `KNOWLEDGE_MAP.md` regenerated). `mdbook build book` OK; `bash knowledge-map/scripts/gen_knowledge_map.sh` (62 facts / 565 keys) + `check_knowledge_map.sh` OK; `bash scripts/check_memory_architecture.sh` OK; `cargo test --test book_examples` **3/3 passed** (the new repro block carries the `<!-- book-test: skip -->` sentinel — byte-identical book-runnable contract preserved). The book seed-3 before/after is byte-verified against `anvil --seed 3` output and the ON form is downstream-clean (Verilator `-Wall` 2012/2017/2023 rc=0 + both Yosys rc=0 + Icarus rc=0). No `src/` touched ⇒ `cargo check/clippy/fmt/test --lib` unaffected. Closes `.12b.3` / `.12b` / `.12` — the sixth structured surface delivered end-to-end; lane returns to no current frontier. | `done` |
| `2026-06-22` | `STRUCTURED-EMISSION-EXPANSION.12b.2b` | **Repo-owned `tool_matrix` gate** (`src/bin/tool_matrix.rs`: `--multi-output-task-gate` + `ScenarioSet::MultiOutputTaskSweep` + `build_multi_output_task_sweep_scenarios`/`multi_output_task_focus_config` + `ModuleReport.emitted_multi_output_task` (`__mt(` detection) + `CoverageSummary.saw_multi_output_task_emit` + `MatrixReport.multi_output_task_gate` + the merge/early-return-gap/units-floor/mutual-exclusion + 5 proofs + 8 ModuleReport + 1 Cli fixture updates + the `test_cli`/`MatrixReport`-fixture defaults; README + USER_GUIDE + CODEBASE_ANALYSIS gate entries). `cargo check --all-targets` clean; `cargo clippy --bin tool_matrix -- -D warnings` clean; `cargo fmt --all --check` clean; `cargo test --bin tool_matrix` **84 passed** / 1 ignored (79 + 5 new gate proofs); `cargo test --test snapshots` **6/6 byte-identical** (harness-only). **Banked clean** `anvil-multi-output-task-gate-r1` (`--multi-output-task-gate --yosys-mode both --iverilog-compile`): 3 scenarios / 12 modules / **6 emitting a multi-output task** / `coverage_gaps = []` / `saw_multi_output_task_emit = true` / Verilator `12/0` / Yosys without-abc `12/0` / Yosys with-abc `12/0` / Icarus compile `12/0`. Frontier → `.12b.3`. | `done` |
| `2026-06-22` | `STRUCTURED-EMISSION-EXPANSION.12b.2a` | **Metric + schema bump** (`src/metrics.rs` `num_emitted_multi_output_tasks` field + `compute()` + lib proof `metrics_count_emitted_multi_output_tasks`; `src/introspect/mod.rs` `SCHEMA_VERSION` `1.13→1.14` + doc comment + 3 assertions; `src/mcp/mod.rs` 8 assertions; `docs/AGENT_INTROSPECTION_SCHEMA.md` new `1.13→1.14` changelog + defines/early-example/checklist; README `--introspect`+`analyze` current refs; USER_GUIDE `--sv-version --introspect` row; `CODEBASE_ANALYSIS.md` envelope line brought current from a drifted "1.12" → "1.14" + the `1.12→1.13`/`1.13→1.14` list entries). `cargo check --all-targets` clean; `cargo clippy --all-targets -- -D warnings` clean; `cargo fmt --all --check` clean; `cargo test --lib` **600 passed** / 2 ignored (599 + the new metric proof; all `schema_version` assertions green at `1.14`); `cargo test --test snapshots` **6/6 byte-identical** (default-off). End-to-end `--introspect`: default (seed 42) ⇒ `schema_version "1.14"` + `introspection.module_metrics.num_emitted_multi_output_tasks 0`; forced `multi_output_task_emit_prob=1.0` (seed 42) ⇒ `"1.14"` + `1`. Historical "landed at schema X" attributions left intact (`num_mealy_fsm_modules` @ 1.13). The book/src example JSON refresh is deferred to `.12b.3` (the Mealy `.2b.2a`/`.2b.3` precedent). Frontier → `.12b.2b`. | `done` |
| `2026-06-22` | `STRUCTURED-EMISSION-EXPANSION.12b.1` | **Live emitter change** (`src/config.rs` `multi_output_task_emit_prob` knob + default + `0.0..=1.0` validation + dump-config; `src/main.rs` `--multi-output-task-emit-prob` clap flag + overlay struct + apply; `src/ir/types.rs` `Module.multi_output_task_groups: BTreeMap<NodeId,Vec<NodeId>>`; new `src/ir/multi_output_task_emit.rs` `annotate_multi_output_task_groups` (one-roll-per-leader pairing on a shared non-constant operand + the `in_fanin` bounded backward DFS) + `src/ir/mod.rs` registration; `src/gen/mod.rs` two call-site rolls after `task_emit`, before `cone_function` (single + design); `src/ir/cone_function_emit.rs` `sibling_marked` extended to exclude multi-output members; `src/emit/sv.rs` `mo_task_member` set + the multi-output task decl/call section (`render_multi_output_task_decl`/`render_multi_output_task_call`/`multi_output_task_params`, body via the reused `render_cone_gate_expr` with an empty interior_set) + the per-gate passthrough; `DEVELOPMENT_NOTES.md` + `CODEBASE_ANALYSIS.md` updated). `cargo check --all-targets` clean; `cargo clippy --all-targets -- -D warnings` clean; `cargo fmt --all --check` clean; `cargo test --lib` **599 passed** / 2 ignored (incl. 10 new `multi_output_task_emit` proofs; introspect `schema_version` 1.13 + `umbrella` DUT-byte-identical still green); `cargo test --test snapshots` **6/6 byte-identical** (default-off). **Forced `multi_output_task_emit_prob=1.0` downstream sweep** (`anvil-mo-sweep/`, comb-only `terminal_reuse_prob=0.6` `max_depth=2` `min_outputs=2`, seeds 1/7/42/100): `__mt(` emitted on all four (2–6 tasks each), Verilator `--lint-only` `-Wall` **delta=0 vs OFF** across **1800-2012/2017/2023**, Yosys without-abc + with-abc + Icarus `iverilog -g2012` clean; the seed-7 module is **exhaustively sim-equivalent** to the inline OFF reference over all 128 input values (`GEN-RTL SIM-EQUIV OK`); seed-100's lone `-Wall` warning is a pre-existing `UNUSEDSIGNAL` on an unrelated `concat_0` wire (identical ON/OFF). No metric / no schema bump (the metric + schema `1.13 → 1.14` land at `.12b.2`; the knob rides the version — the `.10b.1`/`.6b.1` precedent). | `done` |
| `2026-06-22` | `STRUCTURED-EMISSION-EXPANSION.12a` | **Design-detail leaf, no source change** (a `DEVELOPMENT_NOTES.md` design-detail entry + the `.12a`→`.12b` frontier move; no `src/` touched). Grounded in a fresh read of `src/ir/task_emit.rs` (`gate_qualifies`), `src/ir/cone_function_emit.rs` (`admissible`/`sibling_marked`/`compute_use_counts`), `src/emit/sv.rs` (`render_gate_task_decl`/`render_gate_task_call` + `cone_function_params`/`cone_operand_ref`/`render_cone_gate_expr` ~`1490`–`1600` + the per-gate assign loop ~`468`–`560` + `node_ref` ~`1189`), `src/gen/mod.rs` (the `soft_union→function_emit→generate_loop→task_emit→cone_function` roll chain ~`93`–`135` + the design mirror ~`306`–`353`), `src/config.rs` (the `*_emit_prob` defaults/validation/`Option<f64>` CLI overlay), `src/ir/types.rs` (`Module.intern_gate` operand-topological invariant confirmed: `node_id = nodes.len()` then `push` ⇒ operands < gate id). Resolved all seven points (replicate the predicate; `multi_output_task_groups: BTreeMap<NodeId,Vec<NodeId>>`; one-roll-per-leader pairing on a shared non-constant operand; the `in_fanin` bounded backward DFS soundness check; dedup-formal rendering reusing `render_cone_gate_expr` with an empty `interior_set` + `__mt`/`__mtv` naming + one always_comb per group + per-member passthrough; run after `task_emit` before `cone_function` + extend `sibling_marked`; the knob/flag/metric-schema-`1.13→1.14`/gate + gate-shape calibration) + recorded the `.12b` impl shape (pre-split `.12b.1`/`.12b.2`/`.12b.3`). `bash scripts/check_memory_architecture.sh` ✅; `bash knowledge-map/scripts/gen_knowledge_map.sh` (61 facts / 551 keys) + `check_knowledge_map.sh` ✅ (no card change — `0025` already carries `answers:`); `mdbook build book` ✅. No source touched ⇒ `cargo check/clippy/fmt/test` unaffected. | `done` |
| `2026-06-22` | `STRUCTURED-EMISSION-EXPANSION.11` | **Design/decision leaf, no source change.** Decision `0025` (`docs/decisions/0025-structured-emission-sixth-surface-multi-output-task.md`) + `INDEX.md` row + tree split (`.11` done + `.12` impl pending, pre-split `.12a`/`.12b`; frontier → `.12`). **Fresh empirical tool-acceptance + simulation-equivalence probe** (this session, `anvil-se-motask-probe/`): a **2-output combinational `task automatic`** with a deduplicated input list (one formal shared by both outputs), called once from `always_comb`, accepted with **zero `%Warning`** by **Verilator 5.046 `-Wall --lint-only`** under `--language 1800-2012`, `1800-2017`, **and** `1800-2023` (the lone transient warning was a `DECLFILENAME` filename≠module-name probe artifact, gone once the file matched the module name), **Yosys 0.64 both modes** (`synth -noabc` and `abc -fast; opt -fast; check`), and **Icarus `iverilog -g2012`**, and **`iverilog`+`vvp` prove it bit-equal to the inline two `assign`s** over 5000 random vectors (`SIM-EQUIV OK`). `bash scripts/check_memory_architecture.sh` ✅ (`0025` indexed); `bash knowledge-map/scripts/gen_knowledge_map.sh` + `check_knowledge_map.sh` ✅ (decision `0025` carries `answers:`); `mdbook build book` ✅. No source touched ⇒ `cargo check/clippy/fmt` unaffected (`cargo check --all-targets` was clean at session start; `cargo test --lib` 589 + snapshots 6/6 per the prior bank still hold). | `done` |
...
| --- | --- | --- |
| `STRUCTURED-EMISSION-EXPANSION.19b.3` | `STRUCTURED-EMISSION-EXPANSION.19b.3 — ninth-surface user docs` (this commit) | DOCS-ONLY ⇒ DUT byte-identical (no `src/`). Closes `.19b`/`.19` — the NINTH structured surface delivered end-to-end. `book/src/structured-emission.md` ninth-surface section (byte-verified seed-1 before/after) + chapter-intro surface list; the 4 deferred book example-JSON `schema_version` `1.16 → 1.17` (`agent-mcp.md` + `api-tools.md` ×3); `casez_mux_if_emit_prob` entries in `book/src/knobs.md` / `USER_GUIDE.md` / README + a `--casez-mux-if-gate` matrix-gate entry; a `docs/knowledge/casez-mux-if-emit.md` KM card, `KNOWLEDGE_MAP.md` regenerated (70 facts / 672 keys). `mdbook build` clean + `book_examples` 3/3 + `check_knowledge_map` OK. Tree stays `active` at a no-frontier boundary (future `.20+`). |
| `STRUCTURED-EMISSION-EXPANSION.19b.2b` | `STRUCTURED-EMISSION-EXPANSION.19b.2b — tool_matrix --casez-mux-if-gate` (`925c103`) | The ninth-surface downstream gate, templated 1:1 on `--case-mux-if-gate`. `--casez-mux-if-gate` flag + `ScenarioSet::CasezMuxIfSweep` + `build_casez_mux_if_sweep_scenarios` + `casez_mux_if_focus_config` (comb-only, `casez_mux_if_emit_prob=1.0`, `casez_mux_prob=0.9`, BOTH `comb_mux_prob`+`case_mux_prob` zeroed) × 3 strategies + METRIC-KEYED `ModuleReport.emitted_casez_mux_if` + `saw_casez_mux_if_emit` + merge/report/run-plan/dispatch/mutual-exclusion/gap-arm/3 match arms/2 slug arms/`test_cli`/8 fixtures + 5 cargo proofs. Co-located 1-line fmt-drift fix in the `.19b.2a` `metrics.rs` test. `cargo check/clippy(-D warnings)/fmt` clean; `--bin tool_matrix` + `--lib casez_mux_if` + snapshots 6/6 green. Banked clean `anvil-casez-mux-if-gate-r1` (3 scenarios / 12 modules / 12 emitting / 108 chains / `coverage_gaps = []` / 12/0 Verilator + both Yosys + Icarus). `.19b.2` closes; frontier → `.19b.3`. Default-off / DUT byte-identical. |
| `STRUCTURED-EMISSION-EXPANSION.19b.2a` | `STRUCTURED-EMISSION-EXPANSION.19b.2a — casez-mux-if metric + introspection schema 1.17` (`3d944fd`) | Metric + schema bump. `Metrics::num_emitted_casez_mux_if_chains` (`= m.casez_mux_if_gates.len()`, exact) + a lib proof ⇒ introspection schema `1.16 → 1.17` across all current-output refs (introspect/mcp assertions, schema doc + a `1.16→1.17` changelog entry, README/USER_GUIDE/CODEBASE envelope); per-metric landing attributions intact, book JSONs deferred to `.19b.3`. `cargo test --lib` 636, snapshots 6/6 byte-identical; live `--introspect` `1.17` + `0`/`1`. Default-off / DUT byte-identical. Frontier → `.19b.2b`. |
| `STRUCTURED-EMISSION-EXPANSION.19b.1` | `STRUCTURED-EMISSION-EXPANSION.19b.1 — CasezMux masked priority-chain (live)` (`61b8c8e`) | Live emitter change — the ninth structured surface goes live. New `src/ir/casez_mux_if_emit.rs` + `Module.casez_mux_if_gates` + `ir/mod.rs` reg + `casez_mux_if_emit_prob` knob/`--casez-mux-if-emit-prob` flag + two `gen/mod.rs` rolls (after `case_mux_if`) + the `emit/sv.rs` `CasezMux` masked-chain body branch (`(sel & care) == val`, no `__cv`) + the trailing-default suppression tweak + 9 lib proofs. `cargo test --lib` 635, snapshots 6/6 byte-identical; forced sweep clean across Verilator `-Wall` (Δ=0) + both Yosys + Icarus + ON-vs-OFF sim-equiv. No schema bump (`.19b.2` bumps `1.16→1.17`). Default-off / DUT byte-identical. Frontier → `.19b.2`. |
| `STRUCTURED-EMISSION-EXPANSION.19a` | `STRUCTURED-EMISSION-EXPANSION.19a — CasezMux masked priority-chain design-detail` (`f266d54`) | Design-detail leaf, no source change — grounds decision `0029` in the real code for the ninth surface. A `DEVELOPMENT_NOTES.md` entry pinning all `.19b` impl points (dynamic-selector `CasezMux` predicate; `casez_mux_if_gates` carrier; the in-place masked `(sel & care) == val` `if`/`else if` emitter branch, no `__cv`; run last; knob/flag; metric + schema `1.16→1.17`; metric-keyed `--casez-mux-if-gate` + focus config zeroing `comb_mux_prob`+`case_mux_prob`; byte-identical/RNG) + the `.19b` proof plan; records the empirical `==?`-disqualified / masked-AND-chosen result. DOCS-ONLY ⇒ DUT byte-identical. Frontier → `.19b.1`. |
| `STRUCTURED-EMISSION-EXPANSION.18` | `STRUCTURED-EMISSION-EXPANSION.18 — pick CasezMux masked chain surface` (`6aeecf6`) | Design/decision leaf, no source change — picks the **NINTH** structured surface. Decision `0029`: a procedural `always_comb` `if`/`else if` **masked** priority chain `(sel & care_mask) == value_masked` projecting a wildcard `CasezMux`, generalizing the eighth surface's bare-equality `CaseMux` chain. Fresh probe disqualified the `==?` form (Yosys-rejected) and proved the masked-AND form clean across all repo tools + exhaustively sim-equivalent. Own `casez_mux_if_emit_prob` knob + `num_emitted_casez_mux_if_chains` metric (schema `1.16→1.17`) + metric-keyed `--casez-mux-if-gate`. DOCS-ONLY ⇒ DUT byte-identical. Frontier → `.19` (impl, pre-split `.19a`/`.19b`). |
...
| `STRUCTURED-EMISSION-EXPANSION.17b.1` | `STRUCTURED-EMISSION-EXPANSION.17b.1 — CaseMux if/else-if priority-chain emit-projection (live surface)` | Live emitter change — the eighth structured surface goes live. New `src/ir/case_mux_if_emit.rs` (`annotate_case_mux_if_gates`: a `GateOp::CaseMux` whose selector `operands[0]` is NOT a `Node::Constant` — the dynamic-selector test — excluding `CasezMux` + all sibling marks, run **last**; 9 lib proofs incl. the constant-selector exclusion + the end-to-end emit proof) + `Module.case_mux_if_gates: BTreeSet<NodeId>` + `ir/mod.rs` registration + `Config::case_mux_if_emit_prob` knob + `--case-mux-if-emit-prob` flag + two guarded `gen/mod.rs` rolls (after `mux_if`, single + design) + the `emit/sv.rs` structured-case body branch (`case…endcase` → `if`/`else if` chain over the same operand refs + `W'h0` default; the parallel `case`/`endcase` suppressed for a marked gate via the `!contains(...)` tail guard). Implemented exactly per `.17a` (no deviations). No metric/schema bump (the `.17b.2` metric bumps `1.15→1.16`; the knob rides the version). `CODEBASE_ANALYSIS.md` + `DEVELOPMENT_NOTES.md` updated. `cargo check`/`clippy -D warnings`/`fmt --check` clean; `cargo test --lib` 616→**625** (9 new proofs); `--bin tool_matrix` **89** (unaffected); snapshots **6/6 byte-identical** (default-off). Forced `case_mux_if_emit_prob=1.0` sweep (seed 2: 7 `CaseMux` → 7 chains / 14 `else if` / 0 residual `case`): Verilator `-Wall` Δ=0 vs OFF (2012/2017/2023) + both Yosys modes + Icarus clean; generated-RTL ON-vs-OFF **sim-equiv over 20000 vectors** (`GEN-RTL SIM-EQUIV OK`). Default-off / DUT byte-identical. Pre-split `.17b` → `.17b.1`/`.17b.2`/`.17b.3`; frontier → `.17b.2`. |
| `STRUCTURED-EMISSION-EXPANSION.17a` | `STRUCTURED-EMISSION-EXPANSION.17a — CaseMux priority-chain impl design-detail` | Design-detail leaf, no source change — grounds decision `0028` in the real code for the eighth surface's impl. A `DEVELOPMENT_NOTES.md` entry pinning all seven impl points against a fresh read of `src/ir/mux_if_emit.rs` (the annotation-pass template), `src/emit/sv.rs` (`render_static_structured_gate`'s `CaseMux` static-collapse test `constant_value(operands[0]).is_some()` + the structured-case `always_comb` loop), `src/gen/mod.rs` (the seven-pass roll chain), `src/config.rs`, `src/main.rs`, `src/ir/types.rs`. Key calls: candidate = a **dynamic-selector** `CaseMux` (`operands[0]` not a `Node::Constant` — the inverse of the static-collapse test; checked locally in `ir/`), `CasezMux` excluded (the `casez` masked-comparison follow-up); the emitter swaps the `always_comb` body **in place** (`case…endcase` → `if…else if` over the same operand refs + `W'h0` default) with **no `__cv` passthrough** (a `CaseMux` is already an `always_comb` var); `Module.case_mux_if_gates: BTreeSet<NodeId>`; run last; the `case_mux_if_emit_prob` knob + `--case-mux-if-emit-prob` flag; the `num_emitted_case_mux_if_chains` metric (schema `1.15 → 1.16`, exact because static selectors are excluded); the **metric-keyed** `--case-mux-if-gate` / `saw_case_mux_if_emit` gate (no new text token — strictly more robust than scanning `if(==)` which FSM decode blocks also contain) + a `case_mux_prob`-biased focus config; the byte-identical/RNG argument. `.17` split → `.17a` (done) + `.17b` (impl pending, pre-split `.17b.1`/`.17b.2`/`.17b.3`); frontier → `.17b.1`. Self-checks clean; DUT byte-identical. |
| `STRUCTURED-EMISSION-EXPANSION.16` | `STRUCTURED-EMISSION-EXPANSION.16 — pick CaseMux if/else-if priority-chain surface` | Design/decision leaf, no source change — picks the **EIGHTH** structured surface. Decision `0028` (`docs/decisions/0028-structured-emission-eighth-surface-case-mux-priority-chain.md`, with `answers:`) + `INDEX.md` row + `KNOWLEDGE_MAP.md` regenerated. The surface = a default-off, valid-by-construction procedural `always_comb` `if`/`else if` **priority chain** projection of a `CaseMux` (the N-way selection it renders today as a parallel `case (sel) … default`) — the recorded decision-`0027` follow-up. Distinct from the seventh surface (N-way chain vs single 2:1 `Mux` conditional) and the existing `case` render (sequential-priority vs parallel-match). Simpler than the seventh surface: the `CaseMux` is already an `always_comb`-written `logic` var, so no `__cv` passthrough — only the block body swaps `case…endcase` → `if…else`. Candidate = a dynamic-selector `CaseMux` (`render_static_structured_gate` None); `CasezMux` masked chain = the follow-up. Own `case_mux_if_emit_prob` knob + `num_emitted_case_mux_if_chains` metric (schema `1.15 → 1.16` at impl) + `--case-mux-if-gate` / `saw_case_mux_if_emit` (metric-keyed detection — no new text token). Fresh probe this session (`scratchpad/probe8/`): Verilator `-Wall` 2012/2017/2023 + both Yosys modes (no warnings) + Icarus clean, iverilog `vvp` sim-equiv to the parallel `case` over 20000 vectors + exhaustive sweep (`EQUIV OK`). Also flipped the stale parent `.15`/`.15b` `active`→`done` (children all done; the `.5`/`.6`/`.6b`/`.13` closed-parent convention). Split `.16` (design, this leaf) + `.17` (impl, pre-split `.17a`/`.17b`) + future `.18+`. ROADMAP owner-directed-lane #2 + `docs/TASK_TREE.md` row updated. Docs-only ⇒ DUT byte-identical. Frontier → `.17`. |
...
| `STRUCTURED-EMISSION-EXPANSION.12b.3` | `STRUCTURED-EMISSION-EXPANSION.12b.3 — multi-output task user docs (sixth surface end-to-end)` | Docs-only closeout: a `## The sixth surface: a multi-output task automatic` section in `book/src/structured-emission.md` (byte-verified seed-3 before/after — the shared `i_2` becomes the single formal `a1` feeding both outputs; the shared-non-constant-operand + fan-in-independence rules; members keep their wires; metric @ schema `1.14` + the gate) + the `multi_output_task_emit_prob` knob entry in `book/src/knobs.md` / `USER_GUIDE.md` / README "Current CLI truth" + KM how-to card `multi-output-task-emit` (62 facts / 565 keys) + the deferred book example-JSON `schema_version` `1.13 → 1.14` refresh. `mdbook build` + `check_knowledge_map` + `check_memory_architecture` + `cargo test --test book_examples` 3/3 green. Closes `.12b.3` / `.12b` / `.12` — the sixth structured surface delivered end-to-end. DUT byte-identical. Nothing retired. |
| `STRUCTURED-EMISSION-EXPANSION.12b.2b` | `STRUCTURED-EMISSION-EXPANSION.12b.2b — repo-owned --multi-output-task-gate` | The repo-owned `tool_matrix --multi-output-task-gate` (templated on `--cone-function-gate`): `ScenarioSet::MultiOutputTaskSweep` + `build_multi_output_task_sweep_scenarios`/`multi_output_task_focus_config` (comb-only `multi_output_task_emit_prob=1.0` × 3 strategies; high `terminal_reuse_prob=0.6` + shallow `max_depth=2` + `min_outputs≥2`) + `ModuleReport.emitted_multi_output_task` (`__mt(` detection) + `saw_multi_output_task_emit` + `MatrixReport.multi_output_task_gate` + early-return gap arm + units floor + mutual-exclusion + 5 proofs + 8 ModuleReport + 1 Cli fixtures + README/USER_GUIDE/CODEBASE gate entries. Banked clean `anvil-multi-output-task-gate-r1` (3 scenarios / 12 modules / 6 emitting / `coverage_gaps=[]` / `12/0` all tools). `cargo test --bin tool_matrix` 84 + snapshots 6/6. Default-off / DUT byte-identical. Closes `.12b.2b` / `.12b.2`; frontier → `.12b.3`. |
| `STRUCTURED-EMISSION-EXPANSION.12b.2a` | `STRUCTURED-EMISSION-EXPANSION.12b.2a — multi-output task metric (schema 1.13→1.14)` | Metric: `Metrics::num_emitted_multi_output_tasks` (= `m.multi_output_task_groups.len()`, `#[serde(default)]`) in introspection `module_metrics` ⇒ `SCHEMA_VERSION` `1.13→1.14` (the new derived metric bumps; the `.12b.1` knob rode the version) + lib proof. Bumped all current-output schema refs (doc comment + const + 3 introspect + 8 mcp assertions + schema doc changelog/defines/checklist + README + USER_GUIDE; the CODEBASE envelope line brought current from a drifted "1.12"); historical attributions intact; book example JSONs deferred to `.12b.3` (Mealy precedent). `cargo test --lib` 600 + snapshots 6/6; end-to-end `--introspect` default `0` / forced `1` at schema `1.14`. Default-off / DUT byte-identical. Closes `.12b.2a`; frontier → `.12b.2b`. |
| `STRUCTURED-EMISSION-EXPANSION.12b.1` | `STRUCTURED-EMISSION-EXPANSION.12b.1 — multi-output task automatic emit-projection (live surface)` | Live emitter change: `multi_output_task_emit_prob` knob + `--multi-output-task-emit-prob` flag + `Module.multi_output_task_groups` + new `src/ir/multi_output_task_emit.rs` (one-roll-per-leader pairing on a shared non-constant operand + the `in_fanin` bounded backward DFS soundness check, run after `task_emit` before `cone_function`) + `cone_function` sibling exclusion + `src/emit/sv.rs` decl/call section (reusing `render_cone_gate_expr` with an empty interior_set for the dedup body) + per-gate passthrough + 11 lib proofs. Members keep their wires (co-equal roots, no use-count rule). No metric / no schema bump (the metric + schema `1.13→1.14` at `.12b.2`; the knob rides the version). Default-off / DUT byte-identical (snapshots 6/6, lib 589→599). Forced sweep `anvil-mo-sweep/`: `__mt(` 4/5 seeds, Verilator `-Wall` Δ=0 vs OFF (2012/2017/2023) + Yosys both + Icarus clean, seed-7 exhaustively sim-equiv to the inline reference (128 inputs). Frontier → `.12b.2`. |
| `STRUCTURED-EMISSION-EXPANSION.12a` | `STRUCTURED-EMISSION-EXPANSION.12a — multi-output task impl design-detail` | Design-detail (no source): a `DEVELOPMENT_NOTES.md` entry grounding decision `0025` in the real `task_emit.rs` / `cone_function_emit.rs` / `sv.rs` (the cone render primitives) / `gen/mod.rs` / `config.rs` / `types.rs`, resolving all seven impl points: replicate the admissibility predicate; `Module.multi_output_task_groups: BTreeMap<NodeId,Vec<NodeId>>` (leader→partners); the one-roll-per-leader pairing on a shared non-constant operand; the `in_fanin` bounded backward DFS soundness check (operand-topological `NodeId` invariant); deduplicated-formal rendering reusing `render_cone_gate_expr` with an empty `interior_set` + `__mt`/`__mtv` naming + one always_comb per group + per-member passthrough (members keep their wires); run after `task_emit` before `cone_function` + extend `sibling_marked`; the `multi_output_task_emit_prob` knob + `--multi-output-task-emit-prob` flag + `num_emitted_multi_output_tasks` metric (schema `1.13→1.14`) + `--multi-output-task-gate` / `saw_multi_output_task_emit` + gate-shape calibration. Split `.12` into `.12a` (done) + `.12b` (impl pending, pre-split `.12b.1`/`.12b.2`/`.12b.3`); frontier → `.12b`. No source change; self-checks clean. |
| `STRUCTURED-EMISSION-EXPANSION.11` | `STRUCTURED-EMISSION-EXPANSION.11 — pick multi-output task surface + decision 0025` | Design/decision leaf (no source): decision `0025` picks the sixth structured surface — a default-off, valid-by-construction **multi-output combinational `task automatic`** (generalize the single-gate task from one `output` to several: a co-supported group of qualifying gates co-emitted into ONE task with deduplicated input formals + per-member output args, called once from `always_comb`). First cut = a **pair** (`k=2`) sharing a **non-constant** operand; **soundness rule** = mutual fan-in independence (no cycle through the shared task; cheap bounded backward DFS over the operand-topological `NodeId` invariant). Reuses the cone-function dedup render primitives + the single-gate task candidate predicate (`feedback_full_factorization`). Own `multi_output_task_emit_prob` knob + `--multi-output-task-emit-prob` flag (single-gate surface stays byte-identical; reusing `task_emit_prob` rejected) + `num_emitted_multi_output_tasks` metric (schema `1.13 → 1.14` at impl) + `--multi-output-task-gate` / `saw_multi_output_task_emit` (`__mt(` detection). Fresh probe (`anvil-se-motask-probe/`): 2-output task Verilator `-Wall` 2012/2017/2023 + Yosys both + Icarus clean + iverilog-sim-equiv (5000 vec). The recorded deferred runner-up (decision `0016` `.9` probe). `INDEX.md` row; KM picks up `0025` (carries `answers:`); tree split `.11`/`.12` (pre-split `.12a`/`.12b`)/`.13+`; frontier → `.12`. No source change; self-checks clean. |
...
- `2026-06-23`: **`.19b.2a` landed — the ninth-surface metric + introspection schema bump
  (`1.16 → 1.17`).** `Metrics::num_emitted_casez_mux_if_chains` (`= m.casez_mux_if_gates.len()`,
  `#[serde(default)]`, exact because static-selector `CasezMux` are excluded) + the
  `metrics_count_emitted_casez_mux_if_chains` lib proof ⇒ introspection `SCHEMA_VERSION` `"1.16"
  → "1.17"` across all current-output refs (`introspect/mod.rs` const + doc-comment history + 3
  assertions; 8 `mcp/mod.rs` assertions; `docs/AGENT_INTROSPECTION_SCHEMA.md` current-version ×3
  + a new `1.16→1.17` changelog entry; README envelope ×2 + USER_GUIDE envelope ×1 +
  `CODEBASE_ANALYSIS.md` envelope), with per-metric `@ 1.16` landing attributions intact and the
  5 book `agent-mcp.md` example JSONs deferred to `.19b.3` (the `.17b.2a`/`.17b.3` precedent). The
  metric BUMPS the schema (new derived `Metrics` field) whereas the `.19b.1` knob rode the
  version. `cargo test --lib` 635→636; snapshots 6/6 byte-identical; clippy `-D warnings` + fmt
  clean; live `--introspect` `1.17` + metric `0` default (seed 2) / `1` forced (seed 4).
  Default-off / DUT byte-identical. Frontier → `.19b.2b`.

- `2026-06-23`: **`.19b.1` landed — the LIVE ninth structured surface.** The procedural
  `always_comb` `if`/`else if` **masked** priority-chain emit-projection of a dynamic-selector
  `CasezMux` gate (decision `0029`) goes live behind the opt-in `casez_mux_if_emit_prob` knob.
  New `src/ir/casez_mux_if_emit.rs` (`annotate_casez_mux_if_gates` — the `GateOp::CasezMux`
  predicate requiring a non-`Constant` selector + `operands.len() >= 4`, excluding `CaseMux` +
  `case_mux_if` + all sibling marks, run **last** after `case_mux_if`; 9 lib proofs) +
  `Module.casez_mux_if_gates: BTreeSet<NodeId>` + `ir/mod.rs` registration +
  `Config::casez_mux_if_emit_prob` + `--casez-mux-if-emit-prob` flag + two guarded `gen/mod.rs`
  rolls + the `emit/sv.rs` `GateOp::CasezMux` body branch (`casez…endcase` → masked
  `if`/`else if`: per arm `care_mask = ~wildcard_mask & sel_mask`, `value_masked =
  pattern_value & care_mask` via the existing `constant_value`/`bitmask` helpers, emitting
  `(sel & SW'h{care}) == SW'h{val}` + trailing `else W'h0`; **no `__cv`**) + the `:769`
  trailing-default suppression tweak. Implemented exactly per `.19a`. No metric/schema bump
  (the `.19b.2` metric bumps `1.16 → 1.17`; the knob rides the version). `cargo test --lib`
  626→635 (9 new proofs); snapshots 6/6 byte-identical; clippy `-D warnings` + fmt clean.
  Forced `casez_mux_if_emit_prob=1.0` sweep (`scratchpad/sweep19`, 6 seeds): every `casez`
  block → a masked chain (0 residual `casez`); Verilator `-Wall` Δ=0 vs OFF (2012/2017/2023);
  both Yosys modes + Icarus clean; ON-vs-OFF sim-equiv (seed 1 8/8 + seed 4 128/128).
  Default-off / DUT byte-identical. Frontier → `.19b.2`.

- `2026-06-23`: **`.19a` landed — the ninth-surface impl design-detail (no source change).**
  A `DEVELOPMENT_NOTES.md` entry grounding decision `0029` in a fresh read of the real code
  and pinning all `.19b` impl points: the dynamic-selector `CasezMux` candidate predicate
  (`operands[0]` not a `Node::Constant`, `len >= 4`; `CaseMux` + `case_mux_if`/sibling marks +
  `param_env` excluded); the `Module.casez_mux_if_gates: BTreeSet<NodeId>` carrier; the
  **in-place** `emit/sv.rs` `CasezMux` body branch computing per-arm `care_mask =
  !wildcard_mask & sel_mask` + `value_masked = pattern_value & care_mask` from the arm's
  `(value, mask)` constants (the existing `constant_value`/`bitmask` helpers) and emitting
  `(sel & SW'h{care}) == SW'h{val}` chained + trailing `else W'h0` — **no `__cv`** (a
  `CasezMux` is already an `always_comb` var) — plus the `:769` trailing-default suppression
  tweak; run `annotate_casez_mux_if_gates` last; the `casez_mux_if_emit_prob` knob/flag; the
  `num_emitted_casez_mux_if_chains` metric (schema `1.16 → 1.17`, exact); the metric-keyed
  `--casez-mux-if-gate` / `saw_casez_mux_if_emit` gate + a focus config zeroing both
  `comb_mux_prob` and `case_mux_prob` (both roll before `casez_mux` in `cone.rs`); the
  byte-identical/RNG argument. Records the empirical form selection (`sel ==? pattern`
  DISQUALIFIED — Yosys rejects `==?`; the masked-AND form ships) + the `.19b` proof plan
  (~9 lib proofs + the live source shape). Frontier → `.19b.1`. No source change ⇒ DUT
  byte-identical.
...
- `2026-06-23`: **`.17b.2a` landed — the eighth-surface metric + introspection schema
  ...
  m.case_mux_if_gates.len()`, `#[serde(default)]`) + a lib proof ⇒ introspection
  `SCHEMA_VERSION` `"1.15" → "1.16"` across all current-output refs (`introspect/mod.rs`
  const + doc-comment + 3 assertions; 8 `mcp/mod.rs` assertions;
  `docs/AGENT_INTROSPECTION_SCHEMA.md` current-version ×3 + a new `1.15→1.16` changelog
  entry; README ×2 + USER_GUIDE ×1 + `CODEBASE_ANALYSIS.md` envelope refs), with the
  historical `num_emitted_mux_if_blocks @ 1.15` landing attributions left intact and the
  book example JSONs deferred to `.17b.3` (the `.15b.2`/`.15b.3` precedent). The metric is
  **exact** — the annotation pass excludes constant-selector `CaseMux`, so every counted
  gate emits exactly one chain. `cargo check`/`clippy -D warnings`/`fmt` clean; `cargo test
  --lib` 625→**626** (the new `metrics_count_emitted_case_mux_if_chains` proof; all
  introspect + mcp `schema_version` assertions green at `"1.16"`); snapshots **6/6
  byte-identical** (default-off — the metric is a post-hoc RTL-invisible count). Live
  `--introspect`: schema `1.16` + `num_emitted_case_mux_if_chains` `0` default (seed 42) /
  `53` forced (seed 2). Default-off / DUT byte-identical. `.17b.2` pre-split → `.17b.2a`
  (this) + `.17b.2b` (the repo-owned `tool_matrix --case-mux-if-gate`); frontier →
  `.17b.2b`. The lane stays `active`.
- `2026-06-22`: **`.17b.1` landed — the LIVE eighth structured surface (the first source
  change of the eighth surface).** The procedural `always_comb` `if`/`else if` **priority
  chain** emit-projection of a dynamic-selector `CaseMux` gate (decision `0028`) goes live
  behind the opt-in `case_mux_if_emit_prob` knob: new `src/ir/case_mux_if_emit.rs`
  (`annotate_case_mux_if_gates` — the dynamic-selector `CaseMux` predicate `operands[0]`
  not a `Node::Constant`, excluding `CasezMux` + all sibling marks, run **last**; 9 lib
  proofs) + `Module.case_mux_if_gates: BTreeSet<NodeId>` + `ir/mod.rs` registration +
  `Config::case_mux_if_emit_prob` + `--case-mux-if-emit-prob` flag + two guarded
  `gen/mod.rs` rolls (after `mux_if`, single + design) + the `emit/sv.rs` structured-case
  body branch (the `GateOp::CaseMux` arm renders the `if`/`else if` chain over the same
  operand refs + `W'h0` default when marked; the parallel `case`/`endcase` is suppressed
  via the `!contains(...)` tail guard). Implemented exactly per the `.17a` design (no
  deviations). **Simpler than the seventh surface** — a `CaseMux` is already an
  `always_comb`-written `logic` var, so no `<g>__cv` output-var + passthrough, only the
  block body swaps. No metric/schema bump (the `.17b.2` metric bumps `1.15 → 1.16`; the
  knob rides the version). `cargo check`/`clippy -D warnings`/`fmt --check` clean;
  `cargo test --lib` 616→**625** (9 new proofs incl. the constant-selector exclusion + the
  end-to-end emit proof); `--bin tool_matrix` **89** (unaffected); snapshots **6/6
  byte-identical** (default-off). Forced `case_mux_if_emit_prob=1.0` sweep
  (`scratchpad/probe8/`, seed 2: 7 `CaseMux` → 7 chains / 14 `else if` / 0 residual
  `case`): Verilator `-Wall` **Δ=0 vs OFF** (2012/2017/2023) + both Yosys modes + Icarus
  clean; generated-RTL ON-vs-OFF **sim-equiv over 20000 vectors** (`GEN-RTL SIM-EQUIV
  OK`). `CODEBASE_ANALYSIS.md` + `DEVELOPMENT_NOTES.md` updated. Default-off / DUT
  byte-identical. Pre-split `.17b` → `.17b.1`/`.17b.2`/`.17b.3`; frontier → `.17b.2`
  (the metric + the repo-owned `--case-mux-if-gate`). The lane stays `active`.
- `2026-06-22`: **`.17a` landed — the eighth-surface impl design-detail (no source
  change).** A `DEVELOPMENT_NOTES.md` entry grounds decision `0028` in a fresh read of the
  real code and pins all seven impl points for the `CaseMux` → `if`/`else if`
  priority-chain surface. The pass mirrors `src/ir/mux_if_emit.rs` but is **simpler**: a
  `CaseMux` is already an `always_comb`-written `logic` var, so there is **no** `<g>__cv`
  output-var + passthrough — only the `always_comb` body swaps `case…endcase` →
  `if…else if`. Resolved: (0) the candidate is a **dynamic-selector** `CaseMux`
  (`operands[0]` not a `Node::Constant` — the inverse of `render_static_structured_gate`'s
  `CaseMux` static-collapse test `constant_value(operands[0]).is_some()`; checked locally
  in `ir/`), excluding `CasezMux` (its `casez` `?`-wildcards need a masked comparison — the
  follow-up) + sibling marks (vacuous) + `param_env`; (1) `Module.case_mux_if_gates:
  BTreeSet<NodeId>`; (2) the in-place emitter body branch reusing the same operand refs +
  `W'h0` default; (3) run `annotate_case_mux_if_gates` last (eighth guarded `gen/mod.rs`
  call site after `mux_if`); (4) `case_mux_if_emit_prob` knob + `--case-mux-if-emit-prob`
  flag mirroring `mux_if_emit_prob`'s six `config.rs` touch points; (5)
  `num_emitted_case_mux_if_chains` metric (schema `1.15 → 1.16`) — exact because static
  selectors are excluded; (6) the **metric-keyed** `--case-mux-if-gate` /
  `saw_case_mux_if_emit` gate (no new text token — more robust than scanning `if(==)`
  which FSM decode blocks also contain) + a `case_mux_prob`-biased focus config; (7) the
  byte-identical/RNG argument. `.17b` proof plan (~8 lib proofs + snapshots 6/6 + forced
  ON-vs-OFF sweep Δ=0 + sim-equiv) + the `.17b.1`/`.17b.2`/`.17b.3` pre-split + rejected
  alternatives recorded. `check_doctrines` (4) + `check_knowledge_map` +
  `check_memory_architecture` + `mdbook build` green; no `src/` touched ⇒ DUT
  byte-identical (the `.15b.2` bank holds). `.17` split → `.17a` (done) + `.17b` (impl
  pending); frontier → `.17b.1`. The lane stays `active`.
- `2026-06-22`: **`.16` landed — picked the EIGHTH structured surface (design/decision
  leaf, no source change).** Decision `0028` picks a default-off, valid-by-construction
  procedural `always_comb` `if`/`else if` **priority chain** emit-projection of a
  `CaseMux` gate (the N-way selection it renders today as a parallel `case (sel) …
  default` statement) — the recorded decision-`0027` follow-up. It is genuinely distinct
  from the seventh surface (the N-way `CaseMux` chain vs the single 2:1 `Mux`
  conditional; `GateOp::CaseMux` vs `GateOp::Mux`) and from the existing `case` render
  (sequential-priority `if`/`else if` vs parallel-match `case`). Simpler than the
  seventh surface: a `CaseMux` is **already** an `always_comb`-written `logic` var, so
  **no** `<g>__cv` output var + passthrough is needed — only the `always_comb` body swaps
  `case…endcase` → `if…else`. Candidate = a dynamic-selector `CaseMux`
  (`render_static_structured_gate` returns `None`); `CasezMux` (its `casez` wildcards
  need a masked comparison) is the recorded follow-up. Own `case_mux_if_emit_prob` knob +
  `num_emitted_case_mux_if_chains` metric (schema `1.15 → 1.16` at impl) +
  `--case-mux-if-gate` / `saw_case_mux_if_emit` (keyed on the metric `> 0` — no new text
  token). Chosen over nested/multi-level `generate` (no by-construction source) and
  `interface`/`modport` (empirically disqualified since `.7`). **Fresh empirical probe**
  this session (`scratchpad/probe8/`): Verilator `-Wall` 2012/2017/2023 + Yosys both
  modes (32 cells, no warnings) + Icarus + iverilog `vvp` sim-equiv to the parallel
  `case` over 20000 vectors + an exhaustive selector sweep (`EQUIV OK`). `INDEX.md` row +
  `KNOWLEDGE_MAP.md` regenerated; the stale parent `.15`/`.15b` flipped `active`→`done`
  (children all done; the `.5`/`.6`/`.6b`/`.13` closed-parent convention); ROADMAP
  owner-directed-lane #2 + `docs/TASK_TREE.md` row updated. `mdbook build` +
  `check_doctrines` (4) + `check_knowledge_map` + `check_memory_architecture` green; no
  `src/` touched ⇒ DUT byte-identical. Split `.16` (design) + `.17` (impl, pre-split
  `.17a`/`.17b`) + future `.18+`; frontier → `.17`. The lane stays `active`.
...
- `2026-06-22`: **`.15b.2` landed — the seventh-surface metric + repo-owned gate.**
  ...
  introspection `SCHEMA_VERSION` `1.14 → 1.15` (bumped across all current-output refs:
  `introspect/mod.rs` const + doc-comment + 3 lib assertions, 8 `mcp/mod.rs`
  assertions, `docs/AGENT_INTROSPECTION_SCHEMA.md` §7 current-version statements + a new
  `1.14 → 1.15` changelog entry, README ×2 + USER_GUIDE ×1 + `CODEBASE_ANALYSIS.md`
  envelope refs; the `num_emitted_multi_output_tasks @ 1.14` historical attributions
  left intact; book example JSONs deferred to `.15b.3` per the `.12b.2a`/`.12b.3`
  precedent). Repo-owned `tool_matrix --mux-if-gate` (`ScenarioSet::MuxIfSweep` + a
  Mux-biased `mux_if_focus_config` [comb-only, `mux_if_emit_prob=1.0`,
  `comb_mux_prob=0.9` + `comb_mux_encoding_prob=1.0` forcing the encoded
  chained-ternary path that builds plain `GateOp::Mux` gates] × the three strategies
  + `ModuleReport.emitted_mux_if` [`__cv` detection] + `CoverageSummary.saw_mux_if_emit`
  + merge + `MatrixReport.mux_if_gate` + run-plan units/fail-on-gap + mutual-exclusion +
  select/build dispatch + summarize block + early-return gap arm + slug arms + 5
  cargo-portable proofs + 8 fixture extensions), templated on `--multi-output-task-gate`.
  `cargo check/clippy(-D warnings)/fmt` clean; `cargo test --lib` **616** (615 + 1) / 2
  ignored; `cargo test --bin tool_matrix` **89** (84 + 5) / 1 ignored; snapshots **6/6
  byte-identical** (default-off). Banked clean `anvil-mux-if-gate-r1`
  (`--yosys-mode both --iverilog-compile`): 3 scenarios, 12 modules, `coverage_gaps=[]`,
  `saw_mux_if_emit=true`, 12/12 emit a `__cv` block / 215 total
  `num_emitted_mux_if_blocks`, Verilator 12/0 + both Yosys 12/0 + Icarus 12/0, gate exit
  0. Default-off / DUT byte-identical. Frontier → `.15b.3` (user docs).
- `2026-06-22`: **`.15b.1` landed — the LIVE seventh structured surface (the first
  source change). The procedural `always_comb` `if`/`else` emit-projection of a 2:1
  `Mux` gate (decision `0027`) goes live** behind the opt-in `mux_if_emit_prob` knob:
  new `src/ir/mux_if_emit.rs` (`annotate_mux_if_gates` — the `GateOp::Mux` candidate
  predicate excluding all six sibling marks, run last; 10 lib proofs) +
  `Module.mux_if_gates: BTreeSet<NodeId>` + `ir/mod.rs` registration + the
  `mux_if_emit_prob` knob/flag + two guarded `gen/mod.rs` rolls (after `cone_function`,
  single + design) + the `emit/sv.rs` procedural-block section (`logic [w-1:0]
  <name>__cv;` + the `always_comb if/else`, operand refs via `node_ref`) + the
  gate-assign-loop `<name>__cv` passthrough. Implemented exactly per `.15a` (no
  deviations); `CODEBASE_ANALYSIS.md` + `DEVELOPMENT_NOTES.md` updated. No
  metric/schema bump (the `.15b.2` metric bumps `1.14→1.15`; the knob rides the
  version). Default-off / DUT byte-identical (`cargo check`/`clippy`/`fmt` clean,
  `cargo test --lib` 605→615 incl. 10 new proofs, snapshots 6/6). Forced
  `mux_if_emit_prob=1.0` sweep (`anvil-muxif-genproof.*`, 5 seeds): every plain
  mux (121–195/seed) → `__cv`, Verilator `-Wall` Δ=0 vs OFF + both Yosys modes (no
  warnings) + Icarus clean; comb-only ON-vs-OFF iverilog sim-equiv over 20000
  vectors. Pre-split `.15b` → `.15b.1` (done) + `.15b.2` (metric + `--mux-if-gate`) +
  `.15b.3` (docs); frontier → `.15b.2`.
- `2026-06-22`: **`.15a` landed — the seventh-surface impl design-detail (no source):
  a `DEVELOPMENT_NOTES.md` entry grounding decision `0027` in the real
  `task_emit.rs` / `sv.rs` / `gen/mod.rs` / `config.rs` / `types.rs` and resolving
  all seven impl points** (the `GateOp::Mux` candidate predicate excluding the six
  sibling marks since the pass runs last; `Module.mux_if_gates: BTreeSet<NodeId>`;
  the emitter `always_comb if/else` `<name>__cv` block + the gate-assign-loop
  passthrough; run `annotate_mux_if_gates` last ⇒ no other pass changes; the
  `mux_if_emit_prob` knob + flag; the `num_emitted_mux_if_blocks` metric schema
  `1.14→1.15`; the `--mux-if-gate` gate + `saw_mux_if_emit` + calibration; the
  byte-identical/RNG argument). Pre-split `.15` → `.15a` (done) + `.15b` (impl,
  pre-split `.15b.1`/`.15b.2`/`.15b.3`); frontier → `.15b.1`. `check_doctrines` +
  `check_knowledge_map` + `check_memory_architecture` + `mdbook build` green. No
  source change; DUT byte-identical.
- `2026-06-22`: **`.14` landed — picked the SEVENTH structured surface (a procedural
  ...
  output-var + passthrough mechanism. Fresh probe (`anvil-ifelse-probe.*`):
  Verilator `-Wall` 2012/2017/2023 + both Yosys modes (no warnings) + Icarus clean,
  iverilog `vvp` sim-equiv to the inline ternary over 20000 vectors. Own
  `mux_if_emit_prob` knob + `num_emitted_mux_if_blocks` metric (schema `1.14 → 1.15`
  at impl) + `--mux-if-gate` / `saw_mux_if_emit` (`__cv` detection); the N-way
...
- `2026-06-22`: **`.13c` landed — the user-facing docs closeout of the first
  *deepening* of the sixth surface (wider `k>2` multi-output `task automatic`
  groups); `.13c` / `.13` all close — the deepening is delivered end-to-end and the
  lane returns to no current frontier (open-ended; future surfaces `.14+`).**
  Docs-only / DUT byte-identical: `book/src/structured-emission.md` — the
  sixth-surface intro reframed (co-supported pair → co-supported group `k>=2`, up to
  8) + a new `### Wider groups (k > 2)` subsection with a **real seed-22 three-member
  task** (`shr_0`/`mux_0`/`mux_1`; the shared select `a0` feeds both `o1` and `o2`,
  `a2` feeds both `o0` and `o1`) + the connected-co-support / all-member-independence
  rules + the 8-member cap + the Δ=0/sim-equiv note; the "What gets wrapped" + "How
  anvil proves it" bullets generalized pair → group (k=3 in the gate bank + new
  library proofs at every size). The `multi_output_task_emit_prob` knob entry
  updated in `book/src/knobs.md` / `USER_GUIDE.md` / README "Current CLI truth". The
  KM card `docs/knowledge/multi-output-task-emit.md` refreshed (title + 4 new answers
  incl. "can a multi-output task have more than two outputs" / the group-size cap /
  "connected co-support", evidence, body pair → `k>=2` greedy extension + cap, bank →
  `anvil-mo-k3-gate-r1`); `KNOWLEDGE_MAP.md` regenerated (64 facts / 589 → 593
  keys). No decision-`0025` mutation (the ADR already records the `.13` follow-up;
  the tree Decisions section cross-refs). `mdbook build` + `check_knowledge_map` +
  `check_memory_architecture` + `cargo test --test book_examples` 3/3 green. No
  `src/` touched ⇒ cargo unaffected.
- `2026-06-22`: **`.13b` landed — the LIVE `k>2` widening of the sixth surface (the
  multi-output combinational `task automatic`).** The whole change is in
  `src/ir/multi_output_task_emit.rs`: a `const MAX_MULTI_OUTPUT_TASK_GROUP_MEMBERS = 8`
  + two helpers (`connected_co_support` = shares a non-constant operand with at
  least one current member; `independent_of_all` = mutually fan-in-independent with
  every current member) + the partner loop rewritten from "find the first partner
  and break" to "greedily admit every eligible ungrouped higher-`NodeId` candidate
  up to the cap" (a group forms iff `>= 1` partner ⇒ `k=2` is the exact subset).
  **No emitter / config / metric / schema / knob change** — the emitter was already
  written k-agnostic, confirmed by the new 3-output emit proof. 5 new k>2 lib proofs
  + all 10 existing pair proofs green (15 total); doc comments + `CODEBASE_ANALYSIS.md`
  brought current (pair → `k>=2`); a `DEVELOPMENT_NOTES.md` impl-time note.
  Default-off / DUT byte-identical (`cargo test --lib` 600→605, snapshots 6/6 — the
  default `0.0` path never calls the pass). Gate re-bank `anvil-mo-k3-gate-r1`
  `coverage_gaps = []` / `saw_multi_output_task_emit = true` / `12/0` Verilator +
  both Yosys + Icarus with a **k=3 group** present; forced sweep seed 22 emits a
  genuine k=3 task `shr_0__mt(o0,o1,o2,…)`, Verilator `-Wall` **Δ=0 vs OFF** + iverilog
  **sim-equiv over 20000 vectors**. Frontier → `.13c` (user docs).
- `2026-06-22`: **`.13a` landed — the design-detail for the first *deepening* of
  the sixth surface: widen the multi-output `task automatic` group from the
  first-cut pair (`k=2`) to a bounded `k>2` (the recorded `.13` follow-up of
  decision `0025`).** Docs-only / no source: a `DEVELOPMENT_NOTES.md` design-detail
  entry that (after a fresh read) confirms the **emitter is already k-agnostic**
  (`multi_output_task_params` / `render_multi_output_task_decl` /
  `render_multi_output_task_call` iterate `members` of any length; the carrier is
  `BTreeMap<NodeId,Vec<NodeId>>`), so the **only `.13b` source change** is widening
  `annotate_multi_output_task_groups`. Pins the greedy group-extension (keep the
  per-leader inline `gen_bool(prob)` roll; collect all eligible partners up to a
  cap), **connected co-support** (shares a non-constant operand with at least one
  current member — leader-anchored star rejected), **generalized mutual fan-in
  independence** with all current members (inductively maintained ⇒ cycle-free at
  any `k`), a named `const MAX_MULTI_OUTPUT_TASK_GROUP_MEMBERS = 8`, the
  **no-new-knob / no-new-metric / no-schema-bump** decision (the metric counts
  groups), and the byte-identical/RNG argument (default `0.0` ⇒ the pass is not
  called; `prob=1.0` `gen_bool(1.0)` short-circuits ⇒ zero RNG draws). `.13b` proof
  plan + rejected alternatives recorded. Tree/`docs/TASK_TREE.md` registered `.13`/
  `.13a`/`.13b`/`.13c`; frontier → `.13b`. `check_memory_architecture` +
  `check_knowledge_map` + `mdbook build` ✅; no `src/` ⇒ cargo unaffected.
- `2026-06-22`: **`.12b.3` landed — the multi-output task user-facing docs; `.12b.3`
  / `.12b` / `.12` all close — the SIXTH structured surface (decision `0025`) is
  delivered end-to-end; the lane returns to no current frontier (open-ended).**
  Docs-only / DUT byte-identical: a `## The sixth surface: a multi-output task
  automatic` section in `book/src/structured-emission.md` (byte-verified seed-3
  before/after — the inline `xor_0 = i_1 ^ i_2; not_0 = ~i_2;` pair becomes one
  `task automatic xor_0__mt(...)` where the shared `i_2` is the single formal `a1`
  feeding both outputs — the co-supported sink; the shared-non-constant-operand +
  fan-in-independence rules; members keep their wires; the metric @ schema `1.14` +
  the `--multi-output-task-gate`; a skip-sentinelled repro) + the
  `multi_output_task_emit_prob` knob entry in `book/src/knobs.md` / `USER_GUIDE.md` /
  README "Current CLI truth" + the KM how-to card `multi-output-task-emit`
  (cross-linked to decision `0025`, with a working reverify; KM 61→62 facts /
  551→565 keys) + the deferred `book/src/{api-tools,agent-mcp,api-introspection,
  api-reference}.md` example-JSON `schema_version` `1.13 → 1.14` refresh (the
  historical `num_mealy_fsm_modules @ 1.13` refs in sequential.md/knobs.md stay).
  `mdbook build` + `check_knowledge_map` + `check_memory_architecture` +
  `cargo test --test book_examples` 3/3 green; the book before/after is
  byte-verified against `anvil --seed 3` output and downstream-clean.
  **SIX structured surfaces delivered end-to-end.**

- `2026-06-22`: **`.12b.2b` landed — the repo-owned `tool_matrix
  --multi-output-task-gate`; `.12b.2b` / `.12b.2` close; frontier → `.12b.3`.**
  Templated on `--cone-function-gate`: `src/bin/tool_matrix.rs` gains
  `--multi-output-task-gate` + `ScenarioSet::MultiOutputTaskSweep` +
  `build_multi_output_task_sweep_scenarios`/`multi_output_task_focus_config`
  (comb-only `multi_output_task_emit_prob=1.0` × 3 construction strategies;
  node-id + e-graph; high `terminal_reuse_prob=0.6` + shallow `max_depth=2` +
  `min_outputs>=2` so co-supported fan-in-independent sibling pairs exist) +
  `ModuleReport.emitted_multi_output_task` (`prepared.sv_text.contains("__mt(")`,
  distinct from the single-gate `__t(` and the cone `__cf(`) +
  `CoverageSummary.saw_multi_output_task_emit` (lit on Verilator success + Yosys
  clean) + `MatrixReport.multi_output_task_gate` + the merge + the early-return gap
  arm + the units floor + the mutual-exclusion + 5 cargo-portable proofs + the 8
  ModuleReport + 1 Cli fixture updates + README/USER_GUIDE/CODEBASE gate entries.
  **Banked clean** `anvil-multi-output-task-gate-r1` (3 scenarios / 12 modules
  / 6 emitting a multi-output task / `coverage_gaps = []` /
  `saw_multi_output_task_emit = true` / `12/0` Verilator + both Yosys modes +
  Icarus compile). `cargo test --bin tool_matrix` 84 + snapshots 6/6 byte-identical.
  Default-off / DUT byte-identical.

- `2026-06-22`: **`.12b.2a` landed — the multi-output task metric + introspection
  schema `1.13 → 1.14`; `.12b.2` split into `.12b.2a` (done) + `.12b.2b` (the
  `tool_matrix` gate); frontier → `.12b.2b`.** `Metrics::num_emitted_multi_output_tasks`
  (`= m.multi_output_task_groups.len()`, `#[serde(default)]`) in `metrics::compute()`
  + a lib proof; surfaced in introspection `module_metrics` ⇒ `SCHEMA_VERSION`
  `1.13 → 1.14` (the new derived metric bumps; the `.12b.1` knob rode the version).
  Bumped all current-output schema refs (the doc comment + the `SCHEMA_VERSION`
  const + the 3 introspect + 8 mcp `schema_version` assertions + the schema doc's
  `1.13→1.14` changelog/defines/early-example/checklist + README + USER_GUIDE
  current refs + the `CODEBASE_ANALYSIS.md` envelope line — brought current from a
  drifted "1.12" to "1.14" with the missing `1.12→1.13`/`1.13→1.14` list entries
  added); historical "landed at schema X" attributions left intact; the book/src
  example JSON `schema_version` refresh is deferred to `.12b.3` (the Mealy
  `.2b.2a`/`.2b.3` precedent). `cargo test --lib` 600 + snapshots 6/6 byte-identical;
  end-to-end `--introspect` default `0` / forced `1` at schema `1.14`. Default-off /
  DUT byte-identical.

- `2026-06-22`: **`.12b.1` landed — the multi-output combinational `task automatic`
  LIVE surface (the FIRST source change of the sixth surface); `.12b` split into
  `.12b.1` (done) + `.12b.2` (metric+gate) + `.12b.3` (docs); frontier → `.12b.2`.**
  Live emitter change implementing decision `0025` per the `.12a` design:
  `Config::multi_output_task_emit_prob` (default `0.0`) + the
  `--multi-output-task-emit-prob` CLI flag + `Module.multi_output_task_groups:
  BTreeMap<NodeId, Vec<NodeId>>` + new `src/ir/multi_output_task_emit.rs`
  (`annotate_multi_output_task_groups`: one `gen_bool(prob)` roll per ungrouped
  leader, paired with the next ungrouped candidate sharing a non-constant operand
  and mutually fan-in-independent via the `in_fanin` bounded backward DFS) +
  `src/ir/mod.rs` registration + two guarded `gen/mod.rs` rolls (after `task_emit`,
  before `cone_function`) + `cone_function_emit::sibling_marked` extended to exclude
  multi-output members + the `src/emit/sv.rs` decl/call section
  (`render_multi_output_task_decl`/`render_multi_output_task_call`/`multi_output_task_params`,
  the deduplicated-formal body reusing `render_cone_gate_expr` with an **empty
  `interior_set`**) + the per-gate passthrough `assign <m> = <m>__mtv;` (members
  keep their module wires — co-equal roots, no use-count rule) + 11 lib proofs.
  Two notes: the cone primitives composed for free (the `feedback_full_factorization`
  reuse the ADR predicted); members keep their wires. No metric / no schema bump
  (the metric + schema `1.13 → 1.14` land at `.12b.2`; the knob rides the version —
  the `.10b.1`/`.6b.1` precedent). Default-off / DUT byte-identical (`cargo test
  --lib` 589→599, snapshots 6/6). Forced `multi_output_task_emit_prob=1.0` sweep
  (`anvil-mo-sweep/`, 4 seeds): `__mt(` fires 4/5, Verilator `-Wall` Δ=0 vs
  OFF (2012/2017/2023) + Yosys both modes + Icarus clean, seed-7 exhaustively
  sim-equivalent to the inline reference (all 128 inputs).

- `2026-06-22`: **`.12a` landed — the multi-output task impl design-detail; `.12`
  split into `.12a` (done) + `.12b` (impl pending, pre-split `.12b.1`/`.12b.2`/`.12b.3`);
  frontier → `.12b`.** Design-detail leaf, no source change (a `DEVELOPMENT_NOTES.md`
  entry). Grounded decision `0025` in the real `src/ir/task_emit.rs`,
  `src/ir/cone_function_emit.rs`, `src/emit/sv.rs` (the cone render primitives
  `cone_function_params`/`cone_operand_ref`/`render_cone_gate_expr` — reused with an
  **empty `interior_set`** so every member operand resolves to a folded literal or
  its dedup param `a{i}`), `src/gen/mod.rs` (the roll chain), `src/config.rs`,
  `src/metrics.rs`, `src/ir/types.rs` (confirmed the operand-topological `NodeId`
  invariant). Pinned all seven impl points: replicate the admissibility predicate;
  `Module.multi_output_task_groups: BTreeMap<NodeId,Vec<NodeId>>` (leader→partners);
  one-roll-per-leader pairing on a shared non-constant operand; the `in_fanin`
  bounded backward DFS soundness check; dedup-formal rendering + `__mt`/`__mtv`
  naming + one always_comb per group + per-member passthrough (members keep their
  wires); run after `task_emit` before `cone_function` + extend `sibling_marked`;
  the knob/flag/metric-schema-`1.13→1.14`/gate + gate-shape calibration (high
  `terminal_reuse_prob` + shallow `max_depth`). No source change; self-checks clean.
...
- `2026-06-22`: **`.11` landed — picked the SIXTH structured surface (the
  ...
  **co-supported group** of qualifying combinational gates (the single-gate task
  ...
  (`anvil-se-motask-probe/`): a 2-output combinational task is Verilator
...
- `2026-06-17`: **`.10b.1` landed — the multi-gate-cone `function automatic` live
  ...
  `.10b.1` (done) + `.10b.2` (metric+gate) + `.10b.3` (docs); frontier →
  `.10b.2`.** Live emitter change implementing decision `0016` per the `.10a`
  design: `Config::cone_function_emit_prob` (default `0.0`, config-file-only,
  ...
  — a complete-coverage `compute_use_counts`, the post-order **single-use**
  cone-walk, greedy node-order selection with interior reservation, run last after
  all four sibling projections) + `src/ir/mod.rs` registration + two guarded
  `src/gen/mod.rs` rolls (single + design, after `task_emit`) + the `src/emit/sv.rs`
  cone-decl section + interior-suppression in the net-decl **and** gate-assign loops
  + four render helpers (`render_cone_function_decl`/`render_cone_function_call`/
  `render_cone_gate_expr`/`cone_function_params` + `cone_operand_ref`) + 8 lib
  proofs. Two impl wrinkles handled (recorded in `DEVELOPMENT_NOTES.md`): `node_ref`
  resolves inputs/constants/flops intrinsically (so the body needs a dedicated
  resolver-based renderer, not a `render_gate` names-override), and an absorbed
  interior gate must suppress its module **wire** decl + inline assign
  (`use_count==1` makes it provably safe). No metric / no schema bump (the metric
  `num_emitted_cone_functions` + schema `1.10 → 1.11` land at `.10b.2`; the knob
  rides the version, the `.6b.1`/`.4b.1` precedent). Default-off / DUT
  byte-identical (`cargo test --lib` 493 → 501 incl. 8 new proofs; snapshots 6/6).
  Forced `cone_function_emit_prob=1.0` sweep (`anvil-cf-sweep/`, 8 seeds): 18
  cone functions emitted, Verilator `-Wall` Δ=0 vs OFF + Yosys both modes + Icarus
  all clean. `CODEBASE_ANALYSIS.md` gains the `ir/cone_function_emit.rs` module
  entry. Nothing retired.
- `2026-06-17`: **`.10a` landed — the multi-gate-cone `function automatic` impl
  design-detail; `.10` split into `.10a` (done) + `.10b` (impl pending); frontier
  → `.10b`.** Design-detail leaf, no source change (a `DEVELOPMENT_NOTES.md`
  entry). Grounded decision `0016` in the real `src/ir/function_emit.rs`
  (`gate_qualifies`), `src/introspect/analyze.rs` (`build_cone`/`visit` — the
  post-order fan-in walk + support-leaf boundary rules), `src/emit/sv.rs`
  (`render_gate_function_decl`/`_body`/`_call` — extend to a multi-statement
  body), `src/gen/mod.rs` (the roll chain — run cone-function last), and
  `src/config.rs` (the knob list), and resolved all five open questions: separate
  `cone_function_emit_prob` knob; walk-roots/roll-per-cone selection with interior
  reservation; absorb only admissible (non-structured/non-`Slice`),
  non-sibling-marked, **single-use-within-cone** interior gates (else boundary
  param; `Constant` folds inline); post-order topo body + module-wire-name locals
  + ascending-NodeId params; run last + `Module.cone_function_gates:
  BTreeMap<NodeId, Vec<NodeId>>` + emitter interior-suppression; the
  `num_emitted_cone_functions` metric (schema `1.10 → 1.11`) +
  `--cone-function-gate` / `saw_cone_function_emit`. `.10b` impl shape recorded.
  `scripts/check_memory_architecture.sh` + KM gen/check + `mdbook build book`
...
- `2026-06-17`: **`.8b` landed — the FOURTH structured surface (the wider-lane
  ...
  + snapshots 6/6 byte-identical (default-off); a per-seed ON-vs-OFF downstream
  sweep (`anvil-gl8b/`, 8 seeds) emits 9 wider-lane part-selects with
  Verilator `-Wall` Δ=0 + Yosys both modes + Icarus rc=0; the
  `--generate-loop-gate` bank (`anvil-generate-loop-gate-8b`) stays
...
- `2026-06-17`: **`.8a` landed — the wider-lane `generate for` part-select impl
  ...
  (`anvil-widelane-probe/`) emits 448 `{N{x}}` replications, 20 with a
...
- `2026-06-17`: **`.7` landed — picked the FOURTH structured surface (the
  ...
  0.64 both modes + Icarus 13.0, `anvil-probe-se4/`) accepts it
...
- `2026-06-16`: **`.6b.2b` landed — the repo-owned `tool_matrix --task-emit-gate`;
  ...
  `anvil-task-emit-gate-r1` (`--task-emit-gate --yosys-mode both
...
- `2026-06-16`: **`.6b.1` landed — the combinational `task automatic` live
  ...
  (`anvil-te-r1/`, 5 seeds, 4–39 tasks each). Frontier → `.6b.2`. Nothing
  retired.
...
- `2026-06-16`: **`.4b.2b` landed — the repo-owned `tool_matrix
  ...
  `anvil-generate-loop-gate-r1` (`--generate-loop-gate --yosys-mode both
...
- `2026-06-16`: **`.4b.1` landed — the `generate for` loop live surface; `.4b`
  ...
  `anvil-gl-r1/`, 62–168 loops each): Verilator `--lint-only` 5/5 rc=0 / 0
...
- `2026-06-16`: **`.2b.2b` landed — the repo-owned `tool_matrix --function-emit-gate`.**
  ...
  (snapshots 6/6). Banked downstream-clean `anvil-function-emit-gate-r1`
...
- `2026-06-16`: **`.2b.1` live surface landed** — the first richer-structured
  ...
  + both Yosys modes + Icarus (`anvil-fe-r2/`). Frontier → `.2b.2` (the
