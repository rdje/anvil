...
- Created: `2026-06-15`
- Last updated: `2026-06-23` (**`.17b.2a` landed — the eighth-surface metric +
...
**Current frontier → `STRUCTURED-EMISSION-EXPANSION.17b.2b` (the repo-owned `tool_matrix
--case-mux-if-gate` for the EIGHTH surface).** The eighth surface's **live source change**
(`.17b.1`) **and its metric + introspection schema bump** (`.17b.2a`) have landed: the
...
seed 2), and `.17b.2` is pre-split `.17b.2a` (metric+schema, done) + `.17b.2b` (the gate).
The procedural `always_comb` `if`/`else if` **priority chain** projection of a
dynamic-selector `CaseMux` gate is live behind the opt-in `case_mux_if_emit_prob` knob —
new `src/ir/case_mux_if_emit.rs` + `Module.case_mux_if_gates` + the `--case-mux-if-emit-prob`
...
+ 9 lib proofs. Default-off / DUT byte-identical (`cargo test --lib` 616→625, snapshots
6/6); a forced `case_mux_if_emit_prob=1.0` sweep (seed 2: 7 `CaseMux` → 7 chains) is
Verilator `-Wall` Δ=0 vs OFF (2012/2017/2023) + both Yosys modes + Icarus clean and
ON==OFF sim-equiv over 20000 vectors. The surface is picked (`.16`, decision
[`0028`](../decisions/0028-structured-emission-eighth-surface-case-mux-priority-chain.md))
+ designed (`.17a`) + live (`.17b.1`) + metered @ schema `1.16` (`.17b.2a`). **`.17b.2b`**
(the repo-owned `tool_matrix --case-mux-if-gate` / `saw_case_mux_if_emit` [metric-keyed, no
new text token] + a `case_mux_prob`-biased focus config) is the next frontier, then
`.17b.3` (user docs).
