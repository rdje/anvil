...
- Created: `2026-06-15`
- Last updated: `2026-06-16` (**`.6a` landed — the combinational `task automatic`
  impl design-detail; `.6` split into `.6a` (done) + `.6b` (impl pending);
  frontier → `.6b`.** Design-detail leaf, no source change (a `DEVELOPMENT_NOTES.md`
  entry + the tree split). Grounded decision `0014` in the real emitter (the
...
**Active frontier: `STRUCTURED-EMISSION-EXPANSION.6b`** (implement the third
structured surface — the combinational `task automatic` emit-projection,
decision `0014`, grounded by the `.6a` design-detail). `.6` is now an `active`
container split into `.6a` (design-detail — **done `2026-06-16`**) + `.6b` (impl
— `pending`). `.5` (decision `0014`) is done. The first (combinational
...
`generate`, `interface`/`modport`, richer tasks) are `.7+`, each its own
decision. Nothing retired.

| Order | Leaf | Status | Why next |
| --- | --- | --- | --- |
| 1 | `STRUCTURED-EMISSION-EXPANSION.6b` | `pending` | Implement the `.6a` design: `Config::task_emit_prob` + `Module.task_emit_gates` + new `src/ir/task_emit.rs` (`annotate_task_emit_gates`, the function-emit candidate predicate **plus** exclusion of the sibling projections) + the two guarded gen-time call-site rolls (after generate_loop) + the `to_sv_with_modules` `task_emit_gate` accessor + the `task automatic` decl (body via the reused `render_gate_function_body`) + the `logic <wire>__tv` var + the `always_comb <wire>__t(...)` call + the assign-RHS swap + lib proofs + the `num_emitted_combinational_tasks` metric (schema `1.9→1.10`) + the repo-owned `tool_matrix --task-emit-gate` (`ScenarioSet::TaskEmitSweep` + `ModuleReport.emitted_combinational_task` + `saw_combinational_task_emit`) + book/USER_GUIDE/KM. Default-off / DUT byte-identical (snapshots untouched). Pre-split `.6b.1` (live surface) + `.6b.2` (metric + gate) + `.6b.3` (docs closeout) per the `.4b` precedent. |
