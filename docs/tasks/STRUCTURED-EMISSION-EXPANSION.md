...
- Created: `2026-06-15`
- Last updated: `2026-06-22` (**`.12b.1` landed — the multi-output combinational
...
- ID: `STRUCTURED-EMISSION-EXPANSION.12b.2`
  Status: `pending`
  Goal: `The metric + schema bump + repo-owned gate: Metrics::num_emitted_multi_output_tasks (= m.multi_output_task_groups.len(), #[serde(default)]) surfaced in introspection module_metrics => SCHEMA_VERSION 1.13 -> 1.14 (bump all current-output schema refs: assertions + schema doc + README + USER_GUIDE + book example JSONs + CODEBASE_ANALYSIS envelope; historical attributions intact) + a lib proof; the tool_matrix --multi-output-task-gate (ScenarioSet::MultiOutputTaskSweep + build_multi_output_task_sweep_scenarios/focus config [comb-only multi_output_task_emit_prob=1.0 x 3 strategies, terminal_reuse_prob high + shallow] + ModuleReport.emitted_multi_output_task [__mt( detection] + saw_multi_output_task_emit + MatrixReport + early-return gap arm + proofs + fixtures) + README/USER_GUIDE/CODEBASE gate entries. Banked clean (Verilator + both Yosys + Icarus). Default-off / DUT byte-identical.`
  Result: `Pending.`
...
**Frontier: `STRUCTURED-EMISSION-EXPANSION.12b.2`** (metric + introspection schema
`1.13 → 1.14` + the repo-owned `tool_matrix --multi-output-task-gate` for the
**sixth** structured surface — the multi-output combinational `task automatic`,
decision `0025`; `.12a` design-detail + `.12b.1` **live surface** done). **FIVE**
structured surfaces are
