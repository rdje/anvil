...
- Created: `2026-06-15`
- Last updated: `2026-06-17` (**`.10b.1` landed — the multi-gate-cone `function
...
**Frontier: `STRUCTURED-EMISSION-EXPANSION.10b.2`** (the metric + repo-owned gate
for the fifth surface). The tree stays `active`. **Four** structured surfaces are
...
the **FIFTH** surface is shipping: the **multi-gate-cone `function automatic`**
(`.9` design + `.10a` design-detail + `.10b.1` **live surface** all done, decision
`0016`; `.10b.2` metric+gate + `.10b.3` user docs pending). Future surfaces —
...
| --- | --- | --- | --- |
| 1 | `STRUCTURED-EMISSION-EXPANSION.10b.2` | `pending` | Metric + repo-owned gate: `Metrics::num_emitted_cone_functions` (= `m.cone_function_gates.len()`) in introspection `module_metrics` ⇒ schema MINOR bump `1.10 → 1.11` (bump all current-output schema refs; historical attributions intact) + a lib proof; `tool_matrix --cone-function-gate` (`ScenarioSet::ConeFunctionSweep` + `build_cone_function_sweep_scenarios` [comb-only `cone_function_emit_prob=1.0` × 3 strategies] + `ModuleReport.emitted_cone_function` [`__cf(` detection] + `saw_cone_function_emit` + early-return gap arm + proofs) + README/USER_GUIDE/CODEBASE_ANALYSIS gate entries. Banked clean (Verilator + both Yosys + Icarus). Default-off / DUT byte-identical. Templated on `--task-emit-gate` / `--function-emit-gate`. |
| 2 | `STRUCTURED-EMISSION-EXPANSION.10b.3` | `pending` | User-facing closeout (docs-only): a `## The fifth surface: a multi-gate-cone function automatic` section in `book/src/structured-emission.md` (byte-verified before/after) + the `cone_function_emit_prob` knob entry in `book/src/knobs.md` / `USER_GUIDE.md` / README "Current CLI truth" + a KM how-to card. `mdbook build` + KM gen/check + `check_memory_architecture` + `book_examples` 3/3. Closes `.10b.3` / `.10b` / `.10` — the fifth structured surface delivered end-to-end. |
