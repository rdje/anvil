...
- Created: `2026-06-15`
- Last updated: `2026-06-23` (**`.17b.2b` landed — the eighth-surface repo-owned
...
**Current frontier → `STRUCTURED-EMISSION-EXPANSION.17b.3` (the eighth surface's user
docs: book/knobs/USER_GUIDE/README/KM for the `case_mux_if_emit_prob` knob).** The eighth
surface's **live source change** (`.17b.1`), its **metric + introspection schema bump**
(`.17b.2a`), and its **repo-owned downstream gate** (`.17b.2b`) have all landed. The
`num_emitted_case_mux_if_chains` metric (`= m.case_mux_if_gates.len()`) is surfaced at
introspection schema **`1.16`** (live `--introspect` confirms `0` default / `53` forced on
seed 2), and the repo-owned `tool_matrix --case-mux-if-gate` (`ScenarioSet::CaseMuxIfSweep`
+ a `case_mux_prob`-biased `case_mux_if_focus_config` × 3 strategies + the **metric-keyed**
`emitted_case_mux_if`/`saw_case_mux_if_emit`) is **banked clean**
(`/tmp/anvil-case-mux-if-gate-r1`: 3 scenarios / 12 modules / 12 emitting a chain / 83
chains / `coverage_gaps = []` / 12/0 Verilator + both Yosys + Icarus). The procedural
`always_comb` `if`/`else if` **priority chain** projection of a dynamic-selector `CaseMux`
gate is live behind the opt-in `case_mux_if_emit_prob` knob — new
`src/ir/case_mux_if_emit.rs` + `Module.case_mux_if_gates` + the `--case-mux-if-emit-prob`
flag + two `gen/mod.rs` rolls (after `mux_if`) + the `emit/sv.rs` structured-case body
branch (`case…endcase` → `if`/`else if`, the parallel `case` suppressed for a marked gate)
+ 9 lib proofs. Default-off / DUT byte-identical (`cargo test --lib` 616→625→626, `--bin
tool_matrix` 89→94, snapshots 6/6). The surface is picked (`.16`, decision
[`0028`](../decisions/0028-structured-emission-eighth-surface-case-mux-priority-chain.md))
+ designed (`.17a`) + live (`.17b.1`) + metered @ schema `1.16` (`.17b.2a`) + gated
(`.17b.2b`). **`.17b.3`** (user docs) is the remaining frontier; on its completion `.17b`
/ `.17` close and the EIGHTH surface is delivered end-to-end.
