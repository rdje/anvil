...
- Created: `2026-06-15`
- Last updated: `2026-06-17` (**`.10b.2` landed — the cone-function metric
...
**Frontier: `STRUCTURED-EMISSION-EXPANSION.10b.3`** (the user-facing docs
closeout for the fifth surface). The tree stays `active`. **Four** structured
...
(`.7`+`.8`, decision `0015`) — and the **FIFTH** surface (the **multi-gate-cone
`function automatic`**, decision `0016`) is shipping: `.9` design + `.10a`
design-detail + `.10b.1` **live surface** + `.10b.2` **metric + repo-owned
`--cone-function-gate`** all done (banked clean `/tmp/anvil-cone-function-gate-r1`,
introspection schema `1.11`); only `.10b.3` (user docs) remains. Future surfaces —
multi-output tasks, nested/multi-level `generate`, `interface` / `modport` — are
`.11+`, each its own decision when picked (none retired).

| Order | Leaf | Status | Why next |
| --- | --- | --- | --- |
| 1 | `STRUCTURED-EMISSION-EXPANSION.10b.3` | `pending` | User-facing closeout (docs-only): a `## The fifth surface: a multi-gate-cone function automatic` section in `book/src/structured-emission.md` (byte-verified before/after) + the `cone_function_emit_prob` knob entry in `book/src/knobs.md` / `USER_GUIDE.md` / README "Current CLI truth" + a KM how-to card. `mdbook build` + KM gen/check + `check_memory_architecture` + `book_examples` 3/3. Closes `.10b.3` / `.10b` / `.10` — the fifth structured surface delivered end-to-end. |
