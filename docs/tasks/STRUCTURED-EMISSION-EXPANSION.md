...
- Created: `2026-06-15`
- Last updated: `2026-06-22` (**`.17a` landed — the eighth-surface impl design-detail
...
**Current frontier → `STRUCTURED-EMISSION-EXPANSION.17b.1` (the live source change for the
EIGHTH surface).** The eighth surface is **picked + designed** (`.16` done, decision
[`0028`](../decisions/0028-structured-emission-eighth-surface-case-mux-priority-chain.md))
and its **impl design-detail is grounded** (`.17a` done — a `DEVELOPMENT_NOTES.md` entry
pinning all seven impl points against the real `mux_if_emit.rs` template +
`render_static_structured_gate`'s `CaseMux` static-collapse test + the structured-case
emitter loop + the roll chain + the config/types touch points; `.17` pre-split `.17a`
done + `.17b` impl). The surface = a default-off, valid-by-construction procedural
`always_comb` `if`/`else if` **priority chain** emit-projection of a dynamic-selector
`CaseMux` gate (the N-way selection it renders today as a parallel `case (sel) … default`
statement) — the recorded decision-`0027` follow-up, empirically probed clean this
session (Verilator `-Wall` 2012/2017/2023 + both Yosys modes + Icarus + iverilog sim-equiv
over 20000 vectors + an exhaustive selector sweep). `.17b` (impl, pre-split `.17b.1` live
[`case_mux_if_emit_prob` knob + `Module.case_mux_if_gates` + new
`src/ir/case_mux_if_emit.rs` + two `gen/mod.rs` rolls after `mux_if` + the `emit/sv.rs`
structured-case `case`→`if`/`else if` body branch + ~8 lib proofs] / `.17b.2` metric @
schema `1.15 → 1.16` + `tool_matrix --case-mux-if-gate` / `saw_case_mux_if_emit`
[metric-keyed] / `.17b.3` docs) is the next frontier.
