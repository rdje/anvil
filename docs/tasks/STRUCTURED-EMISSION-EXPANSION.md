...
- Created: `2026-06-15`
- Last updated: `2026-06-22` (**`.12b.2b` landed — the repo-owned `tool_matrix
...
- ID: `STRUCTURED-EMISSION-EXPANSION.12`
  ...
  Result: `In progress. .12a (design-detail) done — every .12b impl choice pinned in DEVELOPMENT_NOTES (the candidate predicate, the Module carrier, the selection + leader-roll, the in_fanin soundness DFS, the deduplicated-formal rendering reusing the cone primitives, the ordering/exclusion, the knob/metric/gate split). .12b (impl) pending, pre-split .12b.1 (live surface) / .12b.2 (metric + schema 1.13->1.14 + tool_matrix gate) / .12b.3 (docs).`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.12b`
  ...
  Result: `In progress. .12b.1 (live surface) done — the knob + CLI flag + Module field + src/ir/multi_output_task_emit.rs + two rolls + cone_function exclusion + emitter rendering + 11 lib proofs + the forced downstream sweep all landed, default-off / DUT byte-identical. .12b.2 (metric + schema 1.13->1.14 + tool_matrix gate) and .12b.3 (docs) pending.`
...
- ID: `STRUCTURED-EMISSION-EXPANSION.12b.3`
  Status: `pending`
  Goal: `User-facing closeout (docs-only): a ## The sixth surface: a multi-output task automatic section in book/src/structured-emission.md (byte-verified before/after) + the multi_output_task_emit_prob knob entry in book/src/knobs.md / USER_GUIDE.md / README "Current CLI truth" + a KM how-to card. mdbook build + check_knowledge_map + check_memory_architecture + book_examples green. Docs-only / DUT byte-identical.`
  Result: `Pending.`

## Current Frontier

**Frontier: `STRUCTURED-EMISSION-EXPANSION.12b.3`** (the user-facing docs closeout —
book/USER_GUIDE/README/KM + the deferred book schema-JSON refresh — for the
**sixth** structured surface, the multi-output combinational `task automatic`,
decision `0025`; `.12a` design-detail + `.12b.1` live surface + `.12b.2a`
metric/schema + `.12b.2b` **repo-owned gate** done, banked clean
`/tmp/anvil-multi-output-task-gate-r1`). **FIVE** structured surfaces are
