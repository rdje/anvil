...
- Created: `2026-06-15`
- Last updated: `2026-06-22` (**`.13a` landed — the design-detail for the first
  *deepening* of the sixth surface: widen the multi-output `task automatic` group
  from the first-cut pair (`k=2`) to a bounded `k>2` (the recorded `.13` follow-up
  of decision `0025`). Docs-only / no source: a `DEVELOPMENT_NOTES.md` design-detail
  entry pinning the greedy group-extension algorithm (connected co-support +
  generalized mutual fan-in independence + a bounded
  `MAX_MULTI_OUTPUT_TASK_GROUP_MEMBERS` const cap), the **no-new-knob / no-new-metric /
  no-schema-bump** decision, and the byte-identical/RNG argument — the emitter is
  already **k-agnostic** (verified: `multi_output_task_params` /
  `render_multi_output_task_decl` / `render_multi_output_task_call` iterate
  `members` of any length), so the only `.13b` source change is
  `annotate_multi_output_task_groups`. Frontier → `.13b` (live impl).** Prior:
...
**Frontier: `STRUCTURED-EMISSION-EXPANSION.13b`** (the live impl of the first
*deepening* of the sixth surface — widen the multi-output `task automatic` group
from the pair (`k=2`) to a bounded `k>2`). `.13a` (design-detail) is **done**: the
emitter is already k-agnostic, so the only `.13b` source change is widening
`annotate_multi_output_task_groups` to greedily collect all eligible partners
(connected co-support + generalized mutual fan-in independence) up to a const cap —
no new knob / metric / schema, default-off / DUT byte-identical. After `.13b` the
frontier moves to `.13c` (user docs). Further future surfaces (nested/multi-level
`generate`, `interface`/`modport`) remain `.14+`.
