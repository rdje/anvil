...
- Created: `2026-06-15`
- Last updated: `2026-06-22` (**`.17b.1` landed — the LIVE eighth structured surface (the
...
- ID: `STRUCTURED-EMISSION-EXPANSION.17b`
  ...
  Children: `STRUCTURED-EMISSION-EXPANSION.17b.1`, `STRUCTURED-EMISSION-EXPANSION.17b.2`, `STRUCTURED-EMISSION-EXPANSION.17b.3`
...
**Current frontier → `STRUCTURED-EMISSION-EXPANSION.17b.2` (the metric + repo-owned gate
for the EIGHTH surface).** The eighth surface's **live source change has landed** (`.17b.1`
done): the procedural `always_comb` `if`/`else if` **priority chain** projection of a
...
[`0028`](../decisions/0028-structured-emission-eighth-surface-case-mux-priority-chain.md))
+ designed (`.17a`) + live (`.17b.1`). **`.17b.2`** (the `num_emitted_case_mux_if_chains`
metric @ introspection schema `1.15 → 1.16` + the repo-owned `tool_matrix
--case-mux-if-gate` / `saw_case_mux_if_emit` [metric-keyed, no new text token] + a
`case_mux_prob`-biased focus config) is the next frontier, then `.17b.3` (user docs).
