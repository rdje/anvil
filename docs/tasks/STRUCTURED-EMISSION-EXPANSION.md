...
- Created: `2026-06-15`
- Last updated: `2026-06-22` (**`.13b` landed — the LIVE k>2 widening of the sixth
...
**Frontier: `STRUCTURED-EMISSION-EXPANSION.13c`** (the user-facing docs closeout of
the first *deepening* of the sixth surface — note the now-`k>2` multi-output
`task automatic` groups in `book/src/structured-emission.md` + the knob entries +
the KM card). `.13a` (design-detail) and `.13b` (the live impl) are **done**: the
widening landed entirely in `src/ir/multi_output_task_emit.rs`
(`annotate_multi_output_task_groups` now greedily admits all eligible partners —
connected co-support + generalized mutual fan-in independence — up to
`MAX_MULTI_OUTPUT_TASK_GROUP_MEMBERS = 8`), no new knob / metric / schema /
emitter change, default-off / DUT byte-identical (snapshots 6/6); the gate re-bank
is `coverage_gaps = []` / `12/0` all tools with a k=3 group present, and a forced
sweep proves a k=3 task Verilator `-Wall` Δ=0 vs OFF + sim-equiv over 20000
vectors. After `.13c` the lane returns to no current frontier. Further future
surfaces (nested/multi-level `generate`, `interface`/`modport`) remain `.14+`.
