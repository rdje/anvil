...
- Created: `2026-06-15`
- Last updated: `2026-06-22` (**`.16` landed — picked the EIGHTH structured surface
...
**Current frontier → `STRUCTURED-EMISSION-EXPANSION.17` (impl of the EIGHTH surface).**
The eighth surface is now **picked and designed** (`.16` done, decision
[`0028`](../decisions/0028-structured-emission-eighth-surface-case-mux-priority-chain.md)):
a default-off, valid-by-construction procedural `always_comb` `if`/`else if` **priority
chain** emit-projection of a `CaseMux` gate (the N-way selection it renders today as a
parallel `case (sel) … default` statement) — the recorded decision-`0027` follow-up,
empirically probed clean this session (Verilator `-Wall` 2012/2017/2023 + both Yosys
modes + Icarus + iverilog sim-equiv over 20000 vectors + an exhaustive selector sweep).
`.17` (impl, pre-split `.17a` design-detail + `.17b` impl: `case_mux_if_emit_prob` knob +
the gen-time annotation + the emitter `if`/`else if` body branch + the
`num_emitted_case_mux_if_chains` metric @ schema `1.15 → 1.16` + `tool_matrix
--case-mux-if-gate` / `saw_case_mux_if_emit` + book/USER_GUIDE/KM) is the next frontier.
