...
- Created: `2026-06-15`
- Last updated: `2026-06-23` (**`.19b.1` landed — the LIVE ninth structured surface (the
...
- ID: `STRUCTURED-EMISSION-EXPANSION.19b.2`
  Status: `pending`
  Goal: `The metric + repo-owned gate: Metrics::num_emitted_casez_mux_if_chains (= m.casez_mux_if_gates.len(), #[serde(default)]) surfaced in introspection module_metrics ⇒ schema 1.16 → 1.17 (SCHEMA_VERSION + all current-output assertions/doc/README/USER_GUIDE/CODEBASE refs; historical attributions intact; book JSONs deferred to .19b.3) + a lib proof; AND a repo-owned tool_matrix --casez-mux-if-gate (ScenarioSet::CasezMuxIfSweep + a casez_mux_if_focus_config forcing casez_mux_if_emit_prob=1.0 over comb-only DUTs × 3 strategies, with casez_mux_prob biased high and BOTH comb_mux_prob and case_mux_prob zeroed — both roll before casez_mux in cone.rs) + METRIC-KEYED ModuleReport.emitted_casez_mux_if (= num_emitted_casez_mux_if_chains > 0) + CoverageSummary.saw_casez_mux_if_emit + merge + gap arm + proofs + fixtures, banked clean. Default-off / DUT byte-identical.`
  Acceptance: `cargo test --lib green incl. the metric proof + the schema 1.17 assertions; cargo test --bin tool_matrix green incl. the new gate proofs; snapshots 6/6 byte-identical; banked clean tool_matrix --casez-mux-if-gate (coverage_gaps = [] / saw_casez_mux_if_emit / Verilator + both Yosys + Icarus). Committed through COMMIT.md per leaf.`
...
**Current frontier → `STRUCTURED-EMISSION-EXPANSION.19b.2` (metric + repo-owned gate).**
**The NINTH structured surface is LIVE (`.18` + `.19a` + `.19b.1` done).** `.19b.1` (done)
