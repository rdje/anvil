...
- Created: `2026-06-15`
- Last updated: `2026-06-16` (**`.4b.1` landed — the `generate for` loop live
...
- ID: `STRUCTURED-EMISSION-EXPANSION.4b.2`
  Status: `pending`
  Goal: `The repo-owned downstream gate + metric for the generate for loop surface: a num_emitted_generate_loops metric (= m.generate_loop_gates.len()) surfaced in introspection ⇒ schema MINOR bump 1.8→1.9 (the .2b.2a precedent) + a saw_generate_loop_emit coverage fact + a tool_matrix --generate-loop-gate flag + ScenarioSet::GenerateLoopSweep + build_generate_loop_sweep_scenarios (a replication-rich comb-only DUT forcing generate_loop_emit_prob=1.0 across the three construction strategies) + ModuleReport.emitted_generate_loop SV-text detection + early-return gap enforcement, proving Verilator + both Yosys modes + Icarus accept the emitted loops warning-clean. Bank a clean report. Default-off / DUT byte-identical. Template: --function-emit-gate. Pre-split into .4b.2a (metric + schema bump) + .4b.2b (the tool_matrix gate) if the slice is too broad (the .2b.2 precedent).`
...
**Active frontier: `STRUCTURED-EMISSION-EXPANSION.4b.2`** (the repo-owned
downstream gate + the `num_emitted_generate_loops` metric for the `generate for`
loop surface). `.4b` is now an `active` container split into `.4b.1` (live
surface — **done `2026-06-16`**) + `.4b.2` (gate + metric — `pending`) + `.4b.3`
(docs closeout — `pending`). `.4` and `.4a` (design-detail) are done. `.3`
(decision `0013`) is done. The first structured surface (the combinational
...
| --- | --- | --- | --- |
| 1 | `STRUCTURED-EMISSION-EXPANSION.4b.2` | `pending` | The repo-owned downstream gate + metric: a `num_emitted_generate_loops` metric (`= m.generate_loop_gates.len()`) surfaced in introspection ⇒ schema MINOR bump `1.8→1.9` (the `.2b.2a` precedent) + a `saw_generate_loop_emit` coverage fact + a `tool_matrix --generate-loop-gate` flag + `ScenarioSet::GenerateLoopSweep` + `build_generate_loop_sweep_scenarios` (a replication-rich comb-only DUT forcing `generate_loop_emit_prob=1.0` across the three construction strategies) + `ModuleReport.emitted_generate_loop` SV-text detection + early-return gap enforcement, banked clean across Verilator + both Yosys + Icarus. Template: `--function-emit-gate`. Forced-sweep evidence already banked at `/tmp/anvil-gl-r1/`. Pre-split `.4b.2a` (metric + schema) + `.4b.2b` (the gate) if too broad (the `.2b.2` precedent). Default-off / DUT byte-identical. |
