...
- Created: `2026-06-15`
- Last updated: `2026-06-17` (**`.10a` landed — the multi-gate-cone `function
...
**Frontier: `STRUCTURED-EMISSION-EXPANSION.10b`** (the impl of the fifth surface).
The tree stays `active`. **Four** structured surfaces are delivered end-to-end —
the combinational `function automatic` (`.1`+`.2`), the `generate for` loop
(`.3`+`.4`), the combinational `task automatic` (`.5`+`.6`), and the wider-lane
`generate for` part-select (`.7`+`.8`, decision `0015`) — and the **FIFTH** surface
is in progress: the **multi-gate-cone `function automatic`** (`.9` design + `.10a`
design-detail both done, decision `0016`; `.10b` impl pending). Future surfaces —
...
| --- | --- | --- | --- |
| 1 | `STRUCTURED-EMISSION-EXPANSION.10b` | `pending` | Impl of the fifth surface per the `.10a` design: new `src/ir/cone_function_emit.rs` (`annotate_cone_function_gates` — root selection + the post-order cone-walk + single-use absorption + mutual exclusion + interior reservation) + `Config::cone_function_emit_prob` + `Module.cone_function_gates: BTreeMap<NodeId, Vec<NodeId>>` + the `src/emit/sv.rs` `render_cone_function_decl` (support-leaf params + topo-ordered function-local temps + return root) + the call + the absorbed-interior inline-suppression + lib proofs + the `num_emitted_cone_functions` metric (schema `1.10 → 1.11`) + the `--cone-function-gate` / `saw_cone_function_emit` proof (Verilator + both Yosys + Icarus) + book/USER_GUIDE/KM. Default-off / DUT byte-identical (snapshots untouched; the single-gate `function_emit` surface untouched). Pre-split `.10b.1` live / `.10b.2` metric+gate / `.10b.3` docs if warranted. |
