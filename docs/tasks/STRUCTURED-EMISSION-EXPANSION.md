...
- Created: `2026-06-15`
- Last updated: `2026-06-22` (**`.15a` landed — the seventh-surface impl
...
**Active frontier: `.15b.1` — the live seventh-surface source change** (the
procedural `always_comb` `if`/`else` emit-projection of a `Mux` gate, decision
[`0027`](../decisions/0027-structured-emission-seventh-surface-procedural-if-else.md)).
`.14` (design / decision `0027`) and `.15a` (impl design-detail — the
`DEVELOPMENT_NOTES.md` entry grounding the seven impl points in the real
`task_emit.rs` / `sv.rs` / `gen/mod.rs` / `config.rs` / `types.rs`) are both `done`,
no source change. The 2:1 `Mux` (rendered today as the `(sel)?(a):(b)` ternary) is
projected into a procedural conditional that writes a per-gate `<wire>__cv` output
var, the existing net driven by a passthrough `assign` — the **first
procedural-`if`/`else` construct** in the lane (none of the six delivered surfaces
emits a procedural conditional). The empirical probe is clean: Verilator `-Wall`
2012/2017/2023 + both Yosys modes (no warnings) + Icarus, and iverilog `vvp`
sim-equiv to the inline ternary over 20000 vectors.

`.15b.1` (next) is the **first source change**: `Config::mux_if_emit_prob` (default
`0.0`) + `--mux-if-emit-prob` flag + `Module.mux_if_gates: BTreeSet<NodeId>` + new
`src/ir/mux_if_emit.rs` (`annotate_mux_if_gates`, the `GateOp::Mux` candidate
predicate excluding all six sibling marks) + `src/ir/mod.rs` registration + the two
guarded `gen/mod.rs` rolls (run **last**, after `cone_function`) + the `src/emit/sv.rs`
procedural-block section (`logic [w-1:0] <name>__cv;` + the `always_comb if/else`,
operand refs via the existing `node_ref`) + the gate-assign-loop `<name>__cv`
passthrough branch + the lib proofs. No metric/schema bump (the `.15b.2` metric
bumps `1.14 → 1.15`; the knob rides the version — the `.12b.1` precedent).
Default-off / DUT byte-identical (snapshots 6/6); a forced `mux_if_emit_prob = 1.0`
sweep proving the `always_comb if/else` fires + Verilator `-Wall` Δ=0 vs OFF + both
Yosys + Icarus + iverilog sim-equiv to the inline ternary. Then `.15b.2` (metric +
`--mux-if-gate`) + `.15b.3` (user docs). The N-way `CaseMux` → `if`/`else if`
