...
- Created: `2026-06-15`
- Last updated: `2026-06-22` (**`.12a` landed — the multi-output task impl
...
- ID: `STRUCTURED-EMISSION-EXPANSION.12b`
  ...
  Result: `Pending. Pre-split into .12b.1 (the live surface: knob + CLI flag + Module field + src/ir/multi_output_task_emit.rs + two rolls + cone_function exclusion + emitter rendering + lib proofs + forced downstream sweep; no metric/schema bump — the knob rides the version) + .12b.2 (the num_emitted_multi_output_tasks metric + introspection schema 1.13->1.14 + the repo-owned tool_matrix --multi-output-task-gate banked clean) + .12b.3 (book/USER_GUIDE/README/KM user-facing closeout).`

## Current Frontier

**Frontier: `STRUCTURED-EMISSION-EXPANSION.12b`** (impl of the **sixth** structured
surface — the multi-output combinational `task automatic`, decision `0025`; `.12a`
design-detail **done**, pre-split `.12b.1` live / `.12b.2` metric+gate / `.12b.3`
docs). **FIVE** structured surfaces are
