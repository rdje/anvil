...
| --- | --- | --- |
| `.3` | `dbc8d62` — `NEGATIVE-CONTROL-HARNESS.3 — the mutation refuses to no-op (tree CLOSED)` | The R2 helper + one `TOOLBOX.md` row. Controlled on itself through a seam; two first-draft defects recorded. Shell instrument only ⇒ DUT byte-identical. **Tree closed.** |
