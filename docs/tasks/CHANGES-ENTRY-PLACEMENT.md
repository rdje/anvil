...
- Created: `2026-07-31`
- Last updated: `2026-08-01` (`.4` **done** — the mechanism question answered on measurement: **ship it**, recorded as decision [`0045`](../decisions/0045-changes-entry-placement-authoring-path-check.md). 3 fires / 766 commits, **0 false alarms**, and it found a **third** offender nobody knew about. Frontier `.5` — implement it)
...
- ID: `CHANGES-ENTRY-PLACEMENT.4`
  ...
  Commit: `CHANGES-ENTRY-PLACEMENT.4`
...
| 5 | `CHANGES-ENTRY-PLACEMENT.5` | `pending` | **Next.** Implement `0045` as a registered doctrine: the script, a `DOCTRINES` registry row, and the `ENUMERATION-PARITY`-gated doctrine-id list at **every** fenced site (`README.md`, `book/src/architecture.md`, `docs/knowledge/doctrine-enforcement.md`, `CODEBASE_ANALYSIS.md`) — that list is gated, so the taxonomy change is priced immediately. `0045`'s three negative controls are to be **reproduced, not re-derived**, and the check must **not** be scope-aware: both original offenders were docs-only, which is exactly the exemption that let them through. |
## Decisions
...
| --- | --- | --- |
| `CHANGES-ENTRY-PLACEMENT.4` | `CHANGES-ENTRY-PLACEMENT.4 — the mechanism question, answered on measurement (decision 0045)` | Docs-only. **Ship a mechanism**, keyed on the **authoring path**. 3 fires / 766 commits, 0 false alarms, negative-controlled both ways; found a **third** offender and explained the existing oracle's 0-of-3 blindness. Rules against back-filling the 181 and against ever requiring a hash. Registers `.5` to implement it. |
