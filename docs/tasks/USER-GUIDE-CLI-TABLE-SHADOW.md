...
- Created: `2026-08-01`
- Last updated: `2026-08-01` (`.4` **done** — the book's copy deleted by `0033` rung R1; the
  registered "11" was **14**, and 9 of the section's 12 "not exposed via CLI" claims were
  **false**; `.5` registered for the `tool_matrix` block; frontier `.5`)
...
- ID: `USER-GUIDE-CLI-TABLE-SHADOW`
  ...
  Children: `.1` (audit + register), `.2` (decide the table's contract, then repair), `.3` (the mechanism question), `.4` (the book's second copy of the same shadow), `.5` (the book's `tool_matrix` block — a different clap registry)
...
| 5 | `USER-GUIDE-CLI-TABLE-SHADOW.5` | `pending` | **Next.** The third site, measured by `.4`: the §*`tool_matrix` auxiliary binary* block names **22 of 37**. Same defect class, **different authoritative set** (`src/bin/tool_matrix.rs`'s clap `Cli`), so it needs its own extractor and its own contract decision — folding it into `.4` would have broken the one-leaf-per-commit rule and reused an extractor that does not transfer. |
...
- Should the **`tool_matrix`** block be deleted (R1) or gated as a sixth `ENUMERATION-PARITY`
  pair? **Open — it is `.5`'s decision and must be made before a row is written**, exactly as
  `.2` did. The two are not symmetric: `USER_GUIDE.md` has a "Tool matrix sweeps" section that
  could absorb it (favouring R1), but `tool_matrix`'s gate flags are the surface `0033`'s §4
  calls the repo's only **false-green** site, which argues for holding them mechanically.
...
| `USER-GUIDE-CLI-TABLE-SHADOW.4` | `USER-GUIDE-CLI-TABLE-SHADOW.4 — delete the book's copy; 9 of its 12 exemptions were false` | Book-only (`knobs.md`, `architecture.md`). Net −92 lines. Registers `.5` (the `tool_matrix` block, 22 of 37). |
