...
| `USER-GUIDE-CLI-TABLE-SHADOW.7` | `USER-GUIDE-CLI-TABLE-SHADOW.7 — gate the anvil hunt flags; close the tree` | Landed `4f36cde`. Docs + one enforcement script. No `src/` change (the controls' edits to `src/main.rs` were reverted and verified byte-identical to HEAD). Closes the tree: all three clap registries gated. |
