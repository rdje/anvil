...
- Created: `2026-05-16`
- Last updated: `2026-05-20` (**`.2c.2b.1` semantic-alignment fix landed** — `width_expr` SV text now uses the standard SV non-negative-modulo idiom `((x % 8) + 8) % 8 + 1` matching the oracle's `rem_euclid` semantics; new regression proof `width_expr_uses_sv_non_negative_modulo_idiom_and_agrees_for_negative_last_values` pins it (seed 7 P4=-1 → bits=8); full `cargo test` green incl. all 8 lib microdesign tests + 15 portable parity proofs + 1 `#[ignore]`; frontier → `.2c.2b.2`)
...
| --- | --- | --- | --- |
| 1 | `PHASE-7-ORACLE-MICRODESIGN.2c.2b.2` | `pending` (gate-blocked, real-tool re-run) | **`.2c.2b.1` done** — `width_expr`'s SV text changed to the standard SV non-negative-modulo idiom `(((x % 8) + 8) % 8 + 1)` so a SV elaborator evaluates it to the same value the oracle's `rem_euclid` already produced (oracle ≡ SV for every `last.value`). New lib regression proof `width_expr_uses_sv_non_negative_modulo_idiom_and_agrees_for_negative_last_values` pins the bug-fix on the very counterexample seed (`.2a`'s reproducible seed 7 `P4 = -1` → `widths.sig.bits = 8`) + a non-negative collapse case (seed 0 `P4 = 365` → matches the old formula) + a cross-seed structural pin (every reproducibility-set seed's W_SIG line uses the new idiom). Full `cargo test` green: lib 229 (was 228 + 1 regression), `microdesign` 8/8, `tests/microdesign_parity` 15+1 (every `.2c.1`/`.2c.2a` proof still green), `tests/pipeline` 121, `tests/snapshots` 6, bin 5+29+3, doc 0. Portable `cargo test` stays green tool-less. The `#[ignore]` parity gate is now unblocked. `.2c.2b.2` runs `cargo test -- --ignored parity_against_real_yosys_write_json` against real yosys — the seed-7 counterexample is now structurally impossible to surface — verifies exact-agreement on the yosys-supported categories across the full corpus, banks the verified-clean artifact, records **ROADMAP Phase 7 → done** with the explicit "yosys-supported categories" scope caveat (book/README/CODEBASE reconcile). r87 no-aspirational-claims. |
