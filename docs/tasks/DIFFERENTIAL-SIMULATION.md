...
- Created: `2026-05-14`
- Last updated: `2026-05-24` (**`.3b.1` done — pure refactor extracting harness helpers to `src/diff_sim/mod.rs`**): new `src/diff_sim/mod.rs` (~325 lines) carries `pub fn baked_input_vectors` / `mask_to_width` / `fmt_sv_hex` / `is_sequential` / `emit_testbench` / `run_iverilog` / `run_verilator` / `normalize_trace` / `tools_present` (plus private `emit_display_outputs` and 3 unit tests for the pure-input helpers — moved from `tests/diff_sim.rs` to keep them under `cargo test --lib`); `src/lib.rs` declares `pub mod diff_sim;` with the full-factorization-doctrine docstring. `tests/diff_sim.rs` reduced to ~175 lines: `use anvil::diff_sim::{…}` import + the 2 `#[ignore]`-gated focused proofs + 2 cargo-portable smoke proofs that round-trip through a real generated `Module`. Full `cargo test` clean (247 lib + 121 pipeline + 6 snapshots; +3 lib tests from the migrated pure-input proofs); real-tool gate clean against locally-installed iverilog 13.0 + verilator 5.046 (`differential_simulation_combinational` + `differential_simulation_sequential` both ok). `cargo fmt`/clippy(-D warnings)/check all clean. `.3b` further split per the proven Phase-7 discipline: `.3b.1` (extraction, done) + `.3b.2` (`tool_matrix --diff-sim` wiring + per-axis subset selector + `DiffSimReport` + `saw_design_with_cross_simulator_agreement` coverage fact + cargo-portable proofs + real-tool gate, pending). Frontier → `.3b.2`.
...
- ID: `DIFFERENTIAL-SIMULATION.3`
  ...
  Children: `DIFFERENTIAL-SIMULATION.3a`, `DIFFERENTIAL-SIMULATION.3b`
...
- ID: `DIFFERENTIAL-SIMULATION.3b`
  ...
  Children: `DIFFERENTIAL-SIMULATION.3b.1` (done), `DIFFERENTIAL-SIMULATION.3b.2` (pending)
...
| --- | --- | --- | --- |
| 1 | `DIFFERENTIAL-SIMULATION.3b.2` | `pending` (wire `tool_matrix --diff-sim`) | **`.3b.1` done — pure refactor extracting helpers to `src/diff_sim/mod.rs` (`2026-05-24`)**: new `src/diff_sim/mod.rs` (~325 lines) carries the `pub` harness helpers (`baked_input_vectors` / `mask_to_width` / `fmt_sv_hex` / `is_sequential` / `emit_testbench` / `run_iverilog` / `run_verilator` / `normalize_trace` / `tools_present`) + private `emit_display_outputs` + 3 lib unit tests for the pure-input helpers (migrated from `tests/diff_sim.rs`). `src/lib.rs` declares `pub mod diff_sim` with the full-factorization-doctrine docstring. `tests/diff_sim.rs` reduced from ~556 to ~175 lines: `use anvil::diff_sim::{…}` + the 2 `#[ignore]`-gated focused proofs + 2 cargo-portable smoke proofs that round-trip through a real generated `Module`. `cargo fmt`/clippy(-D warnings)/check all clean; `cargo test --lib` 247 passed (+3 from migration); `cargo test --tests` 121 + 6 + 2 clean. Real-tool gate clean against extracted helpers: `differential_simulation_combinational` + `differential_simulation_sequential` both ok against iverilog 13.0 + verilator 5.046. **Pure refactor — byte-identical harness behavior verified by the real-tool gate.** `.3b` further split per Phase-7 discipline. Frontier → `.3b.2` (`tool_matrix --diff-sim` CLI flag + per-axis subset selector + `DiffSimReport` + coverage fact + per-module pipeline). |
