...
- Created: `2026-05-14`
- Last updated: `2026-05-24` (**`.3` split + `.3a` design landed**, docs-only): `DEVELOPMENT_NOTES.md` "Tool-matrix `--diff-sim` wiring + representative-subset selector + coverage fact design" entry landed. Records: (1) **module-extraction decision** — extract `tests/diff_sim.rs` helpers into a new `src/diff_sim/mod.rs` library module (full-factorization doctrine) so `src/bin/tool_matrix.rs` can call them via `use anvil::diff_sim::{…}`; rejected duplication; (2) **CLI flag shape** — `--diff-sim` opt-in flag, orthogonal to `--phase4-hierarchy-gate`, gated on Verilator+Yosys both clean per scenario; (3) **representative-subset selector** — per-axis sampling (first scenario per major axis: combinational, sequential-flop, hierarchy, memory, fsm), capped K=5, deterministic, self-maintaining; rejected random-N and hand-curated lists; (4) **coverage-fact wiring** — new `saw_design_with_cross_simulator_agreement: bool` on `CoverageSummary` (alongside Phase 6's existing facts); (5) **per-scenario `DiffSimReport`** — retains `mismatch_excerpt` per Phase-7 counterexample doctrine; (6) `tools_present()`-friendly no-op when simulators absent. 5 rejected alternatives recorded. Design-only; no code. Frontier → `.3b`.
...
- ID: `DIFFERENTIAL-SIMULATION.3b`
  Status: `pending`
  Goal: `Implement per .3a: extract tests/diff_sim.rs helpers into new src/diff_sim/mod.rs (full-factorization doctrine) + add run_differential façade; update tests/diff_sim.rs to use anvil::diff_sim::{…}; add --diff-sim CLI flag to src/bin/tool_matrix.rs; add per-axis subset selector; add DiffSimReport per-module field; add saw_design_with_cross_simulator_agreement coverage fact + merge into aggregate; wire per-module pipeline after Verilator+Yosys clean. Cargo-portable proofs of the subset selector + coverage-fact merge + CLI parse. Tool-gated #[ignore] end-to-end proof on a machine with both simulators installed. Closes DIFFERENTIAL-SIMULATION.3.`
  Acceptance: `cargo fmt/clippy(-D warnings)/check --all-targets/test green; new src/diff_sim/mod.rs landed with extracted helpers + run_differential façade; tests/diff_sim.rs updated to use anvil::diff_sim::{…} (no logic change, 5+2 ignored still green); src/bin/tool_matrix.rs gains --diff-sim flag + per-axis subset selector + DiffSimReport + coverage fact; cargo-portable proofs of subset selector + coverage merge; tool-gated #[ignore] end-to-end proof exits 0 with saw_design_with_cross_simulator_agreement=true on a machine with both simulators.`
...
| --- | --- | --- | --- |
| 1 | `DIFFERENTIAL-SIMULATION.3b` | `pending` (implement `--diff-sim` wiring + extract `src/diff_sim/mod.rs`) | **`.3a` design landed (`2026-05-24`)** docs-only — `DEVELOPMENT_NOTES.md` "Tool-matrix `--diff-sim` wiring + representative-subset selector + coverage fact design": (1) **module-extraction decision** — extract `tests/diff_sim.rs` helpers into a new `src/diff_sim/mod.rs` library module (full-factorization doctrine); rejected duplication. (2) **CLI flag** — `--diff-sim` opt-in, orthogonal to `--phase4-hierarchy-gate`, gated on Verilator+Yosys both clean per scenario. (3) **Per-axis subset selector** — one per axis (combinational/sequential-flop/hierarchy/memory/fsm), capped K=5, deterministic + self-maintaining; rejected random-N + hand-curated. (4) **`saw_design_with_cross_simulator_agreement`** on `CoverageSummary`. (5) **`DiffSimReport`** with retained `mismatch_excerpt` per Phase-7 counterexample doctrine. (6) **`tools_present()`-friendly no-op** when simulators absent. 5 rejected alternatives. `.3b` proof shape specified. Design-only; no code; `cargo` unchanged-green vs base `79f5659`. `.3` split per the proven Phase 7/8/9 design-first discipline. Frontier → `.3b`. |
