...
| `.3c` | `d825eb4` — `BOOK-PARAGRAPH-BLOBS.3c — lift the fourteen-query roll-call out of its table cell` | Book + `scripts/` + docs only ⇒ DUT byte-identical. Lift proven in two halves; `--allow-move` added after its clause-unit first draft cried wolf. |
