...
For workflow-doc leaves under `MEMORY-ARCHITECTURE-DOC`, do not run the
full suite unless specifically needed. Use focused functional checks such
as `scripts/check_memory_architecture.sh` once installed, `git diff
--check`, `mdbook build book`, and `cargo check --all-targets` where
appropriate.
