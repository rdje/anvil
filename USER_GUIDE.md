...
and recursive non-top direct registered sibling mixed-support routing, recursive non-top unregistered parent-composed mixed-support child-input routing without helper instances, and recursive non-top parent outputs that mix parent data ports with child outputs without helper instances or parent-local state. The earlier
...
are superseded by the full downstream-clean `r54` bank.
...
is superseded by the full downstream-clean `r54` bank for downstream-clean evidence.
...
matrix and is carried forward by the `r54` bank through
`saw_hierarchy_registered_sibling_mixed_support_routing = true`.
