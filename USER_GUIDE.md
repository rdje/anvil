...
| `--tools <list>` | `verilator,yosys` | Comma-separated vetted tools (`verilator` / `yosys` / `iverilog` / `sv2v` / `slang`). A fixed allow-list. (`sv2v` is a SystemVerilog→Verilog-2005 transpile accept/reject column; `slang` is a strict independent SystemVerilog elaborator — the first fact-bearing adapter (it also projects an `--ast-json` view of ports + child instances). Both absent on most hosts ⇒ a friendly no-op until installed.) |
...
the hardened `verilator`/`yosys`/`iverilog`/`sv2v`/`slang` allow-list in an auto-removed
...
- **Tools** — the pure `generate`, `introspect`, `analyze`, `dump_config`, and
  ...
  instance_count), with `target` a **module name** (omit for all modules); `query =
  flop_dependencies` returns the **register-to-register dependency graph** (per flop
  its direct register predecessors/successors + a self-feedback flag), with `target`
  a `"flop:<id>"` (omit for all flops); `query = memory_provenance` returns
  **per-inferrable-memory port provenance** — each memory's shape (addr/data width,
  ...
  hides, with `target` a `"fsm:<id>"` (omit for all FSMs); `query = node_drivers`
  returns **per-node immediate (1-hop) driver adjacency** — each IR node's kind,
  width, gate op (for a gate), and its direct operand drivers (each operand's id,
  kind, and resolved handle) in operand order — the atomic node-level primitive
  complementing the transitive support cone, surfacing each node's gate op, with
  `target` a `"node:<id>"` (omit for all nodes); `query = node_readers` returns the
  **exact transpose** — **per-node immediate (1-hop) reader adjacency**, each IR
  node's readers (the nodes that list it as a direct operand) in ascending node-id
  order, with the provable duality `B ∈ node_drivers(A) ⇔ A ∈ node_readers(B)` so an
  agent can walk the construction DAG in either direction one hop at a time, with
  `target` a `"node:<id>"` (omit for all nodes); `query = instance_provenance`
  (design-only) returns, for each child instance in the top, its module/role + the
  **support cone of each child output port built inside the child module's graph** —
  the third opaque-leaf boundary-opener (opening the `InstanceOutput` leaf, the sibling
  of `memory_provenance`/`fsm_provenance`) and the only query that crosses the module
  boundary, with `target` a child instance **name** (omit for all instances);
  `query = instance_input_bindings` returns the **parent-side dual** — for each child
  instance, the **parent node driving each child input port** (read from the instance's
  input bindings), so chaining it with `instance_provenance` traces a parent signal
  across the module boundary, with `target` a child instance **name** (omit for all
  instances); `query = longest_path` returns, for a target (an output port or
  `"flop:<id>"`, the `output_support` namespace), one representative **longest combinational
  fan-in path** — the ordered chain of interior gates (each with its op) realizing
  `output_support`'s scalar `cone_depth`, ending at a boundary leaf — the witness for the
  scalar `cone_depth` (structural gate-depth, not timing), with `target` an output port name
  or `"flop:<id>"` (omit for all outputs);
  `query = node_reach` returns, for a node (`"node:<id>"`), its **transitive combinational
  fan-OUT** — the boundary sinks it reaches (the output ports it drives + the flop `D` cones it
  feeds + their total) — the transitive complement to `node_readers` and the node-addressed
  generalization of `input_reach`, with `target` a `"node:<id>"` (omit for all nodes).
  `query = reach_path` returns, for a node (`"node:<id>"`), one representative **longest
  combinational fan-OUT path** — the ordered gate chain from the node to a boundary sink (an output
  port or `"flop:<id>"`, the `output_support`/`longest_path` target namespace) — the **forward
  complement to `longest_path`** and the **path-witness for `node_reach`** (`node_reach` reports
  *which* sinks, `reach_path` the longest gate-chain *to* one), provably
  `reach_path(n).sink.is_some() ⇔ node_reach(n).fanout_targets > 0`; structural gate-depth, **not**
  timing; `target` a `"node:<id>"` (omit for all nodes). Unknown
